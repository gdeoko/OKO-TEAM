...
## Среда OKO (облачный Claude Code) — ПРОВЕРЕННЫЙ рецепт рендера

В этой среде обычный Chrome НЕ запускается ремоушеном (старый headless выпилен). Рабочий путь:

```bash
cat > /tmp/chrome-ns.sh <<'SH'
#!/bin/bash
exec /opt/pw-browsers/chromium_headless_shell-1194/chrome-linux/headless_shell --no-sandbox "$@"
SH
chmod +x /tmp/chrome-ns.sh

npx remotion render src/index.ts <CompId> out.mp4 \
  --browser-executable=/tmp/chrome-ns.sh --gl=swangle --log=error
```

- Версии, проверенные вместе: remotion 4.0.245 + @remotion/cli 4.0.245 + react 18.3.1.
- npm install идёт через прокси нормально (~130 пакетов, 1-2 мин).
- Кириллица в системных шрифтах рендерится; кастомные шрифты класть в public/ и подключать @font-face.
- Прогресс не показывать пользователю — только результат после ffprobe-проверки и просмотра кадра.
