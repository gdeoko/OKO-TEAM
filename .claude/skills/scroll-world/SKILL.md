...
## СРЕДА OKO (адаптер: MCP вместо CLI) — читать первым при работе в наших чатах

У нас НЕТ `higgsfield` CLI — есть Higgsfield **MCP** (то же самое, вызывается инструментами).
Маппинг команд скилла на наши инструменты:

| Скилл (CLI) | Наш MCP-инструмент |
|---|---|
| `higgsfield generate create gpt_image_2 …` | `mcp__Higgsfield__generate_image` (model `gpt_image_2` / `nano_banana_2`) |
| `higgsfield generate create seedance_2_0 --start-image … --end-image …` | `mcp__Higgsfield__generate_video` (model `seedance_2_0`, роли `start_image`/`end_image`) |
| фото→GLB | `mcp__Higgsfield__generate_3d` |
| апскейл/вырезка/reframe | `upscale_image` / `remove_background` / `reframe` |
| подбор модели | `mcp__Higgsfield__models_explore` (action recommend/get) |
| локальная картинка на вход | `mcp__Higgsfield__media_upload` → id/URL в роль медиа |

Правила нашей среды (перекрывают CLI-специфику скилла):
- Генерации 3–8 мин — запускать фоном (Bash run_in_background) и поллить `job_display`, не блокировать.
- Скраб-энджин `references/scrub-engine.js` — БЕРЁМ КАК ЕСТЬ (blob-seek уже решает нашу главную граблю: хостинги без HTTP Range). Это подтверждено боем 08.07.
- Энкод строго по Step 6: `-g 8 -crf 20 -movflags +faststart -an` + `unsharp`. Дублировать WebM/VP9 — Playwright-Chromium H.264 не декодирует (наша грабля).
- Правило бесшовных стыков (Step 5) — ГЛАВНОЕ: коннекторы из РЕАЛЬНЫХ кадров соседних клипов (ffmpeg extract), не из стиллов. Иначе «поп» на стыке.
- Self-QA по Step 8 + наш регламент OKO_MAGIC_FIX (хэши кадров 3/3 разные).
- Хостинг: наши зеркала githack + Higgsfield websites. Тяжёлые видео → R2 (когда откроют домен).
- Стоимость прогона N сцен: ~N картинок + (2N−1) видео. Архитектура A (непрерывный форвард-тэйк) экономит: коннекторов нет, но генерится строго последовательно.
