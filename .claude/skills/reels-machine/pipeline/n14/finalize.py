# -*- coding: utf-8 -*- n14 stage2 (composite) + stage3 (karaoke + audio)
...
# overlays: gl (под fx) на границах, затем fx+3D из sched
bounds=plan['bounds']; GLN=16
overlays=[]  # (dir, t, frames)
...
"""
# Стандартное ASS-караоke: одно событие на строку, \k-тайминги по словам.
# Primary=оранж (активное/спетое), Secondary=белое (ещё не спетое) — чистый бренд-эффект.
...
# копируем soyuz как «Souz Grotesk» в fonts папку ролика
...
# impact на цифрах/слэмах
place(S_imp[0], A('s1',0.3), 0.7, 0.5)     # «3 приёма»
place(S_imp[1], A('s4',3.6), 0.7, 0.5)     # «каждые 3 сек»
# pop на появлении графики
place(S_pop[0], A('s1',1.7), 0.4, 0.42)    # steps
place(S_pop[1], A('s4',1.5), 0.4, 0.42)    # gridpop
place(S_pop[0], A('s3',2.4), 0.4, 0.4)     # stamp без зума
# riser нагнетание перед CTA
place(S_riser, A('s5',0.2), 1.6, 0.28)
# ding на CTA
