  ...
  return (
    <AbsoluteFill style={{overflow:"hidden",backgroundColor:"#050505"}}>
      <OffthreadVideo src={staticFile(clip)} muted
        style={{width:"100%",height:"100%",objectFit:"cover",transform:`scale(${punch*drift}) translate(${px}px,${py}px)`,filter:"contrast(1.16) saturate(1.22) brightness(0.82)"}}/>
...
const Flash: React.FC = () => { const f=useCurrentFrame();
  const o=interpolate(f,[0,2,6],[0,0.45,0],{extrapolateRight:"clamp"});
...
const Leak: React.FC<{clip:string}> = ({clip}) => {
  ...
  const o=interpolate(f,[0,durationInFrames/2,durationInFrames],[0,0.5,0]);
...
// слой моушен-графики поверх футажа: [компонент, старт, длительность]
const OVER: [React.FC<any>, number, number, any][] = [
  [CostCounter,   6, 112, {}],
  [Converge,    120,  66, {}],
  [FeaturePills,187, 104, {step:26}],
  [UnlockRow,   289,  87, {step:26}],
  [ClickUI,     376,  58, {}],
  [PriceFree,   434,  38, {}],
  [CertBadge,   472,  33, {}],
  [IncomeGraph, 505,  50, {}],
