...
## ГОЛОС (v16, требование Даниэля): Silero сейчас → клон голоса Владимира потом
- **По умолчанию: Silero v4_ru, голос `eugene`, `put_accent=True, put_yo=True`** — встроенный
  словарь ударений, БЕЗ ошибок в ударениях, бесплатно/безлимитно, офлайн. Эталон: `n14/tts_silero.py`.
  Кормить ЧИСТЫЙ текст (strip_acc убирает ручные U+0301, й/ё сохранять — Silero сам ставит ударения).
  torch уже есть; модель `pipeline/models/v4_ru.pt` авто-качается curl'ом (torch.hub через прокси НЕ ходит).
  Темп SPEED≈0.97 (atempo). НЕ edge-tts — у него ошибки в ударениях (Даниэль забраковал).
- **Клон голоса Владимира (когда пришлёт ≥3 голосовых в @vcodemedia_bot):** bot_poll сохраняет
  голосовые в `vcode/voice/*.ogg`. Если там есть ≥1 образец — вместо Silero использовать
  **XTTS-v2** (`pipeline/motion/xtts_voice.py`, отдельный venv, клон по образцу, бесплатно/безлимит),
  ставить ударения вручную (U+0301) т.к. XTTS в русских ударениях ошибается. Проверять наличие
  `vcode/voice/` в начале сборки — есть образцы → клон, нет → Silero eugene.
