...
## СТУДИЙНЫЙ КЛОН голоса Даниэля — `oko_voice_pro.py` (использовать это!)
**Для уроков Академии и голосового ИИ-агента — только этот пайплайн.** Решает жалобы
владельца: «хвостики/мамблы в конце фраз» и «звучит не студийно».
```bash
...
```
- **Референс:** `oko-app/brand/voice/oko_ref_pro.wav` (по умолчанию) — чистый, 22050 моно,
  собран из проф-записи владельца (файл `…260839.ogg`, самый чистый: пик −4.7дБ, без клиппинга),
  два натуральных спана БЕЗ скороговорки-панграммы. Части: `oko_ref_pro_A.wav` (25с) + `_B.wav` (19.7с).
- **Победивший конфиг XTTS (A/B, интеллигибельность 0.81→0.93):**
  `temperature=0.3, repetition_penalty=10.0, top_k=30, top_p=0.75, length_penalty=1.0,
  split_sentences=False`. Низкая температура = XTTS почти не галлюцинирует.
- **ГЛАВНЫЙ приём против «хвостиков»: tail-trim по whisper word-timestamps.** Каждое
  предложение генерится отдельно, транскрибируется faster-whisper (word timestamps),
  аудио режется по концу последнего РЕАЛЬНОГО слова + 0.18с + fade-out. Хвост-галлюцинация
  режется, если после совпадения есть пауза >0.35с ИЛИ слово чужое (нет в скрипте);
  непрерывное слово из словаря (напр. «ОКО») — оставляется.
- **Темп: БЕЗ atempo=1.5!** С этим рефом натуральный темп XTTS ≈18 симв/с — уже дикторский.
  Разгон убран → нет «чипмунк»-артефактов и эффекта «под водой» (старый рецеп­т жёг 1.5×).
- **Межфразовые вдохи** отсутствуют по построению (посегментная генерация + чистая тишина в стыках).
- **Мастеринг (лёгкий, не «эффект»):** `highpass=75, afftdn nr=10:nf=-32 (слабый),
  deesser 6-8кГц, presence-EQ +1.2дБ@3кГц, loudnorm I=-16:TP=-1.5`. Без реверба/эксайтера/тяжёлого EQ.
  arnndn НЕ используется (нет .rnnn-модели в среде; afftdn мягче и не даёт «под водой»).
- **Тон вывода `--tone`:** `clean` (дефолт, дикторский) | `low` (−1 полутон, rubberband) |
  `warm` (тёплый, +low-mid body, чуть срезаны верха). `--retries N` — лучший из N по интеллигибельности.
- **Ограничение:** если проп.имя «ОКО» стоит ПОСЛЕДНИМ словом фразы, XTTS часто его комкает —
  фразу строить так, чтобы «ОКО» не было в самом конце («Это была Академия ОКО. Увидимся…»).
