...
## БЕСПЛАТНАЯ генерация (Pollinations / Flux — без ключа!)

```bash
.claude/skills/gemini-media/scripts/gen-image-free.sh "prompt in english" out.jpg 1280 720
```

- Без ключа, без биллинга, без лимитов на практике. Модель Flux.
- Промпт лучше на английском, с палитрой и стилем.
- Это ДЕФОЛТНЫЙ путь для картинок, пока у Gemini не включён биллинг.
- Разные варианты — просто перезапусти (seed случайный) или задай &seed= в скрипте.
