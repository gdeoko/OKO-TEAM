# 🤖 RUNBOOK — «Tappio автопилот» = МОСТ В ЖИВУЮ СЕССИЮ. ДЕЛАЙ РОВНО ЭТО.
**Архитектура (проверено, работает):** триггер `trig_014PgDgmAjBXw9jh5hrakecf` будит ЭТУ живую
сессию ежечасно 06-20 UTC (09-23 МСК). Живая сессия + foreground-сборка = единственный надёжный
механизм (свежие контейнеры/nohup виснут на тяжёлых сборках). Один тик = один ролик. Просто выполни
ОДИН foreground-вызов Bash (таймаут 1500000 мс):
```
cd /home/user/OKO-TEAM/tappio-app/factory && bash cron_once.sh
```
Смотри последнюю строку:
- `BUILT <id> …` → ролик собран и опубликован во все 3 соцсети + отчёт в бот. Короткий итог, заверши.
- `QUOTA_DONE …` → дневная норма выполнена. Тихо заверши, ничего не постить.
- `FAILED <id>` / `ENV_NOT_READY` → `tail work/<id>_auto.log` или `work/_setup.log`, почини очевидное,
  повтори ОДИН раз; не вышло — кратко в бот @tappiorder_bot, сценарий оставь в очереди.
`cron_once.sh` сам: git pull → setup среды → квота (`quota.py`, рамп 3→15/день) →
**АВТОДОЗАЛИВКА очереди `gen_scripts.py topup 12`** (сценарии генерятся сами, вручную писать НЕ надо) →
сборка v4 → доставка git-raw на VPS → TikTok+YouTube+Instagram → отчёт в бот → снятие с очереди → commit.
НЕ нужны MCP-коннекторы (всё на bash/curl/git/API). НЕ используй Higgsfield/media_upload в тике.

## ⚖️ РАМП И ЦЕЛЬ (жёстко)
3 ролика/день 17-21 июля (первые 5 дней) → далее **+1 каждые 2 дня равномерно до 15/день** →
накопить **~500 роликов к 31 августа**. Логика в `quota.py` (`quota_for`). Цель кампании:
20 000 подписчиков, 100 000 000 суммарных просмотров — максимизируй, но качество и разнообразие важнее.

## 🎲 ЗАКОН РАЗНООБРАЗИЯ (главное правило Александра — НОЛЬ ШАБЛОНОВ)
Каждый ролик — как С НУЛЯ. Между роликами НЕ повторяются: угол/смысл, кадры (4К стоки), музыка,
SFX, грейд, темп, И — критично — СТРУКТУРА и МЕХАНИКА наложений (состав/порядок/число типов
инфографики). Даже образно механика не должна повторять прошлые ролики. Это зашито в
`gen_scripts.py`: `rng_for(id)` даёт каждому ролику случайные состав/порядок/кол-во наложений (6-11),
кадров (10-16), грейд из пула, темп; билдер дедупит футаж/музыку/SFX по кэшу и реестрам
(`USED_FOOTAGE.md`/`USED_ANIM.md`). При ручной правке сценариев — тот же принцип, сверяйся с реестрами.

## 🔁 ПОЛНЫЙ ПАЙПЛАЙН (последовательность, когда сессия делает ролик осмысленно, не только скриптом)
Живая сессия на тике МОЖЕТ (и должна, когда есть бюджет) обогащать генератор реальным анализом:
1. **Активность** (без фанатизма, лимиты: подписки ≤3, лайки ≤10, комменты ≤3 на площадку; дедуп в
   `ENGAGEMENT_LOG.md`): IG (`ig_state.json`=tappio.app.pro) + YouTube — поиск по нише
   (hidden camera/privacy/airbnb safety, память/фокус, ремонт/замеры) и бренду, подписки/лайки/
   осмысленные комменты у конкурентов. Через VPS (`vps/ig_engage.mjs`).
2. **Аналитика за вчера → бот @tappiorder_bot** (10:00 МСК): просмотры/лайки/комменты/репосты/
   сохранения/прирост по IG+YT+TikTok, прогресс к цели; обнови обучающий вывод (что зашло — усиливай).
3. **Разведка конкурентов**: ~10 роликов от 1M+ в нише (yt-dlp метрики/комменты/описания + WebSearch;
   видео с датацентр-IP часто 403 → бери метаданные/кадры/комменты). Раскадровка/расшифровка/хук →
   `analysis/YYYY-MM-DD-<app>.md`. Тренд дня + рабочий хук.
4. **Сценарий**: свежий угол на основе анализа/тренда/бренда, адаптация. Прогон через маркетинг/
   медийность/humanizer/хук-скиллы. 6 битов ≤11 слов (иначе >45с брак). Ноль повторов по реестрам.
5. **Сборка v4** (`make_reel4.sh`): 20-45с, 10-16 уник. 4К-кадров (смена 3-5с), формы-вставки +
   БОЛЬШЕ анимационной код-инфографики (linechart/donut/gauge/bars/ring/checklist/stat/ticker/chips/
   callout/kinetic — разный состав каждый раз), xfade, уникальная музыка (плавные fade), проф. SFX
   без повторов подряд, обложка кадром 0.
6. **Публикация во все 3** (рецепты ниже): TikTok / YouTube / Instagram Reels. После каждой — проверь
   факт на площадке. Ровно один раз на площадку.
7. **Реестры + обучение + коммит**: `USED_FOOTAGE`/`USED_ANIM`/`ENGAGEMENT_LOG`, вывод обучения, push.
8. **Бан/лимит/протухшая сессия** → сразу в бот @tappiorder_bot.

## 🖼️ ФОРМАТЫ (не только Reels — РОАДМАП, layer-in)
Основной поток сейчас — вертикальные ролики (Reels/Shorts/TikTok). По плану добавляем разнообразие
форматов: **карусели** (обучающие слайды-инфографика), **фото-посты**, **сторис** — автоматически, с
тем же анализом и законом разнообразия. Каркас: HTML-слайды → скриншот Playwright → carousel-постер
на VPS (аналог `ig_reel_state.mjs`). Пока не подключено — при появлении добавить генератор
`gen_carousel.py` + рецепт публикации карусели/сторис и включить их в ротацию форматов.

## 📥 ДОЗАЛИВКА ОЧЕРЕДИ — АВТОМАТИЧЕСКАЯ
`cron_once.sh` сам зовёт `python3 gen_scripts.py topup 12` каждый тик (держит ≥12 сценариев).
Вручную ничего писать не нужно. Хочешь долить руками: `python3 gen_scripts.py topup 20`.
Хочешь свой угол — добавь запись в `ANGLES`/`SHOTQ`/`OVERLAY_POOLS` в `gen_scripts.py` (расширяй пулы,
чем больше углов — тем дольше без повторов нарратива).

## 🛡️ САМОКОНТРОЛЬ 24/7 + УДАЛЕНИЕ ЛИШНИХ (дубли/брак)
**Защита от дублей (в auto_run уже зашита):** перед постом в IG проверяется `posted_reels.json`
(id → `ig_code`). Если id уже публиковали — IG-пост ПРОПУСКАЕТСЯ (`IG:already`, дубль не создаётся).
После удачного поста shortcode рила пишется в реестр (через `vps/ig_list.mjs`). Реестр в git.

**Читать ленту (read-only)** — коды постов, поиск дублей, самопроверка:
```
vexec: cd /opt/oko-poster && IG_N=12 node ig_list.mjs      # JSON [{code,type,alt}], новейшие первыми
```
**Удалить ЛИШНИЙ по коду (безопасно — только наш аккаунт, проверка владельца):**
```
# 1) DRYRUN: доходит до кнопки Delete, НЕ удаляет — проверить, что код верный:
vexec: cd /opt/oko-poster && IG_DRYRUN=1 IG_DELCODE=<code> node ig_delete.mjs
# 2) реальное удаление:
vexec: cd /opt/oko-poster && IG_DELCODE=<code> node ig_delete.mjs      # печатает DELETED <code>
```
Путь: пост → «...» More → **Manage post → Delete → подтвердить**. Чужой пост НЕ трогает
(SKIP_NOT_OWNER); уже удалённый → SKIP_NOT_FOUND. После удаления убери код из `posted_reels.json`.

**Когда удалять:** если `ig_list` показал ДВА рила одного нашего id (случайный повтор) или явный
брак (не тот кроп/обложка/битый). НЕ удалять по совпадению подписи — углы ротуются, подписи законно
повторяются между РАЗНЫМИ роликами. Только явный дубль одного id или брак, после DRYRUN-проверки.

**Каждый тик:** auto_run сам пишет код в реестр и логирует. Раз в день сверяй `ig_list` ↔
`posted_reels.json`, лишнее сноси `ig_delete`, о непонятном/бане/лимите — в бот @tappiorder_bot.

## АКТИВНОСТЬ IG (раз в день): `node vps/ig_engage.mjs` через VPS (лайки ≤10, подписки ≤3, комменты ≤3).
