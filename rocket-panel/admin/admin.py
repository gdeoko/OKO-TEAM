...
class Обработчик(BaseHTTPRequestHandler):
    ...
    def _действие(self, путь, д):
        ...
        if путь == "/api/запрет/сохранить":
            # Базовые слова не приходят и не принимаются: страница их
            # показывает, но не отдаёт обратно. Даже если кто-то
            # подделает запрос, `записать` их не тронет.
            д2 = запрет.записать(свои=д.get("свои"),
                                 исключения=д.get("исключения"))
            print("запрет изменён: своих %d, исключений %d"
                  % (len(д2["свои"]), len(д2["исключения"])), flush=True)
            self._ответ(200, запрет.состояние())
            return
