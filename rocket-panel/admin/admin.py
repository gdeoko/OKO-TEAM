...
# Модуля `запрет` здесь больше нет. Решение владельца 26.09.2026: правка
# списка слов из админки не нужна, а из-за импорта панель вообще не
# поднималась - падала при запуске и отдавала 502. Сама проверка живёт
# там, где она и работает, в боте; админке она была нужна только чтобы
# показывать и править список.
...
# СЕРВИСЫ И ОПЛАТА. Решение владельца 26.09.2026: руководители проекта
# должны сами заходить в каждый сервис и сами платить, не дёргая его и не
# переспрашивая доступы в переписке.
#
# Файл лежит ВНЕ кода и вне git: `/srv/amberry/сервисы.json`, права 600,
# владелец - amberry. Причина та же, по какой там же лежит `каталог.json`:
# код в /opt под ProtectSystem=strict и доступен только на чтение, а
# доступы в репозитории - это доступы у всех, кто когда-либо клонировал
# репозиторий, включая тех, кто давно не в проекте.
ФАЙЛ_СЕРВИСОВ = os.path.join(
    os.environ.get("AMBERRY_DATA_DIR", "/srv/amberry"), "сервисы.json")


def сервисы():
    """Список сервисов для панели. Нет файла - пустой список, не ошибка:
    панель обязана открываться и без него."""
    try:
        with open(ФАЙЛ_СЕРВИСОВ, encoding="utf-8") as ф:
            д = json.load(ф)
    except Exception:
        return {"сервисы": []}
    ряды = д.get("сервисы") if isinstance(д, dict) else д
    return {"сервисы": ряды or []}
...
class Обработчик(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if п.startswith("/api/поддержка/"):
            ...
            # Сводка рядом с перепиской - по той же причине, что и в
            # карточке бота: видеть, что человек заплатил вчера три
            # тысячи, надо ДО того, как начнёшь печатать ответ.
            св = store.сводка(int(кто)) if человек else {}
            self._ответ(200, {"письма": store.поддержка_диалог(int(кто)),
                              "username": человек.get("username"),
                              "баланс": store.balance(int(кто)) if человек else 0,
                              "куплено": св.get("куплено", 0),
                              "работ": св.get("работ", 0),
                              "осечек": св.get("осечек", 0),
                              "с": человек.get("created_at")})
            return
        if п == "/api/рассылки":
        ...
        if п == "/api/карта":
            ...
            return
        if п == "/api/сервисы":
            return self._ответ(200, json.dumps(сервисы(), ensure_ascii=False))
        if п == "/api/источники":
            # Откуда пришли люди и сколько заплатили: метка ?start=ad_<канал>.
            self._ответ(200, {"источники": store.источники()})
            return
        if п == "/api/реклама":
    ...
    def _действие(self, путь, д):
        ...
        if путь == "/api/человек/блок":
            ...
            return
        if путь == "/api/человек/начислить":
            # НАЧИСЛЕНИЕ РУКАМИ ВЛАДЕЛЬЦА. Подарок, компенсация,
            # блогеру на пробу - причин много, и все они его.
            #
            # Карман «welcome», а НЕ «paid», и это не мелочь: в paid
            # лежит выручка, от неё считается половина партнёру и вся
            # отчётность. Подаренные коины деньгами не являются, и
            # попав туда, они раздули бы выручку на ровном месте.
            #
            # Причина пишется в журнал обязательно: через месяц
            # «откуда у него тысяча коинов» - вопрос без ответа, если
            # её не записать.
            # int() сам бросит на мусоре, а do_POST поймает и ответит
            # четырёхсоткой - своей проверки на «число ли это» не надо.
            кто = int(д["tg_id"])
            сколько = int(д.get("сколько") or 0)
            if сколько <= 0 or сколько > 100000:
                self._ответ(400, {"ошибка": "сколько коинов? от 1 до 100000"})
                return
            if not store.user(кто):
                # Начислить можно только тому, кто уже нажал «Начать»:
                # до этого записи о нём нет, и коины лечь некуда.
                self._ответ(404, {"ошибка": "человек ещё не заходил в бота"})
                return
            почему = (str(д.get("почему") or "").strip()
                      or "начислено из админки")
            store.credit(кто, сколько, "welcome", почему,
                         meta={"вручную": True})
            print("начислено вручную: %s +%d (%s)" % (кто, сколько, почему),
                  flush=True)
            self._ответ(200, {"ок": True, "баланс": store.balance(кто)})
            return
