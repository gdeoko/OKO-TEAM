...
<main>
  ...
  <section id="р_сервисы"><h2>Сервисы и оплата</h2>
    <p class="тихо" id="серв_шапка">Всё, за что платит проект: вход, доступы и
    прямая кнопка оплаты. Считается по данным на сервере.</p>
    <div class="ряд" style="margin:10px 0 14px">
      <button class="кн" id="серв_скрыть">Скрыть пароли</button>
      <button class="кн" id="серв_обновить">Обновить</button>
    </div>
    <div id="т_сервисы"></div>
  </section>
...
// владелец 26.09.2026: руководители должны заходить и платить сами, не
// дёргая его. Данные лежат на сервере в `/srv/amberry/сервисы.json`
// (права 600, в git не идёт) и приезжают сюда уже за паролем админки.
let сервисы_скрыты = false;

function рисовать_сервисы(д){
  const ряды = (д.сервисы || []);
  if (!ряды.length){ $("#т_сервисы").innerHTML = "<p class='тихо'>Список пуст.</p>"; return; }
  $("#т_сервисы").innerHTML = `<table class="карточками">
    <tr><th>Сервис</th><th>Логин</th><th>Пароль</th><th>Почта</th>
        <th>Пароль почты</th><th>Стоимость</th><th></th></tr>
    ${ряды.map(с => `<tr>
      <td data-л="Сервис"><b>${эк(с.имя||"")}</b>${с.заметка?`<div class="тихо">${эк(с.заметка)}</div>`:""}</td>
      <td data-л="Логин"><span class="копия" data-т="${эк(с.логин||"")}">${эк(с.логин||"—")}</span></td>
      <td data-л="Пароль"><span class="тайна копия" data-т="${эк(с.пароль||"")}">${эк(с.пароль||"—")}</span></td>
      <td data-л="Почта"><span class="копия" data-т="${эк(с.почта||"")}">${эк(с.почта||"—")}</span></td>
      <td data-л="Пароль почты"><span class="тайна копия" data-т="${эк(с.пароль_почты||"")}">${эк(с.пароль_почты||"—")}</span></td>
      <td data-л="Стоимость">${эк(с.стоимость||"—")}</td>
      <td><div class="ряд">
        ${с.открыть?`<a class="кн" target="_blank" rel="noopener" href="${эк(с.открыть)}">Открыть</a>`:""}
        ${с.оплатить?`<a class="кн главная" target="_blank" rel="noopener" href="${эк(с.оплатить)}">Оплатить</a>`:""}
      </div></td></tr>`).join("")}</table>`;
  // Нажатие на логин или пароль кладёт его в буфер: с телефона выделять
  // мышью нечем, а вводить руками такое - это опечатка через раз.
  document.querySelectorAll("#т_сервисы .копия").forEach(э => {
    э.style.cursor = "pointer";
    э.title = "нажми, чтобы скопировать";
    э.onclick = () => {
      navigator.clipboard?.writeText(э.dataset.т || "");
      const было = э.textContent; э.textContent = "скопировано";
      setTimeout(() => э.textContent = было, 900);
    };
  });
  применить_тайны();
}

function применить_тайны(){
  document.querySelectorAll("#т_сервисы .тайна").forEach(э => {
    э.style.filter = сервисы_скрыты ? "blur(7px)" : "none";
  });
  $("#серв_скрыть").textContent = сервисы_скрыты ? "Показать пароли" : "Скрыть пароли";
}

async function сервисы(){
  try { рисовать_сервисы(await взять("/api/сервисы")); }
  catch(e){ $("#т_сервисы").innerHTML = "<p class='тихо'>Не читается список сервисов.</p>"; }
}
...
function рисовать_рекламу(){
  const план = рк_план();
  const группы = ["все","18+ можно","купить сразу","в плане","живой","с оговорками","рискованный",
  ...
  $("#рк_про").innerHTML = `Собрано ${эк(РК.собрано)} по публичным лентам каналов и рейтингам TGStat.
    ...
    <b>«18+ можно»</b> - канал сам 18+ или уже ставил такую рекламу; <b>«18+ спросить»</b> -
    ответ знает только админ. Возраст аудитории по ленте не определяется: смотрите канал глазами,
    подростковые не берём.`;
  ...
  let сп = РК.каналы.filter(к => РК_ФИЛЬТР==="все" ? true
          ...
          : РК_ФИЛЬТР==="18+ можно" ? к.допуск==="можно"
    ...
    return `<div class="рк ${в?"в_плане":""}">
      <div class="шапка"><div style="flex:1">
        ...
        <div style="display:flex;flex-direction:column;gap:4px;align-items:flex-end">
          <span class="метка ${м}">${эк(к.вердикт)}</span>
          ${к.допуск ? `<span class="метка ${к.допуск==="можно"?"ок":"тихо"}"
            title="${эк(к.допуск_почему||"")}">18+ ${эк(к.допуск)}</span>` : ""}
        </div></div>
      <div class="цифры">
      ...
      <ul>${пункты}${к.допуск_почему ? `<li class="${к.допуск==="можно"?"х":"о"}">${эк(к.допуск_почему)}</li>` : ""}</ul>
...
async function карта_плитки(){
  ...
  const ч = к.часов;
  // При автопополнении остаток мал ВСЕГДА - Vast списывает с банковской
  // карты, как только он падает ниже порога. Красным это гореть не имеет
  // права: тревога одна - списание не пришло (сторож ставит «автосбой»).
  const авто = к.автопополнение && к.автопополнение.включено ? к.автопополнение : null;
  const тревога = авто ? !!к.автосбой : (ч!=null && ч < 24);
  const срок = ч==null ? "машина не запущена" : ч >= 48 ? Math.floor(ч/24)+" сут" : Math.round(ч)+" ч";
  const пл = [
    ["Остаток на Vast", "$"+к.остаток.toFixed(2),
     тревога ? (авто ? "автопополнение не прошло" : "пополнить срочно")
             : (авто ? "пополняется сам" : "")],
  ...
  if (авто) пл.push(["Автопополнение", "$"+авто.сумма.toFixed(0),
                     "как только остаток ниже $"+авто.порог.toFixed(0)]);
