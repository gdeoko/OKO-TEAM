...
<main>
  ...
  <section id="р_запрет"><h2>Запрет слов</h2>
    ...
    <div class="карта">
      <h3>Свои запреты</h3>
      <p class="когда">Запрещают запрос целиком. Регистр и растянутые
         буквы не важны. Добавил или удалил — работает сразу.</p>
      <div class="ряд" style="margin-top:8px">
        <input id="зап_новое" placeholder="слово или оборот"
               style="flex:1;min-width:200px">
        <button class="кн" id="зап_добавить">Добавить</button>
      </div>
      <div id="зап_свои_список" style="margin-top:12px"></div>
    </div>

    <div class="карта">
      <h3>Исключения</h3>
      <p class="когда">Обороты, которые запрещать НЕ надо. Если такой
         оборот есть в запросе, слово внутри него не срабатывает. Этим
         гасятся ложные придирки, не открывая весь список.</p>
      <div class="ряд" style="margin-top:8px">
    ...
    <div class="карта obёртка">
      <h3>Базовые слова</h3>
      <p class="когда">Список про детей. Показан целиком, но из панели не
         удаляется: снести детскую защиту промахом пальца нельзя. Нужно
         убрать слово — скажи, правится в коде. Возраст цифрами
         запрещён до <b id="зап_возраст"></b> лет.</p>
      <div id="зап_базовые" style="margin-top:10px"></div>
    </div>
  </section>
...
// ---- запрет слов ----
let ЗАП = {базовые:[], свои:[], исключения:[]};
...
function рисовать_запрет(){
  const пусто = `<p class="когда">Пока пусто.</p>`;
  $("#зап_свои_список").innerHTML = ЗАП.свои.length
    ? ЗАП.свои.map(с => строка_запрета(с, "свои")).join("") : пусто;
  $("#зап_искл_список").innerHTML = ЗАП.исключения.length
    ? ЗАП.исключения.map(с => строка_запрета(с, "исключения")).join("") : пусто;
...
async function сохранить_запрет(){
  ...
  const о = await послать("/api/запрет/сохранить",
                          {свои: ЗАП.свои, исключения: ЗАП.исключения});
  ЗАП.свои = о.свои; ЗАП.исключения = о.исключения;
  рисовать_запрет();
  $("#зап_ход").textContent = "сохранено";
...
$("#зап_добавить").onclick = () => добавить_запрет($("#зап_новое"), "свои");
...
const ЗАГРУЗКА = {
  async запрет(){
    ...
    $("#зап_возраст").textContent = ЗАП.взрослый_с;
    $("#зап_базовые").innerHTML = (ЗАП.базовые||[])
      .map(с => `<span class="метка">${эк(с)}</span>`).join(" ");
    рисовать_запрет();
  },
  async сводка(){
