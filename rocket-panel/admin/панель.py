...
<main>
  ...
  <section id="р_запрет"><h2>Запрет слов</h2>
    ...
    <div class="карта">
      <h3>Свои слова</h3>
      <p class="когда">Запрещают запрос целиком. По одному в строке.
         Регистр и повторы букв не важны.</p>
      <textarea id="зап_свои" style="min-height:120px"></textarea>
      ...
      <p class="когда">Обороты, которые запрещать НЕ надо. Если такой
         ...
         гасятся ложные придирки.</p>
      <textarea id="зап_искл" style="min-height:100px"></textarea>
      <div class="ряд" style="margin-top:10px">
        <button class="кн" id="зап_сохранить">Сохранить</button>
...
$("#зап_сохранить").onclick = async () => {
  const строки = п => п.value.split("\n").map(с=>с.trim()).filter(Boolean);
  $("#зап_ход").textContent = "сохраняю…";
  const о = await послать("/api/запрет/сохранить", {
    свои: строки($("#зап_свои")), исключения: строки($("#зап_искл"))});
  $("#зап_ход").textContent =
    `сохранено: своих ${о.свои.length}, исключений ${о.исключения.length}`;
  setTimeout(()=>{$("#зап_ход").textContent="";}, 4000);
};
...
const ЗАГРУЗКА = {
  async запрет(){
    const д = await взять("/api/запрет");
    $("#зап_свои").value = (д.свои||[]).join("\n");
    $("#зап_искл").value = (д.исключения||[]).join("\n");
    $("#зап_возраст").textContent = д.взрослый_с;
    $("#зап_базовые").innerHTML = (д.базовые||[])
