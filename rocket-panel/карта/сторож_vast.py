...
**НА СЧЁТЕ ВКЛЮЧЕНО АВТОПОПОЛНЕНИЕ, И ЭТО МЕНЯЕТ ВСЮ ТРЕВОГУ.** Vast сам
списывает с привязанной карты, как только остаток падает ниже порога
(поля `autobill_threshold` и `autobill_amount` в /users/current/). Значит
маленький остаток - НОРМА, а не беда: он маленький нарочно и таким будет
всегда. Пороги «хватит на 72/24/6 часов» при включённом автопополнении
кричали бы круглые сутки и через день перестали бы что-либо значить.

Поэтому тревога ровно одна: **остаток провалился НИЖЕ порога
автопополнения и не поднялся** за ТЕРПЕНИЕ. Это значит, что списание не
прошло - банковская карта, лимит, 3-D Secure, - и только это способно
остановить видеокарту. Пороги по часам остаются для случая, когда
автопополнение на счёте выключено.

Раз в 15 минут (крон root):
  1. спрашивает у Vast остаток, что запущено и как настроено
     автопополнение;
  ...
  5. зовёт владельцев в Телеграм основным ботом: при автопополнении -
     когда оно не прошло, без него - на порогах 72, 24 и 6 часов. Каждая
     тревога один раз, пока остаток снова не поднимется.
...
ПОРОГИ = (72, 24, 6)                 # часов до остановки (без автопополнения)
ТЕРПЕНИЕ = 40 * 60                   # сколько ждём автопополнение, прежде чем звать
...
def снять(ключ):
    """Остаток, запущенные машины и настройка автопополнения.

    Автопополнение НЕ хранится у нас в конфиге нарочно: его правят в
    платёжном разделе Vast, и списанная копия настройки разошлась бы с
    правдой молча. Спрашиваем тем же запросом, что и остаток."""
    кто = vast("/users/current/", ключ)
    авто = {
        "включено": bool(кто.get("has_billing") and кто.get("can_pay")
                         and float(кто.get("autobill_amount") or 0) > 0),
        "порог": round(float(кто.get("autobill_threshold") or 0), 2),
        "сумма": round(float(кто.get("autobill_amount") or 0), 2),
    }
    машины = []
    ...
    return float(кто.get("credit") or 0), машины, авто
...
def главное():
    ...
    сейчас = time.time()
    остаток, машины, авто = снять(ключ)
    ...
    ниже_с = int(было.get("ниже_с") or 0)        # когда провалились под порог
    авто_сказано = bool(было.get("авто_сказано"))

    if авто["включено"]:
        # Остаток мал всегда - так и задумано. Беда одна: он ушёл под
        # порог автопополнения и не поднялся, значит списание не прошло.
        if остаток < авто["порог"]:
            ниже_с = ниже_с or int(сейчас)
            if сейчас - ниже_с >= ТЕРПЕНИЕ and not авто_сказано:
                известить(
                    "<b>Vast: автопополнение не прошло</b>\n\n"
                    f"Остаток <b>${остаток:.2f}</b> - ниже порога "
                    f"${авто['порог']:.2f}, и списание ${авто['сумма']:.2f} "
                    f"не пришло уже {int((сейчас - ниже_с) / 60)} мин.\n"
                    f"Карта ест ${в_час:.2f}/ч; когда остаток кончится, она "
                    "остановится и бот перестанет генерировать.\n\n"
                    "Смотреть банковскую карту и лимиты: "
                    "https://cloud.vast.ai/billing/", бот)
                авто_сказано = True
        else:
            ниже_с, авто_сказано = 0, False
    elif часов is not None:
    ...
    json.dump({
        ...
        "автопополнение": авто, "ниже_с": ниже_с, "авто_сказано": авто_сказано,
        # Для админки: красная плитка - только настоящая беда, а не
        # обычный маленький остаток между списаниями.
        "автосбой": bool(авто["включено"] and ниже_с
                         and сейчас - ниже_с >= ТЕРПЕНИЕ),
    ...
    for ф in (СЕЙЧАС_ФАЙЛ, ИСТОРИЯ):
        os.chmod(ф, 0o644)
    авто_словами = (f"автопополнение ${авто['сумма']:.0f} ниже "
                    f"${авто['порог']:.0f}" if авто["включено"] else
