...
# ОПОРА — СКЕЛЕТ ПОЗЫ С ПРИНЯТОГО ВЛАДЕЛЬЦЕМ КАДРА.
#
...
# текста у него нет.
#
# СКЕЛЕТ, А НЕ ГЛУБИНА — РЕШЕНО ЗАМЕРОМ 24.09.2026.
#
# Опоры две: скелет (DWPose) несёт только «где локоть, где колено», а
# карта глубины (Depth-Anything-V2) несёт ещё и СИЛУЭТ — то есть
# сложение человека с эталона. До 24.09 стояла глубина: она читает
# любую позу, даже сложенные тела, и казалась безопасной, потому что
# отпускается на 60 % прогона.
#
# Не была. Владелец потребовал: «возраст, фигура, форма тела, цвет
# кожи — максимально как на референсе», и замер показал, что ворует
# фигуру именно опора. Один снимок пышной клиентки (тяжёлая грудь,
# мягкий живот, широкие бёдра), одна кнопка, одно зерно, меняется
# только опора:
#
#     глубина 1,4   модельное тело: тонкие ноги, убранный живот
#     скелет  1,8   её тело: та же грудь, тот же живот, те же бёдра
#     без опоры     её тело — то же, что у скелета
#
# На худой клиентке все три совпадают: силуэт эталона ей не мешает,
# он и так худой. То есть глубина портит ровно тех, кто НЕ похож на
# эталон, — тех единственных, кто это заметит.
#
# По позам скелет не уступил: прогон 21 кнопки на двух зёрнах, кадры
# просмотрены глазами. Двадцать кнопок из двадцати одной держат позу
# не хуже глубины, а местами лучше — у «Секса раком» глубина рисовала
# мужчину с женскими волосами, у «Мастурбации раком» срезала голову.
# Единственное исключение — «Поставить раком» (`un_three`): там DWPose
# прочитал позу как стоячую, и по неверному скелету выходит выкройка
# вместо человека. Эта кнопка осталась на глубине.
#
...
ОПОРА_ВИД="поза"
# Скелету нужна сила больше, чем глубине: у глубины на входе залитый
# силуэт, у скелета — тонкие линии на чёрном, и при 1,4 их перебивает
# текст. 1,8 держит позу и при этом не тянет за собой ничего лишнего:
# тянуть нечего, силуэта в скелете нет.
ОПОРА_СИЛА=1.8
ОПОРА_ДО=0.60

# КНОПКИ, У КОТОРЫХ СВОЯ ОПОРА — сила, срок или вид другие.
#
# Одна: «Поставить раком». На принятом кадре женщина стоит на
# четвереньках боком, DWPose прочитал это как стоячую фигуру в профиль
# и отдал скелет стоящего человека — с одной ступнёй и без второй
# руки. Сборка честно нарисовала по нему выкройку из белых лоскутов.
# По карте глубины та же кнопка идёт нормально, поэтому она одна и
# осталась на глубине.
#
# ПРОВЕРЯТЬ НАДО КАРТИНКУ, А НЕ СЧЁТ ТОЧЕК. Список исключений сперва
# собирался по числу найденных людей (`poz_ok.py`), и он оказался не
# тот: у «Секса раком» и «Наездницы» DWPose тоже считает людей
# неверно, а кадры по их скелетам выходят ЛУЧШЕ, чем по глубине.
# Решает только просмотр готовых кадров.
#
# «Снимает лифчик» долго не давалась (второе тело внизу
...
#
# Вид бывает двух родов:
#   {"ключ": {"вид": "поза"|"глубина", "сила": 1.4, "до": 0.6}}
ОПОРА_СВОЯ={
  "un_three": {"вид":"глубина","сила":1.4,"до":0.60},
}
...
# ПЛОСКАЯ ГРУДЬ СЛОВАМИ НЕ ДЕЛАЕТСЯ. Замер 24.09.2026, и это едва ли
# не самая дорогая находка дня.
#
# Владелец: «она может быть и вообще плоская, не дано природой, и на
# референсе это видно». Взяли снимок худой плоской клиентки и прогнали
# un_full при неизменном зерне:
#
#     текст «как на фото»            грудь круглая, чужая
#     текст «стройная» (SMALL,
#       ALMOST FLAT CHEST)           та же круглая грудь
#     плюс прямой приказ «если на
#       снимке плоская — остаётся
#       плоской»                     та же круглая грудь
#     denoise 1,00 / 0,90 / 0,85     та же круглая грудь
#     denoise 0,80 / 0,75 / 0,70     грудь та же, но одежда не снялась
#
# Шесть попыток, ни одна не сдвинула размер. Значит, дело не в словах:
# у сборки свой прочный навык рисовать округлую грудь, и правится он
# только весами. Проверка на той же карте: заказали ей двенадцать
# одетых женщин, из них четыре «с плоской грудью» — плоской вышла одна.
# Сборка не умеет её рисовать даже когда её об этом прямо просят.
#
# ПОЭТОМУ ЛОРА. `Flat Chest (Qwen)` — веса поверх сборки, а не ещё одна
...
def лоры_комфи(силой=False):
    """Какие лоры ВИДИТ ComfyUI — не какие лежат на диске.

    Это разные вещи, и разница стоила молчаливой поломки. ComfyUI читает
    список моделей ОДИН РАЗ при запуске. Файл, доехавший позже (перенос
    на новую карту, докачка), лежит на месте, `os.path.exists` его
    находит — а ComfyUI отвечает `Value not in list: lora_name ... not
    in []` и выбрасывает узел. Задание при этом не падает: оно считается
    БЕЗ лоры и отдаёт человеку кадр, в котором просто нет того, за что
    он платил. Проверено 25.09.2026 на трёх заказах подряд.

    Поэтому спрашиваем сам ComfyUI. Ответ держим полминуты: список
    меняется только при перезапуске службы, а дёргать его на каждый
    заказ незачем.

    Не ответил — возвращаем ПУСТО, то есть считаем без лоры. Ошибиться
    в сторону кадра без лоры дешевле, чем в сторону упавшего заказа.
    """
    if not силой and time.time()-_ЛОРЫ_КЭШ["когда"]<30:
        return _ЛОРЫ_КЭШ["список"]
    try:
        о=json.load(urllib.request.urlopen(
            COMFY+"/object_info/LoraLoaderModelOnly",timeout=10))
        сп=о["LoraLoaderModelOnly"]["input"]["required"]["lora_name"][0]
        _ЛОРЫ_КЭШ["список"]=frozenset(сп)
    except Exception as e:                              # noqa: BLE001
        print("список лор не прочитался:",str(e)[:120],flush=True)
        _ЛОРЫ_КЭШ["список"]=frozenset()
    _ЛОРЫ_КЭШ["когда"]=time.time()
    return _ЛОРЫ_КЭШ["список"]


def _лора(g, имя, сила):
    """Подмешать веса к модели. Условия не трогаем: лора учит рисовать,
    а не понимать текст, и `LoraLoaderModelOnly` — ровно про это.

    НЕТ ФАЙЛА — РАБОТАЕМ БЕЗ НЕГО, А НЕ ПАДАЕМ.

    Умолчание ползунка «как на фото» берёт середину, то есть силу БОЛЬШЕ
    НУЛЯ, — значит лора подмешивается почти к каждому заказу. Пока файл
    лежал на карте, этого никто не замечал; при переезде на другую
    площадку он не доехал, и `LoraLoaderModelOnly` стал валить задание
    целиком. Человек платит коины и получает ошибку — за файл, который
    всего лишь уточняет размер груди.

    Поэтому отсутствие файла понижает требование: кадр выйдет с тем
    размером, который рисует сама сборка. Панель об этом честно
    сообщает в `/api/готовность`, а заказ доходит.
    """
    if not имя or float(сила) <= 0:
        return g
    if имя not in лоры_комфи():
        print(f"ComfyUI не видит лору {имя} — считаю без неё, заказ не роняю",
              flush=True)
        return g
    g["20"]={"class_type":"LoraLoaderModelOnly",
             "inputs":{"model":["1",0],"lora_name":имя,
                       "strength_model":float(сила)}}
    g["7"]["inputs"]["model"]=["20",0]
    return g


def плоскость(заказ, images, чей=0):
    """Сила лоры: число от бота или «авто» — замер по снимку.

    Число приходит, когда человек САМ выбрал фигуру в боте: он знает
    своё тело лучше любой меры, и спорить с ним нечем. «Авто» приходит
    на умолчании «как на фото» — там мы и правда не знаем, и мера
    решает (см. `плоскость_по_снимку` в приёмке).

    `чей` — НОМЕР ЖЕНСКОГО СНИМКА, и он обязателен именно у пар. В
    сценах МЖ первым идёт МУЖЧИНА (так требует сборка, иначе выходят
    две женщины), а мера, наведённая на мужскую грудь, честно отвечает
    «плоская»: замер 24.09.2026 дал 0,97 и 1,00 на двух мужских
    снимках. Без этого номера лора на полной силе прилетала бы
    ЖЕНЩИНЕ в кадре — за то, что рядом с ней стоит мужчина.
    """
    if str(заказ).strip().lower() in ("авто", "auto"):
        if not images:
            return 0.0
        try:
            import приёмка
            н = чей if 0 <= чей < len(images) else 0
            return приёмка.плоскость_по_снимку(os.path.join(IN, images[н]))
        except Exception as e:                          # noqa: BLE001
            # Мера — удобство, а не условие работы: не смогла — кадр
            # всё равно считается, просто без лоры.
            print("плоскость: осечка", str(e)[:120], flush=True)
            return 0.0
    try:
        return max(0.0, min(1.5, float(заказ or 0)))
...
def опора_файл(ключ, вид=None):
    ...
    if not ключ or "/" in ключ or "\\" in ключ:
        return None
    папка=ВИДЫ_ОПОР.get(вид or ОПОРА_ВИД,ВИДЫ_ОПОР[ОПОРА_ВИД])[0]
    откуда=os.path.join(папка,ключ+".png")
    if not os.path.exists(откуда):
        return None
    имя="opora_%s_%s.png" % (вид or ОПОРА_ВИД,ключ)
...
def _опора(g,w,h,имя,сила,до,вид=None):
    """Прицепить опору к обоим условиям. См. «ОПОРА» в шапке.
    ...
    g["51"]={"class_type":"SetUnionControlNetType",
             "inputs":{"control_net":["50",0],
                       "type":ВИДЫ_ОПОР.get(вид or ОПОРА_ВИД,
                                            ВИДЫ_ОПОР[ОПОРА_ВИД])[1]}}
...
def wf_photo(p,w,h,seed,images=None,neg=None,denoise=1.0,
    ...
    if опора:
        ...
        g=_опора(g,w,h,опора,
                 ...
                 опора_вид or своя.get("вид",ОПОРА_ВИД))
...
#
# Wan обучен на 16 кадрах в секунду — это записано прямо в ComfyUI
# (`comfy_extras/nodes_wan.py`: «model trained with 16 fps», и сам узел
# ставит `fps = 16`). Мы же просили 24 кадра на секунду и склеивали их
# обратно в 24. Выходило два изъяна разом:
#
#   1. ДВИЖЕНИЕ В ПОЛТОРА РАЗА БЫСТРЕЕ. Модель считает, что 240 кадров
#      это пятнадцать секунд действия; мы проигрывали их за десять.
#      Отсюда суетливость, на которую и жаловался владелец.
#   2. ТРЕТЬ КАДРОВ ЛИШНЯЯ. Пятисекундный ролик — 121 кадр вместо 81,
#      десятисекундный — 241 вместо 161. А внимание у видеомодели идёт
#      по всему ролику разом, поэтому лишняя треть кадров стоит БОЛЬШЕ
#      трети времени.
#
# Поднимать обратно до плавных 32 к/с — дело интерполяции (RIFE), а не
# генерации: доснять кадры дешевле, чем выдумать их моделью.
ВИДЕО_FPS=16
...
def готовность():
    ...
    for имя,путь in (("сборка фото", os.path.join(CKPT_DIR,CKPT_PHOTO)),
                     ...
                     ("лора плоской груди",
                      os.path.join(ЛОРЫ_DIR,ЛОРА_ПЛОСКАЯ))):
        if not os.path.exists(путь):
            беды.append(f"нет файла «{имя}»: {путь}")
    # ФАЙЛ НА ДИСКЕ — ЕЩЁ НЕ ЗНАЧИТ, ЧТО COMFYUI ЕГО ВИДИТ.
    #
    # Список моделей он читает при запуске. Лора, доехавшая позже,
    # лежит на месте и проверку выше проходит, а ComfyUI молча
    # выбрасывает узел — человек получает кадр без того, за что
    # заплатил, и никто об этом не узнаёт. Лечится перезапуском службы,
    # но сперва надо заметить.
    if (os.path.exists(os.path.join(ЛОРЫ_DIR,ЛОРА_ПЛОСКАЯ))
            and ЛОРА_ПЛОСКАЯ not in лоры_комфи(силой=True)):
        беды.append(f"ComfyUI не видит лору «{ЛОРА_ПЛОСКАЯ}», хотя файл "
                    f"на диске: список читается при запуске — "
                    f"перезапустить службу comfy")
    # Считаем опоры ОБОИХ видов, а не только ходового: исключения
    # берут свой вид, и папка исключения пустой быть не может.
    for вид,(папка,_) in ВИДЫ_ОПОР.items():
        сколько=len([f for f in os.listdir(папка)
                     if f.endswith(".png")]) if os.path.isdir(папка) else 0
        if сколько < ОПОР_ЖДЁМ:
            беды.append(f"опор «{вид}» {сколько} из {ОПОР_ЖДЁМ}: "
