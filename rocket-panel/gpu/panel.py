...
# КНОПКИ, У КОТОРЫХ СВОЯ ОПОРА.
#
# Общая настройка — компромисс, и на одной кнопке он вышел боком.
#
# «Снимает лифчик» снят В ПОЛНЫЙ РОСТ, и при силе 1,4 сборка
# пририсовывала внизу кадра ВТОРОЕ ТЕЛО крупным планом. На трёх зёрнах
# подряд — значит дело не в зерне. Разбор 24.09.2026, четыре пробы:
#
#   сила 0,8 / 1,0 / 1,15   призрака нет, но и позы нет: она садится
#   сила 1,4, срок 0,3      поза верная, призрак на месте
#   сила 1,25, пол погашен  один раз вышло идеально и не повторилось
#   опора СКЕЛЕТОМ          призрака нет ни на одном зерне, поза сидя
#
# Выбран скелет. У него нет фона вовсе — он несёт только «где локоть,
# где колено», и подрисовывать ему нечего. Цена известна и записана: на
# этой кнопке человек СИДИТ, а на принятом кадре стоит. Действие при
# этом верное — майка поднята, грудь открыта, — а второе тело это брак,
# который виден с первого взгляда. По-хорошему нужен новый эталон,
# снятый сидя, или решение владельца оставить как есть.
#
# Держим исключения поимённо, а не подгоняем общую настройку под худшую
# кнопку: остальным двадцати глубина при 1,4 нужна, см. «Поставить
# раком».
ОПОРА_СВОЯ={"ph_below":{"вид":"поза","сила":1.4,"до":0.60}}
