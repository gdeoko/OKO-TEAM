...
def опора_файл(ключ):
    ...
    Карты глубины лежат отдельной папкой, а `LoadImage` умеет читать
    только из `input`, поэтому нужная карта переносится туда при первом
    ...
    if not ключ or "/" in ключ or "\\" in ключ:
        return None
    откуда=os.path.join(ОПОРЫ,ключ+".png")
    if not os.path.exists(откуда):
        return None
    имя="opora_%s.png" % ключ
...
def _опора(g,w,h,имя,сила,до):
    ...
    g["51"]={"class_type":"SetUnionControlNetType",
             "inputs":{"control_net":["50",0],"type":"depth"}}
