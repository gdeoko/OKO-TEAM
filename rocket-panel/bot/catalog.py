...
def правки(сейчас=None):
    ...
    if метка != _КЭШ["метка"]:
        ...
        # Дерево пересобирается ТУТ ЖЕ: своя кнопка обязана появиться в
        # боте так же, как правка названия — сама, без перезапуска.
        # `_ИЗ_КОДА` пуст ровно до конца сборки дерева, и на первом
        # чтении при загрузке модуля пересборка пропускается: класть
        # кнопки ещё некуда.
        if _ИЗ_КОДА:
            пересобрать_свои(_КЭШ["правки"])
    return _КЭШ["правки"]
...
# ---------------------------------------------------------------------
# СВОИ КНОПКИ ВЛАДЕЛЬЦА
#
# Требование 24.09.2026: «добавлять какие-то новые промпты и кнопки…
# полное редактирование всего». До этого каталог был закрытым списком:
# правились названия и откровенные строки, но завести НОВУЮ кнопку
# можно было только правкой кода.
#
# Своя кнопка — это тот же `Scene`, только собранный не из кода, а из
# файла правок. Технический слой (свет, объектив, анатомия, кожа,
# запреты) она получает ровно такой же, как все остальные: он
# собирается автоматически из `prompts` и от способа заведения кнопки
# не зависит. Владелец пишет то же, что и для кнопки из кода — что
# происходит в кадре.
#
# ЧТО У СВОЕЙ КНОПКИ ИНАЧЕ: её можно удалить по-настоящему. У кнопки
# из кода за спиной три тысячи знаков промпта, и стёртая из браузера
# она бы не вернулась — поэтому такие прячутся. У своей за спиной
# только строка владельца, которую он сам и написал.
# ---------------------------------------------------------------------

def запасной_узел():
    """Куда класть свою кнопку, если ветка не выбрана или её больше нет.

    Считается по дереву, а не записан ключом. Записанный ключ тут уже
    подвёл: в первой версии здесь стояло выдуманное `un_solo_strip`,
    такого узла в дереве нет, и своя кнопка молча не появлялась нигде —
    владелец искал бы, куда она делась. Дерево за сутки меняется
    трижды, и любой ключ, повторённый вторым экземпляром, однажды
    разойдётся с ним.
    """
    for у in УЗЛЫ:
        if у.scenes:
            return у.key
    return УЗЛЫ[0].key if УЗЛЫ else ""

# Свои кнопки, разложенные по узлам в прошлый раз. Нужны, чтобы при
# пересборке снять СТАРЫЕ со своих мест: иначе переименованная или
# перенесённая кнопка осталась бы в дереве дважды.
_СВОИ_В_ДЕРЕВЕ = {}


def свой_сценарий(ключ, поля):
    """Собрать `Scene` из записи владельца."""
    вид = поля.get("вид") or "i2i"
    if вид not in pricing.JOBS:
        вид = "i2i"
    блок = Блок(откровенное=поля.get("строка") or "")
    # `фон="референс"` — та же обстановка, что на присланном снимке.
    # Это умолчание всего каталога: человек прислал своё фото и ждёт
    # свою комнату, а не выдуманную (правка владельца от 21.09.2026).
    return Scene(ключ, поля.get("название") or ключ, вид, блок,
                 поля.get("подпись") or "", фон="референс")


def пересобрать_свои(п=None):
    """Разложить свои кнопки владельца по веткам дерева.

    Зовётся из `правки()` при каждом изменении файла, поэтому кнопка
    появляется в боте сама, без перезапуска — как и правка названия.
    """
    п = правки() if п is None else п
    # Снять то, что клали в прошлый раз.
    for ключ, узел_ in list(_СВОИ_В_ДЕРЕВЕ.items()):
        у = _УЗЛЫ.get(узел_)
        if у:
            у.scenes = [s for s in у.scenes if s.key != ключ]
        _ПО_КЛЮЧУ.pop(ключ, None)
        _СВОИ_В_ДЕРЕВЕ.pop(ключ, None)
    for ключ, поля in (п or {}).items():
        if str(поля.get("свой") or "") != "1":
            continue
        # Ключ кнопки из кода своей записью не перебиваем: это стёрло
        # бы сценарий с промптом и подменило его строкой из браузера.
        if ключ in _ИЗ_КОДА:
            continue
        у = _УЗЛЫ.get(поля.get("узел") or "") or _УЗЛЫ.get(запасной_узел())
        if not у:
            continue
        s = свой_сценарий(ключ, поля)
        s.узел = у
        у.scenes.append(s)
        _ПО_КЛЮЧУ[ключ] = s
        _СВОИ_В_ДЕРЕВЕ[ключ] = у.key
    return _СВОИ_В_ДЕРЕВЕ


def свои():
    """Ключи кнопок, заведённых владельцем."""
    return [к for к, в in (правки() or {}).items()
            if str(в.get("свой") or "") == "1" and к not in _ИЗ_КОДА]


def узлы_для_своих():
    """Куда можно положить свою кнопку: ветки, где лежат сценарии."""
    return [{"ключ": у.key, "назв": у.назв("ru"),
             "путь": " · ".join(_путь_узла(у))}
            for у in УЗЛЫ if у.scenes or у.key in _СВОИ_В_ДЕРЕВЕ.values()]


def _путь_узла(у):
    цепь = []
    пока = у
    while пока is not None:
        цепь.append(пока.назв("ru"))
        пока = пока.родитель
    return list(reversed(цепь))


def добавить_свой(название, строка, строка_рус="", вид="i2i", узел_="",
                  подпись="", название_en=""):
    """Завести новую кнопку. Возвращает её ключ."""
    п = текущие_правки()
    ключ = свободный_ключ(название, п)
    п[ключ] = {"название": название, "название_en": название_en,
               "строка": строка, "строка_рус": строка_рус,
               "скрыт": "", "свой": "1", "вид": вид,
               "узел": узел_ or запасной_узел(), "подпись": подпись}
    данные.сохранить(п)
    перечитать()
    return ключ


def убрать_свой(ключ):
    """Удалить свою кнопку НАСОВСЕМ. Кнопку из кода так не убрать."""
    if ключ in _ИЗ_КОДА:
        return False
    п = текущие_правки()
    if ключ not in п:
...
def текущие_правки():
    return {к: dict(в) for к, в in (данные.загрузить() or {}).items()}


def свободный_ключ(название, п=None):
    """Ключ для новой кнопки: `sv_` плюс номер.

    Латиница и номер, а не транслит названия: ключ уезжает в
    `callback_data` телеграма, где 64 байта на всё, и кириллица в
    процентах съела бы их втрое быстрее.
    """
    п = текущие_правки() if п is None else п
    занято = set(п) | set(_ИЗ_КОДА)
    н = 1
    while "sv_%d" % н in занято:
        н += 1
    return "sv_%d" % н
...
def женский_снимок(ключ_сцены):
    """Каким по счёту идёт ЖЕНСКИЙ снимок в заказе. Нумерация с нуля.

    Нужно карте: она меряет по снимку, нужна ли лора плоской груди, и
    мужская грудь для этой меры «плоская» всегда (замер 24.09.2026:
    0,97 и 1,00 на двух мужских снимках). Наведи её на первый снимок
    сцены МЖ — и лору на полной силе получит ЖЕНЩИНА, за то, что рядом
    с ней стоит мужчина.

    У МЖ первым идёт мужчина (см. `порядок_фото`), значит женский —
    второй. Везде остальном женский снимок первый: у одиночных он
    единственный, у ЖЖ оба женские, а у ММ женщины нет вовсе — там
    мера всё равно ничего не решает, лоры у мужских сцен не бывает.
    """
    ч = str(ключ_сцены or "").split("_")
    return 1 if len(ч) >= 2 and ч[1] == "mf" else 0
...
class Scene:
    ...
    def подп(self, яз="ru"):
        if язык.нормальный(яз) == "en":
            ...
            if свои:
                return свои[1]
        своя = правка(self.ключ_правок, "подпись")
        if своя:
            return своя
        return ПОДПИСЬ_ПОД_ПОСТАНОВКУ.get(self.ключ_правок) or self.подпись
    ...
    def промпт(self, вид=None, место=None, сложение=None):
        ...
        # ФОН БЕРЁТСЯ С ПЕРВОГО СНИМКА И У ПАРЫ ТОЖЕ (правило владельца
        # от 24.09.2026: «если не выбрал — фон такой же, как у человека
        # на референсе»).
        #
        # Раньше пара его не брала: считалось, что двое приходят из
        # разных комнат и «сохрани ту же комнату» — противоречие.
        # Замер на карте 24.09.2026 это опроверг. Комнату держит не
        # текст, а стартовый латент, а он один — первый снимок. Тот же
        # промпт, те же снимки, менялся только denoise:
        #
        #     1.00  комната выдумана: ни мастерской, ни кирпича
        #     0.85  КИРПИЧНАЯ МАСТЕРСКАЯ С ПЕРВОГО СНИМКА, поза на
        #           месте, оба человека свои
        #     0.75  одежда не уходит, кадр в кашу
        #
        # То есть 0,85 — та же цифра, что у одиночных сцен, и ровно по
        # той же причине (см. `DENOISE_ФОН` в `bot.py`).
        фон = self.фон
    ...
    def фон_с_референса(self, место=None):
        ...
        У пары стартовый латент — ПЕРВЫЙ снимок, то есть комната
        первого человека. Это и есть ответ на вопрос «чья комната»,
        когда двое пришли из разных мест.
        """
        if место is not None and not место.референс:
            return False
...
ЖЁСТКАЯ_ОДИНОЧНАЯ = {
    ...
    #
    # ПРИЗРАК ВНИЗУ КАДРА ШЁЛ НЕ ОТСЮДА. Целый вечер 24.09.2026 ушёл
    # на подбор опоры: сила 0,8 / 1,0 / 1,15 / 1,25 / 1,4, срок 0,3 и
    # 0,6, погашенный пол на карте глубины, опора скелетом вместо
    # глубины, три редакции этого текста. Везде одно из двух: либо
    # стойка в полный рост и ОГРОМНОЕ второе тело крупным планом внизу,
    # либо чистый кадр, но она сидит.
    #
    # Причина нашлась в СТРОКЕ ВЛАДЕЛЬЦА, а не в постановке. Она
    # говорила: «after which she shows her tits CLOSE-UP. Naked, visible
    # pussies...» — и попадала в собранный промпт ТРИЖДЫ, в самое
    # начало, куда сборка смотрит внимательнее всего. Кадр в полный рост
    # и крупный план тел одновременно невозможны, и модель мирила их
    # по-своему: рисовала полный рост, а снизу пририсовывала требуемый
    # крупный план отдельным телом.
    #
    # Строка переписана на странице каталога под принятый кадр (владелец
    # разрешил: «хочешь промпт перепиши, но чтобы было как надо»).
    # После этого — четыре зерна из четырёх, кадр тот самый, никаких
    # подпорок в опоре не понадобилось.
    #
    # Вывод на будущее: если кадр упрямо выходит не тот, сперва читать
    # СОБРАННЫЙ промпт целиком и искать в нём противоречие, а уже потом
    # крутить опору. Противоречие в тексте опорой не лечится.
    "ph_below": (
...
def _пара(состав_key, состав_рус, состав_англ, расст_key, title, подпись,
    ...
    return Scene(ключ, title, вид, Блок(**поля), подпись,
                 # Комната берётся с ПЕРВОГО снимка, как у одиночных
                 # сцен: правило владельца от 24.09.2026 «если не
                 # выбрал — фон такой же, как у человека на референсе».
                 # Выбранное место это перебивает, см. `промпт`.
                 фон="референс",
...
# ПОДПИСЬ ПОД КНОПКОЙ — ЧТО ИМЕННО ПРИДЁТ.
#
# Названия кнопок владелец переписал сам («Кунилингус», «Наездница»,
# «Поставить раком»), а подписи остались прежние — от ОБЩЕЙ геометрии,
# по которой кнопки собирались до жёсткой постановки. Выходило
# «Кунилингус · Двое рядом, камера напротив»: подпись описывала не тот
# кадр, который человек получит, а тот, которого больше нет.
#
# Здесь подпись описывает ПРИНЯТЫЙ ВЛАДЕЛЬЦЕМ КАДР — тот самый, что
# висит примером под кнопкой и который бот воспроизводит опорой по
# глубине. Ключ — `ключ_правок`, то есть общий у фотографии и ролика:
# кнопка одна, и обещание у неё одно.
#
# Своя подпись владельца со страницы каталога сильнее этой: см. `подп`.
ПОДПИСЬ_ПОД_ПОСТАНОВКУ = {
    # Раздевание
    "un_close":     "Лёжа, ноги врозь, крупный кадр от бёдер",
    "un_full":      "Стоя во весь рост, лицом к камере",
    "un_back":      "Со спины, наклонилась вперёд",
    "un_three":     "На четвереньках, вполоборота, лицо к камере",
    "un_sit":       "Сидя, колени врозь, юбка задрана",
    "un_lie":       "Лёжа на спине, камера на уровне бёдер",
    # Интим
    ...
    # МЖ пара
    "pr_mf_near":   "Она на четвереньках, он сзади на коленях",
    "pr_mf_face":   "Он на спине, она сверху лицом к камере",
    "pr_mf_behind": "Она полусидит, он между её ног, языком",
    "pr_mf_pov":    "Он стоит, она на коленях перед ним",
    # ЖЖ лесби
    "pr_ff_near":   "Одна сидит на краю кровати, вторая на коленях",
    "pr_ff_face":   "Одна лежит на боку, вторая сзади, языком",
    "pr_ff_behind": "Обе на четвереньках, камера сзади и низко",
    "pr_ff_close":  "Одна стоит нагнувшись, вторая сзади, вид сбоку",
    "pr_ff_pov":    "Одна полусидит на подушках, вторая между её ног",
}
...
ЖЁСТКАЯ_ПОСТАНОВКА = {
    ...
    ("ff", "face"): (
        "Both women are completely naked, bare skin, nothing on either "
        "of them. THE BLONDE lies on her front at the LEFT of the "
        "frame, rolled onto her side so that her hip and buttocks turn "
        "up toward the camera: her head rests low at the left edge, her "
        "face turned to the lens and clearly visible, and one of her "
        "own hands reaches back to spread her buttock open. Her raised "
        "buttocks and her vulva between them are at the centre of the "
        "picture. THE DARK-HAIRED ONE comes in from the RIGHT with her "
        "upper body low and her head at the same height, her face in "
        "profile pressed between the blonde's buttocks from behind, her "
        "tongue on the blonde's vulva. Both heads are low, one at each "
        "end of the frame, the hips between them; the two bodies do not "
    ...
    ("ff", "close"): (
        ...
        "blonde's chest is bare skin with her own nipples in plain "
        "view, her hips are bare skin and her back is bare skin with no "
        "strap and no band on it. The dark-haired one's chest is bare "
        "skin with her own nipples in plain view and her hips are bare "
        "skin.. THE BLONDE STANDS on the floor at the LEFT with her "
        "legs apart and HAS BENT FAR FORWARD from the hips, her hands "
        "on her own knees, her back horizontal and her buttocks raised "
        "high at the RIGHT, her head turned back over her shoulder so "
        "her whole face is to the lens and her own bare breasts hang "
        "free below her chest. THE DARK-HAIRED ONE IS DOWN ON THE FLOOR "
        "BEHIND HER at the RIGHT, sitting back on her heels and "
        "crouched low with her head below the blonde's raised buttocks "
        "and tilted up into them: HER MOUTH IS PRESSED ON THE BLONDE'S "
        "VULVA from behind, her own face seen in profile. One woman "
        "stands high, the other is low on the floor."),
    ...
    ("ff", "behind"): (
        "THE CAMERA IS BEHIND THEM, low down. THE BLONDE has nothing on "
        "her: her chest is bare skin with her own nipples in plain view "
        "and her hips are bare skin. THE DARK-HAIRED ONE has nothing on "
        "her either: her chest is bare skin with her own nipples in "
        "plain view, and HER HIPS AND BUTTOCKS ARE BARE SKIN — no "
        "panties on her.. TWO NAKED WOMEN ARE ON ALL FOURS SIDE BY SIDE "
        "on the bed, both facing away from the camera, their bodies "
        "PARALLEL and NOT TOUCHING each other at all, with dark bedding "
        "visible in the gap between them. THE BLONDE is on the LEFT and "
        "THE DARK-HAIRED ONE is on the RIGHT. Both have their knees "
        "apart, their backs arched down and their bare buttocks raised "
        "toward the lens, so that BOTH OF THEIR BARE VULVAS SHOW "
        "BETWEEN THEIR THIGHS FROM BEHIND, side by side across the "
        "middle of the picture. Each has turned her head back over her "
        "own shoulder and both faces are to the camera, smiling."),
...
# ЧТО ПРИШЛО ИЗ КОДА. Снимок до того, как в дерево легли свои кнопки
# владельца. По нему отличается кнопка, которую можно удалить
# насовсем, от кнопки, которую можно только спрятать.
_ИЗ_КОДА = frozenset(_ПО_КЛЮЧУ) | frozenset(_УЗЛЫ)

# Первое чтение правок уже было при загрузке модуля — тогда класть
# свои кнопки было некуда. Раскладываем сейчас.
перечитать()
...
# У КАКИХ КНОПОК ЕСТЬ ОПОРА — карта глубины с принятого кадра.
#
# Владелец отобрал 21 кадр и потребовал те же позы «в 10 из 10
# случаев». Жёсткая постановка (дословный текст принятого кадра) даёт
# шесть-семь: сборка читает около первой тысячи знаков и дальше
# сочиняет. Опора задаёт геометрию картинкой, и потолка текста у неё
# нет — см. «ОПОРА» в `gpu/panel.py`.
#
# Список ровно по файлам в `/home/ubuntu/ГЛУБИНА` на карте. Кнопка без
# принятого кадра опоры не получает: приложить чужую значило бы выдать
...
def опора(ключ_сцены):
    """Ключ карты глубины для сценария, или пустое.

    Ролик и его фотография — одна кнопка с двумя ключами, и опора у них
    обязана быть одна: ролик считается двумя проходами, и первый из них
    эта самая фотография. Поэтому «ac_close» спрашивает опору у
    «ph_close», а «pr_mf_near» — у «pf_mf_near», ровно как жёсткая
    постановка в `_ЖЁСТКО_ДЛЯ`.
    """
    к = str(ключ_сцены or "")
    if к in С_ОПОРОЙ:
        return к
    if к.startswith("ac_"):
        к = "ph_" + к[3:]
    elif к.startswith("pr_"):
        к = "pf_" + к[3:]
    return к if к in С_ОПОРОЙ else ""
...
def дерево():
    ...
    def сцена(s):
        ...
        return {
            ...
            # Страница показывает ту подпись, которую видит КЛИЕНТ, а не
            # умолчание из кода: иначе владелец правит название, глядя на
            # описание кнопки, которой больше нет.
            "подпись": s.подп(),
            "пара": s.пара,
            "коины": s.coins,
            ...
            "скрыт": скрыт(s.key),
            # Кнопку, заведённую владельцем, можно удалить насовсем —
            # у неё за спиной только его строка. У кнопки из кода там
            # три тысячи знаков промпта, и её можно лишь спрятать.
            "свой": s.key in свои_ключи,
            "вид": s.job,
