...
class Scene:
    ...
    def действие(self, яз="ru"):
        ...
        У английского есть запасной путь: если владелец не написал
        короткую английскую подпись, показывается строка для модели.
        Она длинная и техничная, зато правдивая — а пустая строка на
        экране оплаты не говорит ничего.
        """
        if язык.нормальный(яз) == "en":
            return правка(self.key, "строка_eng") or self.откровенное
...
ВИДИМЫЕ = [п for п in CATEGORIES if п.scenes]
...
def дерево():
    ...
    def сцена(s):
        ...
        return {
            ...
            "строка_eng": своё.get("строка_eng", ""),
...
def известные_ключи():
    ...
    return set(_ПО_КЛЮЧУ) | {prompts.ОБЯЗАТЕЛЬНОЕ_КЛЮЧ}
