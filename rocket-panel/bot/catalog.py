...
## Дерево (решение владельца 21.09.2026)
    ...
    РАЗДЕТЬ (фото)   -> Соло -> Раздевание · Интим
                     -> Групповое (все варианты сразу)
    ВИДЕО            -> Соло · Групповое
    СВОЙ ПРОМПТ      -> Фото · Видео

Уровней стало три, и они неровные: у «Соло» внутри есть ещё разбивка, у
«Группового» нет. Поэтому узел дерева один и тот же класс `Узел`: у
него либо дети, либо сценарии. Городить отдельные «раздел» и
«подраздел» с жёсткой глубиной — значит переписывать навигацию при
каждой правке дерева.

## Один текст — две цены

Строку владельца пишут один раз, а работает она дважды: в «Видео ·
Соло» роликом за 5 😏 и в «Раздеть · Соло · Интим» фотографией за 1 😏.
...
def скрыт(ключ):
    """Владелец убрал пункт из бота. Одно поле на все уровни: у узлов,
    сценариев и мест ключи разные и не пересекаются."""
...
class Scene:
    ...
    def промпт(self, вид=None, место=None):
    ...
    def prompt_фото(self, место=None):
    ...
    def button(self, яз="ru", уточнить=False):
        ...
        return f"{имя_} · {self.coins} {pricing.СИМВОЛ}"
...
ПАРЫ_ВИДЕО = [
    _пара(ключ, рус, англ, rk, rt, rp, поля)
...
ПАРЫ_ФОТО = [
    _пара(ключ, рус, англ, rk, rt, rp, поля, вид="i2i", приставка="pf",
          зеркало=f"pr_{ключ}_{rk}")
    for ключ, _t, рус, англ in СОСТАВЫ
    for rk, rt, rp, поля in РАССТАНОВКИ
]
