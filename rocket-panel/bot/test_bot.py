...
class Цены(unittest.TestCase):
    ...
    def test_печатный_прайс_показывает_всё(self):
        """`python3 pricing.py` — то, чем прайс читают глазами: на него
        ссылается README вместо своей таблицы.

        Тест сторожит ровно одно: печать не умалчивает. Забытая в ней
        ступень хуже отсутствия команды — владелец сверит цены,
        недостающей не увидит и решит, что её нет вовсе.
        """
        текст = "\n".join(pricing.прайс_строками())
        for п in pricing.PACKS:
            self.assertIn(str(п["rub"]), текст, f"{п['id']}: цены нет в печати")
        for k, j in pricing.JOBS.items():
            self.assertIn(j.title, текст, f"{k}: вида нет в печати")

    def test_печатный_прайс_не_обещает_четверть_везде(self):
        """Скидка к рынку у нас РАЗНАЯ — от 30 % на входном до 7 % на
        крупном (см. `test_каждый_пакет_дешевле_конкурента`). Печать
        обязана называть её у каждой ступени своим числом.

        Прежний README обещал «25 % на каждом пакете» и на седьмой
        ступени врал втрое. Поэтому единой цифры скидки в печати нет
        вовсе: её неоткуда взять правдиво.
        """
        for п in pricing.PACKS:
            выгода = round((1 - pricing.vs_market(п)) * 100)
            self.assertIn(f"дешевле рынка на {выгода:>2} %",
                          "\n".join(pricing.прайс_строками()),
                          f"{п['id']}: своей выгоды в печати нет")
...
class ПравкиВладельца(unittest.TestCase):
    ...
    def test_нижнее_меню_по_решению_владельца(self):
        """Пять кнопок со значками: создать, файлы, кабинет, пополнить
        и поддержка. Каждая обязана быть узнаваемой ботом - иначе
        нажатие падает в «сначала выбери, что делаем»."""
        import ui, bot
        подписи = [к["text"] for ряд in ui.нижнее("ru")["keyboard"] for к in ряд]
        self.assertEqual(подписи,
                         ["🫦 Создать", "🗂️ Файлы", "🛠️ Кабинет",
                          "🩷 Пополнить", "💬 Поддержка"])
        for п in подписи:
    ...
    def test_пакеты_по_двое_в_ряд_и_со_скидкой(self):
        ...
        # Снизу два непакетных ряда: «Безлимит на месяц» и «Назад».
        # Безлимит стоит здесь же, но пакетом не является — это месяц
        # вместо штук, и считать его ступенью коинов нельзя.
        пакеты = [р for р in клава[:-1]
                  if not any(b.get("callback_data") == "m:unlim" for b in р)]
        for ряд in пакеты:
        ...
        # А сам безлимит на экране быть обязан: это второй товар, и
        # спрятать его значит не продать.
        все_данные = [b.get("callback_data") for ряд in клава for b in ряд]
        self.assertIn("m:unlim", все_данные, "кнопка безлимита пропала")
...
class ОткудаБерётсяФон(unittest.TestCase):
    ...
    def test_у_пары_комната_тоже_с_первого_снимка(self):
        """Правило владельца от 24.09.2026: «если не выбрал - фон такой
        же, как у человека на референсе». Пар это касается тоже.

        Раньше пара комнату не брала: считалось, что двое приходят из
        разных мест и «та же комната» - противоречие, какая из двух.
        Замер на карте 24.09.2026 это снял: комнату держит не текст, а
        СТАРТОВЫЙ ЛАТЕНТ, а он один - первый снимок. При denoise 0,85
        кирпичная мастерская с первого снимка осталась на месте, поза и
        оба человека тоже.
        """
        пара = [s for s in catalog.все_сценарии() if s.пара]
        self.assertTrue(пара)
        for s in пара:
            self.assertTrue(s.фон_с_референса(), s.key)
            # У ролика комнату решает ПЕРВЫЙ проход - фотография, с
            # которой он начинается. В тексте самого ролика про
            # обстановку сказано другое: «та же комната, что в первом
            # кадре».
            self.assertIn("COPIED from the FIRST reference",
                          s.prompt_фото(места.КАК_НА_ФОТО), s.key)

    def test_выбранное_место_сильнее_и_у_пары(self):
        """Выбрал душ - значит душ, а не своя комната."""
        s = catalog.scene("pf_mf_near")
        душ = места.место("sc_shower")
        self.assertFalse(s.фон_с_референса(душ))
        self.assertNotIn("COPIED from the FIRST reference",
                         s.промпт(место=душ))
    ...
    def test_ответ_совпадает_с_промптом(self):
        ...
        Примета в тексте у одиночной сцены и у пары разная: у первой
        целый абзац «EDIT THIS PHOTOGRAPH», у второй одна строка - на
        абзац у парного промпта нет длины.
        """
        for s in catalog.все_сценарии():
            for м in (None, места.КАК_НА_ФОТО, места.место("sc_shower")):
                if prompts.семейство(s.job) != "i2i":
                    continue
                примета = ("COPIED from the FIRST reference" if s.пара
                           else "EDIT THIS PHOTOGRAPH")
                self.assertEqual(s.фон_с_референса(м),
                                 примета in s.prompt_фото(м),
                                 f"{s.key} / {м and м.key}")
...
class СложениеБуквальноеИПовторённое(unittest.TestCase):
    ...
    Проверяем на ЯВНОМ выборе «стройная»: с 24.09.2026 умолчание —
    «как на фото», и оно нарочно не называет размера (см.
    ВыборФигуры.test_умолчание_как_на_фото). Механизм же — буквальные
    слова, повтор в конце, адресность у пары — остался тот самый."""

    def test_в_промпте_буквальные_слова(self):
        p = catalog.scene("un_full").промпт(сложение="стройная")
    ...
    def test_у_пары_сложение_адресное(self):
        ...
        p = catalog.scene("pf_mf_near").промпт(сложение="стройная")
        self.assertIn("She is petite with a small, almost flat chest", p)

    def test_у_пары_умолчание_тоже_доезжает(self):
        """У парной сборки жёсткий лимит длины, и строка про сложение
...
class ВыборФигуры(unittest.TestCase):
    ...
    С 24.09.2026 умолчание другое — «как на фото»: оно не описывает
    фигуру, а запрещает её сочинять. Выбор руками остался для тех, кто
    хочет не себя, а красивее."""
    ...
    def test_умолчание_как_на_фото(self):
        """Человек, который ничего не выбирал, должен получить СЕБЯ, а
        не модельную фигуру. До 24.09.2026 умолчанием была «стройная»,
        и она перебивала референс."""
    ...
    def test_умолчание_не_запрещает_полноту(self):
        ...
        — и плоской, и пышной. Пышная клиентка получала чужую грудь."""
        н = prompts.негатив(catalog.scene("un_full").промпт())
        self.assertNotIn("busty", н)
        self.assertNotIn("curvy", н)
        self.assertNotIn("milf", н)

    def test_стройная_сама_запрещает_полноту(self):
        """Запрет не выкинут, а переехал туда, где он к месту: его
        просит только явный выбор «стройная»."""
        н = prompts.негатив(catalog.scene("un_full").промпт(сложение="стройная"))
        self.assertIn("busty", н)
        self.assertIn("large breasts", н)
...
class НижнееМеню(unittest.TestCase):
    ...
    def test_подписи_именно_те_что_просил_владелец(self):
        ...
        Поддержка добавлена пятой 25.09.2026 по его же просьбе и стоит
        ОТДЕЛЬНОЙ СТРОКОЙ: её ищут в беде и ищут глазами, а в паре с
        «Пополнить» она читалась бы как ещё один денежный пункт."""
        import ui
        ряды = ui.нижнее("ru")["keyboard"]
        было = [к["text"] for ряд in ряды for к in ряд]
        self.assertEqual(было, ["🫦 Создать", "🗂️ Файлы",
                                "🛠️ Кабинет", "🩷 Пополнить",
                                "💬 Поддержка"])
        self.assertEqual(ряды[-1], [{"text": "💬 Поддержка"}])
...
class КартаПросыпаетсяПодЗапрос(unittest.TestCase):
    """Карта гасится в простое: круглые сутки она стоит около 780 $ в
    месяц, а работает несколько часов в день. Первый за вечер человек
    будит её сам, и это главный риск схемы: он ждёт минуты и может не
    дождаться вовсе, если свободного железа в регионе нет."""

    def setUp(self):
        os.environ.setdefault("ROCKET_BOT_TOKEN", "test")
        import bot, железо
        self.bot, self.железо = bot, железо

    def test_деньги_списываются_после_карты_а_не_до(self):
        """Списать коины, а потом выяснить, что поднять нечего, значит
        взять деньги за ожидание."""
        import inspect
        и = inspect.getsource(self.bot.run_job)
        карта = и.find("железо.нужна")
        деньги = и.find("store.spend")
        self.assertGreater(карта, 0, "подъём карты из run_job пропал")
        self.assertGreater(деньги, 0)
        self.assertLess(карта, деньги, "карта поднимается ПОСЛЕ списания")

    def test_отказ_не_оставляет_человека_занятым(self):
        """Иначе после неудачного подъёма бот считал бы его вечно
        занятым и не пускал к следующей кнопке."""
        import inspect
        и = inspect.getsource(self.bot.run_job)
        хвост = и[:и.find("store.spend")]
        self.assertIn("busy.discard(u)", хвост)

    def test_нет_железа_и_поломка_это_разные_ответы(self):
        """«Попробуй через десять минут» и «мы уже смотрим» человек
        читает по-разному, и в первом случае он вернётся."""
        self.assertIn("ген.карта_не_встала", язык.СТРОКИ)
        self.assertIn("ген.карта_сломалась", язык.СТРОКИ)
        self.assertIn("десять минут", язык.СТРОКИ["ген.карта_не_встала"]["ru"])

    def test_человеку_сказано_что_коины_целы(self):
        """Молчание тут читается как «списали и потеряли»."""
        for ключ in ("ген.бужу", "ген.карта_не_встала", "ген.карта_сломалась"):
            self.assertIn("оин", язык.СТРОКИ[ключ]["ru"], ключ)

    def test_поднятая_карта_не_показывает_экран_ожидания(self):
        """Днём карта работает, и лишний экран на каждую кнопку это
        мусор в переписке."""
        self.assertTrue(hasattr(self.железо, "нужна_проверена"))

    def test_адрес_карты_перечитывается_а_не_кэшируется(self):
        """Туннель выдаёт НОВЫЙ адрес при каждом подъёме. Если бот
        держит его с момента запуска, то после пробуждения он стучится
        по мёртвому адресу, и вылечить это можно только перезапуском
        бота, то есть обрывом чужого запроса."""
        import gpu as gpu_mod
        self.assertIsInstance(gpu_mod.Gpu.base, property)

    def test_адрес_берётся_из_файла(self):
        import tempfile, gpu as gpu_mod
        д = tempfile.mkdtemp()
        ф = os.path.join(д, "адрес.txt")
        старый = gpu_mod.ФАЙЛ_АДРЕСА
        try:
            gpu_mod.ФАЙЛ_АДРЕСА = ф
            g = gpu_mod.Gpu("https://из-окружения", "rocket", "x")
            self.assertEqual(g.base, "https://из-окружения")
            with open(ф, "w", encoding="utf-8") as о:
                о.write("https://из-файла/\n")
...
class ЧужаяРаботаНеУбивается(unittest.TestCase):
    """Самое опасное место во всём хозяйстве.

    Ответ «карта мертва» ведёт к перезапуску служб на ней. Ошибочный
    ответ убивает чужую ОПЛАЧЕННУЮ работу. Разыграть это на запуске
    проще простого: один заказал десятисекундный ролик, карта считает
    его двадцать минут и отвечает медленно; второй нажал кнопку; бот
    спросил «жива ли карта», получил «нет» — и перезапустил её вместе
    с чужим роликом."""
    ...
    def test_занятая_карта_это_отдельный_ответ(self):
        """«Занята» и «сломалась» человек читает по-разному: в первом
        случае он подождёт две минуты и вернётся."""
        import inspect
        self.assertIn('"занята"', inspect.getsource(self.ж.нужна))
        self.assertIn("ген.карта_занята", язык.СТРОКИ)

    def test_человеку_сказано_что_коины_целы(self):
        т = язык.СТРОКИ["ген.карта_занята"]["ru"]
        self.assertIn("оин", т)
        self.assertTrue("подожд" in т.lower() or "ещё раз" in т.lower())

    def test_бот_различает_три_причины(self):
        import inspect
        и = inspect.getsource(__import__("bot").run_job)
...
class ПостояннаяКартаНеБудится(unittest.TestCase):
    ...
    работает, её никто не создаёт и не удаляет.

    Попытка «поднять» постоянную карту означала бы создать ВТОРУЮ
    машину за наши деньги — при том что первая работает и оплачена.
    """

    def setUp(self):
        import железо
        self.было = железо.ПОСТОЯННАЯ
        железо.ПОСТОЯННАЯ = True
        self.железо = железо

    def tearDown(self):
        self.железо.ПОСТОЯННАЯ = self.было

    class _Карта:
        def __init__(self, жива):
            self.настроена = True
            self._жива = жива
            self.звали = 0
    ...
    def test_живая_карта_принимается_сразу(self):
        к = self._Карта(True)
        ок, что = self.железо.нужна(к)
        self.assertTrue(ок)
        self.assertEqual(что, "")

    def test_молчащую_карту_не_поднимаем(self):
        """Молчит — это поломка, а не «спит». Создавать вторую машину
        нельзя ни при каких условиях."""
        к = self._Карта(False)
        ок, что = self.железо.нужна(к)
        self.assertFalse(ок)
        self.assertEqual(что, "ошибка")

    def test_управление_картой_выключено(self):
        """`включено()` — это про почасовую карту. В постоянном режиме
