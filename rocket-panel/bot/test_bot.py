...
class ПарныйПромптКороткий(unittest.TestCase):
    ...
    def test_место_подставляется_короткой_формой(self):
        ...
        # Сцена без жёсткой постановки: на «Раком» теперь стоит
        # дословный текст принятого кадра, и мерить на нём потолок
        # длины бессмысленно (см. тест выше).
        p = catalog.scene("pf_mf_face").промпт(место=м)
