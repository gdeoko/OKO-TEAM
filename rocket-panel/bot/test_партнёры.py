...
class ПоддержкаИПриглашение(unittest.TestCase):
    ...
    def test_кнопка_поддержки_есть_в_кабинете(self):
        ...
        эквайринга."""
        self.assertIn("https://t.me/AMBERRYsupport_bot?start=m",
                      self._кн(self.ui.меню_кабинета("ru")))

    def test_поддержка_ведёт_в_отдельный_бот(self):
        """Схема руководителя: поддержка - отдельный бот с группой
        менеджеров. Кнопка - ссылка, у копии партнёра со своей меткой."""
        self.assertIn("https://t.me/AMBERRYsupport_bot?start=m",
                      self._кн(self.ui.меню_поддержки("ru")))
        with mock.patch.dict(os.environ, {"AMBERRY_PARTNER": "555"}):
            self.assertIn("https://t.me/AMBERRYsupport_bot?start=p555",
                          self._кн(self.ui.меню_поддержки("ru")))
...
class КлавиатураДиалога(unittest.TestCase):
    ...
    def test_кнопки_диалога_узнаются_ботом(self):
        """Старая клавиатура у кого-то ещё висит: нажатие обязано
        вернуть обычное меню, а не уйти строкой в модель."""
        import bot
