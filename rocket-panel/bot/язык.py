...
def друзей(n, яз):
    ...
    return склонить(n, "друг", "друга", "друзей")
...
СТРОКИ = {
    ...
    "гл.шапка": {
        ...
        #
        # ТЕКСТ ПОВТОРЯЕТ ПОДПИСЬ НА ЗАСТАВКЕ, и повторяет нарочно.
        # Картинка и строка под ней стоят в одном сообщении: когда они
        # обещают разное («оживляет» на кадре, «раздень и оживи» в
        # тексте), человек читает это как небрежность. Меняешь одно -
        # меняй и второе, лозунг у продукта один.
        "ru": "{привет}AMBERRY раздевает и оживляет любое фото.",
        "en": "{привет}AMBERRY undresses any photo and brings it to life.",
    },
    ...
    # описывало ТОВАР, а не выгоду, и в меню читалось как настройка.
    # «Заработай с нами» говорит, зачем её нажимать.
    "фр.кнопка": {"ru": "Заработай с нами", "en": "Earn with us"},
    "фр.экран": {
    ...
    # КЛИЕНТУ НЕ ПОКАЗЫВАЮТ ВНУТРЕННОСТИ. Здесь стояло «{почему}», и
    # человеку прилетало «[Errno 13] Permission denied:
    # /srv/amberry/снимки/...». Он не знает, что это, чинить ему нечего,
    # и выглядит это как сломанный сервис. Техническая причина уходит в
    # журнал и в базу, человеку - что делать дальше.
    "фото.не_приняла": {"ru": "Снимок не принялся. Пришли его ещё раз, "
                              "пожалуйста - обычно со второго раза проходит.",
                        "en": "The photo did not go through. Please send it "
    ...
    "ген.осечка": {
        "ru": "Не получилось с первого раза. Коины вернула, "
              "на счету <b>{баланс}</b>. Попробуй ещё раз.",
        "en": "It did not work the first time. Coins refunded, balance "
              "<b>{баланс}</b>. Please try again.",
    },
    "ген.сломалось": {
        "ru": "Что-то сломалось у меня. Коины вернула - <b>{баланс}</b>.",
        "en": "Something broke on my side. Coins refunded - <b>{баланс}</b>.",
    },
    "ген.кадр_не_ушёл": {"ru": "Кадр не ушёл в работу. Попробуй ещё раз.",
                         "en": "The frame did not go through. Please try again."},
    ...
    # ДОСКА БЕЗ ИМЁН. Почему именно так - в `store.доска_рефереров`:
    # назвать вслух того, кто пользуется ботом 18+, мы не вправе.
    # Поэтому в строках нет места под ник, и подставить его некуда даже
    # по ошибке.
    "доска.заголовок": {
        "ru": "<b>Кто позвал больше всех</b>",
        "en": "<b>Who invited the most</b>",
    },
    "доска.строка": {"ru": "{место}. {сколько} {друзей}",
                     "en": "{место}. {сколько} {друзей}"},
    "доска.моё": {
        "ru": "Ты на <b>{место} месте</b>: {сколько} {друзей}.",
        # «#2», а не «2nd»: порядковые окончания в английском требуют
        # своей таблицы (1st, 2nd, 3rd, 11th), а решётка читается
        # одинаково на любом числе.
        "en": "You are <b>#{место}</b>: {сколько} {друзей}.",
    },
    "доска.меня_нет": {
        "ru": "Позовёшь первого - встанешь в этот список.",
        "en": "Invite your first and you are on this list.",
    },
    "доска.пусто": {
        "ru": "Пока не позвал никто. Первое место свободно.",
        "en": "Nobody has invited anyone yet. First place is open.",
    },
    "доска.без_имён": {
        "ru": "Имён в списке нет - ни твоего, ни чужих.",
        "en": "No names on this list - not yours, not anyone's.",
    },
    ...
    # ДОКУМЕНТЫ - ССЫЛКОЙ, А НЕ ТЕКСТОМ В СООБЩЕНИИ.
    #
    # Здесь лежали три абзаца условий, и владелец их снял: «внутри
    # бота текст - так не пойдёт». Он прав, и не только про красоту.
    # Условия длиннее экрана телефона никто не читает, они отодвигают
    # меню вниз, а поправить их задним числом нельзя - у каждого, кто
    # уже нажал старт, в переписке навсегда останется старая
    # редакция. Страница правится в одном месте и обновляется у всех.
    #
    # Ссылка ВСТРОЕНА В ТЕКСТ, а не только висит кнопкой: кнопку под
    # сообщением легко проскочить взглядом, а подчёркнутые слова в
    # самой фразе читаются вместе с ней. Кнопка при этом остаётся -
    # это два разных способа нажать одно и то же.
    #
    # {ссылка} подставляет `bot.py`. Её может не быть (страница ещё не
    # создана) - тогда берётся вариант без ссылки, и бот показывает
    # условия текстом по кнопке, как раньше.
    # ОПЛАТА СЧЁТОМ - единственный способ, как только подключена касса.
    "кн.счёт": {"ru": "Оплатить", "en": "Pay"},
    "оп.счёт_руб": {
        "ru": "Счёт на {сумма} руб. Оплати по кнопке и вернись сюда - "
              "нажми «Я оплатил», и всё зачислится.",
        "en": "Invoice for {сумма} RUB. Pay via the button, come back and "
              "press «I have paid» - it will be credited."},
    "оп.касса_нет": {"ru": "Касса не отвечает", "en": "Checkout is silent"},
    "оп.касса_не_настроена": {
        "ru": "Оплата сейчас недоступна: {почему}. Мы уже это видим.",
        "en": "Payment is unavailable right now: {почему}. We can see it."},
    "усл.коротко": {
        "ru": "<b>Тематика Adult. Только 18+.</b>\n\n"
              "Пользуясь ботом, ты принимаешь "
              "<a href=\"{ссылка}\">условия, требования и оферту</a>.",
        "en": "<b>Adult content. 18+ only.</b>\n\n"
              "By using this bot you accept the "
              "<a href=\"{ссылка}\">terms, requirements and offer</a>.",
    },
    "усл.коротко_без_ссылки": {
        "ru": "<b>Тематика Adult. Только 18+.</b>\n\n"
              "Пользуясь ботом, ты принимаешь условия, требования и "
              "оферту - они ниже.",
        "en": "<b>Adult content. 18+ only.</b>\n\n"
              "By using this bot you accept the terms, requirements and "
              "offer - they are below.",
    },
    "усл.текст": {
        "ru": (
