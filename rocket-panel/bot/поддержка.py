...
Статус - значком в названии ветки, его видно в списке без открытия:
...
# Значок статуса в названии ветки.
ЗНАЧОК = {"wait": "🔴", "work": "🟢", "closed": "✅"}

# Розовый - ближайший к бренду из шести цветов, что разрешает телеграм.
ЦВЕТ_ВЕТКИ = 16749490
...
СЛУЖЕБНЫЕ = (
    ("ветка.правила", "📌 Как отвечать"),
    ("ветка.шаблоны", "📝 Шаблоны ответов"),
    ("ветка.новые", "🔔 Новые обращения"),
)
ОБЩИЙ = "💬 Общий чат"
...
class Поддержка:
    ...
    def название(self, u, состояние):
        ...
        return ("%s %s · %s" % (ЗНАЧОК[состояние], имя, ник))[:128]
    ...
    def завести_ветку(self, u, кто):
        ...
        о = self.tg("createForumTopic", chat_id=self.группа,
                    name=self.название(u, "wait"), icon_color=ЦВЕТ_ВЕТКИ)
        if not о.get("ok"):
            return None
        ветка = о["result"]["message_thread_id"]
