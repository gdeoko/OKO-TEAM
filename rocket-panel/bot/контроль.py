...
def ключ():
    """Сначала бесплатный, платный — только запасным.

    Платный ключ разрешён владельцем 23.09.2026 отдельно и именно для
    приёмки: бесплатная квота Gemini кончается за день, а без неё
    приёмка молча пропускает брак — то есть не работает. Порядок
    сохранён прежний: бесплатный тратится первым, платный подхватывает,
    когда тот кончился.

    Разрешение точечное. На генерации, апскейлы и прочее оно НЕ
    распространяется — там правило прежнее: за платное спрашивают.
    """
    return (os.environ.get("GEMINI_KEY_FREE")
            or os.environ.get("AMBERRY_QC_KEY")
            or os.environ.get("GEMINI_KEY_PAID") or "").strip()
