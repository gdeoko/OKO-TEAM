...
def послать_заставку(chat, подпись, kb=None):
    """Заставка с подписью и меню. Нет файла — просто текст.

    Заставка это украшение, и падать из-за неё нельзя: не нашлась,
...
def on_callback(cb):
    ...
    if data == "m:unlim":
        ...
        if до > time.time():
            ...
            return
        send(chat, t("безл.экран", я, быстрых=безлимит.БЫСТРЫХ,
