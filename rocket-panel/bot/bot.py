...
ДЕЙСТВИЯ_НИЗА = {
    ...
    "низ.поддержка": lambda chat, u: экран(
        chat, "sup", t("подд.экран", яз(u)), ui.меню_поддержки(яз(u))),
}
...
def ответить_клиенту(chat, админ, кому, текст):
    ...
    if о.get("ok"):
        ...
        отвечают.pop(админ, None)
        send(chat, "Ответ ушёл. Обращение закрыто.",
             ui.меню_обращения(кому, закрыто=True))
        return True
    # Человек мог выгнать бота - тогда отвечать больше некуда, и
    # висеть в режиме ответа незачем.
    отвечают.pop(админ, None)
    send(chat, "Не доставлено: человек закрыл бота или заблокировал его. "
               "Ответ не записан.")
    return False
...
# КАКОЙ АДМИН КОМУ СЕЙЧАС ОТВЕЧАЕТ. В памяти, а не в базе: состояние
# живёт минуту и восстанавливается одним нажатием «Ответить» под тем
# же обращением - оно никуда из переписки не девается.
отвечают = {}
...
def on_callback(cb):
    ...
    if data.startswith("sup:"):
        # Отвечать может только админ. Кнопка приходит в его личный
        # чат, но callback можно подделать - проверяем, а не верим.
        if u not in ADMINS:
            answer(cid); return
        ...
        if что == "rep":
            отвечают[u] = кому
            answer(cid, "Пишите ответ")
            send(chat, "Напишите ответ для <code>%d</code> следующим "
                       "сообщением. Он уйдёт от имени бота.\n\n"
                       "Чтобы передумать - /otmena" % кому)
            return
        if что == "done":
            store.поддержка_диалог(кому)      # помечает прочитанным
            отвечают.pop(u, None)
            answer(cid, "Закрыто")
            try:
    ...
    if data == "m:sup":
        answer(cid)
        экран(chat, "sup", t("подд.экран", я), ui.меню_поддержки(я))
        return
...
def on_update(up):
    ...
    # ОТВЕТ АДМИНА ПЕРЕХВАТЫВАЕТСЯ ДО ВСЕГО ОСТАЛЬНОГО.
    #
    # Иначе его текст уйдёт в поддержку как обычное сообщение - то
    # есть админ напишет ответ сам себе, а клиент не получит ничего и
    # будет ждать.
    if u in отвечают:
        кому = отвечают[u]
        if text in ("/otmena", "/отмена", "/cancel"):
            отвечают.pop(u, None)
            send(chat, "Отменено."); return
        if text.startswith("/"):
            # Команду выполняем как обычно, но режим ответа снимаем:
            # человек явно передумал отвечать.
            отвечают.pop(u, None)
        else:
            ответить_клиенту(chat, u, кому, text); return
