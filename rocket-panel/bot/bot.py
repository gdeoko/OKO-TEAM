...
# Сколько строк в доске рефереров. Пять, а не десять: доска стоит под
# готовым приглашением, и длинный список отодвигает саму кнопку «позвать»
# за край экрана телефона - то есть мешает ровно тому, ради чего заведён.
ДОСКА_МЕСТ = 5
...
def позвать_друзей(chat, u):
    ...
    # ДОСКА - ЧАСТЬЮ ЭТОГО ЖЕ ЭКРАНА, а не отдельной кнопкой.
    #
    # Отдельную кнопку надо сперва захотеть нажать, а хотеть человеку
    # пока нечего: он не знает ни что доска есть, ни какое у него место.
    # Здесь же он уже стоит с решением «звать или не звать», и своё
    # место - последний довод, который на это решение влияет.
    экран(chat, "ref", "\n\n".join([
        t("пригл.заголовок", я),
        ...
        ui.текст_доски(store.доска_рефереров(ДОСКА_МЕСТ, tg_id=u), я),
...
def on_text(chat, u, text):
    ...
    if not st:
        # Вольная строка вне всякого сценария - чаще всего вопрос. Здесь
        # на него никто не ответит, поэтому сразу даём дорогу в бот
        # поддержки, а не инструкцию по меню: человек, написавший «не
        # пришла оплата», ищет людей, а не кнопки.
        я = яз(u)
        send(chat, t("подд.вольная", я), ui.меню_поддержки(я))
        return
    kind = st["kind"]
    job = pricing.job(kind)
...
# --- ПОДДЕРЖКА -------------------------------------------------------
#
# Поддержка - ОТДЕЛЬНЫЙ бот @AMBERRYsupport_bot и группа менеджеров, где
# у каждого клиента своя ветка (`поддержка.py`, схема руководителя от
# 25.09.2026). Здесь от неё осталась одна кнопка-ссылка: в основном
# боте никто не отвечает, и переписка с клиентами тут не ведётся.


def открыть_поддержку(chat, u):
    """Экран поддержки с кнопкой в бот поддержки.

    Висящий сценарий снимаем: человек выбрал «Раздеть», передумал и
    пошёл в поддержку - следующая его строка не должна уйти в модель
    как промпт и списать коины.
    """
    waiting.pop(u, None)
    я = яз(u)
...
def on_callback(cb):
    ...
    if data == "cl:done" or data.startswith("sup:"):
        # Кнопки прежней поддержки внутри бота. Телеграм хранит
        # переписку вечно, и старая кнопка обязана вести туда, где
        # поддержка живёт теперь, а не молчать.
        answer(cid)
...
def on_update(up):
    ...
    # Кнопки «Завершить диалог» и «Отойти» от прежней поддержки внутри
    # бота. Нижняя клавиатура живёт в чате, у кого-то она ещё висит:
    # нажатие возвращает обычное меню, а не уходит строкой в модель.
    if text in ПОДПИСИ_ЗАВЕРШИТЬ or text in ПОДПИСИ_ОТОЙТИ:
        завести()
        send(chat, t("низ.обновлено", яз(u)), ui.нижнее(яз(u)))
        return
