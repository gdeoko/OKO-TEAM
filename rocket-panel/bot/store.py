...
class Store:
    ...
    def _migrate(c):
        ...
        #
        # Хранится ВРЕМЯ, а не «да/нет», нарочно: условия однажды
        # изменятся, и тогда понадобится знать, кто видел старую
        # редакцию, а кто новую. Флаг такого не расскажет, а время
        # расскажет.
        if "terms_at" not in have:
            c.execute("ALTER TABLE users ADD COLUMN terms_at INTEGER")

        # Платный безлимит на месяц. `unlim_until` — до какого времени
        # он действует, `unlim_fast` — сколько работ осталось в быстрой
        # полосе. Оба пусты у всех, кто его не покупал.
        #
        # Два поля, а не одно, потому что это две разные вещи: срок
        # кончается сам по часам, а быстрая полоса — по числу работ, и
        # после неё человек продолжает пользоваться, только медленно.
        for стлб in ("unlim_until", "unlim_fast"):
            if стлб not in have:
                c.execute(f"ALTER TABLE users ADD COLUMN {стлб} INTEGER")
    ...
    def условия_показать(self, tg_id):
        """Надо ли показать человеку условия использования.

        Отвечает и СРАЗУ ЗАПОМИНАЕТ показ — тем же приёмом, что и
        `низ_устарел`, и по той же причине: между ответом «да» и
        записью влезает следующее сообщение того же человека, и условия
        прилетают ему дважды подряд.

        Мы спрашиваем «показывали ли», а не «согласился ли». Согласие
        даётся самим фактом пользования — так написано в тексте, и
        отдельной кнопки «принимаю» нет нарочно: кнопка, которую все
        жмут не читая, не делает согласие осмысленнее, а вход в продукт
        загораживает.
        """
        u = self.user(tg_id)
        if not u or u.get("terms_at"):
            return False
        with self._db() as c:
            c.execute("UPDATE users SET terms_at=? WHERE tg_id=?",
                      (int(time.time()), tg_id))
        return True

    def условия_когда(self, tg_id):
        """Когда человеку показали условия. Ноль — ещё не показывали."""
        return (self.user(tg_id) or {}).get("terms_at") or 0
    ...
    def доска_рефереров(self, верх=5, tg_id=None):
        """Сколько человек позвали лучшие и какое место занял этот.

        ИМЁН ЗДЕСЬ НЕТ И НЕ БУДЕТ, и это не недоделка. Доска у бота 18+
        называла бы вслух тех, кто им пользуется: «@ник привёл семерых»
        читается как заявление о человеке, которое он никому не давал
        делать. Соседний экран у нас умеет стереть человека насовсем
        (`забыть`) - и было бы странно давать такое право, попутно
        печатая его ник в общем списке.

        Поэтому доска состоит из чисел: сколько позвал первый, сколько
        второй. Соревнование от этого не пропадает - место своё человек
        видит (`моё`), а догонять он догоняет число, а не фамилию.

        Считаем по `users.invited_by`, а не по журналу начислений: это
        «сколько людей пришло», а не «сколько раз мы заплатили». Руками
        начисленный коин доску не двигает.

        Возвращает {"верх": [число, ...], "моё": {"место", "позвано"} или
        None}. Место - по числу приглашённых; у равных счётов место
        одинаковое, как в спорте.
        """
        with self._db() as c:
            счёты = [r["n"] for r in c.execute(
                "SELECT COUNT(*) n FROM users"
                " WHERE invited_by IS NOT NULL"
                " GROUP BY invited_by ORDER BY n DESC")]
        итог = {"верх": счёты[:max(0, int(верх))], "моё": None}
        if tg_id is not None:
            позвано = self.сводка(tg_id)["позвано"]
            if позвано:
                # Место - сколько счётов СТРОГО больше моего, плюс один.
                # Так двое с одинаковым счётом стоят на одном месте, а не
                # делят его по тому, кто раньше завёлся.
                итог["моё"] = {"место": sum(1 for n in счёты if n > позвано) + 1,
                               "позвано": позвано}
        return итог
    ...
    def безлимитный(self, tg_id):
        """Этому человеку коины не считаются.

        Два разных безлимита, и они не смешиваются:

        * СПИСОК из окружения — владелец, тестировщики, свои. Вечный,
          бесплатный, правится только перезапуском бота.
        * КУПЛЕННЫЙ месяц — обычный клиент, заплативший за тариф.
          Кончается по часам сам.

        Здесь они сходятся в один ответ нарочно: всему, что считает
        деньги, разницы нет — коины не списываются ни у тех, ни у
        других. Разница есть у ОЧЕРЕДИ, и её спрашивают отдельно, через
        `безлимит_быстрая`.
        """
        if self.безлимит_до(tg_id) > time.time():
            return True
    ...
    def безлимит_до(self, tg_id):
        """До какого времени действует КУПЛЕННЫЙ месяц. Ноль — нет."""
        return int((self.user(tg_id) or {}).get("unlim_until") or 0)

    def безлимит_дать(self, tg_id, секунд, быстрых):
        """Включить месяц. Купил, не дождавшись конца прошлого — сроки
        складываются, а быстрая полоса наливается заново: он заплатил
        за неё второй раз."""
        сейчас = int(time.time())
        было = self.безлимит_до(tg_id)
        до = (было if было > сейчас else сейчас) + int(секунд)
        with self._db() as c:
            c.execute("UPDATE users SET unlim_until=?, unlim_fast=? "
                      "WHERE tg_id=?", (до, int(быстрых), tg_id))
        return до

    def безлимит_быстрая(self, tg_id):
        """Осталась ли у человека быстрая полоса, и тратит ОДНУ.

        Отвечает и сразу списывает — тем же приёмом, что `низ_устарел`.
        Вызывающий обязан после «да» пустить работу вперёд очереди.

        Ложь означает не «нельзя», а «в медленном режиме»: работа
        всё равно будет сделана, просто подождёт свободной карты.
        """
        if self.безлимит_до(tg_id) <= time.time():
            return False
        u = self.user(tg_id) or {}
        осталось = int(u.get("unlim_fast") or 0)
        if осталось <= 0:
            return False
        with self._db() as c:
            c.execute("UPDATE users SET unlim_fast=? WHERE tg_id=?",
                      (осталось - 1, tg_id))
        return True

    def безлимит_быстрых(self, tg_id):
        """Сколько быстрых работ осталось. Не тратит."""
        if self.безлимит_до(tg_id) <= time.time():
            return 0
        return int((self.user(tg_id) or {}).get("unlim_fast") or 0)
