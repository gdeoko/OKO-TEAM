...
# NoNewPrivileges ВЫКЛЮЧЕН НАРОЧНО, и вот почему.
#
# Боту и админке нужно поднимать и гасить копии партнёров, а это
# systemctl, то есть sudo. NoNewPrivileges запрещает любой setuid, и
# sudo под ним не работает вовсе - рубильник в админке молча отвечал
# «не вышло».
#
# Что при этом НЕ открывается: sudoers разрешает ровно один скрипт,
# /usr/local/sbin/amberry-partner, и ничего больше. Скрипт принимает
# только цифровой id и токен вида BotFather (проверка на строку
# целиком, см. партнёры/партнёр.sh), трогает только службы
# amberry@<цифры> и пишет только в /etc/amberry/<цифры>.env. Файл
# этот читает служба, которая сама работает под amberry, - то есть
# root через эту дорогу не достаётся никак.
#
# Остальная защита на месте: чужой корень только на чтение, свой /tmp,
# домашние каталоги закрыты, писать можно в один каталог.
