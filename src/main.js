...
const HOVER_RADIUS = 1.45, HOVER_FORCE = 0.035;   // чуть ШИРЕ след касания (как у igloo)
// RETURN_TOUCH ниже + TOUCH_DAMP выше → вмятина от касания затягивается ДОЛЬШЕ и плавнее (текучий след)
const RETURN_SHAPE = 0.075, RETURN_TOUCH = 0.012, TOUCH_DAMP = 0.86;
  ...
  const mat = new THREE.ShaderMaterial({
    ...
    vertexShader: `attribute float size; attribute float aglow; varying vec3 vColor; varying float vSh; varying float vGlow; varying float vFade;
      ...
      void main(){ vColor=color; vGlow=aglow;
        ...
        // ЖИВАЯ МАССА КАК У IGLOO: КАЖДАЯ частица бродит ПО-СВОЕМУ — свой набор случайных фаз,
        // частот и направлений → разнообразные траектории по ВСЕЙ форме, медленно и плавно.
        // Смещение отсчитывается от «домашней» точки (position) и ОГРАНИЧЕНО по амплитуде →
        // силуэт мозга НЕ разрушается; иногда частица чуть выходит за контур и мягко возвращается.
        vec3 p = position;
        // 4 независимых псевдослучайных числа на частицу (из её позиции) — уникальный «характер»
        float h1 = fract(sin(dot(position, vec3(12.989, 78.233, 37.719))) * 43758.5453);
        float h2 = fract(sin(dot(position, vec3(39.346, 11.135, 83.155))) * 24634.6345);
        float h3 = fract(sin(dot(position, vec3(73.156, 52.235, 09.151))) * 13758.1234);
        float h4 = fract(sin(dot(position, vec3(26.651, 31.728, 61.213))) * 31415.9265);
        // каждая частица — своя скорость (медленная) и свои фазы по 3 осям → разные направления
        float spd = 0.30 + h4 * 0.35;                       // индивидуальная медленная скорость
        float ph1 = h1 * 6.2831;                            // фазовые сдвиги (рандом по частице)
        float ph2 = h2 * 6.2831;
        float ph3 = h3 * 6.2831;
        // независимое блуждание по каждой оси (Лиссажу-подобная траектория, у всех разная)
        vec3 wob;
        wob.x = sin(uTime*spd + ph1) * 0.55 + sin(uTime*spd*1.7 + ph2) * 0.45;
        wob.y = sin(uTime*spd*1.3 + ph2) * 0.55 + cos(uTime*spd*0.9 + ph3) * 0.45;
        wob.z = cos(uTime*spd*1.1 + ph3) * 0.55 + sin(uTime*spd*1.9 + ph1) * 0.45;
        // базовая амплитуда блуждания (по всей фигуре, у каждой чуть своя)
        float amp = 0.085 + h3 * 0.075;
        p += wob * amp;
        // РЕДКИЕ ВЫЛАЗКИ за контур: ~18% частиц медленно выходят наружу и так же мягко возвращаются
        float exMask = smoothstep(0.82, 1.0, h2);
        float ex = (sin(uTime*spd*0.7 + ph1*1.3) * 0.5 + 0.5) * exMask * 0.16;
        p += normalize(position + vec3(0.0001)) * ex;
