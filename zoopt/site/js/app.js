...
function initJourney(){
  ...
  const moonLight=new THREE.PointLight(0xFFE7A0,230,120,1.8); moonLight.position.set(2.5,7,-30); scene.add(moonLight);
  ...
  const moon=new THREE.Mesh(new THREE.IcosahedronGeometry(4.6,5),
    new THREE.MeshStandardMaterial({color:0xFFD500,emissive:0xFFC400,emissiveIntensity:1.05,roughness:.55}));
  ...
  moon.position.set(2.5,5.5,-32); scene.add(moon);
  ...
  moonGlow.scale.set(14,14,1); moonGlow.position.copy(moon.position); scene.add(moonGlow);
  ...
  function station(x,y,z,silhouettes){
    ...
    const lamp=new THREE.PointLight(0xFFD500,12,12,2); lamp.position.set(0,.6,1.4); g.add(lamp);
    scene.add(g); stations.push(g); return g;
  }
  station(-6.5,0, 3.5, [['sil_dog.png',2.6,0]]).rotation.y=0.59;
  station( 6.5,0,-3.5, [['sil_cat.png',2.6,0]]).rotation.y=-0.56;
  station(-6.0,0,-10.5,[['sil_rodent.png',1.8,-1.0],['sil_aqua.png',1.8,1.1]]).rotation.y=0.58;
  ...
  portal.add(pring2);
  const plight=new THREE.PointLight(0xFFD500,22,22,2); portal.add(plight);
  ...
  const makers=[()=>bone(matGreen),()=>bone(matIvory),
    ...
    ()=>new THREE.Mesh(new THREE.OctahedronGeometry(.32,0),matYellow),
    ()=>new THREE.Mesh(new THREE.IcosahedronGeometry(.34,0),matDark),
    ()=>new THREE.Mesh(new THREE.TorusGeometry(.34,.13,14,28),matGreen)];
  const floaters=[];
  const COUNT=MOBILE?16:26;
  for(let i=0;i<COUNT;i++){
    const o=makers[i%makers.length]();
    let x=rnd(-10,10); if(Math.abs(x)<4.6) x+=Math.sign(x||1)*4.6;
    o.position.set(x,rnd(-1.2,3.6),rnd(-17,9.5));
    o.rotation.set(rnd(0,6),rnd(0,6),rnd(0,6)); o.scale.setScalar(rnd(.65,1.35));
  ...
  /* --- утка-игрушка (классика, CC0) --- */
  new GLTFLoader().load('assets/models/duck.glb',(g)=>{
    const d=g.scene; const box=new THREE.Box3().setFromObject(d);
    const s=1.5/box.getSize(new THREE.Vector3()).y; d.scale.setScalar(s);
    d.position.set(4.4,-0.4,7.6); d.rotation.y=-.8; scene.add(d);
    d.userData={y0:d.position.y}; floaters.push(Object.assign(d,{userData:{sp:.2,ax:new THREE.Vector3(0,1,0),ph:1,bob:.5,y0:d.position.y}}));
  },undefined,()=>{});
  ...
  const CHNAMES=['00 / пролог','01 / собакам','02 / кошкам','03 / малыши и вода','04 / портал опт'];
  let lastCh=0;
  function hud(p){
    let active=0;
    chs.forEach((el,i)=>{
  ...
  function loop(){
    ...
    cam.lookAt(lookPos.x+mt.x*2.2, lookPos.y-mt.y*1.6, lookPos.z);
...
/* ═════════════════════════════════════════════════════════════════════
   3D-ВИТРИНА — интерактивная модель (зажми и крути)
   ═════════════════════════════════════════════════════════════════════ */
function initShowcase(){
  ...
  const ring=new THREE.Mesh(new THREE.TorusGeometry(1.85,.05,16,80),
    new THREE.MeshStandardMaterial({color:0xFFD500,emissive:0xFFC400,emissiveIntensity:2.2,roughness:.4}));
  ...
  controls.autoRotate=!RM; controls.autoRotateSpeed=1.3;
  ...
  new GLTFLoader().load('assets/models/fox.glb',(g)=>{
    ...
    model.position.set(-c.x,-b2.min.y,-c.z); model.rotation.y=-.5;
    model.traverse(o=>{if(o.isMesh)o.material.envMapIntensity=1.1;});
    stage.add(model);
    mixer=new THREE.AnimationMixer(model);
    actions=g.animations.map(cl=>mixer.clipAction(cl));
    if(actions[0]){current=actions[0];current.play();}
  },undefined,e=>console.warn('fox',e));
  ...
  document.querySelectorAll('.pill3d').forEach(btn=>btn.addEventListener('click',()=>{
    ...
    if(current&&current!==next){next.reset().play();next.crossFadeFrom(current,.4,false);}else next.reset().play();
    current=next;
  }));

  const composer=new EffectComposer(renderer);
  composer.addPass(new RenderPass(scene,cam));
  composer.addPass(new UnrealBloomPass(new THREE.Vector2(1,1),.7,.8,.7));
...
function initScroll(){
  ...
  ScrollTrigger.create({trigger:'#show',start:'top 80',end:'max',
    onToggle:s=>nav.classList.toggle('solid',s.isActive)});
