...
function rc_pass_new() {
    ...
    $ok = rc_api_update(rc_pass_file(), function ($d) use ($t) {
        $сейчас = time();
        $чисто = [];
...
function rc_pass_drop($t) {
    ...
    rc_api_update(rc_pass_file(), function ($d) use ($t) {
...
/* A successful response requires a committed write. */
function rc_api_update($file, callable $fn) {
    $result = rc_json_update($file, $fn);
    if ($result === false) { http_response_code(503); out(['ok' => false, 'error' => 'storage']); }
    return $result;
}
function rc_api_write($file, $data) {
    if (!rc_json_write($file, $data)) { http_response_code(503); out(['ok' => false, 'error' => 'storage']); }
    return true;
}
function rc_csv_cell($value) {
    $value = (string)$value;
...
function rate_ok($bucket, $limit, $window) {
    ...
    $now = time();
    $hit = false;
    rc_api_update($f, function ($d) use ($ip, $now, $limit, $window, &$hit) {
...
function rate_global($bucket, $limit, $window) {
    ...
    $now = time();
    $ok = false;
    rc_api_update($f, function ($d) use ($now, $limit, $window, &$ok) {
...
if ($action === 'track') {
    ...
    if (!rate_ok('track', 400, 300)) { http_response_code(429); out(['ok' => false, 'error' => 'too_many']); }
    ...
    $сайт = rc_site();
    $fresh = [];
    rc_api_update(stat_file(null, $сайт), function ($d) use ($events, $ua, $sid, $refHost, &$fresh, $сайт) {
        $d['site'] = $сайт;
        ...
        foreach ($events as $e) {
            ...
            $id = substr((string)($e['id'] ?? ''), 0, 100);
            $key = $id !== '' ? hash('sha256', $sid . ':' . $id) : '';
            if ($key !== '' && isset($d['received'][$key])) continue;
            $type = mb_substr((string)($e['t'] ?? ''), 0, 20, 'UTF-8');
        ...
        if (count($d['received']) > 30000) $d['received'] = array_slice($d['received'], -25000, null, true);
...
if ($action === 'lead_status') {
    ...
    $found = false;
    rc_api_update(RC_LEADS, function ($d) use ($id, $st, &$found) {
...
if ($action === 'lead_note') {
    ...
    $found = false;
    rc_api_update(RC_LEADS, function ($d) use ($id, $note, &$found) {
        foreach (($d['items'] ?? []) as $i => $it) {
            if (($it['id'] ?? '') === $id) {
                $d['items'][$i]['note'] = $note;
...
if ($action === 'lead_delete') {
    need_key();
    $id = inp('id');
    rc_api_update(RC_LEADS, function ($d) use ($id) {
...
if ($action === 'export') {
    ...
    foreach (array_reverse($d['items'] ?? []) as $x) {
        fputcsv($out, array_map('rc_csv_cell', [$x['ts'] ?? '', $x['kind'] ?? '', $x['name'] ?? '', $x['contact'] ?? '',
...
if ($action === 'nodes_save') {
    ...
    $видели = [];
    foreach ($add as $n) {
        if (!is_array($n) || count($n) < 5) out(['ok' => false, 'error' => 'node']);
        for ($k = 1; $k <= 4; $k++) {
            if (!isset($n[$k]) || !is_numeric($n[$k]) || !is_finite((float)$n[$k])) out(['ok' => false, 'error' => 'coordinates']);
        }
        if ((float)$n[3] != (int)$n[3] || (float)$n[4] != (int)$n[4]) out(['ok' => false, 'error' => 'node']);
        $имя = trim((string)$n[0]);
        ...
        if ($имя === '') out(['ok' => false, 'error' => 'name']);
        if ($ш < -90 || $ш > 90 || $д < -180 || $д > 180) out(['ok' => false, 'error' => 'coordinates']);
        if ($р < 0 || $р > 4) out(['ok' => false, 'error' => 'region']);
        if ($м < 0 || $м > 7) out(['ok' => false, 'error' => 'mask']);
    ...
    rc_api_write(RC_NODES, ['add' => $чисто, 'hide' => $скрыт, 'ts' => date('c')]);
...
if ($action === 'связка') {
    ...
    if ($токен !== '') {
        ...
        if ($свеж) {
            ...
            if (!empty($j['ok'])) {
                $бот = $j['result'];
                rc_api_write(RC_DATA . '/tg_me.json', ['ts' => time(), 'бот' => $бот]);
