...
$RC = [
    ...
    /* ── У КАЖДОЙ ПЛОЩАДКИ СВОИ ВЕТКИ ─────────────────────────
       Темы были одни на все сайты, и в чат заказчика всё сыпалось
       вперемешку: ошибка VPN приходила под заголовком «Rocket CDN», а
       сводка VPN шла хвостом «Остальные площадки» внутри отчёта CDN.
       Владелец: «что-то у тебя в чат всё, а должно быть по веткам
       своим, как CDN».

       Общие ключи ниже остаются и работают как ЗАПАСНЫЕ: пока у сайта
       своей ветки не заведено, его сообщения идут в общую и не
       пропадают. Заведена своя - уходят в неё. Разбор в rc_topic(). */
    'tg_topic_form'  => '',       /* id темы «Формы», общая запасная */
    'tg_topic_error' => '',       /* id темы «Ошибки», общая запасная */
    'tg_topic_stat'  => '',       /* id темы «Аналитика», общая запасная */
    'tg_topic_form_cdn'  => '',   /* CDN · Форма */
    'tg_topic_error_cdn' => '',   /* CDN · Ошибки */
    'tg_topic_stat_cdn'  => '',   /* CDN · Аналитика */
    'tg_topic_form_vpn'  => '',   /* VPN · Форма (формы на VPN нет, ветка про запас) */
    'tg_topic_error_vpn' => '',   /* VPN · Ошибки */
    'tg_topic_stat_vpn'  => '',   /* VPN · Аналитика */
    'tg_username'    => 'rocket_cdn_bot',

    /* ── ФОРМА ОБРАТНОЙ СВЯЗИ ТОЛЬКО У CDN ────────────────────
       Слово заказчика: «форму обратной связи руководитель сказал не
       нужно на VPN, а на CDN можно оставить». Держим это настройкой, а
       не вырезанным кодом: передумают - включается одной строкой.
       Ключ читает api.php перед приёмом заявки. */
    'lead_sites'  => ['cdn'],
...
/* Читаем под общей блокировкой. Изменение режет файл на месте под
   своей блокировкой, а чтение раньше шло мимо неё - и попадание в
   это окно давало обрезанный JSON. Разбор молча возвращал значение
   по умолчанию: пустой список заявок вместо содержимого, без ошибки
   и без записи в журнал. Замер на четырёх писателях и одном читателе:
   242 потери на 2500 чтений без блокировки и ноль с ней. Дороже всего
   это стоило бы в cron.php, где той же функцией снимается дневная
   резервная копия заявок. Теперь чтение ждёт конца записи, а битый
   разбор попадает в журнал. */
function rc_json_read($file, $def = []) {
    ...
    /* Сообщаем о битом файле ровно один раз за вызов: сам журнал
       редких сообщений тоже читается этой же функцией, и без замка
       битый журнал уводил бы разбор в бесконечную рекурсию. */
    static $вжурнале = false;
    if (!$вжурнале) {
        $вжурнале = true;
        rc_log_rare('json_bad', 'битый JSON: ' . basename($file));
        $вжурнале = false;
    }
    return $def;
}
...
function rc_json_update($file, callable $fn) {
    ...
    /* Сбрасываем буфер ДО снятия блокировки: иначе порядок держится
       только на том, что поток пишет насквозь. Потерь на замере не
       было ни так, ни так, но полагаться тут на буферизацию потока
       нечего - строка закрывает окно по построению. */
    fflush($fh);
    @flock($fh, LOCK_UN);
    fclose($fh);
    return $data;
...
/* ── КУДА ПИСАТЬ: СВОЯ ВЕТКА ПЛОЩАДКИ, ИНАЧЕ ОБЩАЯ ───────────
   Вид это «form», «error» или «stat». Имя площадки из rc_sites().

   Порядок один и тот же везде: сначала спрашиваем ветку этой
   площадки, потом общую. Своей нет - сообщение всё равно дойдёт, и
   это важнее ровности: молчащий бот хуже сообщения не в той ветке.

   Игра живёт внутри VPN, поэтому её ветки это ветки VPN: заводить
   третью пару тем ради подраздела значит дробить чат на то, что
   человек всё равно читает вместе. */
function rc_topic($вид, $сайт = null) {
    $вид = preg_replace('~^tg_topic_~', '', (string)$вид);
    if ($вид === '') return null;
    $с = (string)$сайт;
    if ($с === 'game') $с = 'vpn';
    if ($с !== '' && isset(rc_sites()[$с === 'vpn' ? 'vpn' : $с])) {
        $своя = rc_cfg('tg_topic_' . $вид . '_' . $с);
        if ($своя !== '' && $своя !== null) return $своя;
    }
    /* ── СВОЕЙ ВЕТКИ НЕТ: ИЩЕМ БЛИЖАЙШУЮ СВОЮ, А НЕ ОБЩИЙ ЧАТ ──
       Ветка «Ошибки» заведена не у каждой площадки, а падать в общий
       чат такому сообщению нельзя: именно так ошибка VPN оказалась в
       ветке «Чат» рядом с перепиской, и владелец это увидел первым.

       Поэтому сначала пробуем другую ветку ТОЙ ЖЕ площадки: ошибка в
       её аналитике всё равно стоит рядом со своими числами. И только
       если у площадки нет ни одной ветки, идём в общие. */
    $поблизости = ($вид === 'error') ? ['stat', 'form'] : (($вид === 'stat') ? ['error'] : ['stat']);
    if ($с !== '') {
        foreach ($поблизости as $в2) {
            $з = rc_cfg('tg_topic_' . $в2 . '_' . $с);
            if ($з !== '' && $з !== null) return $з;
        }
    }
    $общая = rc_cfg('tg_topic_' . $вид);
    return ($общая === '' || $общая === null) ? null : $общая;
}

function rc_notify($text, $markup = null, $topic_key = null, $сайт = null) {
    $chat = rc_cfg('tg_chat');
    $topic = $topic_key ? rc_topic($topic_key, $сайт) : null;
    if ($chat) {
