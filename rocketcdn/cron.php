...
if ($force || ($hour >= (int)rc_cfg('report_hour', 9) && ($state['daily'] ?? '') !== $today)) {
    /* ── СКОЛЬКО ПЛОЩАДОК, СТОЛЬКО ОТЧЁТОВ ──────────────────────
       Был один отчёт с хвостом «Остальные площадки», и весь VPN в нём
       умещался в две строки внутри ветки CDN. Владелец: «по VPN
       сделать всё так же, но в другие ветки».

       Теперь каждая площадка получает СВОЙ полный разбор и уходит он
       в СВОЮ ветку (rc_topic). Ветки у площадки нет - сообщение идёт
       в общую и не пропадает. */
    foreach (rc_report_sites(1, 1) as $к) {
        rc_notify(rc_report_daily($к), null, 'tg_topic_stat', $к);
    }

    /* По понедельникам добавляем недельный срез, тоже по площадкам */
    if (date('N') === '1' || $force) {
        foreach (rc_report_sites(1, 7) as $к) {
            rc_notify(rc_report_period(7, 'Итоги недели', $к), null, 'tg_topic_stat', $к);
        }
    }
