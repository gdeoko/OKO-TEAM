...
# Поправка размера для тех, у кого габарит задаёт не сам предмет.
#
# Подгонка вписывает рисунок по его рамке, и у Сатурна рамку задают
# кольца: шар выходит вдвое мельче Юпитера, хотя стоит в том же ряду.
# Глаз читает такой ряд как неровный. Поэтому кольцам разрешено слегка
# выйти за край - стикеру это можно, а шар встаёт вровень с соседями.
ПОПРАВКА = {"saturn": 1.20}
...
def main():
    ...
    for ключ, знак, подпись, движение, ист, второй in ПАК:
        ...
        dest, n, br, мал = F.собрать_ключ(src, ключ, движение, выдача, втор,
                                          эмодзи=эмодзи,
                                          поправка=ПОПРАВКА.get(ключ, 1.0))
