...
# профили плавности: вход в ключ и выход из него
EASE = {
    # мягкий приход и мягкий уход - общий случай
    "smooth": ((0.42, 1.0), (0.58, 0.0)),
    # резкий старт, долгое торможение: так падают и прилетают предметы
    "out": ((0.16, 1.0), (0.30, 0.0)),
    # медленный разгон: предмет трогается с места
    "in": ((0.70, 1.0), (0.85, 0.0)),
    # почти удар: короткое движение с жёсткой остановкой
    "snap": ((0.08, 1.0), (0.20, 0.0)),
    "linear": ((1.0, 1.0), (0.0, 0.0)),
}


def anim(frames, easing=True, ease="smooth"):
    """Ключевые кадры: [(кадр, значение), ...].

    Профиль плавности задаётся на всю дорожку или на отдельный ключ -
    тогда кадр пишется как (кадр, значение, "профиль"). Разная плавность
    на разных участках и отличает живое движение от ватной протяжки.
    """
    keys = []
    for i, fr in enumerate(frames):
        t, v = fr[0], fr[1]
        prof = fr[2] if len(fr) > 2 else (ease if easing else "linear")
        k = {"t": t, "s": v if isinstance(v, list) else [v]}
        if i < len(frames) - 1:
            (ix, iy), (ox, oy) = EASE.get(prof, EASE["smooth"])
            k["i"] = {"x": [ix], "y": [iy]}
            k["o"] = {"x": [ox], "y": [oy]}
        keys.append(k)
    return {"a": 1, "k": keys}
...
def spring(t0, t1, a, b, over=0.14, settle=0.05):
    """Движение с отскоком: проскочить цель и вернуться.

    Живой предмет не встаёт в точку с первого раза - он её проскакивает
    и качается, затухая. Без этого анимация выглядит механической.
    """
    def mix(k):
    ...
    return anim([
        (t0, mix(0.0), "out"),
        (t0 + int(d * 0.72), mix(1.0 + over), "out"),
        (t0 + int(d * 0.88), mix(1.0 - settle), "smooth"),
        (t1, mix(1.0), "smooth"),
    ])
