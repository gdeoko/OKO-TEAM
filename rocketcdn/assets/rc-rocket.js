...
/* ── Маршрут корабля по странице ─────────────────────────────
   Одна замкнутая петля на весь фильм: корабль обходит кадр по
   большому кругу - вверх вдоль правого борта, поверху налево, вниз
   по левому и назад направо. Прокрутка ведёт его по этой петле, и
   каждый следующий раздел встречает корабль на новом месте.

   Главное правило петли - ровная глубина. Прежний маршрут нырял к
   самому объективу (z доходил до -0.6) и тут же уходил на -5:
   корабль то занимал полкадра, то пропадал точкой в глубине.
   Владелец описал это дословно: «в hero ракета справа, начинаю
   листать, и она не летит за скроллом, а просто исчезает и потом
   слева появляется». Это была не анимация, а телепорт, и вина не
   анимации, а маршрута.

   Теперь глубина держится в узком поясе (от -4.6 до -6.4), а
   выразительность даёт не наезд, а сама форма петли, крен на
   вираже и след. Корабль всё время одного размера, и глаз следит
   за ним, а не ищет его заново на каждом экране. */
function buildPath() {
  var v = function (x, y, z) { return new T.Vector3(x, y, z); };
  return new T.CatmullRomCurve3([
    v( 7.2,  0.4, -5.0),   /* пролог: правый борт, чуть выше середины */
    v( 5.6,  3.0, -5.4),   /* старт: пошла вверх вдоль правого поля */
    v( 0.6,  3.9, -6.0),   /* набор высоты: верх кадра */
    v(-5.0,  2.8, -5.6),   /* облака: перевалила налево */
    v(-7.2, -0.4, -5.2),   /* коридор: левое поле */
    v(-4.6, -3.2, -6.0),   /* сеть: низ слева */
    v( 0.8, -3.9, -5.6),   /* вход в атмосферу: низ кадра */
    v( 5.4, -2.6, -4.8)    /* маршрут: справа снизу, замыкает петлю */
  ], true, "catmullrom", 0.5);
}
...
Rocket.prototype.layout = function (p, dt) {
  ...
  if (scAct && (scAct.act === "pad" || p < 0.075) && app < 0.01 && (this.landK || 0) < 0.01) {
    ...
    if (padK > 0.01) {
      ...
      /* На телефоне доля 0.23 по высоте ставила корабль под самой
         кромкой кадра: в первом экране был виден один нос, и
      ...
      /* Куда ставим корабль в первом кадре телефона.

         Раньше это был правый ВЕРХ, и корабль шёл прямо по строке
         заголовка: приёмка сняла кадр, где по буквам «Доставка
         контент» идёт корпус. Слоями это вылечено (корабль ушёл за
         текст), но само место осталось неудачным - за буквами от него
         видна одна полоска.

         Свободная зона на телефоне одна: правый край на уровне списка
         отметок. Строки там короткие, справа от них пусто, и корабль
         входит в кадр целиком, не заслоняя ничего. */
      var pxA = this.C.mobile
        ? pw * (0.78 - arch * 0.16 - hp * 0.05)
        : pw * (0.84 - arch * 0.20 - hp * 0.05);
      var pyA = this.C.mobile
        ? ph * (0.70 - arch * 0.14 - hp * 0.10)
