...
/* Третья ступень деградации - это «попроще», а не «выкинуть».
   Читаем её каждый кадр заново: ступень назначает rc-motion уже на
...
function frame() {
  ...
  var k = fade(d);
  /* Экраны гаснут вместе со сходящейся диафрагмой входа (rc-airlock,
     доля «вход»).

     Замер поймал их вторым швом, помельче главного: доля люка на
     подмене доходила не до единицы, а до половины, растворение по
     ней ещё не начиналось - и обе голограммы пропадали ровно тем же
     кадром, которым менялась сцена, по правилу html.rc-inside
     .rc-gate. Теперь их ведёт тот же проход, что и весь вход:
     источник один, и к самой узкой точке в кадре их уже нет. */
  var вход = (g.RC_AIRLOCK && typeof g.RC_AIRLOCK.вход === "number") ? g.RC_AIRLOCK.вход : 0;
  if (вход > 0) {
    var порог = 1 - вход * 1.7;
    if (порог < 0) порог = 0;
    if (порог < k) k = порог;
  }
  ...
  /* Упрощённый ход: доля идёт пятью ступенями, и на каждую тратится
     одна запись в стиль вместо сотни. Экраны остаются - исчезать им
     на слабом устройстве не за что. */
  var прощеЛи = простой();
  if (прощеЛи) k = Math.round(k * 5) / 5;
  if (Math.abs(k - lastK) < (прощеЛи ? 0.19 : 0.01)) return false;
  lastK = k;
  ...
  /* Наезд: при растворении экраны уходят на зрителя вместе с
     ...
     сквозь них», а не «выключили». На третьей ступени наезда нет:
     это украшение, а не сценарий. */
  if (!прощеЛи) wrap.style.setProperty("--gate-z", (Math.max(0, d - 0.74) * 5).toFixed(3));
}

function boot() {
  if (reduced || root.classList.contains("rc-reduced")) return;
  /* Здесь стоял отказ строить экраны на третьей ступени деградации,
     и это была дыра в сценарии: у человека на слабом устройстве
     дверь корабля открывалась в пустоту, а на стене рубки его потом
     встречали карточки, которых он в проёме не видел. Ступень
     означает «попроще», а не «выкинуть» - и оформление это уже
     знало: правило html[data-degrade="3"] .rc-gate-card в
     rc-gate.css снимает переходы, но только у тех экранов, которые
     построены. Строим всегда, дешевле их делает простой(). */
