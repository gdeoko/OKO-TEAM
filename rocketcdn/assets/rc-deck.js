...
function влить(c, src, q, sw, sh) {
  ...
  if (Math.abs(дх) + Math.abs(ду) < мера * 0.012) {
    c.save();
...
function клавиша(c, w, h, k, сила, ru, наЭкране) {
  ...
  var тл = Math.max(1, Math.min(w, h) * 0.045);
  var x = тл, y = тл, ww = w - x * 2, hh = h - y * 2;

  /* ── Объём ───────────────────────────────────────────────────
     Заказчик сказал прямо: «добавь объёма, чтобы круто смотрелось».
     Клавиша перестаёт быть наклейкой не от толщины обводки, а от
     того, что на ней лежит свет: сверху фаска ловит подсветку рубки,
     тело чуть светлее к верхнему краю, а снизу приходит слабый
     отсвет от дна ниши.

     Всё это ТОЛЬКО добавленным светом: слой кладётся на снимок
     складыванием, затемнить он ничего не может. Так и правильно -
     тень на клавише уже нарисована на самом снимке, наше дело блики.

     Нажатую клавишу переворачиваем: фаска сверху гаснет, отсвет
     снизу разгорается. Так ведёт себя колпачок, ушедший в нишу. */
  var вниз = сила;                       /* 0 отпущена, 1 утоплена */

  /* Тело: свет держится у верхней кромки и сходит на нет к середине */
  c.save();
  плитка(c, x, y, ww, hh, r);
  c.clip();
  var тело = c.createLinearGradient(0, y, 0, y + hh);
  тело.addColorStop(0, "rgba(123,209,236," + (0.10 + вниз * 0.06).toFixed(3) + ")");
  тело.addColorStop(0.55, "rgba(66,178,220,0)");
  тело.addColorStop(1, "rgba(66,178,220," + (0.02 + вниз * 0.07).toFixed(3) + ")");
  c.fillStyle = тело;
  c.fillRect(0, 0, w, h);
  c.restore();

  /* Фаска: обводка градиентом, яркая сверху, гаснет к низу */
  плитка(c, x, y, ww, hh, r);
  var фаска = c.createLinearGradient(0, y, 0, y + hh);
  var вверху = (0.62 + сила * 0.30) * (1 - вниз * 0.45);
  var внизу = 0.16 + вниз * 0.52;
  фаска.addColorStop(0, "rgba(207,233,245," + вверху.toFixed(3) + ")");
  фаска.addColorStop(0.42, "rgba(66,178,220," + (0.20 + сила * 0.40).toFixed(3) + ")");
  фаска.addColorStop(1, "rgba(66,178,220," + внизу.toFixed(3) + ")");
  c.strokeStyle = фаска;
  c.lineWidth = тл;
  c.stroke();

  /* Верхняя кромка отдельной тонкой линией: она и даёт ощущение
     физической грани, а не нарисованного контура. */
  c.save();
  плитка(c, x, y, ww, hh, r);
  c.clip();
  c.beginPath();
  c.moveTo(x + r * 0.6, y + тл * 0.5);
  c.lineTo(x + ww - r * 0.6, y + тл * 0.5);
  c.strokeStyle = "rgba(226,244,252," + ((0.42 + сила * 0.35) * (1 - вниз * 0.7)).toFixed(3) + ")";
  c.lineWidth = Math.max(0.8, тл * 0.55);
  c.stroke();
  /* Отсвет от дна ниши по нижней кромке */
  c.beginPath();
  c.moveTo(x + r * 0.8, y + hh - тл * 0.5);
  c.lineTo(x + ww - r * 0.8, y + hh - тл * 0.5);
  c.strokeStyle = "rgba(66,178,220," + (0.10 + вниз * 0.42).toFixed(3) + ")";
  c.lineWidth = Math.max(0.8, тл * 0.5);
  c.stroke();
  c.restore();
...
function создать(wrap) {
  ...
  function кадр(dt) {
    ...
    for (var s0 = 0; s0 < п_.length; s0++) {
      var кв0 = места(ужать(п_[s0]["угол"], 0.10), п_[s0]["мест"], п_[s0]["зазор"]);
