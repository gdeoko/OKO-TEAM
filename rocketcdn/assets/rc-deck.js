...
function клавиша(c, w, h, k, сила, ru, наЭкране) {
  ...
  g1.addColorStop(0, "rgba(66,178,220," + (0.05 + вниз * 0.10).toFixed(3) + ")");
  g1.addColorStop(0.42, "rgba(58,150,190," + (0.10 + вниз * 0.22).toFixed(3) + ")");
  g1.addColorStop(1, "rgba(40,110,150," + (0.03 + вниз * 0.14).toFixed(3) + ")");
  c.fillStyle = g1;
  c.fillRect(0, 0, w, h);
  ...
  var g2 = c.createRadialGradient(w * 0.5, h * 0.46, 0, w * 0.5, h * 0.46,
                                  Math.max(w, h) * 0.58);
  g2.addColorStop(0, "rgba(123,209,236," + (0.16 + сила * 0.42).toFixed(3) + ")");
  g2.addColorStop(0.55, "rgba(66,178,220," + (0.05 + сила * 0.16).toFixed(3) + ")");
  ...
  g3.addColorStop(0, "rgba(207,233,245," + ((0.20 + сила * 0.22) * (1 - вниз * 0.55)).toFixed(3) + ")");
  ...
  g4.addColorStop(0, "rgba(66,178,220," + (0.08 + вниз * 0.30).toFixed(3) + ")");
  ...
  var мелко = экрВыс < 46;
  var сз = Math.min(w, h) * (мелко ? 0.56 : 0.42);
  var K = g.RC_KEYS;
  if (K && K.icon) {
    c.save();
    c.translate((w - сз) / 2, h * (мелко ? 0.5 : 0.42) - сз * (мелко ? 0.5 : 0.58));
  ...
  /* На мелком месте подпись не помогает, а мешает: буквы в шесть
     точек не читаются, зато съедают половину плитки и превращают
     аккуратную клавишу в кашу. Ниже порога оставляем один значок и
     делаем его крупнее - смысл держит рисунок, а не текст. */
  if (мелко) return;
  ...
  function кадр(dt) {
    ...
    for (var s = 0; s < п_.length; s++) {
      ...
      for (var j = 0; j < кв.length; j++, n++) {
        ...
        бк.save(); клавиша(бк, б2.w, б2.h, k, сила[n], ru, самоеТесное); бк.restore();
