...
function qualityHint() {
  ...
  /* ── ВИДЕОКАРТА ГОЛОСУЕТ НАРАВНЕ С ПАМЯТЬЮ И ЯДРАМИ ─────────
     Память и ядра про графику не знают. Телефон с восемью ядрами и
     шестью гигабайтами при видеокарте начального уровня получал
     ступень ноль, то есть полный буфер и все частицы, и заказчик на
     такой машине видел двадцать-тридцать кадров.

     Слабая карта ставит НЕ МЕНЬШЕ первой ступени: это снижает буфер и
     число прозрачных частиц, но оставляет в кадре все планеты, весь
     материал и всё движение. Вторую ступень даём только там, где
     слабая карта совпала со слабой машиной: это уже действительно
     дешёвый телефон, и ему лучше меньше частиц, чем рывки.

     Вердикт берём готовый (rc-gpu.js): список моделей один на сайт. */
  var картаСлабая = window.RC_СЛАБАЯ_КАРТА === true;
  if (mem <= 1 || cpu <= 2) return 3;
  if (mem <= 2 || cpu <= 4 || (c && c.saveData)) return 2;
  if (картаСлабая && (mem <= 4 || cpu <= 6)) return 2;
  if (mem <= 4 || cpu <= 6 || картаСлабая) return 1;
  return 0;
}
