...
/* ── МЕРКА ПУЛЬТА: ОДИН КОД НА ОБА САЙТА ─────────────────────────
   Та же мерка стоит у соседа (rocketvpn, rv-финал.js `пультМерка`), и
   сверяет их `tools/рубки-рядом.mjs`. Две копии здесь неизбежны:
   сайты раздаются отдельно, общего модуля под мерки нет, а мерка
   обязана считаться ОДНОЙ формулой - иначе расхождение покажет не
   разницу рубок, а разницу мерок.

   Габарит берём у самой вещи, а не у параметров сборки: параметры
   можно передать верные и всё равно получить другой пульт, если
   модуль внутри что-то доберёт сам. */
function пультМерка() {
  if (!W3 || !cabin || !W3.cam) return null;
  var T3 = W3.T || g.THREE;
  ...
  /* Экранный габарит считаем по ВОСЬМИ углам коробки, а не по центру:
     пульт стоит наклонно, и по центру его ширина выйдет заниженной.
     Углы за спиной камеры в мерку не берём - проекция выбрасывает их
     на другую сторону кадра и растягивает рамку на весь экран из
     ничего. */
  var лево = 1e9, право = -1e9, верх = 1e9, низ = -1e9, видно = 0;
  for (var i = 0; i < 8; i++) {
    var т = new T3.Vector3(
      (i & 1) ? box.max.x : box.min.x,
      (i & 2) ? box.max.y : box.min.y,
      (i & 4) ? box.max.z : box.min.z
    );
    var вид = т.clone().applyMatrix4(W3.cam.matrixWorldInverse);
    if (вид.z > -0.01) continue;
    т.project(W3.cam);
    видно++;
    var эx = т.x * 0.5 + 0.5, эy = 0.5 - т.y * 0.5;
    if (эx < лево) лево = эx;
  ...
  return {
    ...
    "кадр": видно < 4 ? null : {
      ...
      "ширина": +(право - лево).toFixed(4), "высота": +(низ - верх).toFixed(4),
      "углов": видно
    }
  };
}
...
g.RC_FLIGHT = {
  ...
  "замерРубки": function () {
    ...
    return {
      ...
      "пультМир": (пульт && пульт["мир"]) || null,
      "пультДоГлаза": (пульт && пульт["доГлаза"]) || null,
      "пультКадр": (пульт && пульт["кадр"]) || null,
