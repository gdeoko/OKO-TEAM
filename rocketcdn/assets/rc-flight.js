...
/* ── Один мир, два продукта ───────────────────────────────────
   ...
   назвать дырой зла, поглощающей все свободы в сети». Имя ведомства
   с метки снято по слову руководства, разбор у самой строки.
...
function строитьСлова() { return {
  ...
  vpn: {
    ...
    /* ── ВЕДОМСТВО НЕ НАЗЫВАЕМ ─────────────────────────────────
       Здесь стояло «РКН» на метке тела и в двух подписях. Слово
       руководства: «в игре чёрную дыру всё же лучше убрать "РКН", от
       греха...». Названо ведомство - названа и мишень, а сайт продаёт
       связь, а не спор с кем-то.

       Смысл сцены от этого не теряется: дыра остаётся дырой, которая
       глотает всё и ничего не отдаёт, и обходим мы её ровно так же.
       Меняется только табличка. */
    дыраТитул:   RU ? "ДЫРА" : "THE PIT",
    дыраПодпись: RU ? "ДЫРА ЗЛА" : "PIT OF EVIL",
    дыраПуть:    RU ? "ДЫРА ЗЛА · поглощает все свободы в сети"
                    : "THE PIT · it swallows every freedom online",
    ...
    дыраСнимок:  RU ? "ДЫРА ЗЛА · из неё не возвращается ничего"
                    : "THE PIT · nothing returns from it",
...
var CAPTIONS = [
  ...
  /* Здесь стояла строка про чёрную дыру. Дыры больше нет, а точка
     маршрута осталась: на ней корабль идёт дальней дугой к прыжку. */
  { p: 0.63, t: RU ? "ДАЛЬНИЙ КРАЙ · тут кеша нет ни у кого"
                   : "FAR EDGE · nobody caches this far out" },
...
function NET_TOTAL() {
  /* Родная система: Солнце, восемь планет и Луна. Дыры тут больше
     нет, и знаменатель обязан был съехать вместе с ней: пока он
     считал одиннадцать, «сеть развёрнута полностью» становилось
     недостижимым - последний узел ставить было не на что. */
  var n = 10;
  for (var u = 1; u < UNIVERSES.length; u++) {
...
var ПОВЕРХНОСТЬ = {
 ...
 "УЗЕЛ": "sat",
 "УЗЕЛ СЕТИ": "earth",
 /* Имя ведомства с метки снято по слову руководства (см. блок
    подписей выше), но в поиске оба слова оставлены: у человека, который
    видел прежнюю версию, запрос по ним обязан продолжать работать. В
    выдачу они ничего не выводят, только открывают то же досье. */
 "ЮПИТЕР": "jupiter"
};
...
function showHome(on) {
  if (!W3) return;
  /* Дыры в списке нет намеренно: узел собран, но в сцену не добавлен,
     и возврат домой не имеет права зажечь его обратно. */
  var list = [W3.earth, W3.moon, W3.mars, W3.saturn, W3.comet,
...
/* Доли спектральных классов взяты для ВИДИМОГО неба, а не для всех
   ...
   настоящие границы классов. Таблица одна на живое звёздное поле и
   на звёзды спиральных галактик: разойтись они не должны. Панорама
   звёзд больше не рисует вовсе - она несёт только газ и пыль. */
var КЛАССЫ_НЕБА = [
...
function skyTexture(mob) {
  ...
  for (i = 0; i < NEB.length; i++) {
    ...
    /* Мазков двенадцать, и они шире: тридцать мелких давали не облако,
       а россыпь цветных пятен, и на телефоне владельца они читались
       теми же «неестественными пятнами», что и ореолы звёзд. Широкий
       и редкий мазок складывается в ровную дымку, а прозрачность
       остаётся выше ступени яркости, чтобы не вернуть дизеринг. */
    for (var q = 0; q < 12; q++) {
      var ox = nx + (rnd() - 0.5) * nr * 2.0, oy = ny + (rnd() - 0.5) * nr * 1.4;
      var orr = nr * (0.22 + rnd() * 0.36);
      gr = x.createRadialGradient(ox, oy, 0, ox, oy, orr);
      gr.addColorStop(0, "rgba(" + NEB[i][2] + "," + (0.034 + rnd() * 0.040).toFixed(3) + ")");
  ...
  /* ── ЗВЁЗД В ПАНОРАМЕ БОЛЬШЕ НЕТ ─────────────────────────────
     Владелец про космос игры: «полупрозрачные белые пятна летают, ну
     не реалистично это выглядит, и из-за этого как каша смотрится».
     Пятна нашлись замером, а не на глаз: `tools/checks/кто-мутит.mjs`
     гасит слои неба по одному на замороженном кадре и считает долю
     светлого без деталей. Панорама снимала 1.06 процента кадра из
     2.38 - вдвое больше любого другого слоя, и на снимке без неё небо
     разом становится россыпью острых точек.

     Причина не в том, КАК тут рисовали звезду, а в том, ГДЕ.
     Панорама натянута на сферу радиусом 4200 и живёт в текстуре
     2048 на телефоне. В кадр при поле зрения 72 градуса попадает
     около четырёхсот её текселей по высоте, а экран телефона отдаёт
     под них две с половиной тысячи точек: каждый тексель растянут
     вшестеро. Звезда в один тексель выходит мягким квадратом в шесть
     пикселей, звезда с ореолом - кляксой в тридцать. Поджимать ореол
     тут уже пробовали дважды (11.09 и ранее), и оба раза пятна
     возвращались: растягивает их сфера, а не радиус кисти.

     Поэтому точки неба целиком отданы ЖИВОМУ звёздному полю
     (`starMats` ниже): там звезда это точка в мире, её размер задан в
     ПИКСЕЛЯХ экрана, и растянуть её нечем ни на каком устройстве.
     Панорама несёт только то, что по природе мягкое и от растяжения
     не портится: полосу Млечного Пути, балдж, облака газа и тёмные
     пылевые прожилки. Разделение честное - крупное и размытое на
     сфере, мелкое и острое точками.

     Заодно это дешевле: девять с половиной тысяч кистей по холсту
     2048 на 1024 при каждой сборке неба больше не рисуются. */
...
function buildWorld() {
  ...
  /* Стартовая плотность держится того же потолка, что и потолок
     регулятора ниже: иначе первые секунды полёта идут в одной
     плотности, а дальше кадр незаметно переезжает в другую. */
  r.setPixelRatio(Math.min(g.devicePixelRatio || 1, tiny ? 1.0 : 2.0));
  ...
  /* Доля поля на слабых устройствах поднята. Раньше телефон получал
     меньше половины неба, и это сходило с рук, пока вторую половину
     россыпи рисовала панорама. Панорама звёзд больше не рисует, и та
     же доля разом превратилась в пустое чёрное окно - владелец
     увидел это первым: «так чище, но всё-таки звёзды и какие-то
     объекты нужны, просто чёрное некрасиво». Поле стоит один вызов
     отрисовки на всё небо, и платить за него надо именно там, где
     смотрят. */
  var starScale = tiny ? 0.62 : (particleBudget ? 0.78 : (qualityHint ? 0.9 : 1));
  var starMats = (function () {
    /* ── СКОЛЬКО ЗВЁЗД В НЕБЕ ────────────────────────────────
       Всё небо теперь держит одно это поле: панорама отдала ему свою
       россыпь, потому что растягивала её в пятна (см. skyTexture).

       Число считается от кадра, а не берётся на глаз. Поле разложено
       по полной сфере, это 12.57 стерадиана. В кадр при поле зрения
       72 градуса по высоте и вертикальном экране попадает около
       0.72 стерадиана, то есть примерно один процент неба из
       семнадцати. При семнадцати тысячах в окне стояло меньше тысячи
       точек, и половина из них уходила под порог отсечения в шейдере
       как слишком слабые: окно читалось чёрным.

       Шестьдесят тысяч дают в кадре три с половиной тысячи, из них
       заметных около тысячи - столько глаз и читает как «полное
       звёзд небо». Стоит это одного вызова отрисовки: точки мельче
       пяти пикселей, слабые отбрасываются в шейдере до растеризации,
       а вершин шестьдесят тысяч это меньше, чем в одной планете. */
    var n = Math.round(60000 * starScale);
    ...
    for (var i = 0; i < n; i++) {
      /* ── ГДЕ СТОИТ ЗВЕЗДА ────────────────────────────────
         Две трети поля раскиданы ровно по сфере: это гало, и оно
         даёт небо в любую сторону. Треть собрана к ПЛОСКОСТИ ДИСКА,
         и это Млечный Путь.

         Прежде плоскость диска отмечалась только прибавкой к
         светимости, то есть в полосе звёзды были те же по числу, но
         поярче. В жизни наоборот: полоса это прежде всего СГУЩЕНИЕ,
         звёзд там на порядок больше на ту же площадь, и именно
         поэтому она читается рекой, а не просто светлым местом.
         Гауссово отклонение от плоскости собираем суммой трёх
         равномерных - предельная теорема даёт колокол дешевле, чем
         логарифм с корнем.

         Ровное распределение по сфере берём через косинус широты:
         равномерная широта сбивает звёзды к полюсам, и на потолке
    ...
    var мат = new T.ShaderMaterial({
      ...
      vertexShader: [
        ...
        /* Нижняя граница размера - РОВНО ОДНА точка буфера, и это не
           придирка. Слабое большинство поля считалось по 0.7 пикселя,
           то есть меньше точки: расстояние в gl_PointCoord у такого
           фрагмента почти всегда больше единицы, и шейдер отбрасывал
           звезду до того, как она успевала загореться. Три четверти
           неба не рисовались вовсе, и владелец увидел ровно это -
           «просто чёрное, некрасиво». Одна точка это минимум, ниже
           которого звезды на экране не бывает. */
      ...
      fragmentShader: [
        ...
        "    ray = (hx + vy) * 0.26 * vTop;",
        "  }",
        /* Свободный член это яркость самой слабой видимой звезды.
           Шестнадцать сотых на чёрном дают сорок из двухсот
           пятидесяти пяти, и это ещё различимо на мониторе, но не на
           телефоне при дневном свете. Поднят до двадцати двух сотых:
           слабое большинство неба должно быть видно, иначе его можно
           было и не считать. */
  ...
  /* -- ЧЁРНОЙ ДЫРЫ В НЕБЕ БОЛЬШЕ НЕТ --------------------------------
     Слово руководства через владельца: «чёрную дыру вообще убрать
     надо, чёрную дыру просто убираем». До этого её уже переименовали
     из ведомства в «ДЫРУ ЗЛА» - теперь снимаем совсем.

     Сам узел остаётся собранным и НЕ добавляется в сцену. Причина не
     в лени: на его место ссылается счёт маршрута (`at.hole`), выбор
     отрезка пути и наведение. Удали объект - и весь путь надо
     пересчитывать заново, а это правка на порядок больше и с другим
     риском. Не добавленный в сцену узел не рисуется никогда, то есть
     в небе его нет, а числа маршрута остаются целы.

     Из всего, что видит человек, дыра убрана отдельно: меню курса,
     сканер, досье, поиск и отбойник столкновений. */
  hole.visible = false;
  ...
  /* Нажимать не на что: дыры в небе нет. Луч ходит по списку узлов, а
     не по сцене, поэтому неубранный узел ловил бы нажатия вслепую. */
  var pickables = [eBody, moon, mars, saturn.children[0],
  ...
  /* Кома светит на просвет, а не заливает кадр. Непрозрачность стояла
     единицей при сложении цветов, и вблизи облако выбеливало всё
     вместе с ядром внутри - ядра было не видно вовсе. Половина даёт
     то же светлое облако издали и оставляет тело видимым вблизи. */
  var cometCore = new T.Sprite(new T.SpriteMaterial({
    map: glowSprite(64, "rgba(220,244,255,1)", "rgba(160,220,255,0)"),
    transparent: true, opacity: 0.52, depthWrite: false, blending: T.AdditiveBlending
  }));
  ...
  /* ── ЯДРО КОМЕТЫ ─────────────────────────────────────────────
     Свечение выше это КОМА - облако испарившегося газа вокруг ядра.
     Самого ядра тут не было вовсе, и комета выходила светящимся
     шариком с хвостом. Владелец назвал кометы среди объектов, которые
     надо довести до реализма.

     Настоящее ядро выглядит ровно наоборот: это самое ТЁМНОЕ тело
     Солнечной системы, чернее угля (Чурюмова-Герасименко отражает
     шесть процентов света), и форма у него всегда рваная - глыба,
     а не шар. Светит не оно, светит кома вокруг.

     Делаем так: тёмная неправильная глыба внутри светлой комы. Форму
     мнём по вершинам шара своим шумом - геометрия остаётся дешёвой, а
     силуэт перестаёт быть кругом. Плюс две струи: у активной кометы
     газ бьёт из трещин направленными выбросами, и они видны как
     светлые лучи из ядра. */
  var ядроК = (function () {
    /* Сетка мельче, чем кажется нужным: ядро мнут по вершинам, и на
       редкой сетке мятость выходит гранями многогранника, а не
       рельефом. Портрет в упор показал именно это - силуэт из
       заметных плоских кусков. */
    var г = new T.SphereGeometry(2.6, 40, 28);
    var п = г.attributes.position, в = new T.Vector3();
    var сК = 77713;
    function слК() {
      сК ^= сК << 13; сК >>>= 0; сК ^= сК >>> 17; сК ^= сК << 5; сК >>>= 0;
      return сК / 4294967296;
    }
    /* Три волны разной частоты: крупные доли тела, средние впадины,
       мелкая крошка. Одна волна дала бы ровную картофелину. */
    for (var i = 0; i < п.count; i++) {
      в.fromBufferAttribute(п, i);
      var н = в.clone().normalize();
      var к = 1
        + Math.sin(н.x * 1.7 + 0.6) * Math.cos(н.y * 1.3) * 0.34
        + Math.sin(н.y * 3.4 + 1.9) * Math.cos(н.z * 2.8) * 0.19
        + Math.sin(н.z * 7.1 + 4.2) * Math.cos(н.x * 5.9) * 0.075
        /* Впадина-кратер на одном боку: у каждого снятого вблизи ядра
           есть такая яма, и она сразу даёт телу масштаб. */
        - Math.exp(-((н.x - 0.55) * (н.x - 0.55) + (н.y + 0.42) * (н.y + 0.42) +
                     (н.z - 0.72) * (н.z - 0.72)) * 5.5) * 0.30
        + (слК() - 0.5) * 0.035;
      в.copy(н).multiplyScalar(2.6 * к);
      п.setXYZ(i, в.x, в.y, в.z);
    }
    г.computeVertexNormals();
    /* Тёмное не значит чёрное. Первый заход поставил почти угольный
       цвет с почти нулевым собственным свечением, и портрет кометы в
       упор показал ровно дыру в кадре: ядро стоит в яркой коме, свет
       от солнца сюда приходит вскользь, и поверхность не читалась
       никак. У настоящего ядра отражение шесть процентов, но оно
       СТОИТ В СВЕТЕ, и глаз видит серо-бурый камень с бликами льда.
       Поднимаем и цвет, и собственное свечение до того уровня, на
       котором фактура видна, а тело всё равно остаётся самым тёмным
       в кадре. */
    return new T.Mesh(г, new T.MeshPhongMaterial({
      color: 0x6a6157, shininess: 14, specular: 0x3a352c, emissive: 0x2b2620
    }));
  })();
  ядроК.userData.info = cometCore.userData.info;
  /* Мелкая шероховатость приходит из общего слоя реализма: своих карт
     для одного тела рисовать незачем, а без них поверхность гладкая,
     как полированный камень. */
  if (g.RC_REAL && g.RC_REAL.upgradeTree) {
    try {
      /* ── ПОВТОР КАРТЫ МЕЛЬЧЕ, ИНАЧЕ ЭТО ПОЛОСКИ ────────────────
         Стояло два повтора при силе рельефа полтора. На шаре развёртка
         идёт по долготе и широте, поэтому два повтора это не «зерно», а
         ЧЕТЫРЕ широкие полосы поперёк тела: портрет в упор показал
         ядро в рябчатую полоску, будто плетёное.

         Шесть повторов делают ту же карту мелкой крошкой, а не
         рисунком, и силу рельефа при этом надо опустить: на мелком шаге
         полторы единицы дают шум, а не камень. */
      g.RC_REAL.upgradeTree(T, ядроК, function () {
        return { kind: "hull", roughness: 0.94, metalness: 0.02, normalScale: 1.1, repeat: 6, envMapIntensity: 0.35 };
      });
    } catch (eЯ) {}
  }
  comet.add(ядроК);
  /* ── СТРУИ ГАЗА ИЗ ТРЕЩИН: РОССЫПЬ, А НЕ КОНУС ────────────────
     Здесь дважды стоял конус, и дважды портрет в упор показывал одно
     и то же: два плоских светлых клина из ядра. Сначала радиусом
     полторы единицы при ядре в две с половиной, потом сужённый до
     0.55 и почти прозрачный - и всё равно клин, потому что у конуса
     есть КРОМКА. Треугольная кромка на просвет читается бумагой при
     любой непрозрачности, а замена числа её не убирает.

     Владелец про этот кадр сказал прямо: «полупрозрачные белые пятна
     летают, как каша», «нереалистично».

     Настоящий выброс это пыль, а не поверхность. Собираем его тем же
     способом, что и хвост, звёздное поле, пояс и объём галактики -
     россыпью точек с размером В ПИКСЕЛЯХ. Тогда у струи нет кромки
     вовсе, вблизи она рассыпается на крупинки льда, а издали остаётся
     тем же светлым лучом, каким была.

     Раскладка взята у снимков Чурюмова-Герасименко: плотно у самого
     жерла, дальше всё реже, на конце расходится веером. Отсюда шаг по
     длине корнем (у жерла часто, дальше редко), разброс поперёк
     пропорционален длине (веер), яркость падает степенью 1.5 (газ
     разлетается и тускнеет, а не обрывается). */
  var СТРУИ = [[0.9, -0.3, 1.2], [-0.7, 0.8, 0.6]];
  (function () {
    var НАСТРУЮ = 150, ДЛИНА = 7.5, ЖЕРЛО = 3.1, ВЕЕР = 0.16;
    var всего = СТРУИ.length * НАСТРУЮ;
    var поз = new Float32Array(всего * 3), цв = new Float32Array(всего * 3);
    var сС = 20931;
    function слС() {
      сС ^= сС << 13; сС >>>= 0; сС ^= сС >>> 17; сС ^= сС << 5; сС >>>= 0;
      return сС / 4294967296;
    }
    var к = 0;
    for (var ст = 0; ст < СТРУИ.length; ст++) {
      var нп = new T.Vector3(СТРУИ[ст][0], СТРУИ[ст][1], СТРУИ[ст][2]).normalize();
      /* Два поперечника к направлению выброса: по ним и расходится веер. */
      var бок = new T.Vector3(0, 1, 0);
      if (Math.abs(нп.y) > 0.9) бок.set(1, 0, 0);
      var о1 = new T.Vector3().crossVectors(нп, бок).normalize();
      var о2 = new T.Vector3().crossVectors(нп, о1).normalize();
      for (var i = 0; i < НАСТРУЮ; i++) {
        var д = Math.sqrt(слС());              /* густо у жерла */
        var уг = слС() * Math.PI * 2;
        var р = (0.10 + д * ВЕЕР) * ДЛИНА * Math.sqrt(слС());
        var вд = ЖЕРЛО + д * ДЛИНА;
        поз[к * 3] = нп.x * вд + (о1.x * Math.cos(уг) + о2.x * Math.sin(уг)) * р;
        поз[к * 3 + 1] = нп.y * вд + (о1.y * Math.cos(уг) + о2.y * Math.sin(уг)) * р;
        поз[к * 3 + 2] = нп.z * вд + (о1.z * Math.cos(уг) + о2.z * Math.sin(уг)) * р;
        var я = Math.pow(1 - д, 1.5) * (0.55 + слС() * 0.75);
        if (я < 0.03) я = 0.03;
        /* Цвет тот же, что был у конуса: голубоватый лёд на просвет. */
        цв[к * 3] = 0.75 * я; цв[к * 3 + 1] = 0.90 * я; цв[к * 3 + 2] = 1.00 * я;
        к++;
      }
    }
    var гС = new T.BufferGeometry();
    гС.setAttribute("position", new T.BufferAttribute(поз, 3));
    гС.setAttribute("color", new T.BufferAttribute(цв, 3));
    var струи = new T.Points(гС, new T.PointsMaterial({
      size: 2.2, sizeAttenuation: false, vertexColors: true,
      transparent: true, opacity: 0.85, depthWrite: false,
      blending: T.AdditiveBlending
    }));
    струи.name = "струи кометы";
    comet.add(струи);
  })();
  /* ── Хвосты кометы ────────────────────────────────────────
  ...
  var cometTail = (function () {
    ...
    /* ── КРУПИНКА ХВОСТА ЭТО ТОЧКА, А НЕ ШАР ──────────────────
       Тут стоял размер в единицах МИРА с ослаблением по дальности, и
       правка «мельче и ярче» его не вылечила: при ослаблении размер
       на экране растёт обратно расстоянию, и вблизи любая крупинка
       раздувается без предела. Портретная проверка объектов сняла
       комету в упор, и на снимке весь хвост это мягкие белые шары по
       сто пикселей - ровно то, что владелец описал словами
       «полупрозрачные белые пятна летают, нереалистично».

       Размер задаём в ПИКСЕЛЯХ, как у всех остальных россыпей этой
       сцены (звёздное поле, пыль у стекла, галактический объём, пояс
       астероидов). Тогда крупинка остаётся крупинкой с любого
       расстояния, а вблизи хвост читается россыпью льда, как на
       снимках Чурюмова-Герасименко. Издали вид не меняется вовсе:
       там точка и так была в пару пикселей.

       Плотность берём у рендерера, а не у экрана: холст рисуется со
       своей плотностью, и размер в пикселях буфера обязан считаться
       от неё, иначе на телефоне точка выходит вдвое крупнее. */
    var пл = Math.min(2, (r && r.getPixelRatio ? r.getPixelRatio() : (g.devicePixelRatio || 1)));
    return new T.Points(geo, new T.PointsMaterial({
      color: 0xffffff, size: 2.0 * пл, sizeAttenuation: false, map: starDot,
      vertexColors: true,
  ...
  /* ── СПУТНИК RC-SAT ──────────────────────────────────────────
     Здесь стоял ящик и две плоскости: коробка 1.4x1.4x2.8 светло-
     серого цвета плюс два прямоугольника по бокам. Владелец назвал
     этого среди объектов, которые надо довести до реализма, и назвал
     справедливо - на подлёте он читался значком, а не аппаратом.

     Настоящий спутник узнаётся по четырём вещам, и все четыре видны
     даже мельком:

       1. ЗОЛОТАЯ ИЗОЛЯЦИЯ. Корпус обёрнут многослойной плёнкой, и
          она не серая, а тёплая янтарная с сильным зеркальным
          бликом. Это первое, что отличает аппарат от куска металла.
       2. ПАНЕЛИ СОБРАНЫ ИЗ ЯЧЕЕК и стоят на штангах, а не приклеены
          к борту. Ячейки дают ту самую сетку, по которой панель
          читается панелью с любого расстояния.
       3. АНТЕННА. Параболическая тарелка с облучателем на треноге:
          без неё аппарат не аппарат, а бак.
       4. МЕЛОЧЬ, КОТОРАЯ ДАЁТ МАСШТАБ: радиатор, звёздный датчик,
          сопла коррекции. Глаз читает размер по деталям, а гладкая
          коробка кажется игрушкой любого размера.

     Всё собрано из простых тел и после сборки прогоняется через
     RC_REAL.upgradeTree: фактура, шероховатость и отражение неба
     приходят оттуда, руками тут ничего не рисуется. */
  var sat = new T.Group();

  /* Ячейки солнечной панели: рисуем сеткой, а не цветом. Настоящая
     батарея это ряды прямоугольных элементов с тонкими светлыми
     шинами между ними, и именно шины ловят солнце под углом. */
  var ячейкиТекс = (function () {
    var ш = 256, в = 128, к = doc.createElement("canvas");
    к.width = ш; к.height = в;
    var кx = к.getContext("2d");
    кx.fillStyle = "#0a1c33"; кx.fillRect(0, 0, ш, в);
    var стлб = 16, ряд = 8;
    for (var сy = 0; сy < ряд; сy++) {
      for (var сx = 0; сx < стлб; сx++) {
        /* Оттенок ячейки чуть гуляет: кремний неоднороден, и ровный
           цвет по всей панели читается пластиком. */
        var т = 0.86 + ((сx * 7 + сy * 13) % 11) / 42;
        кx.fillStyle = "rgba(" + ((26 * т) | 0) + "," + ((62 * т) | 0) + "," + ((118 * т) | 0) + ",1)";
        кx.fillRect(сx * ш / стлб + 1.5, сy * в / ряд + 1.5, ш / стлб - 3, в / ряд - 3);
      }
    }
    /* Токосъёмные шины поверх ячеек */
    кx.strokeStyle = "rgba(190,208,226,.55)"; кx.lineWidth = 1;
    for (сy = 0; сy <= ряд; сy++) {
      кx.beginPath(); кx.moveTo(0, сy * в / ряд); кx.lineTo(ш, сy * в / ряд); кx.stroke();
    }
    for (сx = 0; сx <= стлб; сx++) {
      кx.beginPath(); кx.moveTo(сx * ш / стлб, 0); кx.lineTo(сx * ш / стлб, в); кx.stroke();
    }
    var т2 = new T.CanvasTexture(к);
    т2.colorSpace = T.SRGBColorSpace || т2.colorSpace;
    т2.anisotropy = 8;
    return т2;
  })();

  /* Корпус: восьмигранная призма в золотой плёнке. Восемь граней, а
     не четыре - блик тогда перебегает по корпусу при обороте, и
     аппарат виден вращающимся даже без единой другой приметы. */
  var satBody = new T.Mesh(
    new T.CylinderGeometry(1.05, 1.05, 2.9, 8),
    new T.MeshPhongMaterial({ color: 0xc8a24a, shininess: 120, specular: 0x6b5a2e, emissive: 0x1a1408 })
  );
  satBody.rotation.x = Math.PI / 2;
  satBody.userData.info = СЛ("спутник");
  sat.add(satBody);

  /* Поясок радиатора: белая полоса поперёк корпуса. Радиатор всегда
     светлый - он сбрасывает тепло, и красят его белым нарочно. */
  var радиатор = new T.Mesh(
    new T.CylinderGeometry(1.12, 1.12, 0.55, 8, 1, true),
    new T.MeshPhongMaterial({ color: 0xe8eef4, shininess: 40, side: T.DoubleSide, emissive: 0x1b2430 })
  );
  радиатор.rotation.x = Math.PI / 2;
  радиатор.position.z = -0.75;
  sat.add(радиатор);

  var штангаМат = new T.MeshPhongMaterial({ color: 0x9aa6b4, shininess: 70, emissive: 0x141c26 });
  /* ── У ПАНЕЛИ ДВЕ РАЗНЫЕ СТОРОНЫ ──────────────────────────────
     Стояла одна плоскость с `DoubleSide`, то есть ячейки кремния были
     нарисованы и на ИЗНАНКЕ крыла. Портрет в упор это и показал:
     дальнее крыло повёрнуто к нам тылом и светится розовым - синий
     кремний, подсвеченный тёплым светом на просвет. У живого аппарата
     тыл панели это тёмная сотовая подложка, она не блестит и не
     синеет вовсе.

     Делаем два полотна спиной к спине: лицо с ячейками наружу, тыл
     тёмный. Плоскостей вдвое больше (восемь вместо четырёх), но это
     восемь треугольников, а не восемь тел со своей физикой. */
  var панельМат = new T.MeshPhongMaterial({
    map: ячейкиТекс, color: 0xffffff, shininess: 150, specular: 0x37527a,
    side: T.FrontSide, emissive: 0x070f1c
  });
  var тылМат = new T.MeshPhongMaterial({
    color: 0x2b2f36, shininess: 12, specular: 0x14171c,
    side: T.FrontSide, emissive: 0x090b0e
  });
  for (var кр = 0; кр < 2; кр++) {
    var зн = кр ? -1 : 1;
    /* Штанга от корпуса к панели */
    var штанга = new T.Mesh(new T.CylinderGeometry(0.10, 0.10, 1.5, 6), штангаМат);
    штанга.rotation.z = Math.PI / 2;
    штанга.position.x = зн * 1.7;
    sat.add(штанга);
    /* Крыло из двух секций: настоящая батарея складная, и стык между
       секциями виден тонкой тенью. */
    for (var сек = 0; сек < 2; сек++) {
      var крыло = new T.Mesh(new T.PlaneGeometry(2.9, 1.9), панельМат);
      крыло.position.set(зн * (2.9 + сек * 3.0), 0, 0);
      /* Малый развал секций: идеально плоское крыло читается
         картонкой, а у живого аппарата секции всегда чуть разведены
         петлями. */
      крыло.rotation.y = зн * (сек ? 0.05 : -0.03);
      sat.add(крыло);
      /* Тыл того же полотна: повёрнут на пол-оборота и отодвинут на
         толщину панели, чтобы две плоскости не спорили за пиксель.

         Отодвигать надо по СОБСТВЕННОЙ оси полотна, а не по оси узла.
         Первый заход вычел два сантиметра из `position.z` спутника, а
         узел спутника в сцене развёрнут - и тыл уехал НА ЗРИТЕЛЯ,
         закрыв ячейки. Портрет показал это сразу: вместо синих крыльев
         два серых полотнища поперёк аппарата.

         `translateZ` идёт по местной оси уже повёрнутого тела: после
         разворота на пол-оборота его плюс Z смотрит туда же, куда
         изнанка лица, то есть ровно назад. */
      var тыл = new T.Mesh(new T.PlaneGeometry(2.9, 1.9), тылМат);
      тыл.position.copy(крыло.position);
      тыл.rotation.y = крыло.rotation.y + Math.PI;
      тыл.translateZ(0.02);
      sat.add(тыл);
    }
  }

  /* Антенна: тарелка чашей наружу, облучатель на треноге */
  var тарелка = new T.Mesh(
    new T.SphereGeometry(1.15, 24, 12, 0, Math.PI * 2, 0, Math.PI * 0.32),
    new T.MeshPhongMaterial({ color: 0xdfe6ee, shininess: 160, specular: 0x8fa2b6, side: T.DoubleSide, emissive: 0x161e28 })
  );
  тарелка.rotation.x = Math.PI * 0.62;
  тарелка.position.z = 1.9;
  sat.add(тарелка);
  var облучатель = new T.Mesh(new T.CylinderGeometry(0.11, 0.16, 0.5, 8), штангаМат);
  облучатель.rotation.x = Math.PI / 2;
  облучатель.position.z = 2.7;
  sat.add(облучатель);
  for (var нг = 0; нг < 3; нг++) {
    var уг = нг * 2.0944;
    var нога = new T.Mesh(new T.CylinderGeometry(0.045, 0.045, 1.1, 5), штангаМат);
    нога.position.set(Math.cos(уг) * 0.42, Math.sin(уг) * 0.42, 2.3);
    нога.rotation.x = Math.PI / 2;
    нога.rotation.z = Math.cos(уг) * 0.34;
    sat.add(нога);
  }

  /* Звёздный датчик и два сопла коррекции: мелочь, которая даёт
     аппарату размер. */
  var датчик = new T.Mesh(new T.CylinderGeometry(0.22, 0.22, 0.7, 10),
    new T.MeshPhongMaterial({ color: 0x2a3340, shininess: 20, emissive: 0x0a0f16 }));
  датчик.position.set(0.7, 0.75, -0.2);
  датчик.rotation.set(-0.5, 0, 0.3);
  sat.add(датчик);
  for (var сп = 0; сп < 2; сп++) {
    var сопло = new T.Mesh(new T.CylinderGeometry(0.07, 0.15, 0.32, 8), штангаМат);
    сопло.position.set(сп ? 0.8 : -0.8, -0.7, -1.5);
    сопло.rotation.x = Math.PI / 2;
    sat.add(сопло);
  }

  /* Физика поверхности приходит из общего слоя реализма: плёнка
     корпуса зеркальная и шершавая, стекло панелей гладкое, штанги
     матовый металл. Своих карт тут не рисуем. */
  if (g.RC_REAL && g.RC_REAL.upgradeTree) {
    try {
      g.RC_REAL.upgradeTree(T, sat, function (о, м) {
        if (м === панельМат) return { kind: "glass", roughness: 0.22, metalness: 0.30, envMapIntensity: 0.40 };
        /* Тыл панели это сотовая подложка: матовая, почти не
           металлическая и без рельефной карты. Через общий слой она
           получила бы блеск стекла и стала бы вторым лицом. */
        if (м === тылМат) return { kind: "panel", roughness: 0.88, metalness: 0.10, envMapIntensity: 0.35 };
        /* ── ПОВТОР КАРТЫ НА КОРПУСЕ ──────────────────────────────
           Повтор тут не стоял вовсе, и карта рельефа растягивалась на
           всю развёртку восьмигранника: портрет в упор показал корпус
           в вертикальную рябь, будто гофрокартон. Золотая плёнка
           экранной изоляции мнётся мелко и НЕРАВНОМЕРНО, поэтому берём
           три повтора и опускаем силу рельефа: на мелком шаге прежние
           девять десятых дают шум вместо складок. */
        if (о === satBody) return { kind: "hull", roughness: 0.34, metalness: 0.92, normalScale: 0.55, repeat: 3, envMapIntensity: 1.4 };
        return { kind: "panel", roughness: 0.52, metalness: 0.70 };
      });
    } catch (eС) {}
  }
  scene.add(sat);
...
function size() {
  if (!W3 || !F.open) return;
  deckSkinSoon();
  /* Нулевая высота окна бывает: часть Android-WebView отдаёт её на
     повороте экрана, и свёрнутое окно тоже. Делить на неё нельзя -
     соотношение уходит в бесконечность, матрица проекции становится
     ненулевой чепухой, и кадр не восстанавливается сам даже когда
     размер вернулся. Единица здесь это заведомо неверное, но живое
     число: следующий же честный размер её перепишет. */
...
function reqPick() {
  ...
  if (uniIdx === 0) {
    /* Солнца в этом списке не было, а узел на нём ставится и в
       ...
       было недостижимо. */
    /* Дыры в списке нет: её сняли со сцены целиком, и всплеск трафика
       звал человека к телу, которого в небе не существует. На табло
       это выходило прямым текстом «ЗАПРОС ТРАФИКА · ЧЁРНАЯ ДЫРА ·
       нужен узел», и поймано это портретной проверкой объектов. */
    var names = [GOAL_NAMES.earth, GOAL_NAMES.moon, GOAL_NAMES.mars, GOAL_NAMES.saturn,
                 ...
                 GOAL_NAMES.uranus, GOAL_NAMES.neptune];
...
function frame(ts) {
  ...
  if (F.stage) окно.length = 0;
    ...
    /* Пол подбора в САЛОНЕ выше, чем в полёте, и по той же причине,
       по которой выше потолок: панель нарисована внутри кадра, и её
       резкость есть резкость кадра. В полёте пульт живёт слоем
       разметки, там холсту можно и просесть. */
    var пол = F.stage ? 1.4 : 0.72, пот = w3["потолокПл"] || тек;
    var нов = 0;
  ...
  /* У двух титров текст зависит от продукта: на сайте VPN Земля это
     узлы VPN. Берём слово в момент показа, а не
...
function open() {
  ...
  /* ── ЖДЁМ СО СЧЁТОМ И СПРАШИВАЕМ, НУЖЕН ЛИ ПОЛЁТ ─────────────
     Ожидание было бессрочным и безусловным: таймер на 220 мс без
     счётчика попыток, и когда модуль доезжал, `open()` звался в любом
     случае. Два следствия, оба видны человеку. Модуль не доехал вовсе
     (обрыв, 404) - таймер тикает до закрытия вкладки. Модуль доехал
     через минуту, а человек за эту минуту закрыл полёт и ушёл читать
     раздел - игра открывалась САМА, поверх того, что он смотрит.

     Даём тридцать секунд (сто тридцать шесть попыток) и спрашиваем
     перед открытием, ждут ли полёт до сих пор. */
  if (!g.RC_CABIN) {
    if (!open._ждуКабину) {
      open._попыток = 0;
      open._ждуКабину = setInterval(function () {
        var сдаться = ++open._попыток > 136;
        if (!g.RC_CABIN && !сдаться) return;
        clearInterval(open._ждуКабину);
        open._ждуКабину = 0;
        if (сдаться && !g.RC_CABIN) {
          if (g.RC_track) g.RC_track("jserr", "flight: rc-cabin не доехал за 30 с", true);
          return;
        }
        /* Полёт с тех пор могли закрыть. Открывать его тогда нельзя:
           человек уже в другом месте страницы. */
        if (!F.хотятПолёт) return;
        open();
      }, 220);
    }
    F.хотятПолёт = true;
...
function close() {
  if (!F.open) return;
  F.open = false;
  /* Полёта больше не ждут. Флаг читает отложенное открытие: модуль
     рубки мог ещё ехать, и без этого он открыл бы игру поверх того,
     что человек уже смотрит вместо неё. */
  F.хотятПолёт = false;
  F.stage = false;
  F.stageK = 0;
  if (F.raf) { cancelAnimationFrame(F.raf); F.raf = null; }
...
function trafBuild() {
  ...
  var geo = new T.BufferGeometry();
  geo.setAttribute("position", new T.BufferAttribute(pos, 3));
  /* Размер пакета в ПИКСЕЛЯХ, как у всех россыпей этой сцены.
     Ослабление по дальности раздувало пакет, пролетающий рядом с
     бортом, в мягкий шар: в сцене это была последняя россыпь, где
     оно ещё оставалось, и белые пятна шли в том числе отсюда. */
  var плТ = Math.min(2, W3.r.getPixelRatio ? W3.r.getPixelRatio() : 1);
  traf = new T.Points(geo, new T.PointsMaterial({
    color: 0xbfe9ff, size: 5 * плТ, sizeAttenuation: false,
...
function cabinBuild() {
  ...
  cabin = g.RC_CABIN.build(T, {
    /* Отрисовщик нужен салону, чтобы знать версию WebGL: под первой
       версией текстуры экранов собираются другим сэмплером, иначе
       читаемый текст рубки ужимается вдвое ещё до растеризации. */
    renderer: W3 && W3.r ? W3.r : null,
    tiny: innerWidth < 760,
    ...
    /* Плотность печати лица секции идёт за плотностью кадра: текстура
       мельче кадра мылит панель до всякой растеризации, текстура
       крупнее кадра памяти стоит зря. Потолок тот же, что у кадра. */
    /* ── ПЛОТНОСТЬ БЕРЁТСЯ У ОТРИСОВЩИКА, А НЕ У ЭКРАНА ──────────
       devicePixelRatio отвечает на другой вопрос: сколько точек у
       экрана на точку разметки. Кадр рисуется со СВОЕЙ плотностью, и
       на мониторе она не единица - подбор поднимает её до 1.8, когда
       машина тянет. Печь лицо по dpr значило печь его в 1.8 раза
       грубее кадра, в который оно встанет, и правка плотности кадра
       этого не чинила: мылило уже на текстуре. Спрашиваем отрисовщик,
       он знает точно; dpr остаётся нижней границей и запасом. */
    "плотность": (function () {
      var пл = (W3 && W3.r && W3.r.getPixelRatio) ? W3.r.getPixelRatio() : 0;
      if (!(пл > 0)) пл = g.devicePixelRatio || 1;
      return Math.min(Math.max(пл, g.devicePixelRatio || 1), tiny ? 1.0 : 3.0);
    })(),
...
function deckFrame(ts, dt) {
  ...
  for (var i = 0; i < мест; i++) {
    ...
    /* Кто под рукой. Спрашиваем у САМОЙ КНОПКИ, а не заводим слежение
       ...
       смотрит глаз. Фокус с клавиатуры считаем наведением тоже -
       человек, идущий по пульту табом, обязан видеть, где он стоит. */
    if (наведён < 0) {
      try {
        if (el.matches(":hover") || el === doc.activeElement) наведён = i;
      } catch (eН) {}
    }
    var q = d["место"](i);
    if (!q) continue;
  ...
  /* Одним числом на весь пульт, а не по клавише: под рукой всегда
     ровно одна, и слою достаточно знать её номер. */
  if (d["навести"]) d["навести"](наведён);
...
/* ── ЧТО МЕРИТЬ У ПУЛЬТА: НЕ ГЕОМЕТРИЮ, А СВЕТ ПОД НИМ ───────────
   ...
   `flatBuild`) считается в экранных координатах - контур, окно, доли
   кадра `frameShare` - и кладётся обводкой поверх кадра. Меша у неё
   нет ни одного.

   Значит рама на обоих сайтах стоит в кадре ОДИНАКОВО ПО ПОСТРОЕНИЮ:
   она зависит от размера кадра, поля зрения и паспорта раскладки, а от
   камеры не зависит вовсе.

   А расходится то, что видно ПОД рамой: замер яркости низа кадра дал
   64.1 у соседа против 7.1 здесь. Рама одна, вырез один, значит в него
   смотрят разные вещи - и разница в СВЕТЕ на настиле. Мерка отдаёт доли
   кадра у рамы и перепись света в рубке. Мерить надо то, что
   расходится, а не то, что удобно измерить. */
function пультМерка() {
  if (!W3 || !cabin || !W3.cam) return { "нет": "мира, рубки или камеры" };
  var T3 = W3.T || g.THREE;
  ...
  /* Перепись света. Ищем от узла рубки вниз: лампа выше него освещает
     весь мир и к рубке отношения не имеет. */
  var лампы = [];
  var корень = cabin.group || null;
  if (корень) {
    корень.updateMatrixWorld(true);
    корень.traverse(function (о) {
      if (!о.isLight) return;
      var м = корень.worldToLocal(о.getWorldPosition(new T3.Vector3()));
      лампы.push({
        "вид": о.type,
  ...
  /* ── ОДНИ И ТЕ ЖЕ ТОЧКИ РУБКИ НА ЭКРАНЕ ─────────────────────────
     Те же точки в МЕСТНЫХ координатах рубки (файл рубки у сайтов байт в
     байт один, значит точки у обоих одни и те же), перевод в мир через
     сам узел, проекция живой камерой. Сосед (rocketvpn) отдаёт их тем же
     кодом, и сверка `tools/рубки-рядом.mjs` кладёт числа рядом.

     Нужно это затем, что снимки рядом показывают пульт у соседа на
     четверть кадра выше нашего при равном глазе, равном поле и равном
     наклоне. При равной геометрии так быть не может, и мерка обязана
     показать, какое из «равных» равно не там, где мерили. */
  var точки = {};
  var узелР = cabin.group || null;
  if (узелР) {
    var R = cabin.R || 3.05, ГЛ = cabin.eye || 1.62, ВЫС = 4.2;
    var кудаСмотрим = {
      "настил у стены": [0, 0, -R],
      "потолок у стены": [0, ВЫС, -R],
      "глаз у стены": [0, ГЛ, -R],
      "стена слева": [-R, ГЛ, 0],
      "стена справа": [R, ГЛ, 0]
    };
    for (var имяТ in кудаСмотрим) {
      if (!Object.prototype.hasOwnProperty.call(кудаСмотрим, имяТ)) continue;
      var т = кудаСмотрим[имяТ];
      var м = узелР.localToWorld(new T3.Vector3(т[0], т[1], т[2]));
      var вид = м.clone().applyMatrix4(W3.cam.matrixWorldInverse);
      if (вид.z > -0.01) { точки[имяТ] = null; continue; }
      м.project(W3.cam);
      точки[имяТ] = [+(м.x * 0.5 + 0.5).toFixed(4), +(0.5 - м.y * 0.5).toFixed(4)];
    }
  }
...
/* ═══════════════════════════════════════════════════════════════
   НИЖЕ МЁРТВЫЙ КОД. НЕ ЗОВЁТСЯ НИЧЕМ И НИГДЕ.

   Плоская панель салона снята с маршрута: её роль забрала передняя
   секция объёмной рубки. Вместе с ней перестали зваться stageBake,
   stagePanel (все её упоминания лежат внутри неё самой), панельПрочь,
   заглушкаОкна и stageWall - проверено счётом вхождений по файлу:
   у stageBake и stageWall ровно по одному, то есть только объявление.

   Держим здесь до следующего захода, а не сносим прямо перед сдачей:
   снос это четыреста строк в трёх местах плюс правила в rc-flight.css
   (.rcf-cabbake), и цена ошибки выше цены лишних килобайт, которые
   браузер всё равно только разбирает, но не исполняет.

   ЧЕГО НЕ ДЕЛАТЬ: не «чинить» это и не звать заново. Панель, поднятая
   поверх объёмной рубки, ляжет вторым слоем на тот же пульт - ровно
   та беда, из-за которой её и сняли. И не трогать панельD: она живая
   и стоит выше по файлу, её зовут три места.
   ═══════════════════════════════════════════════════════════════ */
...
function stageCam(dt) {
  ...
  /* Угол берём как есть: расписание салона держит его нулём от входа
     до пульта, и доводить его к полному кругу больше не нужно - это
     и был оборот. Оставляем только дыхание головы, оно затухает к
     пульту вместе со всем остальным. */
  ...
  /* ── ВОРОТ ПО ОБОРОТУ БОЛЬШЕ НЕТ ──────────────────────────
     Здесь стояла страховка: подъезд к пульту не начинался, пока угол
     оборота не дойдёт до полного круга. Оборот владелец снял с обоих
     сайтов (rc-interior, ENTER_AZ), и угол теперь держится нулём всю
     дорогу - страховка при нуле закрывала подъезд НАВСЕГДА, и камера
     осталась бы стоять у порога. Убираем вместе с оборотом. */
  ...
  /* ── СВЕТ РУБКИ СВЕДЁН С ФИНАЛОМ VPN ────────────────────────
     Владелец 14.09.2026, глядя на два финала рядом: «панель и планеты
     так круче выглядят» - про яркий кадр. Общая мерка двух рубок
     (`tools/рубки-рядом.mjs`) числом говорит то же: сумма сил света у
     нас 4.4 против 8.4 у соседа, и главная разница в потолочной лампе,
     которую мы гасили в ноль (0.068 против 1.85).

     Прежняя мысль была «кадр в конце принадлежит остеклению, а не
     подсвеченным стенам за спиной», и для стен она верна. Но вместе со
     стенами гасла и ПАНЕЛЬ: её лицо освещают ровно эти лампы, и на
     снимке подписи клавиш уходили в тёмное. Поэтому гасим стены (тёплая
     и полусфера остаются притушенными), а потолочную и пультовую
     оставляем в полную силу - как у соседа.

     Числа не подобраны, а взяты замером: потолочная 1.85, пультовая
     3.9 в конце подъезда, полусфера 0.084, тёплая 0.075. */
  var dim = Math.max(0, Math.min(1, (ek - 0.55) / 0.4));
  if (C.deskLight) C.deskLight.intensity = 0.9 + ek * 3.0;
  if (C.hemi) C.hemi.intensity = 1.05 * (1 - dim * 0.92);
  if (C.ceilL) C.ceilL.intensity = 1.85;
  if (C.warmL) C.warmL.intensity = 1.5 * (1 - dim * 0.95);
  /* Диоды дышат */
...
function prebuild() {
  ...
  /* Пустой catch тут однажды уже стоил кадра: ошибка сборки корпуса
     рубки проглатывалась молча, кадр гас, а в консоли было пусто, и
     искать причину пришлось замером яркости по всей ленте. Пишем и в
     консоль, и в телеметрию. Метка "jserr", как у соседних записей. */
  var корпусСобран = false;
  if (g.RC_CABIN) {
    try { cabinBuild(); корпусСобран = true; }
    catch (e) {
      try { console.error("rc-flight: корпус рубки не собрался -", e); } catch (e2) {}
      if (g.RC_track) g.RC_track("jserr", "cabinBuild: " + (e && e.message), true);
    }
  }
  if (DBG) {
    ...
    /* Этап пишется по факту, а не безусловно: сборка могла упасть, и
       тогда замер отчитался бы о корпусе, которого в кадре нет. */
    ЭТАПЫ.push([корпусСобран ? "+ корпус рубки" : "+ корпус рубки (не собрался)",
                +(тК - тМир).toFixed(1)]);
  }
  return true;
}
...
function stage(k) {
  ...
  if (!g.RC_CABIN) {
    F.stageK = k;
    /* Ожидание со счётом, как у соседнего в open(). Бессрочный таймер
       на 220 мс при модуле, который не доедет вовсе (обрыв, 404), тикал
       бы до закрытия вкладки. Число попыток ТО ЖЕ, что у соседа: два
       ожидания одного и того же модуля обязаны сдаваться вместе, иначе
       одно перестанет ждать раньше другого и разобраться, почему рубка
       собралась наполовину, будет не по чему. Тихая сдача уходит в
       телеметрию: без записи она не отличима от успеха. */
    if (!stage._ждуКабину) {
      stage._попыток = 0;
      stage._ждуКабину = setInterval(function () {
        var сдаться = ++stage._попыток > 136;   /* тридцать секунд, как в open() */
        if (!g.RC_CABIN && !сдаться) return;
        clearInterval(stage._ждуКабину);
        stage._ждуКабину = 0;
        if (сдаться && !g.RC_CABIN) {
          if (g.RC_track) g.RC_track("jserr", "flight: rc-cabin не доехал за 30 с (stage)", true);
          return;
        }
...
function stageLite(on) {
  ...
  if (W3.r) {
    ...
    /* Потолок разный у салона и у игры, и разница не вкусовая.
       ...
       своя резкость, кадру он ничего не должен. Отсюда и стояло
       здесь «полтора против двух по космосу не читается». По пульту
       рассуждение верное, по космосу оно оказалось неверным, и
       владелец увидел это первым: звёзды, край планеты, кольца и
       титры голограмм растягивались вместе со всем кадром. Потолок
       игры теперь тоже берётся от экрана (ниже), а расход держит не
       заранее заниженное число, а регулятор плавности. */
    /* ── СТУПЕНИ «ОБОРОТА» БОЛЬШЕ НЕТ ─────────────────────────
       Здесь стояло три ступени, и средняя была под оборот: пока
       камера шла по кругу, кадр держали на 1.15, а двойку давали
       только на подъезде к пульту. Оборот снят (rc-interior.js), и
       человек идёт к панели с первого кадра внутри - значит панель
       обязана быть резкой с первого кадра. Владелец увидел ровно
       середину этой ступени: «почему панель управления в салоне и до
       старта игры такая некачественная».

       В САЛОНЕ пульт нарисован ВНУТРИ кадра: это передняя секция
       обшивки со снимком в развёртке, и его резкость есть резкость
       кадра. В ИГРЕ пульт это слой разметки поверх холста, ему
       плотность кадра не нужна вовсе - там холст рисует только
       космос. Отсюда две ступени вместо трёх. */
    /* В салоне потолок ставим по самому экрану, а не числом.

       Замер на телефоне с плотностью экрана три: холст выходил 655 на
       1417 точек при экране 1170 на 2532, то есть каждая точка кадра
       растягивалась в 1.79 раза - панель мылило ещё до растеризации.
       Двойка это не чинит: при экране в три точки на точку она даёт
       растяг полтора. Ставим потолком сам экран, зажатый тройкой, а
       дальше пусть решает регулятор: он меряет кадр на живом железе и
       сам опустит плотность, если телефон не тянет. Разница с игрой
       честная - там пульт живёт слоем разметки и от плотности холста
       не зависит вовсе, а холст рисует только космос. */
    /* ── В ИГРЕ ПЛОТНОСТЬ ТОЖЕ ПО ЭКРАНУ ──────────────────────
       Здесь стояло 1.35 на телефоне, и рассуждение под этим числом
       было такое: в игре пульт живёт слоем разметки, а холст рисует
       только космос, космосу полутора точек хватит. Первая половина
       верна, вторая нет. На телефоне с плотностью экрана три холст
       выходил 527 на 1139 при экране 1170 на 2532: каждая точка
       кадра растягивалась в 2.2 раза, и растягивался при этом ВЕСЬ
       космос - звёзды, кольца Сатурна, край планеты, титры на
       голограммах. Владелец описал это словами «на телефоне очень
       некачественно, как будто всё мутное, как каша; на компьютере
       ещё более-менее».

       Ставим потолком сам экран, зажатый двойкой. Двойка, а не
       тройка: выше неё на космосе разницы уже не видно, а заливка
       растёт квадратом. Дальше решает регулятор плавности - он
       меряет кадр на живом железе и сам опускает плотность до пола
       0.72, если телефон не тянет. Слабые устройства при этом не
       страдают дважды: ступени упрощения ниже вычитают своё. */
    var cap = on
      ? Math.min(3.0, Math.max(2.0, dpr))
      : (tiny ? 1.0 : Math.min(2.0, dpr));
...
g.RC_FLIGHT = {
  ...
  /* ── ГДЕ СТОИТ ГЛАЗ В РУБКЕ ───────────────────────────────
     Три числа, по которым сосед (rocketvpn) повторяет наш финал:
     высота глаза над настилом, расстояние от глаза до стены с
     панелью и наклон взгляда. Отдаём их живыми, из собранной сцены.

     Зачем наружу. У соседа финал собирается ДРУГИМ способом: у нас
     рубка ставится под камеру полёта (`back = cabin.R - панельD`,
     см. сборку рубки), у него камеру ведёт лента актов, а рубка
     стоит в мире. Один и тот же кадр приходится получать с двух
     сторон, и до сих пор он повторял наши ФОРМУЛЫ пересказом в
     своём коде. Пересказ разошёлся: щит у него стоит на четверть
     метра ближе обшивки, и та же `панельD`, отмеренная от щита,
     ставила глаз дальше пульта, чем у нас. Поймали это замером
     ширины пульта - 85.1 процента кадра против наших 91.0.

     Сверять пересказ было нечем: все проверки рубки смотрят на
     КАДР, внутрь сцены не лезет ни одна. Теперь есть чем, и правится
     это числом, а не подбором.

     Ничего не меняет: только читает. */
  "замерРубки": function () {
    if (!W3 || !cabin || !cabin.center || !W3.cam) return null;
    var T3 = W3.T || (g.THREE);
    var п = W3.cam.getWorldPosition(new T3.Vector3());
    var в = new T3.Vector3(0, 0, -1)
      .applyQuaternion(W3.cam.getWorldQuaternion(new T3.Quaternion()));
    /* Стена с панелью стоит на азимуте рубки, на её радиусе. */
    var впер = new T3.Vector3(Math.sin(cabin.yaw0 || 0), 0, -Math.cos(cabin.yaw0 || 0));
    var стена = cabin.center.clone().addScaledVector(впер, cabin.R);
    /* Габарит пульта: та же мерка стоит у VPN (rv-финал.js,
       `пультМерка`), сверяет `tools/рубки-рядом.mjs` на той стороне.
       Три числа про глаз уже сошлись у обеих рубок, а настил в кадре
       VPN всё равно светил - значит расходится сам пульт, и увидеть
       это можно только измерив его. */
    var пульт = пультМерка();
    return {
      "радиусОбшивки": +cabin.R.toFixed(4),
      "высотаГлаза": +cabin.eye.toFixed(4),
      "дистанцияПанели": cabin.dПан != null ? +cabin.dПан.toFixed(4) : null,
      /* Причину отказа проводим ОТДЕЛЬНЫМ полем: объект с полем «нет»
         правдив, и `пульт["мир"]` иначе выходит undefined - сверка
         печатает «не отдала габарит» вместо самой причины. */
      ...
      /* Живьём, а не по задумке: если расстановка когда-то разойдётся
         с формулой, это будет видно прямо здесь. */
      "глазНадНастилом": +(п.y - cabin.center.y).toFixed(4),
      "глазДоСтеныПанели": +new T3.Vector3(п.x - стена.x, 0, п.z - стена.z).length().toFixed(4),
      "тангажГрадусов": +(Math.asin(Math.max(-1, Math.min(1, в.y))) * 180 / Math.PI).toFixed(4),
      "поле": +W3.cam.fov.toFixed(4),
  ...
  /* Слой пульта наружу. Провал колпачка на снимке кадра не отличить
     от разгоревшегося света: свет перебивает геометрию, и обе меры,
     которые считаются по пикселям, показывают на старом коде то же
     самое, что на новом (проверено). Спрашиваем о движении там, где
     оно считается. Стоит за признаком rcdbg, как остальные служебные
     ходы. */
  _пульт: function () { return DBG ? deck : null; },
  /* ── МИР НАРУЖУ И СТОП-КАДР, ТОЛЬКО ДЛЯ ПРИЁМКИ ───────────
     Владелец просит довести до реализма кометы, спутники, метеориты
     и корабли. Судить об этом по общему кадру нельзя: спутник в нём
     занимает двадцать точек. Нужен портрет каждого объекта вплотную,
     а для портрета надо две вещи - добраться до узла и остановить
     кадр, иначе цикл полёта увезёт камеру обратно на маршрут прежде,
     чем снимок будет сделан.

     Оба хода закрыты признаком отладки: наружу мир не отдаём, там
     его трогать незачем. */
  _мир: function () { return DBG ? W3 : null; },
  _замереть: function (вкл) {
    if (!DBG) return null;
    if (вкл === false) {
      if (!F.raf && (F.open || F.stage)) F.raf = requestAnimationFrame(frame);
      F["_стоп"] = false;
    } else {
      if (F.raf) { cancelAnimationFrame(F.raf); F.raf = null; }
      F["_стоп"] = true;
    }
    return { замер: !!F["_стоп"] };
  },
  /* Нарисовать ОДИН кадр, не запуская цикл: камеру приёмка поставила
     сама, и надо увидеть ровно её, а не следующий шаг полёта. */
  _кадр: function () {
    if (!DBG || !W3) return null;
    try {
      if (W3.post) W3.post.render(W3.scene, W3.cam, performance.now());
      else W3.r.render(W3.scene, W3.cam);
    } catch (e) { return { нет: String(e).slice(0, 120) }; }
    return { нарисован: true };
  },
  ...
  /* ── КТО ИМЕННО МУТИТ КАДР ────────────────────────────────
     Владелец сказал про космос «полупрозрачные белые пятна летают»
     и «как каша». Слоёв, способных дать белёсое пятно, в небе пять:
     панорама на дальней сфере, живое звёздное поле, туманности,
     галактический объём и пыль у стекла. По снимку они неотличимы,
     и прошлые заходы правили то один, то другой наугад.

     Здесь слой гасится по имени, снимок делается ещё раз, и доля
     пятен говорит, чей это был вклад. Ничего не строит и не
     пересобирает - только переключает видимость, поэтому вернуть
     всё на место стоит второго вызова.

     Пример: RC_FLIGHT["слой"]("панорама", false) */
  "слой": function (имя, видно) {
    if (!W3) return null;
    var карта = {
      "панорама": [W3.sky],
      "звёзды": [W3.starShell],
      "туманности": W3.nebSprites || [],
      "объём": [W3.galacticVolume],
      "пыль": [W3.dust],
      "млечный": [W3.milky, W3.gal2, W3.gal3],
      /* Спираль это две разные вещи в одном узле: россыпь точек и
         мягкие подложки под ней (ядро-спрайт и свечение диска).
         Точки резкие и в пятна не попадают, подложки размытые по
         замыслу. Гасить их вместе значит не узнать, кто из двоих
         мутит кадр. */
      "подложки": (function () {
        var сп = [];
        var гр = [W3.milky, W3.gal2, W3.gal3];
        for (var i = 0; i < гр.length; i++) {
          if (!гр[i] || !гр[i].userData) continue;
          if (гр[i].userData["ядро"]) сп.push(гр[i].userData["ядро"]);
          if (гр[i].userData["гало"]) сп.push(гр[i].userData["гало"]);
        }
        return сп;
      })()
    };
    /* Свечение плёнки это не узел сцены, а несколько проходов поверх
       кадра, и гасится оно иначе - снятием самой постобработки.
       Держим его в том же ключе нарочно: для замера это такой же
       слой, и виновника надо искать среди всех шести, а не среди
       пяти «настоящих». */
    if (имя === "свечение") {
      if (видно === false) { W3._постСпрятан = W3._постСпрятан || W3.post; W3.post = null; }
      else if (W3._постСпрятан) { W3.post = W3._постСпрятан; W3._постСпрятан = null; }
      return 1;
    }
    var сп = карта[имя];
    if (!сп) { var к = Object.keys(карта); к.push("свечение"); return к; }
    var n = 0;
    for (var i = 0; i < сп.length; i++) {
      if (сп[i]) { сп[i].visible = !!видно; n++; }
    }
    return n;
  },
