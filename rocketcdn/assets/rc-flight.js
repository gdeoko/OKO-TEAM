...
function skyTexture(mob) {
  ...
  /* Мелкая звёздная пыль: зернистость диска, и только диска.

     Раньше сорок процентов её сыпалось по всему небу, а не в полосу.
     Панорама натянута на сферу радиусом 4200, в кадр попадает малая
     её доля, и каждый её тексель растягивается на экране в десяток
     пикселей: пылинка в один пиксель превращалась в жирную белую
     КЛЯКСУ. Небо выходило снежным, а настоящие звёзды - точки с
     честным параллаксом - тонули среди этих клякс.

     Пыль оставляем внутри полосы Млечного Пути, где она и нужна:
     там она читается зерном на светлом, а не отдельным светилом на
     чёрном. Прозрачности вдвое меньше по той же причине - в
     растянутом виде она складывается сама с собой. */
  var n = tiny ? 3000 : 7000;
  for (i = 0; i < n; i++) {
    u = rnd();
    var sx2 = u * W;
    var sy2 = bandY(u) + (rnd() - 0.5) * H * 0.16;
    var a = 0.05 + Math.pow(rnd(), 2.6) * 0.24;
...
function buildWorld() {
  ...
  var starMats = (function () {
    ...
    /* Цвет чёрного тела по температуре: приближение планковского
       локуса, то самое, по которому строят таблицы цветовой
       температуры для света. Считается при сборке, в кадре не
       стоит ничего, а цвет получается не «на глаз», а по шкале. */
    function планк(K) {
      var t = K / 100, r, gg, b;
      if (t <= 66) { r = 255; }
      ...
      /* Нормируем по самому яркому каналу: яркость звезды несёт
         величина, а тройка отвечает только за оттенок. Без этого
         красные карлики выходили бы ещё и вдвое темнее положенного
         дважды - и по величине, и по цвету. */
    ...
    /* Доли спектральных классов взяты для ВИДИМОГО неба, а не для
       ...
       Температуры - настоящие границы классов. */
    var КЛАССЫ = [
...
function keyHintFrame() {
  ...
  /* ── Подсказка встаёт над ВСЕМ пультом, а не над своей клавишей ──
     ...
     Верхняя кромка приборной плиты известна: это верх ниш клавиш.
     Берём самую высокую клавишу ряда и ставим подсказку НАД ней -
     тогда она не накрывает ни одной клавиши, к какой бы из них ни
     относилась. Если места сверху не хватает (узкий кадр), уводим
     её выше в остекление: там пусто, и читать не мешает ничто. */
  /* Верх плиты берём по клавишам В РАЗМЕТКЕ, а не по узлам
     физических крышек: у крышек в мире нет экранного прямоугольника,
     и цикл по ним не давал ничего - подсказка оставалась над своей
     клавишей и накрывала соседний ряд. Список клавиш кэшируем:
     подсказка считается в кадре, а querySelectorAll по всему слою
     шестьдесят раз в секунду не нужен никому. */
  if (!keyHintFrame.клавиши || keyHintFrame.клавишиT !== ui.wrap.childElementCount) {
    keyHintFrame.клавиши = ui.wrap.querySelectorAll(".rcf-key");
    keyHintFrame.клавишиT = ui.wrap.childElementCount;
  }
  var кл2 = keyHintFrame.клавиши, верхПлиты = sy;
  for (var вк = 0; вк < кл2.length; вк++) {
    var рк = кл2[вк].getBoundingClientRect();
    if (рк.height > 4 && рк.top < верхПлиты) верхПлиты = рк.top;
  }
  /* Ставим НИЗ подсказки на двенадцать точек выше верхней кромки
     плиты. Высота в расчёт не входит вовсе - за неё отвечает
     привязка нижней кромкой в стилях. Это и есть то, чего просили:
     подсказка не заходит на пульт ни при какой длине текста. */
  /* Зазор в двадцать шесть точек, а не двенадцать: под коробкой
     подсказки живёт стрелка-указатель (псевдоэлемент ::after), и в
     прямоугольник самой коробки она не входит. С зазором в двенадцать
     стрелка ложилась на верхний ряд клавиш полоской в пять точек -
     мелочь, но именно из таких мелочей и складывается «надписи
     заходят на панель». */
  var y = верхПлиты - 26;
