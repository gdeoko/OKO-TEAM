...
function buildWorld() {
  ...
  /* Плотность пикселей. Числа 1.2 на телефоне хватало, пока пульт
     ...
     Стартуем с той же двойки, что и потолок в stageLite, иначе
     первые кадры полёта были бы заметно мягче последующих.
     Разгонять кадр это не мешает: страж плавности снимет плотность
     ступенями, если устройство не тянет. */
  r.setPixelRatio(Math.min(g.devicePixelRatio || 1, tiny ? 1.0 : (mob ? 2.0 : 1.8)));
...
function stageLite(on) {
  ...
  if (W3.r) {
    ...
    var cap = (on && !уПульта) ? 1.15 : (tiny ? 1.0 : 2.0);
