...
function skyTexture(mob) {
  ...
  for (i = 0; i < зв; i++) {
    ...
    if (я2 < 0.30) {
      ...
      /* ── ОРЕОЛ ПОДЖАТ (11.09.2026): это и были «пятна в космосе» ──
         Радиус стоял 1.7..6.3 текселя, то есть круг до тринадцати
         текселей, а на телефоне тексель панорамы (2048) занимает
         полтора-два экранных пикселя: на снимке владельца это мягкие
         кляксы по 20-40 пикселей, и луч в каждую из них упёрся ровно
         в эту сферу (tools/кдн-кто-пятна-4.mjs). Яркая звезда на
         снимке это точка с коротким свечением, а не мыльный пузырь.
         Радиус держим в ТЕКСЕЛЯХ панорамы и НЕ множим на размер
         карты. Множитель стоял здесь первым заходом и компенсировал
         не то: на мониторе панорама вчетверо подробнее, но и кадр там
         втрое шире, поэтому один тексель на мониторе занимает 1.76
         экранного пикселя против 0.81 на телефоне. С множителем ореол
         выходил 20 пикселей на мониторе против 5 на телефоне - снимок
         монитора это и показал. Без него 4.9 и 2.3: одна звезда на
         обоих экранах. */
      var рад = 1.0 + я2 * 1.8;
      gr = x.createRadialGradient(пx, пy, 0, пx, пy, рад);
      gr.addColorStop(0, цвет + "1)");
      gr.addColorStop(0.40, цвет + "0.30)");
