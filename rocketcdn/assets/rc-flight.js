...
var buildQ = [], buildBusy = false, buildEpoch = 0, buildFrame = 0, buildTimer = 0;
function cancelBuild() {
  ...
  buildFrame = buildTimer = 0;
}
function buildStep(epoch) {
  if (epoch !== buildEpoch) return;
  buildFrame = buildTimer = 0;
  if (!buildQ.length) { buildBusy = false; return; }
  var job = buildQ.shift();
  try { job(); } catch (e) { if (g.RC_track) g.RC_track('jserr', 'planet: ' + (e.message || e), true); }
  if (epoch !== buildEpoch) return;
  buildFrame = requestAnimationFrame(function () {
    if (epoch !== buildEpoch) return;
    buildFrame = 0;
    buildTimer = setTimeout(function () { buildStep(epoch); }, 0);
  });
}
function buildLater(fn) {
  ...
  if (buildBusy) return;
  buildBusy = true;
  var epoch = buildEpoch;
  buildFrame = requestAnimationFrame(function () { buildStep(epoch); });
}
...
var buildingWorld = null;
function buildWorld() {
  disposeFlightWorld();
  try { return assembleWorld(); }
  catch (error) {
    W3 = buildingWorld;
    disposeFlightWorld();
    throw error;
  } finally { buildingWorld = null; }
}
function assembleWorld() {
  ...
  if (g.RC_GL && g.RC_GL.take) {
    if (!g.RC_GL.take(true)) throw new Error("WebGL context budget exhausted");
    F.glSlot = true;
  }
  var r;
  try {
    r = new T.WebGLRenderer({ canvas: ui.cv, antialias: !tiny, alpha: false, powerPreference: "high-performance" });
    buildingWorld = { r: r, scene: null };
  } catch (error) {
    if (F.glSlot && g.RC_GL) g.RC_GL.give();
    F.glSlot = false;
    throw error;
  }
...
function launchIntent() {
  var query = new URLSearchParams(g.location.search || "");
  if (query.get("flight") !== "1") return;
  // Consume the explicit request once so refresh/back does not restart flight.
  query.delete("flight");
  var suffix = query.toString();
  try { g.history.replaceState(g.history.state, "", g.location.pathname + (suffix ? "?" + suffix : "") + (g.location.hash || "")); } catch (e) {}
  open();
}
function ready() { launchers(); launchIntent(); }
