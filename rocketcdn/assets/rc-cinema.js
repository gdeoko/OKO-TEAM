...
/* На третьей ступени убранство не строим и снимаем уже стоящее:
   карточка остаётся карточкой, комната остаётся комнатой. */
...
function cabin(act) {
  ...
  for (i = 0; i < n; i++) {
    ...
    /* ── В ОТКРЫТЫХ ДВЕРЯХ ТЕКСТА НЕТ ─────────────────────────
       Экраны приходили ВМЕСТЕ с расхождением створок и читались
       текстом, наклеенным в проём. Владелец это отменил прямым
       словом: «по этому разделу тут не нужен текст на сайте vpn в
       открытых дверях и на сайте CDN тоже убери».

       Оборот при этом остаётся и остаётся главным: «на сайте cdn
       этот 360 и весь салон ракеты сделать более качественный и
       текст читаемый при 360». Значит экраны не снимаются, а ЖДУТ
       входа: пока доля входа не дошла до девяти десятых, в проёме
       стоит корабль и ничего кроме. Дальше человек уже внутри, и
       оборот идёт по экранам, как и шёл. */
    var вошли = eIn <= 0.90 ? 0 : (eIn - 0.90) / 0.10;
    vis *= вошли < 0 ? 0 : (вошли > 1 ? 1 : вошли);
    ...
    /* Ноль пишем как `none`, а не как `0px`. Фильтр с нулевым
       ...
       под три десятка. */
    /* ── РАЗМЫТИЕ УБАВЛЕНО, ПОТОМУ ЧТО ЧИТАТЬ ПРОСИЛИ ВСЕ ЭКРАНЫ ──
       Стояло 2.6 точки на полностью отвёрнутый экран, и при обороте
       читался ровно один - тот, что строго перед глазом. Владелец:
       «текст читаемый при 360». Полторы точки оставляют глубину (
       дальний экран всё ещё мягче ближнего) и снимают кашу: буква на
       шестнадцатом кегле переживает полторы точки и не переживает
       две с половиной.

       Ближние экраны не мутнеем вовсе. Порог по углу, а не по номеру:
       «второй слева» может стоять и вплотную, и за плечом. */
    var рз = soft && i !== best && vis < 0.86 ? (0.86 - vis) * 1.9 : 0;
...
/* ── Комната из вёрстки повторяет комнату из объёма ──────────
   ...
   ровно панель. */
/* ── Высота панели по самому длинному тексту ───────────────────
   Одна высота на все экраны - это прямая просьба владельца, и она
   остаётся. Ломалось другое: высота бралась из окна (26vh) и с
   текстом никак не сверялась. При окне 900 пикселей на содержимое
   в 303 приходилась панель в 232 - каждый экран терял по семьдесят
   пикселей, «Дежурная смена инженеров круглосуточно, включая»
   обрывалось на полуслове, из шести вопросов FAQ в кадр попадал
   один.

   Меряем натуральную высоту самой длинной панели и ставим её всем.
   Панели остаются одного размера, текст перестаёт срезаться.
   Мерить надо offsetHeight, а не рамку: рамку экраны отдают уже
   повёрнутой и уменьшенной перспективой, offsetHeight - настоящую
   высоту вёрстки. Потолок в 62% окна оставлен, чтобы панель не
   вылезла за кадр на низком окне; на такой случай в разметке
   остаётся прокрутка внутри панели. */
function fitPanels(ring, cards) {
  if (!ring || !cards || !cards.length) return;
  CSSVAR(ring, "--cab-h-scr", "auto");
  var need = 0;
...
/* ── Комната собирается ВСЕГДА, вопрос только в том, какая ──────
   Здесь стоял отказ по третьей ступени упрощения: на слабом
   устройстве комната с экранами не собиралась вовсе. Это не
   упрощение, а дыра в сценарии - человек доходит до места, где
   должен войти в корабль и оглядеться, и не находит там ничего.

   Отказ остаётся только по двум настоящим приговорам: человек
   попросил меньше движения (reduced) или страница объявлена
   упрощённой целиком (rc-reduced). Оба - выбор, а не показание
   счётчика кадров.

   На третьей ступени комната строится, но БЕЗ дорогого убранства:
   см. упрощённо() ниже. Так человек видит сценарий целиком, а
   платит за это только украшениями. */
function cabinAllowed() {
  return !reduced && !root.classList.contains("rc-reduced");
}

/* Третья ступень упрощения: комнату строим, голограммное убранство
   карточек не строим. Именно оно и стоит дорого: у каждой карточки
   свои слои развёртки, скобок, свечения и луча, и на слабой машине
   это десятки лишних составных слоёв в кадре.

   Читается признак каждый раз заново, а не запоминается: ступень
   умеет и подниматься, и опускаться, и комната обязана следовать за
   ней, а не за тем, какой ступень была в момент сборки. */
function упрощённо() {
...
function collect() {
  ...
  if (rel && ring) {
    ...
    if (on && !упрощённо()) holoBuild(cards); else holoStrip(cards);
