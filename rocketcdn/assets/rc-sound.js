...
/* Треск горения. Замер старого двигателя показал ровный спад от низа к
   верху без единой неровности во времени: спектр как у водопада, и от
   этого он читался механизмом, а не огнём. У настоящего двигателя поток
   в сопле рвётся неупорядоченно, и поверх ровного рёва всё время идут
   короткие резкие всплески - именно они дают слуху понять, что там
   горит, а не крутится. Здесь всплески расставлены случайно, около
   сорока в секунду: реже - слышны отдельные щелчки, чаще - снова
   сливается в ровный шум.

   Всплески складываются друг с другом там, где попали в одно место:
   так и должно быть, неровность в том и состоит, что иногда рвётся
   сразу в двух местах. */
function трескБуфер(ctx, сек) {
  var sr = ctx.sampleRate;
  var n = Math.floor(sr * сек);
  var шов = Math.floor(sr * 0.05);
  var сырое = new Float32Array(n + шов);
  var всплесков = Math.floor(сек * 40);
  var i, k;
  for (k = 0; k < всплесков; k++) {
    var с = Math.floor(Math.random() * (n + шов));
    var дл = Math.floor(sr * (0.004 + Math.random() * 0.020));
    var сила = 0.4 + Math.random() * 0.6;
    for (i = 0; i < дл && с + i < n + шов; i++) {
      сырое[с + i] += (Math.random() * 2 - 1) * сила * спадВсплеска(1 - i / дл);
    }
  }
  for (i = 0; i < шов; i++) {
    var д = i / шов;
    сырое[i] = сырое[i] * д + сырое[n + i] * (1 - д);
  }
  var буф = ctx.createBuffer(1, n, sr);
  var d = буф.getChannelData(0);
  var макс = 0;
...
/* Затухание всплеска: резкий удар и быстрый спад. Вынесено в имя,
   чтобы в теле цикла читался смысл, а не степень. */
function спадВсплеска(x) { return x * x * Math.sqrt(x); }
...
Sound.prototype.build = function () {
  ...
  this.срез = узлы.срез;
  this.рёвG = узлы.рёвG;
  this.выхлопG = узлы.выхлопG;
  this.трескG = узлы.трескG;
  this.bp = узлы.bp;
  this.ng = узлы.ng;
...
/* ── Двигатель ────────────────────────────────────────────────
   ...
   Было и после этого: три слоя шума, но полоса около 82 Гц стояла с
   добротностью 3.4, то есть шириной всего в двадцать четыре герца -
   ухо читает такое как гудение с высотой. А замер показал вторую
   беду: на полной тяге от двадцати герц до четырёх килогерц спектр
   падал на 56 дБ, выше двух килогерц не было почти ничего. Рёва не
   получалось - получался тёмный гул, ровный во времени, как у
   работающего прибора. Заказчик и назвал это нереалистичным.

   Стало: пять слоёв шума и ни одного осциллятора.
     · подвал - коричневый шум ниже 52 Гц, это то, что ощущается
       телом, а не слышится ухом;
     · корпус - тот же шум до 240 Гц с широким подъёмом на 64 Гц;
       подъём взят колокольным фильтром, а не полосовым: он красит
       низ, но не выделяет из него ноту;
     · рёв - розовый шум широкой полосой около 380 Гц с добротностью
       0.5, то есть шириной в две октавы; его тем больше, чем больше
       тяга, и он читается как «мощно»;
     · выхлоп - розовый шум выше 1300 Гц: шипение струи, которое
       появляется только под тягой и уходит на холостом ходу;
     · треск - неровные всплески горения, около сорока в секунду.
   Сверху всё придавлено срезом: тяга открывает его от 420 Гц до
   почти четырёх килогерц, и звук из тёмного гула становится рёвом.
   Высоту не двигает ничто - у ракеты нет ноты. */
Sound.prototype.двигатель = function (ctx, выход) {
  var eg = ctx.createGain();
  /* Значение на первые доли секунды: дальше уровень ведут партитура
     страницы и flightLevel, и подрезка общей громкости живёт там, а
     не здесь. Ставить её в этом месте бесполезно - первый же кадр
     партитуры перезапишет. */
  eg.gain.value = 0.5;

  /* Дрожь горения множит громкость, а не прибавляется к ней: ±30%
     вокруг единицы. Прибавкой это делать нельзя - на малой тяге
     сумма ушла бы в минус, и волна перевернулась бы фазой. Было
     ±18%: замер разброса уровня показал, что этого мало, тяга
     держалась почти ровно, а у настоящего двигателя давление в
     камере гуляет заметно. */
  ...
  /* Срез яркости. Открывается тягой от 420 Гц до почти четырёх
     килогерц: на холостом ходу слышен только низ, под тягой
     добавляется весь верх - без него рёва не бывает. Добротность
     ниже прежней, чтобы срез сводил верх плавно, а не обрубал. */
  var срез = ctx.createBiquadFilter();
  срез.type = "lowpass";
  срез.frequency.value = 420;
  срез.Q.value = 0.4;
  ...
  /* Подвал: десятки герц. Слышно его плохо, ощущается хорошо -
     ровно так и работает настоящий стартовый стол. */
  var подвал = ctx.createBiquadFilter();
  подвал.type = "lowpass"; подвал.frequency.value = 52; подвал.Q.value = 0.7;
  var подвалG = ctx.createGain(); подвалG.gain.value = 1.30;
  корич.connect(подвал); подвал.connect(подвалG); подвалG.connect(eg);

  /* Корпус: гул камеры. Колокольный подъём на 64 Гц с добротностью
     0.9 - это полоса шириной больше октавы, характер она даёт, а
     высоту нет. Прежний полосовой с добротностью 3.4 давал ноту. */
  var корпус = ctx.createBiquadFilter();
  корпус.type = "lowpass"; корпус.frequency.value = 240; корпус.Q.value = 0.5;
  var окрас = ctx.createBiquadFilter();
  окрас.type = "peaking"; окрас.frequency.value = 64;
  окрас.Q.value = 0.9; окрас.gain.value = 5;
  var корпусG = ctx.createGain(); корпусG.gain.value = 0.55;
  корич.connect(корпус); корпус.connect(окрас);
  окрас.connect(корпусG); корпусG.connect(eg);

  /* Розовый буфер один на три слоя: генерация шума заметно дороже
     ...
     один и тот же звук втрое громче. */
  ...
  рёв.type = "bandpass"; рёв.frequency.value = 380; рёв.Q.value = 0.5;
  var рёвG = ctx.createGain(); рёвG.gain.value = 0.06;
  розов.connect(рёв); рёв.connect(рёвG); рёвG.connect(eg);

  /* Выхлоп: шипение струи. Свой источник со сдвигом по буферу -
     иначе он повторял бы рёв и складывался с ним в один звук. */
  var розов2 = ctx.createBufferSource();
  розов2.buffer = розовБуф;
  розов2.loop = true;
  var выхлоп = ctx.createBiquadFilter();
  выхлоп.type = "highpass"; выхлоп.frequency.value = 1300; выхлоп.Q.value = 0.5;
  var выхлопG = ctx.createGain(); выхлопG.gain.value = 0.0;
  розов2.connect(выхлоп); выхлоп.connect(выхлопG); выхлопG.connect(eg);

  /* Треск горения: то, чего не хватало больше всего. На холостом
     ходу его нет, с тягой он выходит вперёд. */
  var трИст = ctx.createBufferSource();
  трИст.buffer = трескБуфер(ctx, 4);
  трИст.loop = true;
  var треск = ctx.createBiquadFilter();
  треск.type = "bandpass"; треск.frequency.value = 900; треск.Q.value = 0.7;
  var трескG = ctx.createGain(); трескG.gain.value = 0.0;
  трИст.connect(треск); треск.connect(трескG); трескG.connect(eg);

  корич.start(); розов.start(); трИст.start();
  розов2.start(ctx.currentTime, розовБуф.duration * 0.33);
  ...
  /* Со сдвигом по буферу: тот же буфер, читаемый с той же точки, дал
     бы два одинаковых шума, то есть один шум вдвое громче, а не два
     независимых слоя. */
  поток.start(ctx.currentTime, розовБуф.duration * 0.66);

  return { eg: eg, срез: срез, рёвG: рёвG, выхлопG: выхлопG,
           трескG: трескG, bp: bp, ng: ng };
};
...
/* Довод - необязательный ответчик: его зовут с true, если звук
   действительно пошёл, и с false, если браузер отказал. Без этого
   ответа rc-music.js не знал, чем кончилась попытка на прокрутке, и
   замер показал, чем это оборачивалось: после отказа браузера музыка
   заводилась только с четвёртого щелчка человека. Молчаливый start()
   и был причиной. */
...
/* Отдаёт true, если звук действительно пошёл. Ответ нужен start(),
   чтобы было что сообщить наверх: раньше отказ браузера отличался от
   успеха только тем, что ничего не звучало. */
Sound.prototype.fadeIn = function () {
  if (this.ctx.state !== "running") { this.hint(); return false; }
  this.on = true;
...
/* ── Уровень синтезированного слоя ───────────────────────────
   Под музыкой синтез отступает: две полноценные фонограммы разом -
   ...
   музыка появилась или пропала, и разнобоя нет. */
Sound.prototype.баланс = function () {
  if (!this.on || !this.ready) return;
  var ц = this._flight ? (this.music() ? 0.14 : 0.20)
                       : (this.music() ? 0.085 : 0.16);
  try { this.master.gain.setTargetAtTime(ц, this.ctx.currentTime, 0.5); } catch (e) {}
};
...
Sound.prototype.loop = function () {
  ...
  function step() {
    ...
    if (self.eg) {
      /* Было 0.30 и 0.55. Слоёв в двигателе стало больше, и замер
         показал прибавку в 2.5 дБ по среднеквадратичному уровню - вся
         она пришлась на верх, который ухо считает громким охотнее
         всего. Задача была сделать звук достовернее, а не громче,
         поэтому оба множителя подрезаны на те же 2.5 дБ. */
      var base = self.music() ? 0.22 : 0.41;
    ...
    /* Разгон. Раньше здесь ехала вверх частота обоих осцилляторов, и
       ...
       Теперь скорость открывает срез и добавляет рёва, шипения струи
       и треска: звук становится ярче и злее, оставаясь на месте по
       высоте. Тембр ведёт скорость корабля, а не его нота. */
    self.срез.frequency.setTargetAtTime(380 + self.vel * 900 + self.fly * 1500, t, 0.15);
    self.рёвG.gain.setTargetAtTime(0.08 + self.vel * 0.30 + self.fly * 0.40, t, 0.25);
    /* Шипение и треск принадлежат тяге, а не странице: где ракеты в
       кадре нет, множитель fly равен нулю и обоих слоёв нет вовсе. */
    if (self.выхлопG) {
      self.выхлопG.gain.setTargetAtTime((0.010 + self.vel * 0.10) * self.fly, t, 0.3);
    }
    if (self.трескG) {
      self.трескG.gain.setTargetAtTime((0.020 + self.vel * 0.16) * self.fly, t, 0.3);
    }
...
Sound.prototype.flight = function (on) {
  ...
  var t = this.ctx.currentTime;
  if (on) {
    if (this.eg) this.eg.gain.setTargetAtTime(this.music() ? 0.30 : 0.45, t, 0.6);
    this.баланс();
  } else if (this.on) {
    this.баланс();
  } else {
...
Sound.prototype.flightLevel = function (k) {
  ...
  var t = this.ctx.currentTime;
  /* Тяга ведёт громкость и тембр, высоту не трогает вовсе. Срез
     открывается почти до четырёх килогерц: тёмный гул на малой тяге
     и полноценный рёв на полной - это и есть разница, которую ухо
     читает как мощность. */
  this.срез.frequency.setTargetAtTime(420 + k * 3200, t, 0.25);
  this.рёвG.gain.setTargetAtTime(0.10 + k * 0.60, t, 0.3);
  if (this.выхлопG) this.выхлопG.gain.setTargetAtTime(0.005 + k * 0.16, t, 0.3);
  if (this.трескG) this.трескG.gain.setTargetAtTime(0.010 + k * 0.26, t, 0.3);
  if (this.ng) this.ng.gain.setTargetAtTime(0.004 + k * 0.035, t, 0.3);
  /* Те же 2.5 дБ подрезки, что и в партитуре страницы: новый двигатель
     шире по спектру и от этого громче на слух при том же числе. */
  if (this.eg) this.eg.gain.setTargetAtTime((this.music() ? 0.22 : 0.37) * (0.5 + k * 0.8), t, 0.35);
};
...
function bind() {
  ...
  /* Музыка появилась или пропала - синтезированный слой обязан
     подвинуться. Один раз на входе этого не сделать: тема заводится
     позже собственного включения звука. */
  addEventListener("rc:music", function () { snd.баланс(); });
