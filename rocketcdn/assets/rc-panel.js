...
function build(T, o) {
  ...
  try {
    /* ── ОТРАЖЕНИЯ БЕРУТСЯ У СЦЕНЫ, А НЕ У ПУЛЬТА ─────────────────
       Своей карты окружения пульт не получает НАМЕРЕННО. Отражает он
       `scene.environment`, который ставит сборка мира той же печёной
       картой (у CDN это rc-flight.js рядом с заводкой плёнки, у VPN
       rv-real.js), и три.js подхватывает окружение сцены сам, когда у
       материала своей карты нет. Поставить сюда envMap руками значит
       перебить общее окружение частным: пульт станет отражать не то,
       что отражает вся рубка вокруг него.

       Здесь стояла проверка `RC_REAL.env.isTexture`. Она писалась от
       настоящей беды: без неё в поле envMap ложилась САМА ФУНКЦИЯ
       печати, три.js считал такое поле истинным, включал ветку
       окружения в шейдере и брал у функции mapping, которого нет -
       отражения выходили случайными. Но `RC_REAL.env` функцией и
       остался, значит проверка не проходила никогда, и ветка была
       мёртвой с первого дня. */
    /* Ветки тут больше нет, и это важнее, чем кажется. Проверка не
       проходила НИКОГДА (RC_REAL.env это функция), значит сила
       окружения оставалась той, что даёт три.js по умолчанию, - и
       оставалась она так на обоих сайтах только по совпадению. Стоило
       одному из них однажды начать отдавать готовую текстуру, и пульт
       там потемнел бы до 0.55, а у соседа остался на единице: два
       разных пульта при требовании «всё 1:1».

       Задаём силу безусловно и тем числом, на котором оба сайта стоят
       сейчас и которое владелец принял. Число одно, ветки нет,
       разойтись нечему. */
