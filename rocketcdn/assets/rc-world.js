...
/* Ход камеры. Приёмка замерила прежние пределы: панорама ±13 px и
   наезд 2,6 % на кадре шириной 1440 - это меньше процента экрана,
   человек такого движения просто не замечает, и «единый мир» держался
   на одном корабле. Поднимаем ход в полтора раза; края по-прежнему
   прикрывает наезд, поэтому обрезки строк не прибавляется. */
var PAN_MAX = 40, TILT_MAX = 28, DIVE_MAX = 0.030;
...
/* Пересев слоёв. Раздел надёжности и часть карточек страница
   перерисовывает заново - при смене языка и при догрузке контента.
   Старые узлы после этого висят в наблюдателе мёртвым грузом, а
   новые никто не берёт под наблюдение, и боковой ход у них
   пропадает до перезагрузки. Поэтому смотрим заново: снимаем сдвиг
   со старых, отпускаем их и проходим по разметке ещё раз. */
function пересев() {
  if (!stage) return;
  for (var i = 0; i < layVis.length; i++) {
    if (g.RC_VAR && g.RC_VAR.del) g.RC_VAR.del(layVis[i], "--w3-shift");
  }
  layVis.length = 0;
  if (layIO) layIO.disconnect();
  seed();
}
addEventListener("rc:content", function () { setTimeout(пересев, 60); });
doc.addEventListener("rc:lang", function () { setTimeout(пересев, 280); });
...
function frame() {
  ...
  /* ── ВЫХОДИМ ИЗ ЦИКЛА, КОГДА ТРАНСФОРМ УЖЕ ПУСТ ─────────────
     Стояло просто `if (power() <= 0) return;`, и это выходило из кадра
     РАНЬШЕ записи трансформа (строка ниже, где цель уже умножена на ту
     же силу и сама сошла бы к нулю). Мир замирал на последнем наружном
     значении: перед салоном акт прохода к трапу держит наезд до 1.14
     на широком экране. Вход в салон гасит трансформ правилом стиля со
     знаком важности, мгновенно и без перехода, а выход возвращает
     ЗАМОРОЖЕННЫЙ наезд одним кадром - скачок на границе акта в обе
     стороны.

     Теперь цикл продолжает считать, пока трансформ не доехал до пустой
     строки, и останавливается сам. Лишней работы это не создаёт:
     схождение идёт по экспоненте и кончается за доли секунды. */
  if (power() <= 0 && lastTf === "") return;
  ...
  var panMax = Math.min(PAN_MAX, innerWidth * 0.023);
  var tiltMax = Math.min(TILT_MAX, innerHeight * 0.020);
  ...
  var zMax = innerWidth < 760 ? 1.07 : 1.14;
...
/* ── ВСПЫШКИ ИЗ ПРОЁМА БОЛЬШЕ НЕТ ────────────────────────────
   Здесь висел слой .w3-flash: на событие «мы в проёме» поверх кадра
   на шесть десятых секунды ложилась белая засветка, и задумана она
   была как «шаг внутрь на свет». На деле она прикрывала СМЕНУ СЦЕНЫ -
   корабль отдавал кадр рубке, и под вспышкой кадр был пуст.

   Владелец 11.09.2026: «сделай так, чтобы после входа в ракету не было
   шва и вспышки какой-то, после чего просто чёрный экран; всё должно
   быть бесшовно: вошли в ракету, подошли к панели и застыли».

   Вспышка снята целиком, а пустоту под ней закрывает не заслонка, а
   порядок: наружная сцена не останавливается, пока рубка не отдала
   свой первый кадр (rc-interior.js, show). Стиль .w3-flash остаётся в
   rc-world.css неиспользованным намеренно: он часть словаря сцены, и
   вернуть засветку, если однажды понадобится, будет правкой одной
   строки, а не поиском по истории. */
