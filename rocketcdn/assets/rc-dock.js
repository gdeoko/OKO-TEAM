...
var таймерШвартовки = 0, таймерФокуса = 0;
function швартовка(кнопка, марка) {
  if (швартуется) return;
  швартуется = true;
  /* Каким причалом человек уходит из космоса. Это единственное место,
     где он выбирает между двумя сайтами сам, и без счёта тут нельзя
     ответить на прямой вопрос: работает ли связка вообще. */
  try { if (g.RC_track_игра) g.RC_track_игра("причал", марка); } catch (eС) {}
  if (узел) узел.setAttribute("data-швартовка", марка);
  кнопка.classList.add("рчл-идёт");
  var пауза = тише() ? 0 : 760;
  таймерШвартовки = setTimeout(function () { таймерШвартовки = 0; швартуется = false; уйти(марка); }, пауза);
}
...
function открыть() {
  ...
  if (первый) таймерФокуса = setTimeout(function () { таймерФокуса = 0; if (открыт()) { try { первый.focus(); } catch (e) {} } }, тише() ? 0 : 120);
  return true;
}

function закрыть(тихо) {
  clearTimeout(таймерШвартовки); таймерШвартовки = 0;
  clearTimeout(таймерФокуса); таймерФокуса = 0;
  швартуется = false;
  if (!узел) return;
