...
var st = {
  ...
  /* Защёлка обратного выхода и самая нижняя точка после него: ими
     отбивается дребезг от отстающей доли люка, см. setProgress. */
...
function setProgress(p) {
  ...
  /* ── Защёлка обратного выхода ──────────────────────────────
     ...
     заметно дальше самой нижней достигнутой точки - на ту же полосу
     гистерезиса. Дребезг на кадре ни тем, ни другим не проходит. */
  if (reverseExit) {
    st["вышлиНазад"] = true;
    st["дноВыхода"] = rawP;
  }
  if (st["вышлиНазад"]) {
    if (st["дноВыхода"] == null || rawP < st["дноВыхода"]) st["дноВыхода"] = rawP;
    ...
    if (доляЛюка < 0.30 || rawP > st["дноВыхода"] + exitBand) {
      st["вышлиНазад"] = false;
      st["дноВыхода"] = null;
    }
  }
  ...
  /* Запасной порог по прогрессу оставлен только для страниц без
    ...
    lockHide();
    if (st.shown) hide();
    /* Ушли из зоны входа целиком - забываем и защёлку обратного
       выхода: следующий заход обязан начинаться с чистого листа, а
       не с памяти о прошлом выходе. */
    if (p < P_LOCK) {
      st.sLock = false; st.sIn = false; st.stop = -1;
      st["вышлиНазад"] = false; st["дноВыхода"] = null;
    }
  }
...
g.RC_INTERIOR = {
  ...
  state: function () {
    return {
      ...
      дно_выхода: st["дноВыхода"] == null ? null : +st["дноВыхода"].toFixed(4)
    };
  }
};
