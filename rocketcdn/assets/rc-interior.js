...
function measure() {
  ...
  /* У каждой остановки не точка, а плато. Это принципиально: с
     ...
     Поэтому на каждый экран отводится отрезок, и внутри него две
     трети камера стоит неподвижно, а треть уходит на переход к
  ...
  for (var i = 1; i <= 7; i++) {
    ...
    PLAN.rel.push(a);
    var s0 = 0.09 + seg * (i - 1);
    KNOT.push([P_IN + span * (s0 + seg * 0.34), a]);   /* встали */
    KNOT.push([P_IN + span * (s0 + seg * 0.94), a]);   /* стоим */
  }
  PLAN.faq = 5 * STEP;
