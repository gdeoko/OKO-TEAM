...
function build(T, opts) {
  ...
  (function () {
    ...
    for (var vi = 0; vi < поз.count; vi++) {
      ...
      uv.setXY(vi, (сX - пк.ox) / пк.dw, 1 - (сY - пк.oy) / пк.dh);
    }
    uv.needsUpdate = true;
    ...
    хол.width = метаФ.w; хол.height = метаФ.h;
    ...
    (function () {
      ...
      им.decoding = "async";
      им.onload = function () {
        хк.clearRect(0, 0, метаФ.w, метаФ.h);
        хк.drawImage(им, 0, 0, метаФ.w, метаФ.h);
        try {
          ...
          if (планД) {
            ...
            /* Плотность подобрана так, чтобы холст деки совпал по
               разрешению с кадром, вписанным в снимок */
            var пл = метаФ.w / пк.dw;
            д.размер(Wк, Hкад, пл);
            д["кадр"](0.016);
            хк.save();
            хк.translate(-пк.ox * пл, -пк.oy * пл);
            хк.scale(пл, пл);
