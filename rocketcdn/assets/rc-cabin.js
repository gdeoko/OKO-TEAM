...
function build(T, opts) {
  ...
  (function () {
    ...
    var пл = метаФ.w / пк.dw;
    var холW = Math.round(кадрW * пл), холH = Math.round(кадрH * пл);
    /* Потолок на сторону: холст 1.32 от кадра при плотности снимка
       на телефоне уходит за три тысячи точек, а это лишние мегабайты
       в видеопамяти на сцене, которую и так просили не утяжелять. */
    var ПОТ = tiny ? 2048 : 3072;
