...
function place(id, x, y, depth, visible) {
  ...
  if (огрП > огрЛ) {
    ...
    var ОРЕОЛ = 18;
    /* ── Само перекрестье обязано быть в проёме ──────────────
       Тут была дыра, и штатная проверка находила её через раз.
       Проверялось ТОЛЬКО, помещается ли карточка. Карточка это
       прямоугольник, который можно перекинуть на другую сторону от
       точки привязки, и она честно помещалась. А сама точка -
       перекрестье с ореолом, то самое, что стоит НА ТЕЛЕ, - при этом
       оставалась снаружи: тело за кромкой остекления, кружок с
       мягким свечением висит на стойке рубки, карточка от него
       уходит внутрь.

       Замер: метка ЛУНАРЕЗЕРВ на доле 0.06, проём в координатах слоя
       0..896, точка привязки на 1054, ореол торчит за кромку на 166
       точек. Ровно те же 166, что видела приёмка.

       Тело, которого не видно сквозь стекло, не может быть подписано:
       подпись показывала бы пальцем в стойку. Прячем. Это то же
       решение, что абзацем ниже, и по той же причине - лучше потерять
       подпись, чем соврать. */
    if ((x + ОРЕОЛ) > огрП || (x - ОРЕОЛ) < огрЛ) {
...
function сторожить() {
  if (сторож) return;
  /* ── СТО ДВАДЦАТЬ, А НЕ ЧЕТВЕРТЬ СЕКУНДЫ ────────────────────────
     Разбор выше обещает четверть секунды, а в коде стояло 120 мс, то
     есть вдвое чаще обещанного. Приводим число к разбору: при живых
     метках это восемь пересчётов в секунду вместо шестнадцати, глазу
     по-прежнему незаметно.

     И сторож больше не бьёт в скрытой вкладке. Он ставил deDirty сам,
     значит работа шла даже когда человек ушёл на другую вкладку и не
     видит ни одной метки: телефон за это платит батареей. */
