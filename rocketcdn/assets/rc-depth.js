...
/* ── Спокойные карточки ──────────────────────────────────────
   Заказчик посмотрел живьём и сказал прямо: карточки слишком
   дёрганые, нужно чтобы стояли, максимум белый отблеск при
   наведении. Отблеск это чистый стиль (.rcd-sheen), он остаётся.
   Уходит всё остальное: наклон под курсором, разворот от места в
   кадре, подъём, парение и послойный разлёт на въезде.

   Заодно это лечит вторую его жалобу, про тормоза при прокрутке.
   Разбор кадра показал, что самая дорогая строка страницы -
   съём прямоугольника у каждой видимой карточки в каждом кадре
   (box в rc-var). Он был нужен ровно для этого движения. Нет
   движения - нет и съёма: цикл по карточкам вообще не заводится.

   Оставить оба поведения переключателем нельзя без второго кода,
   поэтому здесь один флаг, и он же документ решения. */
var ТИХО = true;
...
function collect() {
  ...
  for (i = 0; i < list.length; i++) {
    ...
    /* Экраны салона объёма не получают. Они стоят на стенах рубки,
       и объём им даёт сама комната: собственный наклон и подъём
       спорили бы с её поворотом - именно это владелец и назвал
       «карточки дёргают вверх-вниз, пока я поворачиваюсь». */
    if (el.closest && el.closest(".cin-world")) continue;
    el._rcd = 1;
    claim(el);
    stage(el);
    sheen(el);
...
function frame(dt) {
  ...
  /* ── чтение ── */
  readBuf.length = 0;
  for (i = 0; ТИХО ? false : i < visible.length; i++) {
    rec = visible[i];
    r = BOX(rec.el);
  ...
  var limit = (ТИХО || fast) ? 0 : MAX_TILT;
