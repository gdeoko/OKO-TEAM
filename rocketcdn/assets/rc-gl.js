  ...
  function loadAll() {
    ...
    function ready() {
      ...
      if (!left && !failed) {
        ...
        fire("rc:3d");
        /* Карты поверхностей для салона считаются попиксельно и
           стоят секунды. Раньше их считал тот самый кадр, которым
           открывался люк, и вход читался зависанием. Отдаём их в
           свободные промежутки прямо сейчас: человек ещё читает
           страницу, до корабля идти долго, процессор свободен. */
        try { if (g.RC_REAL && g.RC_REAL["прогрев"] && g.THREE) g.RC_REAL["прогрев"](g.THREE); } catch (eПр) {}
      }
    }
    function fail(file) {
      if (failed) return;
