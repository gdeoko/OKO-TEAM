...
/* ── Настоящее небо ──────────────────────────────────────────
   Дальний план, снятый как астрономический кадр: пылевые полосы
   Млечного Пути, звёздные облака, тёмные разрывы. Такого рисунка не
   даёт ни один процедурный слой - у выпеченных пятен нет ни
   филаментов, ни разрывов, и небо читается запотевшим стеклом.
   Владелец сказал про прежний фон ровно это: звёзды блеклые, Млечный
   Путь пятном.

   Снимок идёт ПОД процедурными звёздами и служит только фактурой:
   резкость и параллакс держат звёзды, которые рисуются поверх. На
   светлой теме небо не показываем вовсе.

   Тянем лениво и не держим страницу: пока картинка не пришла, сцена
   работает по-старому. */
Space.prototype.загрузитьНебо = function () {
  if (this.небо || this.небоЖдём || this.B.lean) return;
  this.небоЖдём = 1;
  var я = this;
  var im = new Image();
  im.decoding = "async";
  im.onload = function () { я.небо = im; };
  im.onerror = function () { я.небоЖдём = 2; };
  var база = "assets/gen/";
  try {
    var т = document.querySelector('script[src*="rc-gl.js"]');
    if (т && т.src) база = т.src.replace(/[?#].*$/, "").replace(/[^/]+$/, "") + "gen/";
  } catch (e) {}
  im.src = база + "deep-sky.webp";
};
...
Space.prototype.frame = function (dt) {
  ...
  /* Снимок неба: самый дальний план, едет медленнее всех остальных.
     Кладётся первым, под выпеченные туманности и под звёзды. */
  if (!this.небо && !this.небоЖдём) this.загрузитьНебо();
  if (this.небо && !light) {
    var нИ = this.небо;
    var нК = Math.max(w / нИ.width, h / нИ.height) * 1.14;
    var нШ = нИ.width * нК, нВ = нИ.height * нК;
    var нX = (w - нШ) / 2 + (panx * 0.014 - p * 0.022) * w;
    var нY = (h - нВ) / 2 + (pany * 0.011 - p * 0.062) * h;
    /* Держим в своих берегах, иначе покажется край снимка */
    if (нX > 0) нX = 0; else if (нX < w - нШ) нX = w - нШ;
    if (нY > 0) нY = 0; else if (нY < h - нВ) нY = h - нВ;
    x.globalAlpha = dim * 0.62;
    x.drawImage(нИ, нX, нY, нШ, нВ);
    x.globalAlpha = 1;
  }
