...
test('Flight world retirement releases post buffers and rebuilds the target hologram in the next scene',()=>{
 const f=fixture();load('rocketcdn/assets/vendor/three.min.js',f.c);const T=f.c.THREE;
 let posts=0,renderers=0;const old={scene:new T.Scene(),cam:new T.PerspectiveCamera(),post:{dispose:()=>posts++},r:{dispose:()=>renderers++,forceContextLoss(){}}};
 Object.assign(f.c,{g:f.c,W3:old,F:{glSlot:false,raf:null},ui:{},cancelBuild(){},убратьДерево(){},cabin:{old:true},stageWallMesh:{old:true}});
 const s=read('rocketcdn/assets/rc-flight.js');vm.runInContext('var proj=null,projFor="";'+between(s,'function projBuild(w3)','function projFrame(')+between(s,'function disposeFlightWorld()','var buildingWorld'),f.c);
 f.c.projBuild(old);const oldProjection=f.c.proj;assert.equal(old.cam.parent,old.scene);
 f.c.disposeFlightWorld();f.c.disposeFlightWorld();assert.equal(posts,1);assert.equal(renderers,1);
 assert.equal(f.c.cabin,null);assert.equal(f.c.stageWallMesh,null);
 const fresh={scene:new T.Scene(),cam:new T.PerspectiveCamera()};f.c.W3=fresh;f.c.projBuild(fresh);
 assert.notEqual(f.c.proj,oldProjection);assert.equal(f.c.proj.parent,fresh.cam);assert.equal(fresh.cam.parent,fresh.scene);
});
test('Postprocessing retirement frees all owned targets, fullscreen geometry and materials once',()=>{
 const f=fixture();load('rocketcdn/assets/vendor/three.min.js',f.c);load('rocketcdn/assets/rc-real.js',f.c);const T=f.c.THREE;
 const made=[];for(const name of ['WebGLRenderTarget','PlaneGeometry','ShaderMaterial','DataTexture']){
  const Original=T[name];T[name]=function(...args){const value=new Original(...args);const rec={name,disposed:0};value.addEventListener('dispose',()=>rec.disposed++);made.push(rec);return value;};
 }
 const renderer={capabilities:{isWebGL2:false,maxSamples:0},extensions:{has:()=>false},getSize:v=>v.set(640,400),getPixelRatio:()=>1};
 const post=f.c.RC_REAL.post(T,renderer,{tier:1});assert.ok(made.filter(x=>x.name==='PlaneGeometry').length===3);
 post.dispose();post.dispose();assert.ok(made.every(x=>x.disposed===1),JSON.stringify(made));assert.equal(f.c.RC_REAL.lastPost,null);
});
function fixture() {
