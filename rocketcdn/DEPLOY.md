...
| Сертификат | выпустится сам, см. раздел 10 |

Открыто наружу: 22, 80, 443 (ufw). Пока домен не переключён, сайт живёт
по адресу `http://217.19.122.132`.

### Что ещё нужно сделать руками

1. **Переключить домен.** Записи `A` для `rocketcdn.ru` и `www.rocketcdn.ru`
   сейчас указывают на `5.101.152.161`, который не отвечает. Их нужно
   перевести на `217.19.122.132`. Для личного кабинета завести `A`-запись
   `lk.rocketcdn.ru` (сейчас имя не резолвится вообще).
2. **Почта.** В `config.local.php` пустые `mail_user`, `mail_pass`, `mail_to`.
   Нужен ящик Gmail и пароль приложения на 16 знаков, см. раздел 3.
3. **Мини-приложение.** Кнопку меню в `@BotFather` можно прописать только на
   `https`, то есть после переключения домена. См. раздел 6.
