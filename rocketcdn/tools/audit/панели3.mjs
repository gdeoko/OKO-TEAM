...
const отчёт = {};
function писать(){ try{writeFileSync(ДОМ+"/все3-"+ЯЗ+".json", JSON.stringify(отчёт,null,1));}catch(e){} }
...
/* Тормоз кадров. В песочнице двадцать браузеров на четыре ядра, и
   цикл отрисовки мира съедает главный поток целиком: замер из
   Playwright не получает ни кванта и висит вечно. Отдаём миру
   четыре кадра в секунду - раскладка и переменные рамы живут, а
   поток освобождается под замеры. */
await pg.addInitScript(() => {
  const ор = window.requestAnimationFrame.bind(window);
  let ждём = false, очередь = [];
  window.requestAnimationFrame = function (cb) {
    очередь.push(cb);
    if (!ждём) {
      ждём = true;
      setTimeout(() => { ждём = false; const q = очередь; очередь = [];
        const t = performance.now(); q.forEach(f => { try { f(t); } catch (e) {} }); }, 250);
    }
    return 1;
  };
