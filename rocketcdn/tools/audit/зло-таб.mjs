...
шаг("обход не обрывается", тел <= 60, `провалов в тело ${тел} из ${путь.length}`);
...
if (доФормы) {
  ...
  const вФормеТеперь = await pg.evaluate(() => {
    ...
    return {
      фокус: э.tagName + (э.id ? "#" + э.id : ""),
      вПульте: !!(э.closest && э.closest(".rc-desk")),
      полей: поля.length
    };
  });
  шаг("форма заявки открывается с клавиатуры", вФормеТеперь.вПульте && вФормеТеперь.полей >= 3,
      `фокус ${вФормеТеперь.фокус} · полей ${вФормеТеперь.полей}`);
} else {
