...
/* Если полёт не открывался, эти два шага сравнивают страницу саму с
   собой и зеленеют ни на чём. Зелёное на несостоявшемся круге врёт
   не меньше красного на исправном, поэтому здесь тоже ПРОПУСК. */
if (!полётЖив) {
  console.log(`ПРОПУСК бюджет контекстов :: круга через полёт не было, ` +
              `сравнивать нечего (used ${глДо ? глДо.used : "?"} -> ${глПосле ? глПосле.used : "?"})`);
  console.log("ПРОПУСК список сцен :: там же");
} else {
