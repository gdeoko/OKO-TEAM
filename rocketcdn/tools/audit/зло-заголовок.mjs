...
const снять = () => pg.evaluate(() => {
  ...
  return {
    ...
    /* Считаем ТОЛЬКО настоящие карточки надёжности.

       Заголовок раздела на время салона переезжает в кольцо и там
       сам получает класс card (см. collect в rc-cinema.js). Пока он
       попадал в этот счёт, число гуляло: 5 до смены языка и 4 после,
       и читалось как потерянная панель. Проверка при этом была
       зелёной - порог стоит на четырёх, - но соврала числом, а
       инструмент, который врёт числом, отправляет искать призрак.
       Замер живьём: кольцо держит пять панелей из пяти во всех трёх
       состояниях, заголовок среди них и переводится вместе со всеми.

       Заголовок теперь считается отдельным признаком, ниже. */
    карточек: document.querySelectorAll("#relGrid .card:not(.cab-head)").length,
    головаВКольце: !!(h && h.closest("#relGrid")),
