...
export async function вПолёт(b, э) {
  ...
  /* ── ЧУЖОЙ ДОМЕН НЕ НАША ОШИБКА ────────────────────────────────
     ...
     chat.rocketcdn.ru. В песочнице у браузера нет доверия к
     сертификату прокси, запрос падает с ERR_CERT_AUTHORITY_INVALID, и
     проверки краснели на этом: «мусор на входе», «форма заявки».

     Ни к мусору, ни к форме это отношения не имеет. На боевом домене
     сертификат настоящий (проверено запросом: 200, 21995 байт), а сам
     виджет к отказу готов - не доехал пакет, на странице не остаётся
     ни кнопки, ни строки в меню (см. rc-chat.js).

     Проверка судит НАШ сайт. Сообщения о загрузке с чужих доменов
     отсеиваем, свои остаются все до одного: красная проверка, которая
     краснеет из-за чужого сертификата, перестаёт что-либо значить. */
  var чужое = /chat\.rocketcdn\.ru|ERR_CERT_AUTHORITY_INVALID/;
  pg.on("pageerror", e => ошибки.push("PE: " + e.message));
  pg.on("console", m => {
    if (m.type() !== "error") return;
    var т = m.text();
    if (чужое.test(т)) return;
    ошибки.push("CE: " + т.slice(0, 160));
  });
