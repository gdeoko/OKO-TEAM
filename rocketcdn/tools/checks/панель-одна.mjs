...
for (const с of СЛУЧАИ) {
  ...
  const из = await pg.evaluate(() => {
    ...
    сцена.traverse((o) => {
      ...
      /* Снимок рубки в тамбуре теперь ПЕЧЁТСЯ в холст: к нему
         применяется та же цветокоррекция, что в полёте, поверх
         кладётся дека с клавишами, а за стеклом рисуется космос -
         иначе тамбур показывал сырой файл с чёрной дырой вместо
         окна и без единой клавиши. У холста нет src, поэтому имя
         исходного снимка текстура несёт в userData, а холст в
         атрибуте. Проверка смотрит все три места: смысл её от этого
         не меняется - она по-прежнему сверяет, что рубка одна и та
         же. */
      const холстИмя = (к && к.getAttribute) ? (к.getAttribute("data-снимок") || "") : "";
      const мапИмя = (м && м.map && м.map.userData && м.map.userData["снимок"]) || "";
      const src = (к && (к.currentSrc || к.src)) || холстИмя || мапИмя || "";
