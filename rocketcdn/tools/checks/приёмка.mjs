...
const цели = await тел.evaluate(async () => {
  ...
  for (const e of подозрительные) {
    ...
    /* Не дотянул до сорока - меряем ещё и по нижнему порогу в
       двадцать четыре. Разница между «палец мажет» и «попасть
       нельзя» именно здесь, и валить их в одну кучу значит однажды
       перестать читать эту строку вовсе. */
    const шире24 = мой(сx - 11, сy) && мой(сx + 11, сy);
    const выше24 = мой(сx, сy - 11) && мой(сx, сy + 11);
    const подпись = ((e.textContent || "").trim().slice(0, 18) || String(e.className).slice(0, 18)) +
                    ` ${Math.round(r.width)}x${Math.round(r.height)}` +
                    (шире ? "" : " узкая") + (выше ? "" : " низкая");
    /* Тесно, но пройти можно: обе стороны от двадцати четырёх и хотя
       бы одна от сорока. */
    if (шире24 && выше24 && (шире || выше)) тесные.push(подпись);
    else мелкие.push(подпись);
  }
  return { мелкие, тесные };
});
/* Порогов два, и это не смягчение правила, а разные беды.

   ДО ДВАДЦАТИ ЧЕТЫРЁХ - беда. Ниже этого палец промахивается всерьёз,
   и никакая теснота ряда такое не оправдывает.

   ОТ ДВАДЦАТИ ЧЕТЫРЁХ, НО НЕ ДО СОРОКА ПО ОДНОЙ СТОРОНЕ - строка
   отчёта, а не беда. Ровно такие точки у каруселей: их зона 34 на 44,
   и вширь она не растёт по геометрии, а не по недосмотру. Точек в
   нижнем ряду двенадцать, экран телефона 412 - двенадцать зон по
   сорок это 480, ряд не помещается, а перекрытые зоны хуже узких:
   палец начинает попадать в соседа. Лента при этом листается пальцем,
   и точки - второй способ, а не единственный.

   Держать из-за этого проверку вечно красной значит приучить не
   читать её вовсе, а тихо занизить порог до двадцати четырёх значит
   потерять настоящие мелкие цели. Поэтому обе строки видны, а падает
   проверка только на первой. */
console.log(`   на телефоне целей мельче 24 точек: ${цели.мелкие.length}` +
            (цели.мелкие.length ? " · " + цели.мелкие.slice(0, 4).join(", ") : ""));
if (цели.тесные.length) {
  console.log(`   тесных (от 24, но не 40 по одной стороне): ${цели.тесные.length}` +
              " · " + цели.тесные.slice(0, 4).join(", "));
}
if (цели.мелкие.length) беды.push(`целей мельче 24 точек на телефоне: ${цели.мелкие.length} (${цели.мелкие.slice(0, 3).join(", ")})`);
