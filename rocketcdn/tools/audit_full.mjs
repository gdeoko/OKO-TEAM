...
for (const э of ЭКРАНЫ) {
  ...
  for (let i = 0; i <= 22; i++) {
    ...
    if (i % 4 === 0) {
      /* Перед снимком ждём дольше, чем при опросе актов.

         В песочнице нет видеокарты, кадр собирает SwiftShader, и
         объёмный слой выдаёт от одного до трёх кадров в секунду. Трёх
         сотых секунды не хватает даже на один кадр: снимок уходил
         наполовину собранным, и половина файлов получалась ровным
         чёрным полем. На такой файл потом смотрят и заводят дефект,
         которого нет: я сам едва не завёл. Две с лишним секунды дают
         слою собраться на любой машине. */
      await p.waitForTimeout(2200);
      /* Одной выдержки мало: на тяжёлых актах (облака, проход, салон)
         слой иногда не успевает и здесь. Проверяем сам файл: ровный
         чёрный кадр жмётся в считанные килобайты, живой весит
         десятки. Меньше тридцати - ждём и снимаем заново, до трёх
         попыток. Дороже на пару секунд, зато в папке не остаётся
         пустых снимков, по которым потом заводят несуществующие
         дефекты. */
      const путь = `${ПАПКА}/${э.имя}-${String(i).padStart(2, "0")}-${a}.jpg`;
      for (let по = 0; по < 3; по++) {
        const буфер = await p.screenshot({ path: путь, type: "jpeg", quality: 62 });
        if (буфер.length > 30000 || по === 2) break;
