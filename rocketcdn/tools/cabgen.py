...
def screens(a, mask_win, h, w):
    """Экраны пульта: циановые прямоугольники в полосе под окном.

    Их положение нужно, чтобы поверх запечённой картинки поставить
    живые: радар с разверткой, карту узлов, телеметрию. Запечённый
    экран остаётся подложкой - живой холст ложится ровно в его
    рамку, и прибор начинает работать.
    """
    r, g, b = a[:, :, 0], a[:, :, 1], a[:, :, 2]
    lum = luminance(a)
    # Порог мягкий: ниши генерятся нарочно пустыми, с еле заметным
    # циановым оттенком - яркого свечения там нет и не должно быть.
    cyan = (b > 26) & (b > r * 1.08) & (g > r * 0.9) & (lum > 18)
    ys, xs = np.nonzero(mask_win)
    win_b = ys.max() / h
    band = np.zeros_like(cyan)
    band[int(win_b * h):, :] = True
    cand = components(cyan & band, min_area=(h * w) // 3000,
                      max_area=(h * w) // 40, min_side=12)
    out = []
    for c in cand:
        ar = c["w"] / max(1.0, c["h"])
        if ar < 0.6 or ar > 6.0:
            continue
        # Плотность: экран это сплошное пятно, а не россыпь точек
        if c["area"] < c["w"] * c["h"] * 0.45:
            continue
        out.append({"x": c["x"] / w, "y": c["y"] / h,
                    "w": c["w"] / w, "h": c["h"] / h})
...
def main():
    ...
    # Общее приглушение кадра.
    #
    # Генератор отдаёт рубку светлее, чем нужно: у него в кадре нет
    # космоса, и он вытягивает экспозицию по раме. В сцене за окном
    # чёрный космос, и на его фоне та же рама читается как освещённая
    # днём комната. Заказчик: «как будто я в каком-то старом ведре».
    # Гасим всё, кроме того, что светится само - лампы и экраны
    # остаются на своей яркости, металл уходит в глубокий графит.
    tone = np.where(lit[:, :, None], 1.0, 0.62)
    a = a * tone
    ...
    # Размытие кромки окна. Полпикселя не хватало: отсечение по
    # прозрачности режет по порогу, и на скруглении шло видимой
    # лесенкой. Полтора пикселя дают порогу плавный переход, и край
    # выходит ровным на любом увеличении.
    ...
    # сверху.
    # Свечение линий держим умеренным: на полную они выгорали в белый,
    # и заказчик увидел пересветы. Полтора не нужно - хватает 1.15.
    emis = np.where(lit[:, :, None], np.minimum(a * 1.15, 255.0), a * 0.88).astype(np.uint8)
    ...
    data = {
        ...
        "экраны": [{k: round(v, 5) for k, v in ee.items()} for ee in screens(a, win, h, w)],
    }
    ...
    print("экранов найдено", len(data["экраны"]))
