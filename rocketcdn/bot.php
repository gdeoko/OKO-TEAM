...
/* Сайт RocketVPN. Ведём на /vpn/ нашего домена.

   Домен rocketcdn.ru наш и ни к какому телеграм-боту не привязан,
   значит эта ссылка не может открыть бота ни при каких условиях. За
   ней лежит тот же самый каталог, что и на rocketvpn.top: копии нет,
   расходиться нечему.

   Прежде вели на /vpn.php, который подписывал ключ створа и уводил
   переходом на rocketvpn.top. Створ у витрины теперь открыт, ключ не
   нужен, а лишний переход между доменами внутри телеграма только
   добавлял поводов открыть страницу не там.
...
$VPN_URL = rtrim(rc_cfg('site_url'), '/') . '/vpn/';
