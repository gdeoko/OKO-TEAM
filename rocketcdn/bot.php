...
/* Сайт RocketVPN. Ведём на его собственный домен.

   Владелец назвал три адреса и ровно три: rocketvpn.top это сайт VPN и
   больше ничего, rocketvpn.top/bot это бот VPN, rocketcdn это сайт CDN.
   Какое-то время здесь стояло зеркало сайта VPN на нашем домене - его
   заводили, пока было неясно, открывает ли домен клиента бота. Зеркало
   снято: сайт живёт на своём домене, и два адреса одной страницы это
   две правды об одном.

   Створ у витрины открыт, ключ не нужен, поэтому ведём прямо на корень,
   без промежуточного /vpn.php с подписью.
...
function save_binding($k, $v) {
    ...
    $d[$k] = $v;
    rc_json_write($f, $d);
    /* Привязки читаются из файла ОДИН раз, при подъёме конфига. Внутри
       запущенного бота он уже прочитан, и без этой строки ответ на
       только что привязанную ветку ушёл бы по старому адресу: человек
       видит «привязано» и сообщение не там, где привязывал. */
    global $RC;
    if (is_array($RC)) $RC[$k] = $v;
}
...
for ($loop = 0; $loop < 6; $loop++) {
    ...
    foreach ($res['result'] as $u) {
        ...
        /* ── ВЕТКА ПРИВЯЗЫВАЕТСЯ ПО СВОЕМУ ЖЕ НАЗВАНИЮ ───────────
           Привязка веток была ручной командой в каждой ветке, и это
           тот шаг, который делают один раз и забывают: завели «VPN ·
           Аналитика», а сводка VPN продолжает идти в ветку CDN, и
           понять это можно только по тому, что её там нет.

           Телеграм сам присылает служебное сообщение, когда ветку
           создали или переименовали, и в нём лежит её НАЗВАНИЕ. Имя
           ветки человек и так пишет осмысленно - «CDN · Аналитика»,
           «VPN · Ошибки», - значит оно и есть привязка. Читаем из него
           площадку и раздел, и ветка встаёт на место сама.

           Разделители в названиях бывают любые (точка, тире, двоеточие,
           пробел), поэтому смотрим на слова, а не на разметку. Не
           разобрали - молчим: привязать не ту ветку хуже, чем не
           привязать никакую. */
        /* ── ИМЯ ВЕТКИ ЛЕЖИТ И В ОБЫЧНОМ СООБЩЕНИИ ──────────────
           Служебное сообщение о создании ветки приходит ОДИН раз, и
           если бот в ту минуту не работал или обновления уже забраны,
           второго шанса нет: ветка стоит заведённая и непривязанная, а
           человеку остаётся вспоминать команду.

           Телеграм кладёт в каждое сообщение ветки ссылку на её первое
           сообщение (`reply_to_message`), а в нём лежит `forum_topic_
           created` с названием. Значит имя ветки можно прочитать из
           ЛЮБОЙ реплики в ней, и привязка случается сама, как только в
           ветке кто-нибудь что-нибудь напишет.

           Перевязываем только пустое: если ветка у этой площадки уже
           назначена, обычная переписка в соседней не имеет права её
           перебить. */
        $тема = $msg['forum_topic_created'] ?? $msg['forum_topic_edited'] ?? null;
        $поПереписке = false;
        if ($тема === null && $topic && !empty($msg['reply_to_message']['forum_topic_created'])) {
            $тема = $msg['reply_to_message']['forum_topic_created'];
            $поПереписке = true;
        }
        if ($тема !== null && $topic) {
            $имяВ = mb_strtolower((string)($тема['name'] ?? ''));
            $площадки = ['cdn' => 'cdn', 'vpn' => 'vpn', 'впн' => 'vpn', 'кдн' => 'cdn'];
            $разделы = ['аналитик' => 'stat', 'статистик' => 'stat', 'отчёт' => 'stat', 'отчет' => 'stat',
                        'форм' => 'form', 'заявк' => 'form',
                        'ошибк' => 'error', 'сбо' => 'error'];
            $с = null; $в = null;
            foreach ($площадки as $сл => $к) if (mb_strpos($имяВ, $сл) !== false) { $с = $к; break; }
            foreach ($разделы as $сл => $к) if (mb_strpos($имяВ, $сл) !== false) { $в = $к; break; }
            $ключВ = $с && $в ? 'tg_topic_' . $в . '_' . $с : '';
            $уже = $ключВ ? (string)rc_cfg($ключВ) : '';
            if ($ключВ && ($уже === '' || $уже === (string)$topic)) {
                $новая = ($уже === '');
                save_binding('tg_chat', (string)$chat);
                save_binding($ключВ, (string)$topic);
                if ($новая) {
                    rc_log("ветка «{$тема['name']}» привязана: {$с}/{$в} = {$topic}");
                    say($chat, 'Ветка узнана по названию: <b>' . htmlspecialchars(rc_sites()[$с]) . '</b>, раздел '
                        . htmlspecialchars($в) . '. Сюда и будет приходить.', null, null, $topic);
                }
            }
            /* Служебное сообщение на этом и кончается, а обычную реплику
               отдаём дальше: в ветке могут писать команды, и съесть их
               ради привязки значило бы сломать всё остальное. */
            if (!$поПереписке) continue;
        }
        if ($text === '') continue;
