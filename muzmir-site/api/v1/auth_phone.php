...
// Стоп-кран SMS: временно отключено до восстановления провайдера (запуск 10:00 МСК).
// Клиенты (login/register) UI уже не показывают, но если кто-то попадёт сюда прямым
// запросом — отвечаем корректной ошибкой, не занимая rate-limit и не создавая
// «отправленных» кодов, которых на деле нет.
json_out([
    'ok' => false,
    'sms_unavailable' => true,
    'error' => 'Вход по SMS временно недоступен. Войдите через почту или ВКонтакте.',
], 503);
