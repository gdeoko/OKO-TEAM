...
function vk_cb_ack_then_process(string $type): void {
    ...
    try {
        ...
        /* --- Вне рабочего времени (кроме вс): шаблон + сохраняем вопрос ---
         * Ответ по существу бот даст утром (cron/chat_offhours_flush.php) или оператор.
         *
         * ГРАФИК НЕ ЗАВИСИТ ОТ ТОГО, ЕСТЬ ЛИ В СООБЩЕНИИ ТЕКСТ.
         *
         * Здесь стояло условие «$text !== '' && нерабочее время». Человек, приславший
         * ОДНО вложение без подписи — скрин заявки, фото диплома, — получал полноценный
         * ответ в любое время суток: текста нет, проверка пропускается, бот идёт
         * отвечать. 13 сентября, в воскресенье в 20:18, так и вышло: на вложение бот
         * ответил по существу, а на следующую реплику того же человека, уже с текстом,
         * прислал «сейчас нерабочее время». Со стороны участника это выглядит как
         * поломка: сначала ответили, потом сказали, что не работают.
         *
         * Считаем сообщением всё, на что вообще можно ответить: текст ИЛИ вложение. */
        if (($text !== '' || $hasAttach) && chat_offhours_hold()) {
        ...
        // Страница ВКонтакте, с которой пишут: по ней запоминается связь с участником,
