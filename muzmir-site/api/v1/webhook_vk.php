...
function vk_cb_ack_then_process(string $type): void {
    ...
    try {
         ...
         * Человеческая пауза остаётся всем: мгновенный ответ на длинный вопрос
         * читается как автомат. Но обычному участнику к ней добавляется очередь,
         * а участник Клуба идёт вне очереди — за это он и платит. */
