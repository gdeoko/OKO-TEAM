    ...
    if ($password !== '') {
        ...
        if ($user) {
            if (($user['password_hash'] ?? '') !== '') {
                ...
                // У аккаунта нет пароля — но это НЕ повод отдать его тому, кто
                // назвал адрес. Раньше здесь пароль просто записывался и человека
                // впускали: достаточно было знать чужую почту, чтобы забрать
                // учётную запись вместе с её заявками и ролью.
                //
                // Пароль такому аккаунту ставится только после подтверждения
                // почты — тем же одноразовым кодом, что и вход по OTP.
                json_out([
                    'ok'    => false,
                    'need'  => 'otp',
                    'error' => 'Для этой почты пароль ещё не задан. Запросите код подтверждения — '
                             . 'после входа по коду пароль можно установить в настройках кабинета.',
                ], 409);
            }
        } else {
