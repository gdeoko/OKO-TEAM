...
/* ОЧЕРЕДЬ: КЛУБУ МОМЕНТАЛЬНО, ОСТАЛЬНЫМ ЧЕРЕЗ ПЯТЬ МИНУТ — И НИ СЛОВА ОБ ЭТОМ.
 *
 * Ответ уже готов, задерживается только его показ: участник Клуба видит его
 * сразу, обычный — через chat_delay_regular_sec.
 *
 * НИКАКИХ «ОТВЕЧУ В ТЕЧЕНИЕ ПЯТИ МИНУТ». Здесь стояло подтверждение приёма
 * вопроса — и оно превращало обычную паузу в объявление о самой себе: живой
 * человек не пишет «вижу Ваш вопрос, отвечу через пять минут», он просто
 * отвечает, когда освободился. Правило владельца дословно: «просто ответ даёт
 * через 5 минут и всё, и ничего лишнего». Окно чата опрашивает историю раз в
 * несколько секунд и покажет ответ сам, как только настанет его срок. */
...
if ($delay > 0) {
    /* Пока идёт пауза, в окно не уходит ничего — ни ответа, ни объявления о том,
       что он готовится. Окно добирает историю опросом и покажет ответ, когда
       наступит visible_at. Кнопки и картинка приедут вместе с ним. */
    json_out([
        'ok'      => true,
        'reply'   => '',
        'actions' => [],
...
function chat_maybe_escalate(string $text, ?int $uid, string $sessionKey, string $greetName): void {
    ...
    /* ЗВАТЬ ЧЕЛОВЕКА — ТОЛЬКО НА ДЕНЬГИ И ЖЁСТКУЮ ЖАЛОБУ (правило от 03.09.2026).
     *
     * Список был шире некуда: «сотрудник», «администратор», «не помог»,
     * «менеджер». Участник пишет «наш сотрудник подал заявку» или «не помогло
     * обновить страницу» — и владельцу летит «нужен оператор». Такие уведомления
     * приходили постоянно и обесценили сами себя: среди них терялся тот один
     * случай, ради которого оператор и нужен.
     *
     * Остались две группы: требование денег и настоящий конфликт (жалоба, угроза
     * инстанциями, обвинение в обмане), плюс прямая просьба позвать человека —
     * «позовите оператора», «соедините с человеком». Одно слово «оператор» больше
     * не срабатывает: оно встречается и в «оператор связи», и в «сотовый оператор». */
    $triggers = ['живой человек', 'с человеком', 'позовите', 'соедините', 'позвать оператора',
                 'нужен оператор', 'дайте оператора', 'жалоб', 'недоволен', 'недовольна',
                 'верните деньги', 'вернуть деньги', 'возврат', 'компенсац',
                 'отвратительн', 'безобразие', 'обман', 'мошенн', 'прокуратур',
