 ...
 * другие, — а самое устойчивое: конкурс, участник и название номера. На ДРУГОЙ
 * конкурс ту же работу подать можно: это другое соревнование, и правило
 * «одна заявка — один материал» там не нарушено. Отклонённые заявки не в счёт:
 * человек имеет право подать заново после отказа.
 *
 * СРАВНИВАЕМ ПО СМЫСЛУ, А НЕ ПО БУКВАМ. Прежняя сверка требовала, чтобы строки
 * совпали ровно, и повтор проходил от одной только разницы в написании:
 * «Хореографическая группа «Виктория»» · ««Лялечка»» и
 * «Хореографический коллектив «Виктория»» · «танец «Лялечка»» — один танец, одна
 * ссылка, две заявки. Теперь у коллектива берётся собственное имя, у номера
 * отбрасывается жанровая приставка (app_dup_person_key / app_dup_work_key). */
if (!$errors) {
    require_once BASE_PATH . '/core/app_fields.php';
    $dupWho  = app_dup_person_key($isGroup ? (string) $group_name : (string) $full_name);
    $dupWork = app_dup_work_key((string) $wt);
    if ($dupWho !== '' && $dupWork !== '') {
        foreach ($comps as $__c) {
            ...
            foreach (all("SELECT number, work_title, full_name, group_name, is_group, status
                         ...
                         [(int) $__c['id']]) as $__a) {
                $__isGrp = (int) ($__a['is_group'] ?? 0) === 1 || trim((string) $__a['group_name']) !== '';
                if (app_dup_person_key($__isGrp ? (string) $__a['group_name'] : (string) $__a['full_name']) !== $dupWho) continue;
                if (app_dup_work_key((string) $__a['work_title']) !== $dupWork) continue;
                $__found = $__a;
                break;
            }
            if ($__found) {
                json_out(['ok' => false, 'error' => 'Этот номер уже подан на конкурс «' . (string) $__c['name']
                    ...
                    . 'в конкурсе один раз — второй раз оценить его нельзя. Если заявку подал педагог, она уже '
                    . 'в работе; с другим номером можно подать отдельную заявку, а этот номер разрешается '
                    . 'заявить на другой конкурс.',
