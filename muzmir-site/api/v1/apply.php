...
/* ОДИН НОМЕР — ОДИН КОНКУРС, А НЕ ВСЕ СРАЗУ.
 *
 * В форме конкурсы отмечаются галочками, и люди отмечают все подряд: за сентябрь
 * из 292 заявок разными работами оказались 182, а 48 материалов ушли в три
 * конкурса каждый — одним нажатием, с одной и той же ссылкой. Дальше жюри
 * оценивает одно выступление трижды и присуждает ему три разных звания, а
 * участник получает три диплома за один номер. Проверка уникальности ссылки
 * этого не ловила: она считала занятость ВНУТРИ конкурса, а тут конкурсы разные.
 *
 * Теперь материал идёт в один конкурс. Правило выключается настройкой
 * apply_one_comp_per_work=0, если центр решит иначе.
 *
 * Сравниваем по ссылке (самое устойчивое — это буквально одна и та же запись) и
 * по паре «участник + название номера», потому что одну работу выкладывают и на
 * Яндекс.Диск, и во ВКонтакте. Отклонённые заявки не в счёт. */
if (!$errors && (string) setting('apply_one_comp_per_work', '1') === '1') {
    $lnk = '';
    if ($video !== '' && is_file(BASE_PATH . '/core/link_unique.php')) {
        require_once BASE_PATH . '/core/link_unique.php';
        if (function_exists('lu_norm')) $lnk = lu_norm($video);
    }
    // 1. Несколько конкурсов в одной подаче — принимаем только один.
    if (count($comps) > 1) {
        $errors['competition_ids'] = 'Один конкурсный номер участвует в одном конкурсе. '
            . 'Выберите конкурс, в котором хотите участвовать с этим номером, — а другой номер '
            . 'можно подать на остальные конкурсы отдельной заявкой.';
    }
    // 2. Этот же материал уже подан на другой открытый конкурс.
    if (!$errors && $lnk !== '') {
        foreach (all("SELECT a.number, a.video_url, c.name AS cname
                        FROM applications a JOIN competitions c ON c.id = a.competition_id
                       WHERE a.status NOT IN ('rejected','deleted','canceled')
                         AND COALESCE(a.video_url,'') <> ''
                         AND c.status IN ('open','judging')") as $__a) {
            if (lu_norm((string) $__a['video_url']) !== $lnk) continue;
            $errors['video_url'] = 'Эта запись уже подана на конкурс «' . (string) $__a['cname']
                . '» (заявка ' . (string) $__a['number'] . '). Один номер участвует в одном конкурсе: '
                . 'второй раз то же выступление оценить нельзя. Для другого конкурса подайте другой номер.';
            break;
        }
    }
}
...
if (!$errors) {
    ...
    /* Когда номер участвует только в одном конкурсе, искать повтор надо по ВСЕМ
     * идущим конкурсам, а не только по выбранным: одну работу выкладывают и на
     * Яндекс.Диск, и во ВКонтакте — ссылки разные, номер тот же. */
    $__scope = (string) setting('apply_one_comp_per_work', '1') === '1'
        ? all("SELECT id, name FROM competitions WHERE status IN ('open','judging')")
        : $comps;
    if ($dupWho !== '' && $dupWork !== '') {
        foreach ($__scope as $__c) {
            $__found = null;
