...
return [
    ...
    // КУДА ОТВЕЧАТЬ. Этот адрес уходит в заголовке Reply-To каждого письма, и
    // почтовые клиенты подставляют его в кнопку «Ответить». Здесь стоял ящик на
    // mail.ru, которым центр не пользуется и который никто не читает: ответы
    // ведомств и участников улетали туда и оставались без ответа. Адрес должен
    // совпадать с официальной почтой центра — на неё же ссылаются документы.
    'mail_reply_to'  => cfg('MUZMIR_MAIL_REPLY_TO', 'kc@музыкальный-мир.рф'),
    ...
    'mail_daily_max'   => (int) cfg('MUZMIR_MAIL_DAILY_MAX', 400),   // потолок массовых/день = 400 успешных (база 200 + Элитный клуб 100 + кабинет 100)
