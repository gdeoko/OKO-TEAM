 ...
 *    Р 7.0.97-2016 в ol_html_for()/lm_addressee(). Подать через приёмную сам
 *    агент не может: она требует авторизации в личном кабинете портала, а
 *    заводить учётную запись в государственной системе от имени владельца
 *    нельзя. Поэтому уходит ответ в переписку с переоформленным обращением, а
 *    пакет для подачи руками собирается отдельно и показывается владельцу.
 *
...
$outDir = BASE_PATH . '/data/letters/ruchnaya-podacha';
...
foreach ([50, 25] as $mid) {
    ...
    /* Ведомству, которое принимает только через приёмную, письмо уходит не как
       новое обращение, а как ответ на его замечание: они попросили переоформить,
       мы отвечаем в ту же переписку и говорим, что подадим через приёмную. */
    $portal = (int) ($r['portal_only'] ?? 0) === 1;
    $formUrl = trim((string) ($r['e_reception_url'] ?? ''));

    if ($portal) {
        // Пакет для подачи руками — рядом, одной папкой, с понятными именами.
        $stem = $outDir . '/' . preg_replace('~[^0-9a-zA-Z]~', '-', $number);
        @copy($pdf, $stem . '_obrashchenie.pdf');
        foreach ($att as $i => $a) if (is_file($a)) @copy($a, $stem . '_prilozhenie-' . ($i + 1) . '.' . pathinfo($a, PATHINFO_EXTENSION));
        echo "  пакет для приёмной: $stem" . "_*\n";
        echo "  приёмная: " . ($formUrl !== '' ? $formUrl : '(ссылка не известна)') . "\n";
        ...
        $body = '<div style="font-family:Georgia,\'Times New Roman\',serif;font-size:15px;line-height:1.62;color:#1B2340;max-width:640px">'
            ...
            . h(date('d.m.Y')) . '.</p>'
            . '<p>Обращение с приложениями будет направлено через интернет-приёмную комитета'
            . ($formUrl !== '' ? ' (' . h($formUrl) . ')' : '') . ', как Вы и просили. '
            . 'Настоящее письмо направляем в ту же переписку, чтобы замечание не осталось без ответа.</p>'
