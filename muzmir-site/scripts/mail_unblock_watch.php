...
/* СТОРОЖ РАБОТАЕТ, ТОЛЬКО ПОКА ЕСТЬ ЧТО СТЕРЕЧЬ.
 *
 * Он появился на время блокировки kc@ и nagradi.on@ Яндексом и каждый час шлёт
 * пробное письмо на почту центра, чтобы поймать момент снятия. Ограничение сняли
 * ещё в августе — письма сайта давно идут со своего домена, — а проба всё это
 * время продолжала капать на ящик по два письма в час, круглые сутки.
 *
 * Признак того, что стеречь нечего: mail_tx_via_gmail=0, то есть письма уже
 * ушли с запасного канала на рабочие ящики. В этом случае выходим молча, ничего
 * не отправляя. Вернётся блокировка — вернётся и признак, и сторож проснётся
 * сам, крон трогать не нужно. Ключ --force для ручной разовой проверки. */
if ((string) setting('mail_tx_via_gmail', '0') !== '1' && !in_array('--force', $argv, true)) {
    echo "Письма сайта идут со своего домена — стеречь нечего, проба не отправляется.\n";
    exit(0);
}
