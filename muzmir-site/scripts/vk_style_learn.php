...
function vsl_strip_greeting(string $t): string {
    ...
    // \b с кириллицей в PCRE не работает без (*UCP): «суток» не считается словом,
    // границы нет, и правило молча не срабатывало — обрывок «суток.» уезжал в канон.
    $t = preg_replace('~^\s*доброго\s+времени\s+суток[,!.\s]*~ui', '', $t) ?? $t;
    // Склейка серии оставляет обрывок «суток.», если «Доброго времени» было
    // отдельной репликой: режем и его.
    $t = preg_replace('~^\s*суток[,!.\s]+~ui', '', $t) ?? $t;
...
while (($line = fgets($fh)) !== false) {
    ...
    /* ОДИН ОТВЕТ — ЭТО ВСЯ СЕРИЯ РЕПЛИК ПОДРЯД.
     *
     * В чате отвечают не абзацем, а очередью коротких сообщений: отдельно
     * «Здравствуйте», отдельно суть, отдельно ссылка, отдельно подпись. Разбор
     * брал ПЕРВУЮ реплику после вопроса и на этом останавливался — из 385 тысяч
     * реплик в обучение попадала сотня, а сами ответы выходили обрезанными по
     * первой строке. Склеиваем серию целиком: и материала на порядок больше, и
     * ответ выглядит так, как его написал человек. */
    for ($i = 1; $i < count($turns); $i++) {
        if (($turns[$i]['who'] ?? '') === 'user') continue;       // это вопрос, а не ответ
        if (($turns[$i - 1]['who'] ?? '') !== 'user') continue;   // не начало серии
        ...
        // Ответ — вся серия реплик центра подряд, склеенная в один текст.
        $parts = [];
        $k = $i;
        for (; $k < count($turns) && ($turns[$k]['who'] ?? '') !== 'user'; $k++) {
            $one = trim((string) $turns[$k]['text']);
            if ($one !== '' && !vsl_is_machine($one)) $parts[] = $one;
        }

        $qc = vsl_clean($q);
        $ac = vsl_strip_greeting(vsl_clean(implode("\n", $parts)));
        $i  = $k - 1;                                             // серию второй раз не разбираем
...
try {
    ...
    foreach ($tpl as $c) {
        ...
        /* Эскалация по телефону — не ответ на вопрос, а действие для конфликта.
           В подсказке по теме она учит уводить разговор в колл-центр вместо
           ответа, что системный промпт как раз запрещает. */
        if (preg_match('~контактный номер телефона|с вами свяжется~ui', $c['a'])) continue;
        /* ОДНА ТЕМА — ОДИН ШАБЛОН НА СМЫСЛ.
           «Уточните: конкурс, ФИО, номер» центр писал десятком чуть разных
           формулировок. Все они об одном, и вместе вытесняют из подсказки
           остальные темы. Схлопываем по скелету — буквам без знаков и эмодзи. */
        $skel = mb_substr(preg_replace('~[^\p{L}]+~u', '', mb_strtolower($c['a'])) ?? '', 0, 34);
        if (isset($seenSkel[$c['topic']][$skel])) continue;
        $seenSkel[$c['topic']][$skel] = 1;
        /* НЕ БОЛЬШЕ ДВУХ ШАБЛОНОВ НА ТЕМУ.
           В подсказку модели и так уходят только два самых частых (chat_canon_prompt),
           а десяток вариаций одной и той же просьбы назвать данные лишь засоряет
           реестр и мешает владельцу править его руками. */
        $perTopicCanon[$c['topic']] = ($perTopicCanon[$c['topic']] ?? 0) + 1;
        if ($perTopicCanon[$c['topic']] > 2) continue;
        ...
        /* Чистим ещё раз перед записью: канон копится по всей выгрузке, и в
           ключ мог попасть вариант, собранный до срезки приветствия. Дешевле
           прогнать ту же функцию повторно, чем искать, какой из вариантов
           оказался длиннее и победил. */
        $ans = vsl_strip_greeting($c['a']);
        if (mb_strlen($ans) < 30) continue;
        /* КАНОН ОБЯЗАН ОТВЕЧАТЬ НА ВОПРОС ТЕМЫ.
         * Частота сама по себе ничего не гарантирует: чаще всего повторяются
         * реакции на присланный файл («Спасибо, вложение получила»), готовые
         * сообщения о выдаче («Ваш Диплом готов») и предложение вступить в клуб.
         * Это реплики по ходу разговора, а не ответы на вопрос — в подсказке по
         * теме они уводят бота в сторону. Проверка это и показала: на «Как
