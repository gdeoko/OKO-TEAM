...
$prefill = [
    ...
    'institution' => '', 'city' => '', 'country' => 'Россия', 'address' => '', 'postal_index' => '',
      ...
      <form class="card apply-card" id="applyForm" novalidate autocomplete="on">
        ...
        <section class="astep active" data-step="comp">
          <div class="astep-head">
            ...
            <?php
            // ЦЕНА — ИЗ БАЗЫ, А НЕ ЧИСЛОМ В ТЕКСТЕ. Здесь стояло «Каждое участие —
            // 500 ₽», и когда владелец поднял «Мировые Таланты» до 1000 ₽, участник
            // на первом же шаге читал одну сумму, а на шаге оплаты видел другую.
            // Конкурсы стоят по-разному, поэтому либо называем единую цену, либо
            // диапазон, либо не называем вовсе — если платных сейчас нет.
            $__prices = [];
            foreach ($comps as $__c) {
                if (!empty($__c['is_paid']) && (int) $__c['price'] > 0) $__prices[] = (int) $__c['price'];
            }
            $__prices = array_values(array_unique($__prices));
            sort($__prices);
            $__cost = !$__prices ? ''
                : (count($__prices) === 1
                    ? 'Участие — ' . $__prices[0] . ' ₽'
                    : 'Участие — от ' . $__prices[0] . ' до ' . end($__prices) . ' ₽, цена указана у каждого конкурса');
            ?>
            <?php /* ОДИН НОМЕР — ОДИН КОНКУРС.
                     Галочки звали отметить всё сразу, и половина заявок сентября
                     ушла в три конкурса одной и той же записью: жюри оценивало
                     одно выступление трижды. Теперь выбор один, и сказано почему. */
                  $__oneComp = (string) setting('apply_one_comp_per_work', '1') === '1'; ?>
            <p style="color:var(--muted);font-size:.88rem;margin:6px 0 0"><?= $__oneComp
                ? 'Выберите конкурс — один номер участвует в одном конкурсе. С другим номером можно подать отдельную заявку.'
                : 'Отметьте один или несколько — оплатите одним чеком.' ?><?= $__cost !== '' ? ' ' . h($__cost) . ', включая электронные основной и дополнительные дипломы.' : '' ?></p>
          </div>
          <div class="comp-list comp-list--multi"<?= $__oneComp ? ' data-one="1"' : '' ?>>
        ...
        <section class="astep" data-step="teacher">
          ...
          <div class="field ff">
            ...
            <div class="hint">Обязательно. Достаточно города — страну подставим сами: «Казань» → «Россия, г. Казань», «Минск» → «Республика Беларусь, г. Минск», «Дубай» → «Объединённые Арабские Эмираты, г. Дубай». Редкий населённый пункт пишите со страной: «Республика Беларусь, г. Столбцы».</div>
