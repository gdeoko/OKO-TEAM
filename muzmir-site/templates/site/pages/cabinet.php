...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!csrf_check()) {
        ...
        // наградные материалы — крон соберёт их заново из обновлённой заявки.
        ...
        flash('Заявка обновлена. Изменения учтены во всех наградных материалах.', 'success');
        redirect('/cabinet#apps');
    }
}
...
// Сколько наградных материалов уже в пути (для честной подписи в кабинете).
      ...
      <div class="cab-main">
        ...
        <div class="cab-panel" id="tab-diplomas" role="tabpanel">
          ...
          <?php if (!$diplomas): ?>
            ...
            // Длинный конкурс наградных материалов не изготавливает: там итоги
            ...
            <div class="cab-card cab-empty">
              ...
              <?php if ($diplomasPending > 0): ?>
                <p><b>Наградные материалы изготовлены</b><?= $_dipDate !== ''
                ...
                <p class="cab-meta" style="margin-top:8px;opacity:.85">Мы показываем в кабинете ровно то,
                  что уже отправлено Вам на почту, — поэтому наградного материала здесь пока нет.</p>
              <?php elseif ($_hasLong): ?>
                <p>По конкурсу с публикацией итогов списком дипломы за участие не изготавливаются.
                  Результат появится в день оглашения, после чего наградные материалы можно заказать
                  в разделе «Награды и заказы» — электронные или оригиналы.</p>
              <?php else: ?>
                <p>Дипломы появятся здесь после того, как жюри оценит работу и наградные материалы уйдут на Вашу почту.</p>
              <?php endif; ?>
            </div>
            ...
            <div class="cab-card reveal" style="--i:<?= $k ?>">
              ...
              <?php elseif (!$isClubOrder): ?>
                ...
                /* ОРИГИНАЛЫ НЕ СКАЧИВАЮТСЯ.
                   ...
                   заверенный печатью наградной материал — его везёт почта. Файла для
                ...
                if ($__hasOrig): ?>
                  <p class="cab-meta" style="margin-top:10px">
                    ...
                    Скачивание для них не предусмотрено: силу имеет только напечатанный оригинал.
                  </p>
                <?php endif; ?>
                <?php if ($trackUrl !== ''): ?>
                  ...
                  <div class="cab-set-body">
                    <form method="post" action="<?= url('/cabinet') ?>" enctype="multipart/form-data" id="profileForm">
                      ...
                      <div class="field">
                        ...
                        <div class="hint">Почта используется для входа и наградных материалов.</div>
                      </div>
