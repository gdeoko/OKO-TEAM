...
// ПОРЯДОК ГАЛЕРЕИ ЗАДАЁТСЯ ОДНИМ ЧИСЛОМ.
//
// Раньше здесь стояли три ключа сразу — дата письма, колонка sort и номер записи.
// Колонка sort заполнялась когда-то руками и с датами давно разъехалась, поэтому
// письмо за январь оказывалось между августовскими, и галерея выглядела как папка
// со сканами, а не как реестр. Теперь sort пересчитывается скриптом
// scripts/fix_ministry_letters.php (он же в кроне): новые сверху, внутри одной
// даты — по алфавиту субъекта. Странице остаётся взять готовый порядок.
//
...
<section class="section">
  <div class="container">
    ...
    <div class="section-head reveal">
      ...
      <p class="ms-lead"><strong>При информационной поддержке Федерального агентства по делам молодёжи (Росмолодежь), Министерств культуры и образования субъектов РФ и портала «Про.Культура.РФ».</strong> Ниже — официальные письма региональных ведомств.</p>
    </div>
    ...
    <div class="partners reveal" style="margin-top:30px">
      <?php foreach ([
          ...
          // Росмолодежь поддержала конкурс «Величие России»: публикация на
          // ресурсах Роспатриотцентра, письмо ЕЛ/8737-08 от 20.08.2026.
          ['emblem_rosmolodezh', 'Росмолодежь'],
