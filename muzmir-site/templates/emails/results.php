...
?>
<h1 style="margin:0 0 18px;font-size:24px;color:#7a2e1e;font-weight:700;">Результаты конкурса</h1>
<p style="margin:0 0 16px;">Здравствуйте<?= $name !== '' ? ', ' . h($name) : '' ?>! Жюри завершило работу по конкурсу «<?= h($competition) ?>». Благодарим Вас за участие.</p>
<?php if ($result !== ''): ?>
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:8px 0 22px;background:linear-gradient(135deg,#fbf1de,#f3e2c4);border-radius:14px;border:1px solid #e6d0a8;">
  <tr><td style="padding:24px;text-align:center;">
    ...
    <div style="font-size:22px;font-weight:700;color:#7a2e1e;">«<?= h($result) ?>»</div>
    <?php if ($score !== ''): ?><div style="margin-top:6px;font-size:14px;color:#8a7658;">Оценка жюри: <?= h($score) ?></div><?php endif; ?>
  </td></tr>
</table>
<?php endif; ?>
<p style="margin:0 0 8px;">Полный протокол и Ваш диплом доступны на странице результатов и в личном кабинете.</p>
