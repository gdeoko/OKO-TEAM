...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && input('do') === 'grade') {
    ...
    if ($appId && $score >= 1 && $score <= 10) {
        ...
        // автопереход к следующему в очереди
        $queue = grading_queue($comp);
        $next = null;
        foreach ($queue as $row) { if ((int)$row['id'] > $appId) { $next = (int)$row['id']; break; } }
        if ($next) admin_redirect('grading', array_filter(['id'=>$next,'competition'=>$comp]));
        admin_redirect('grading', array_filter(['competition'=>$comp]));
    }
    admin_redirect('grading', array_filter(['id'=>$appId,'competition'=>$comp]));
}
...
if ($id = (int) input('id')) {
    ...
    ob_start(); ?>
    <div class="toolbar">
      <a class="btn btn--ghost btn--sm" href="<?= a_link('grading', array_filter(['competition'=>$comp])) ?>"><?= admin_icon('back') ?>К очереди</a>
      <?php if ($pos): ?><span class="badge badge--gold">В очереди: <?= $pos ?> из <?= $total ?></span><?php endif; ?>
      <?php if ($nextId): ?><a class="btn btn--navy btn--sm" href="<?= a_link('grading', array_filter(['id'=>$nextId,'competition'=>$comp])) ?>">Пропустить →</a><?php endif; ?>
    </div>
    ...
    <div class="grid grid-2">
      ...
      <div class="card">
        ...
        <form method="post" action="<?= url('/admin/') ?>" id="gradeForm">
          <?= csrf_field() ?><input type="hidden" name="do" value="grade"><input type="hidden" name="id" value="<?= $id ?>">
          <input type="hidden" name="competition" value="<?= $comp ?>"><input type="hidden" name="score" id="scoreInput" value="<?= h((string)($my['score'] ?? '')) ?>">
...
$rows = all("SELECT a.*, c.name comp,
             ...
             WHERE $w ORDER BY a.id LIMIT 300", array_merge([(int)(current_user()['id']??0)], $args));
...
<form method="post" action="<?= url('/admin/') ?>">
  <?= csrf_field() ?><input type="hidden" name="do" value="bulk_grade"><input type="hidden" name="competition" value="<?= $comp ?>">
  <div class="toolbar">
  ...
  <div class="table-wrap">
    <table class="tbl">
      ...
      <tbody>
        <?php if (!$rows): ?><tr><td colspan="8" class="empty muted" style="text-align:center;padding:28px">Очередь пуста — все заявки оценены</td></tr><?php endif; ?>
        <?php foreach ($rows as $a): ?>
          <tr>
            ...
            <td><a class="btn btn--primary btn--sm" href="<?= a_link('grading', array_filter(['id'=>$a['id'],'competition'=>$comp])) ?>"><?= admin_icon('grading') ?>Оценить</a></td>
          </tr>
        <?php endforeach; ?>
      </tbody>
    </table>
