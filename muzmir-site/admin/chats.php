 ...
 *
 * ЗА ИМЕНЕМ В СЕТЬ НЕ ХОДИМ. Здесь стоял вызов vk_user_name() — запрос к API
 * ВКонтакте на КАЖДЫЙ диалог списка. В списке их до ста двадцати, у запроса
 * таймаут 25 секунд и четыре попытки с паузами: одного медленного ответа ВК
 * хватало, чтобы страница висела минутами.
 *
 * Хуже, что страдала не только эта вкладка. Каждый висящий запрос занимает
 * процесс PHP, а их на сервере четырнадцать. Пока владелец обновлял «Чат-бот»,
 * процессы кончались — и переставала открываться ВСЯ админка: заявки, заказы,
 * рассылки. Ровно это и выглядело как «не работает ни одна вкладка».
 *
 * Имя берём из базы: его сохраняет разбор входящих сообщений ВК в
 * chat_dialogs.title. Не сохранилось — показываем номер собеседника, это
 * честно и мгновенно.
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($do === 'reply' && $sk !== '') {
        ...
        flash(chats_channel($sk) === 'vk'
            /* ПРИЧИНУ НАЗЫВАЕМ НАСТОЯЩУЮ.
             * Здесь на любой отказ писалось «проверьте токен сообщества», и это
             * сбивало с толку: 8 сентября токен был цел, а ВКонтакте держал его
             * в паузе по темпу («Flood control»). Оператор шёл проверять токен
             * вместо того, чтобы просто подождать — ответ дошлётся сам
             * (cron/vk_resend_stuck), как только пауза кончится. */
            ? ($sent ? 'Ответ отправлен участнику во ВКонтакте.' . $tail
                     : ((int) setting('vk_flood_until', '0') > time()
                        ? 'Ответ сохранён. ВКонтакте временно не принимает сообщения от сообщества (пауза по темпу до '
                          . date('H:i', (int) setting('vk_flood_until', '0')) . '); письмо уйдёт само, как только пауза кончится.'
                        : 'Ответ сохранён, но ВКонтакте его не принял. Причина в журнале: data/logs/vk.log.'))
          ...
          <div style="max-width:78%;padding:9px 12px;border-radius:12px;background:<?= $pending ? '#fdf3e0' : ($mine ? '#d9f0e0' : '#fff') ?>;border:1px solid <?= $pending ? '#f0d9a8' : '#e4e7ec' ?><?= $pending ? ';opacity:.85' : '' ?>">
            ...
            <div class="chat-tools" style="margin-top:5px;display:flex;gap:8px">
              ...
              <!-- ВЗЯТЬ ТЕКСТ БОТА В СВОЙ ОТВЕТ.
                   Оператор часто отвечает почти тем же текстом, что подготовил бот:
                   правит пару слов и отправляет. Выделять ответ на странице и
                   копировать руками — лишняя работа, особенно с телефона, и лишний
                   способ потерять кусок текста. Кнопка кладёт в поле ответа ИСХОДНЫЙ
                   текст сообщения, каким он лежит в базе, без следов вёрстки. -->
