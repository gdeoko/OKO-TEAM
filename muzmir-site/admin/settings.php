...
function settings_schema(): array {
    return [
        ...
        // Контакты вынесены в админку: они печатаются в письмах, в положениях и в
        // подвале сайта, а менялись только правкой config.php на сервере.
        'Контакты оргкомитета' => [
            'org_phone'   => ['Телефон', false, '+7 ...'],
            'org_email'   => ['Почта для участников', false, 'info@...'],
            'org_address' => ['Почтовый адрес', false, ''],
        ],
        // Кто чем пишет. Раньше это жило только в config.local.php на сервере, и
        // чтобы завести новый обратный адрес, приходилось лезть в файл по SSH.
        'Отправители писем' => [
            'unisender_api_key'   => ['Ключ сервиса рассылок', true, ''],
            'unisender_from'      => ['Обратный адрес для СВОЕЙ базы', false, 'news@музыкальный-мир.рф'],
            'unisender_from_cold' => ['Обратный адрес для УЧРЕЖДЕНИЙ (холодная база)', false, 'pedagogam@музыкальный-мир.рф'],
            'mail_from_name'      => ['Имя отправителя', false, 'Культурный центр «Музыкальный Мир»'],
            'mail_from_name_cold' => ['Имя отправителя для учреждений', false, 'Культурный центр «Музыкальный Мир»'],
        ],
        // Темп рассылки. Цифры подобраны под окно 9:00–18:00 и крон раз в минуту;
        // менять их стоит, только понимая, что стоит за каждой.
        'Темп рассылки' => [
            'nl_split_konkurs_pct' => ['Доля своей базы, % от дневного объёма', false, '50'],
            'nl_split_inst_pct'    => ['Доля учреждений, %', false, '50'],
            'nl_warmup'            => ['Прогрев отправителя (1 — включён)', false, '1'],
            'nl_warmup_max'        => ['Потолок прогрева, писем в день', false, '10000'],
            'nl_box_burst'         => ['Писем за один прогон крона', false, '25'],
            'nl_service_gap_sec'   => ['Пауза сервиса между письмами, сек', false, '3'],
        ],
        'Подписанты диплома' => [
            'sign1_name' => ['Подпись 1 — ФИО/роль', false, 'Галиулин Данил Дамирович'],
            'sign1_desc' => ['Подпись 1 — должность/регалии', false, 'Председатель Оргкомитета'],
            'sign2_name' => ['Подпись 2 — ФИО', false, 'Ильясов Альберт Ильясович'],
            'sign2_desc' => ['Подпись 2 — должность/регалии', false, 'Председатель аттестационной комиссии'],
        ],
        'Безопасность' => [
...
<div class="flash flash--warning">Сохранённое здесь применяется на сайте сразу. Если тот же ключ задан в окружении сервера (переменные окружения / config.php), приоритет остаётся за окружением — так боевые ключи нельзя перебить из браузера. Секреты показаны маской: оставьте поле пустым, чтобы не менять сохранённое значение.</div>
