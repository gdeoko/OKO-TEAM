...
**Правила диалога (как живой оператор):**
- Приветствие «Здравствуйте, Имя.» — один раз в начале диалога и снова после
  перерыва 24 ч или после завершённого диалога (имя ВК берётся через `vk_user_name()`).
- Подпись «🌍 С уважением, оргкомитет…» — НЕ в каждом сообщении, только при завершении.
- Завершение диалога: пользователь пишет «спасибо/пока/всё понятно» → сразу финал
  «🙏 Благодарим Вас за обращение…» + подпись, диалог помечается `dialog_end`.
- Молчание после ответа: `cron/vk_bot_close.php` (каждые 10 мин) через 30 мин тишины
  шлёт «Есть ли у Вас ещё вопросы?», ещё через 30 мин тишины — финал + подпись.
- Маркеры в `chat_messages`: `assistant_followup` (вопрос «ещё вопросы?»),
  `dialog_end` (диалог закрыт → следующий вопрос снова с приветствием).

**Поведение «как живой человек» (ВК):**
- Персона — живой менеджер колл-центра. Бот НИКОГДА не признаётся, что он ИИ/бот
  (на «вы бот?» → «Я оператор колл-центра, рад(а) помочь»). Стиль: коротко, по фактам,
  без воды и повторов, с лёгкими вариациями формулировок.
- «Печатает…» показывается сразу (`vk_typing` → messages.setActivity), ответ приходит
  через 20-30 сек (короткий/повтор — 8-14 сек). Пауза выполняется ОТДЕЛЬНЫМ процессом
  `cron/vk_send_delayed.php` (exec … &), чтобы не держать php-fpm воркеры (пул из 5!).
- Повторный один и тот же вопрос (`chat_repeat_count`, сравнение по словам ≥0.6) →
  короткий человеческий ответ «уже писал(а) выше», без повторной простыни (анти-троллинг).
