...
function _qr_place_format(array &$m, int $n, int $fmt): void {
    // Копия 1 (у левого-верхнего искателя). Позиция k несёт бит (14-k), MSB-first.
    $pos1 = [[8,0],[8,1],[8,2],[8,3],[8,4],[8,5],[8,7],[8,8],[7,8],[5,8],[4,8],[3,8],[2,8],[1,8],[0,8]];
    foreach ($pos1 as $k => [$r, $c]) {
        $m[$r][$c] = ($fmt >> (14 - $k)) & 1;
    }
    // Копия 2: вертикаль у нижнего-левого искателя (биты 14..8), горизонталь у правого-верхнего (биты 7..0).
    for ($j = 0; $j <= 6; $j++) $m[$n - 1 - $j][8] = ($fmt >> (14 - $j)) & 1;
    for ($t = 0; $t <= 7; $t++) $m[8][$n - 8 + $t] = ($fmt >> (7 - $t)) & 1;
}
