...
function chat_context_from_dialogue(string $sessionKey, string $text): string {
    ...
    if (!$found) return '';
    return "НАЙДЕНО ПО НАЗВАННЫМ В ПЕРЕПИСКЕ ДАННЫМ (номер заявки / ФИО) — отвечай строго по этим заявкам, "
        . "при необходимости вежливо подтверди личность (ФИО, конкурс):\n" . _chat_apps_lines(array_values($found));
}
