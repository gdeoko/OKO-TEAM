...
function chat_user_context(?int $uid): string {
    ...
    if (!$apps) return 'ДАННЫЕ УЧАСТНИКА: у него пока нет поданных заявок. Предложи подать заявку.';
    ...
    $L = ['ДАННЫЕ ЭТОГО УЧАСТНИКА (отвечай точно по его заявкам; если заявок несколько — уточни, по какой из них вопрос):'];
    foreach ($apps as $a) {
        ...
        $line = '— Заявка №' . $num . ' на «' . (string) ($a['comp_name'] ?? '') . '» (' . ($paidComp ? 'платный' : 'бесплатный') . '), статус: ' . $st;
