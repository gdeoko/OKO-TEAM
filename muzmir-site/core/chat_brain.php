...
function chat_repeat_reply(int $count, string $name = ''): string {
    if ($count >= 3) {
        ...
        $v = [
            'Я уже отвечал(а) на этот вопрос чуть выше 🙂 Посмотрите, пожалуйста, моё предыдущее сообщение. Если что-то осталось непонятным — уточните, что именно.',
            'Этот момент я пояснял(а) выше по переписке. Подскажите, что именно осталось неясным — с радостью распишу подробнее.',
        ];
    }
