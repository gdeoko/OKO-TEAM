...
function chat_system_prompt(): string {
    ...
    $L[] = 'ОБЯЗАТЕЛЬНО каждый ответ начинай ровно с фразы (отдельной строкой): «Здравствуйте, доброго времени суток.»';
...
function chat_rule_reply(string $text): string {
    ...
    $tail = "\n\nЕсли нужна помощь человека — Оргкомитет ответит на почту {$orgEmail}." . $sign;
    ...
    if ($has(['контакт', 'телефон', 'почт', 'адрес', 'связат', 'режим', 'работает'])) {
        return 'Контакты Оргкомитета: почта ' . $orgEmail . ', телефон ' . cfgv('org_phone') . '. Режим работы: ' . cfgv('org_hours') . '. Адрес: ' . cfgv('org_address') . '.' . $sign;
    }
    ...
    return 'Спасибо за обращение! Я передал Ваш вопрос специалистам - Оргкомитет ответит на почту ' . $orgEmail . ' в рабочее время (' . cfgv('org_hours') . '). А пока могу подсказать про заявки, стоимость участия, результаты и награды.' . $sign;
}
...
function chat_ensure_greeting(string $reply): string {
    $greeting = 'Здравствуйте, доброго времени суток.';
    $reply = trim($reply);
    if (mb_stripos($reply, 'здравствуйте') === 0) return $reply; // уже поздоровались
    return $greeting . "\n\n" . $reply;
}
