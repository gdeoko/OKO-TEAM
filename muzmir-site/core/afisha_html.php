...
function afisha_terms(array $c): string {
    ...
    /* Главное про этот конкурс - не то, что он бесплатный, а то, что попасть в
       него может только участник ВИП-клуба. Раньше это стояло мелкой строкой под
       лентой, и человек со стороны подавал заявку, которую приходилось
       отклонять. */
    if ((int) ($c['club_only'] ?? 0) === 1) return 'ТОЛЬКО ДЛЯ УЧАСТНИКОВ ВИП-КЛУБА · ПРИЗОВОЙ ФОНД 100 000 ₽';
...
function afisha_note(array $c): string {
    return (int) ($c['club_only'] ?? 0) === 1
        ? 'Участие бесплатное, входит в членство ВИП-клуба · Основной и дополнительный дипломы включены · Подробности в положении конкурса'
        : '';
}
