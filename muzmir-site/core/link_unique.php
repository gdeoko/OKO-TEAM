 ...
 */
function lu_check(string $url, int $compId, int $exceptAppId = 0, string $nomination = ''): array {
    /* У ХУДОЖНИКОВ ПАПКА БЫВАЕТ ОДНОЙ РАБОТОЙ.
     *
     * Изделие прикладного творчества снимают с нескольких сторон, у рисунка
     * присылают общий план и фрагмент — это одна работа в нескольких файлах, и
     * запрещать такую подачу нельзя: участница так и написала 21 августа, что
     * «ни одна ссылка на альбом не отправляется».
     *
     * Здесь папка художникам разрешена, а от повтора защищает второе правило:
     * одна ссылка — одна заявка в конкурсе. Именно оно и отсекает случай школы,
     * приславшей 37 заявок с одной общей папкой: пройдёт только первая. */
    $isArtwork = $nomination !== '' && preg_match(
        '~изобраз|декоратив|дпи|прикладн|фото|живопис|график|скульптур|дизайн~ui', $nomination) === 1;

    $f = $isArtwork ? ['folder' => false, 'files' => 0, 'why' => ''] : lu_is_folder($url);
    if ($f['folder']) {
