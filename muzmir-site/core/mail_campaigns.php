 ...
 * афиши конкурсов с обложками, полосу образцов наград, блок Элитного клуба, плашку поддержки
 * органов культуры и рабочие кнопки-CTA (Подать заявку, Положение, Образцы наград,
 * Поддержка Минкульта, Вступить в Элитный клуб). Все картинки — абсолютными URL (почтовики
...
/**
 * Блок Элитного клуба в теле письма.
 *
...
/** Сетка привилегий Элитного клуба (как раздел «Что даёт членство» на сайте) — плитки без цен. */
function mmc_vip_perks_grid(): string {
    ...
    $perks = [
        ...
        ['✚', 'Моментальный ответ в поддержке', 'Чат отвечает участнику Клуба сразу, вне очереди: обычный ответ занимает 5-15 минут.'],
...
function campaign_subject(string $type, array $opt = []): string {
    ...
    $ab = (int) ($opt['ab'] ?? 0) % 2;
    $variants = [
        'vip'      => ['Приглашение в Элитный клуб «Музыкальный Мир» - участвуйте без ограничений',
                       'Элитный клуб «Музыкальный Мир»: все конкурсы сезона по одной подписке'],
...
function campaign_inner(string $type, array $opt = []): string {
    ...
    switch ($type) {
        ...
        case 'vip':
            // Красиво, как раздел «Элитный клуб» на сайте: CTA вверху и внизу, все привилегии,
