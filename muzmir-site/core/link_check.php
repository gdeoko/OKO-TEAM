...
if (!function_exists('video_verify')) {
    ...
    function video_verify(string $url, string $nomination = ''): array {
        ...
        if ($hostIs('vk.com') || $hostIs('vkvideo.ru')) {
            ...
            $isArtwork = $nomination !== '' && preg_match(
                '~изобраз|декоратив|дпи|прикладн|фото|живопис|график|скульптур|дизайн~ui', $nomination) === 1;
            if ($isArtwork && preg_match('#/(photo-?\d+_\d+|album-?\d+_\d+)#i', $url)) {
                return _lc_result('ВКонтакте', null);
            }
            if (!preg_match('#video(-?\d+_\d+(?:_[a-z0-9]+)?)#i', $url, $m)) {
                return _lc_bad('VK Видео', $isArtwork
                    ? 'Дайте ссылку на саму работу: фотографию ВКонтакте (…/photo123_456) или файл на Яндекс.Диске, Google Диске, в Облаке Mail.ru.'
                    : 'Дайте ссылку на видео VK вида …/video-123_456.');
            }
            $vid = $m[1];
        ...
        if ($hostIs('disk.yandex.ru') || $hostIs('disk.yandex.com') || $hostIs('yadi.sk')) {
            ...
            /* У ХУДОЖНИКА КОНКУРСНАЯ РАБОТА — КАРТИНКА, А НЕ ВИДЕО.
             *
             * Эти две проверки отсекали по изобразительному искусству и
             * фотографии всё подряд: папку с фотографиями работы — «дайте ссылку
             * на видеофайл», сам снимок — «этот файл не является видео». Подать
             * заявку было нельзя в принципе, о чём участница и написала
             * 21 августа. Для таких номинаций изображение и альбом — нормальная
             * форма подачи; для остальных требование видео остаётся. */
            $isArtwork = $nomination !== '' && preg_match(
                '~изобраз|декоратив|дпи|прикладн|фото|живопис|график|скульптур|дизайн~ui', $nomination) === 1;
            if (($j['type'] ?? '') === 'dir' && !$isArtwork) {
            ...
            if ($mt !== '' && $mt !== 'video' && !($isArtwork && in_array($mt, ['image', 'document', 'compressed'], true))) {
                return _lc_bad('Яндекс.Диск', $isArtwork
                    ? 'Этот файл на Яндекс.Диске не похож на изображение работы (' . $mt . '). Приложите фотографию конкурсной работы.'
                    : 'Этот файл на Яндекс.Диске не является видео (' . $mt . '). Приложите конкурсное видео.');
            }
            $ts = null;
        ...
        if ($hostIs('drive.google.com') || $hostIs('docs.google.com')) {
            // Художнику папка Google Диска разрешена: там лежат снимки одной
            // работы с разных сторон, и требовать один файл незачем.
            $isArtwork = $nomination !== '' && preg_match(
                '~изобраз|декоратив|дпи|прикладн|фото|живопис|график|скульптур|дизайн~ui', $nomination) === 1;
            if ($isArtwork && preg_match('#drive\.google\.com/drive/(u/\d+/)?folders/#i', $url)) {
                return _lc_result('Google Диск', null);
            }
            if (!preg_match('#/d/([a-zA-Z0-9_-]{10,})#', $url, $m) && !preg_match('#[?&]id=([a-zA-Z0-9_-]{10,})#', $url, $m)) {
                return _lc_bad('Google Диск', $isArtwork
                    ? 'Дайте ссылку на файл или папку Google Диска с фотографией работы.'
                    : 'Дайте прямую ссылку на файл Google Диска (…/file/d/…/view).');
            }
