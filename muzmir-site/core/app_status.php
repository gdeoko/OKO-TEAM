...
function app_state(array $app, bool $forAdmin = false): array {
    ...
    // У КОНКУРСА С ОГЛАШЕНИЕМ СПИСКОМ РЕЗУЛЬТАТ ОТКРЫВАЕТ ПУБЛИКАЦИЯ, А НЕ ПИСЬМО.
    //
    // Правило «участник видит только дошедшее» верно для короткого конкурса, где
    // результат приходит письмом каждому. У длинного итоги объявляются списком:
    // письма может не быть вовсе, а звание уже названо публично. Без этой ветки
    // сто участников «Величия России» после оглашения 28 августа увидели бы в
    // кабинете «На оценке» и пустое место вместо звания.
    $resultShown = $forAdmin ? $hasResult : ($resultSent || ($isList && $listPub && $hasResult));
    $dipAllSent  = $forAdmin
