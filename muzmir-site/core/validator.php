...
function v_video(string $url): array {
    ...
    // Живость ссылки — HEAD-запрос с таймаутом (тихий фолбэк: недоступность DNS не валит валидацию).
    if (!v_url_alive($url)) {
        return ['ok' => false, 'platform' => $platform, 'reason' => 'Ссылка недоступна, проверьте доступ по ссылке'];
    }
