...
function vf_direct_link(string $url): array {
    ...
    switch (vf_platform($url)) {
        ...
        case 'yandex_disk':
            ...
            /* ФАЙЛ ЕСТЬ, А ССЫЛКИ НА СКАЧИВАНИЕ НЕТ — ЭТО НЕ ВИНА УЧАСТНИКА.
             *
             * У заявки #1607 Яндекс отдаёт полные метаданные (имя «Полный номер
             * горизональный.mp4», 433 МБ), а на запрос скачивания возвращает
             * пустой href: так бывает, когда у владельца диска исчерпан лимит
             * раздачи. В браузере запись при этом открывается, и снимать
             * человека с конкурса за «недействительную ссылку» нельзя. Отличаем
             * этот случай словами — дальше по коду он не превращается в отказ. */
            $meta2 = json_decode(vf_http('https://cloud-api.yandex.net/v1/disk/public/resources?public_key='
                     . rawurlencode($url))['body'], true);
            if (is_array($meta2) && (string) ($meta2['type'] ?? '') === 'file') {
                return $fail('Яндекс.Диск: файл на месте, но диск не выдаёт ссылку на скачивание '
                           . '(обычно исчерпан лимит раздачи у владельца) — нужен просмотр человеком');
            }
        ...
        case 'vk':
            ...
            $vid = '';
            /* КЛЮЧ ДОСТУПА ОТДЕЛЯЕТСЯ ПОДЧЁРКИВАНИЕМ, А НЕ ВОПРОСИТЕЛЬНЫМ ЗНАКОМ.
             *
             * Прежнее выражение принимало за ключ и то, что идёт после «?»: у
             * ссылки vkvideo.ru/video162427751_456239204?pl=162427751_-2
             * получался идентификатор «162427751_456239204_pl», ВК на такой
             * отвечал пустотой, и девять работ последнего прогона выглядели
             * как «запись закрыта настройками приватности» — при том что API
             * отдаёт их свободно, по четыре файла на каждую. */
            if (preg_match('~(?:video|clip)/?(-?\d+)_(\d+)(?:_([A-Za-z0-9]{6,}))?~', $url, $m)) {
            ...
            if ($vid !== '') {
                ...
                if ($token !== '') {
                    ...
                    if ($item) {
                        /* БЕРЁМ ЛУЧШЕЕ ДО 720p, А ЕСЛИ ЕГО НЕТ — ЧТО ЕСТЬ.
                         *
                         * Прежде отбирались только дорожки не выше 720p, и
                         * запись, выложенная одним файлом mp4_1080, не
                         * скачивалась вовсе: девять работ последнего прогона
                         * получили «запись закрыта настройками приватности»,
                         * хотя ВК отдаёт их свободно. Лишний вес мы всё равно
                         * срежем при подготовке дорожек. */
                        $files = (array) ($item['files'] ?? []);
                        $best = ['h' => 0, 'u' => ''];      // лучшее в пределах 720p
                        $any  = ['h' => PHP_INT_MAX, 'u' => ''];   // самое лёгкое из остального
                        foreach ($files as $k => $u) {
                            if (!preg_match('~^mp4_(\d{3,4})$~', (string) $k, $mm)) continue;
                            $h = (int) $mm[1];
                            if ($h <= 720) {
                                if ($h > $best['h']) $best = ['h' => $h, 'u' => (string) $u];
                            } elseif ($h < $any['h']) {
                                $any = ['h' => $h, 'u' => (string) $u];
                            }
                        }
                        if ($best['u'] === '' && $any['u'] !== '') $best = $any;
                        if ($best['u'] !== '') {
                            return ['ok' => true, 'url' => $best['u'], 'name' => 'vk_' . $best['h'] . '.mp4',
                                    'size' => 0, 'why' => ''];
                        }
                        // Совсем без mp4 бывает у записей, лежащих только потоком:
                        // ffmpeg читает плейлист так же, как обычный файл.
                        foreach (['hls_ondemand', 'hls', 'dash_ondemand'] as $k) {
                            if (!empty($files[$k])) {
                                return ['ok' => true, 'url' => (string) $files[$k],
                                        'name' => 'vk.m3u8', 'size' => 0, 'why' => ''];
                            }
                        }
                    }
                }
