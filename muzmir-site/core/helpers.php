...
/**
 * ЛОГОТИП НА СТРАНИЦЕ САЙТА — ФАЙЛОМ, А НЕ СТРОКОЙ ВНУТРИ HTML.
 *
 * logo_data_uri() отдаёт настоящий логотип в base64 — это обязательно там, где
 * картинку неоткуда взять: письмо, диплом, PDF. Но на обычной странице такой
 * логотип весит 172 КБ и приезжает ЗАНОВО при каждом открытии: браузеру нечего
 * кэшировать, строка лежит внутри разметки. В админке на этом уходило три
 * четверти веса страницы — двести с лишним килобайт на каждый переход по
 * вкладке, и с телефона панель выглядела как «не открывается».
 *
 * Файлом логотип весит 29 КБ и скачивается один раз на всё время работы.
 * Логотип тот же самый, настоящий, в тех же пропорциях — меняется только
 * способ доставки.
 */
function logo_web_src(): string {
    static $u = null;
    if ($u === null) {
        $u = is_file(BASE_PATH . '/public/assets/img/logo_muzmir_256.webp')
            ? asset('img/logo_muzmir_256.webp')
            : asset('img/logo_muzmir_256.png');
    }
    return $u;
}
...
/**
 * РАЗБОР ЗВАНИЯ УЧАСТНИКА — ОДИН НА ВСЁ ПРИЛОЖЕНИЕ.
 *
 * Звание сравнивали кусками строки: «содержит "i степ"». В «ЛАУРЕАТ III
 * СТЕПЕНИ» такая проверка находит последнюю «i» перед словом «степени» — и
 * третья степень засчитывалась за первую. В кабинете от этого горело чужое
 * достижение «Лауреат I» (у 274 человек), а в разбивке по званиям всё, где
 * есть слово «степени», падало в первую строку: вторая и третья всегда
 * показывали ноль. Дипломант с первой степенью тоже считался лауреатом.
 *
 * Отдаёт: 'gp' | 'laur1' | 'laur2' | 'laur3' | 'dipl1' | 'dipl2' | 'dipl3'
 *         | 'dipl' | 'member' | '' (нет звания).
 */
function result_rank(?string $result): string {
    $r = mb_strtolower(trim((string) $result));
    if ($r === '') return '';
    if (str_contains($r, 'гран')) return 'gp';

    $isLaur = str_starts_with($r, 'лауреат');
    $isDipl = str_starts_with($r, 'дипломант');
    if (!$isLaur && !$isDipl) return str_contains($r, 'участник') ? 'member' : '';

    // Степень — отдельным словом: римская или арабская. Границы слова
    // обязательны, иначе «III» снова прочтётся как «I».
    $deg = 0;
    if (preg_match('~(?:^|\s)(iii|ii|i|1|2|3)(?=\s|$)~u', $r, $m)) {
        $deg = ['i' => 1, '1' => 1, 'ii' => 2, '2' => 2, 'iii' => 3, '3' => 3][$m[1]] ?? 0;
    }
    if ($deg === 0) return $isLaur ? 'laur3' : 'dipl';
    return ($isLaur ? 'laur' : 'dipl') . $deg;
}
