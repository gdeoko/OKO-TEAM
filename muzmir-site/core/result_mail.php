...
function results_long_mail_send_batch(array $appIds, string $vkUrl = '', string $docxAbs = '', string $docxUrl = '', string $posterUrl = ''): bool {
    ...
    /* ОТМЕТКА ОБ ОТПРАВКЕ СТАВИТСЯ ВСЕЙ ПАЧКЕ.
     *
     * Письмо здесь одно на участника и содержит ВСЕ его заявки, а отметки не
     * получала ни одна: после оглашения итогов «Величия России» 718 заявок
     * числились неотправленными, хотя письма ушли. Из-за этого кабинет и
     * чат-бот отвечали «результат ещё не направлен», аудит поднимал ложную
     * тревогу, а повторная рассылка грозила задвоить письма участникам. */
    if ($queued) {
        $now = date('Y-m-d H:i:s');
        foreach ($apps as $__a) {
            try {
                update('applications', ['result_sent_at' => $now], 'id=:id', ['id' => (int) $__a['id']]);
            } catch (\Throwable $e) { /* отметка не критична для доставки письма */ }
        }
    }
    return $queued;
}
