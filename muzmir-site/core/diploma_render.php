...
/**
 * ПУТЬ ДЛЯ ХРАНЕНИЯ В РЕЕСТРЕ — ВЕБ-АДРЕС, А НЕ МЕСТО НА ДИСКЕ.
 *
 * Генератор возвращает абсолютный путь («/var/www/muzmir/public/diplomas/...»),
 * и он же уходил прямо в diplomas.pdf_path. В админке ссылка «файл» строится
 * от корня сайта — и по такой записи открывалось «не найдено»; так было у 242
 * наградных документов, и после ручной правки набежало ещё четыре, потому что
 * чинили данные, а не место, где они появляются.
 *
 * Каталог наградных документов лежит в public, поэтому веб-адрес получается
 * отбрасыванием пути до него. Чужой путь возвращаем как есть — молча портить
 * запись хуже, чем сохранить непривычную.
 */
function diploma_store_path(?string $abs): string {
    $abs = trim((string) $abs);
    if ($abs === '') return '';
    $root = rtrim(BASE_PATH . '/public', '/');
