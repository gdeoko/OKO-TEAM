...
function lm_render(string $number, array $o): array {
    ...
    // ИМЯ ФАЙЛА И НОМЕР ДОКУМЕНТА — РАЗНЫЕ ВЕЩИ.
    //
    // Имя строилось из номера, поэтому второй документ с тем же исходящим
    // (сопроводительное письмо) приходилось звать «03092026/002-soprovod» — и
    // этот служебный хвост печатался на бланке и уходил в QR проверки
    // подлинности, где такого номера нет. Файл различаем ключом 'file_stem',
    // номер на бланке остаётся настоящим.
    $stemSrc = trim((string) ($o['file_stem'] ?? '')) !== '' ? (string) $o['file_stem'] : $number;
    $stem = 'ol-' . preg_replace('~[^0-9a-zA-Z]~', '-', $stemSrc);
    $jpg  = $dir . '/' . $stem . '.jpg';
    $pdf  = '';
...
function lm_mail_institution(array $inst, string $number, array $comps, string $unsubUrl = ''): array {
    ...
    $inner .= lm_callout('<b>Педагогам-кураторам — бесплатно.</b> Преподаватель, подготовивший '
        ...
        . 'без оплаты</b>, отдельным наградным материалом на каждого ученика. Принимаются в '
    ...
    if ($instId > 0 && function_exists('partner_join_url')) {
        ...
        /* ВТОРОЙ ПУТЬ: ПРОСТО ОТВЕТИТЬ НА ПИСЬМО.
         *
         * За месяц рассылки на страницу подтверждения зашли двое: в школе почту
         * часто читают через ведомственный шлюз, где ссылки вырезаны или
         * заблокированы, а делопроизводитель привык отвечать письмом, а не жать
         * кнопки в письме. Ответ разбирается автоматически (cron/inbox_actions),
         * и учреждение принимается так же, как по ссылке. */
        $inner .= lm_p('Если ссылка не открывается — просто <b>ответьте на это письмо словом «СОГЛАСНЫ»</b>. '
            . 'Этого достаточно: мы оформим партнёрство и пришлём сертификат и доступ в кабинет.',
            'font-size:14px;color:#4a4a55;margin-top:-4px');
        if ($slug !== '') {
...
function lm_mail_institution_short(array $inst, string $number, array $comps, string $unsubUrl = ''): array {
    ...
    $inner .= lm_p('Исходящий №' . h($number) . ' от ' . h(function_exists('ru_date') ? ru_date(date('Y-m-d')) : date('d.m.Y'))
        . '. Положения конкурсов и образцы наградных материалов — на официальном сайте '
...
function lm_mail_thanks(array $inst, string $number, int $works = 0, array $teachers = []): array {
    ...
    if ($works > 0) {
        $inner .= lm_callout('Обучающиеся Вашего учреждения представили на аттестацию жюри <b>' . $works
            ...
            . '. Каждая получила профессиональную оценку и наградной материал.');
    }
...
/**
 * СОПРОВОДИТЕЛЬНОЕ ПИСЬМО ОТДЕЛЬНЫМ ФАЙЛОМ.
 *
 * Система электронного документооборота части ведомств устроена так, что
 * входящим считается сопроводительное письмо, а всё остальное — приложения к
 * нему. Обращение, пришедшее одно, там не регистрируют: «Документ к
 * рассмотрению не принимается – отсутствует сопроводительное письмо» —
 * так «Культура.РФ» вернула наше обращение дважды, 12.08 и 01.09.
 *
 * Документ короткий и по форме: кому, что направляем, перечень приложений с
 * числом листов, подпись. Ничего убеждающего в нём быть не должно — весь текст
 * просьбы лежит в самом обращении.
 *
 * @param array $r        строка ministries
 * @param string $number  исходящий номер обращения
 * @param array $attNames названия приложений (без самого обращения)
 * @return string путь к PDF или '' если собрать не удалось
 */
function lm_cover_pdf(array $r, string $number, array $attNames = []): string {
    $org  = (string) ($r['org'] ?? '');
    $role = (string) ($r['person_role'] ?? '');
    $fio  = (string) ($r['person'] ?? '');

    $items = array_merge(
        ['Обращение об информационной поддержке всероссийского творческого конкурса '
         . 'исх. №' . $number . ' — на 1 л. в 1 экз.'],
        array_map(static fn(string $a): string => rtrim($a, '.') . ' — в электронном виде.', $attNames)
    );

    $body = '<p>Культурный центр «Музыкальный Мир» направляет в Ваш адрес обращение '
          . 'об информационной поддержке всероссийских творческих конкурсов для детей и молодёжи '
          . 'исх. №' . h($number) . ' от ' . h(date('d.m.Y')) . '.</p>'
          . '<p>Участие в конкурсах бесплатное. Просим рассмотреть обращение по существу '
          . 'и направить ответ на фирменном бланке ведомства.</p>';

    try {
        $doc = lm_render($number, [
            'file_stem'   => $number . '-soprovod',   // отдельный файл, номер тот же
            'kind'        => 'support',
            'plain'       => true,
            'no_approve'  => true,               // сопроводительное ничего не утверждает
...
function lm_addressee(string $role, string $org, string $fio): array {
    ...
    if (count($parts) >= 2) {
        ...
        // Инициалы ПЕРЕД фамилией — как и написано в правиле выше и как в примере
        // ГОСТа («В. А. Лагунину»). Код печатал их после фамилии, вопреки
        // собственному комментарию.
        $ini = mb_substr($parts[1], 0, 1) . '.';
        if (isset($parts[2])) $ini .= ' ' . mb_substr($parts[2], 0, 1) . '.';
        $out[] = $ini . ' ' . ol_surname_dative($parts[0], $fio);
...
function ol_surname_dative(string $surname, string $fio = ''): string {
    ...
    // Прилагательные фамилии: «Горький» → «Горькому», «Толстой» → «Толстому».
    if (preg_match('~(ий|ый)$~u', $s)) return preg_replace('~(ий|ый)$~u', 'ому', $s);
    // «-ой» по этому же правилу склоняется только у длинных фамилий. У коротких
    // «-ой» не окончание прилагательного, а часть корня, и правило их калечит:
    // председателю комитета Ленинградской области ушло обращение «Цой Владимир
    // Олегович» (а до правки реквизита вышло бы «Цому»), и ведомство вернуло
    // документ с просьбой исправить фамилию. Верно — «Цою Владимиру Олеговичу».
    if (preg_match('~ой$~u', $s)) {
        return mb_strlen($s) >= 6 ? preg_replace('~ой$~u', 'ому', $s) : mb_substr($s, 0, -1) . 'ю';
    }
    // Прочие фамилии на «-й»: «Гайдай» → «Гайдаю», «Бабий» уже ушёл выше.
    if (preg_match('~й$~u', $s)) return mb_substr($s, 0, -1) . 'ю';
