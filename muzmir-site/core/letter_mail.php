...
function lm_mail_institution(array $inst, string $number, array $comps, string $unsubUrl = ''): array {
    ...
    if ($instId > 0 && function_exists('partner_join_url')) {
        ...
        if ($slug !== '') {
            ...
            $inner .= lm_p('Для вашего учреждения уже подготовлены номер в реестре партнёров'
                . ($no !== '' ? ' <b>' . h($no) . '</b>' : '')
                . ' и постоянная ссылка для участников: <a href="' . h($link) . '" style="color:#8B6F1F">'
                . h($link) . '</a>. Заявки, поданные по ней, засчитываются учреждению. Ссылка начнёт '
                . 'работать сразу после подтверждения партнёрства.',
