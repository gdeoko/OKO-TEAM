...
function ag_prompt(array $app, array $rubric, array $pitch = [], bool $isImage = false): string {
    ...
    $L[] = 'ВОЗРАСТ В КАДРЕ. Заявленную возрастную категорию с виду не проверить: сорокашестилетнюю женщину легко принять за подростка при плохом свете, а рослого школьника — за взрослого. Если возраст в кадре расходится с заявкой, НЕ считай это нарушением и НЕ предлагай отказ: напиши сомнение в internal_note, решать будет человек. Ошибка здесь снимает с конкурса живого участника.';
    $L[] = 'МОНТАЖ И СКЛЕЙКА — НЕ ОСНОВАНИЕ ДЛЯ ОТКАЗА. Титр в начале, обрезанные края, переход между планами есть почти в каждой записи со школьного концерта. Отметь монтаж в formal, если он есть, но отказ из этого не следует: центр по этому пункту работы не снимает.';
...
function ag_parse_json(string $raw): ?array {
    ...
    /* ОБОРВАННЫЙ ОТВЕТ ДОЧИТЫВАЕМ, А НЕ ВЫБРАСЫВАЕМ.
     *
     * Мост отдаёт ответ модели из браузера, и он приходит обрезанным на
     * середине: последний критерий не дописан, скобки не закрыты. json_decode
     * такое не берёт, и работа объявлялась неоценённой — хотя разбор по всем
     * основным критериям в ответе уже есть. 02.09 так потерялись пять работ:
     * запись скачана, модель отработала, а результат ушёл в мусор.
     *
     * Здесь мы отрезаем незавершённый хвост до последней целой пары
     * «ключ: значение» и закрываем скобки, которые модель не успела закрыть.
     * Дальше вызывающий код всё равно проверяет, что критерии на месте
     * (empty($out['criteria'])), и считает балл только по заполненным — так что
     * половинчатый разбор не превратится в заниженную оценку: если критериев
     * не хватит, работа честно уйдёт человеку. */
    if ($a !== false) {
        $frag = mb_substr($t, $a);
        // Обрубаем по последней закрытой скобке значения — там заведомо
        // законченная структура, а всё после неё модель не дописала.
        $cut = max((int) mb_strrpos($frag, '}'), (int) mb_strrpos($frag, ']'));
        if ($cut > 0) {
            $frag = mb_substr($frag, 0, $cut + 1);
            $depth = 0; $inStr = false; $esc = false; $stack = [];
            $len = mb_strlen($frag);
            for ($i = 0; $i < $len; $i++) {
                $ch = mb_substr($frag, $i, 1);
            ...
            if ($inStr) $frag .= '"';
            $frag = rtrim(rtrim($frag), ',');
            while ($stack) { $frag .= array_pop($stack) === '{' ? '}' : ']'; }
            $j = json_decode($frag, true);
            if (is_array($j)) {
                error_log('ag_parse_json: ответ модели пришёл оборванным, дочитан по последней целой записи');
                return $j;
            }
        }
    }
...
function ag_grade_application(int $appId, array $opt = []): array {
    ...
    if ($onBridge) {
        ...
        if (!$prep['ok']) {
            ...
            $closed = (bool) preg_match('~закрыт|удален|удалён|не отдаётся|приватн|недоступ~ui', $why)
                   && !preg_match('~файл на месте~ui', $why);
            if ($closed) {
    ...
    if (!$dl['ok']) {
         ...
         *
         * «Запись закрыта настройками приватности», «файл по ссылке не
         * отдаётся» — по положению это п. 8.10: ссылка должна быть доступна
         * вплоть до оглашения результатов. Девять работ последнего прогона
         * получили такой ответ и легли в список технических неудач, где жюри их
         * не увидит, — а причина у них готовая и участнику понятная.
         *
         * Прочие неудачи (оборванная закачка, нехватка места, сбой ffmpeg) сюда
         * не попадают: там виноваты мы, и отказывать участнику не за что. */
        $why = (string) $dl['why'];
        $closed = (bool) preg_match('~закрыт|удален|удалён|не отдаётся|приватн|недоступ~ui', $why);
        // «Файл на месте, но диск не выдаёт ссылку» — наша беда, не участника.
        if ($closed && !preg_match('~оборвал|не полностью|повреждённ|файл на месте~ui', $why)) {
            $reason = "Ссылка на конкурсный материал недействительна: она должна быть актуальна "
                    . "вплоть до оглашения результатов (п. 8.10 положения).\n" . $why;
            try {
                q("UPDATE grading_runs SET status='ok', title='ТРЕБУЕТ ПРОВЕРКИ', total=0,
                  ...
                  [mb_substr($reason, 0, 900),
                   'Оценка не проводилась: запись по ссылке недоступна. ' . $why, $runId]);
            } catch (\Throwable $e) {}
            return ['ok' => true, 'run_id' => $runId, 'total' => 0.0,
                    'title' => 'ТРЕБУЕТ ПРОВЕРКИ', 'why' => $reason];
        }
    ...
    if (ag_transport() === 'bridge') {
        ...
        if ($br['ok'] && ag_parse_json($br['text'])) {
            ...
            // Длина ответа в логе обязательна: по первым тремстам знакам не
            // отличить «модель ответила мусором» от «ответ обрезан на середине»,
            // а лечится это по-разному.
            error_log('ai_grade(' . $appId . '): мост не дал разбор ('
                . ($br['why'] ?: 'ответ не разобран') . ', длина ответа '
                . mb_strlen((string) ($br['text'] ?? '')) . ' знаков) — иду через ключ. Ответ моста: '
    ...
    /* НЕ ТО ПРОИЗВЕДЕНИЕ — МОЖЕТ БЫТЬ НЕ ТОТ ФАЙЛ.
     *
     * В папке #1632 лежат две безымянные съёмки с телефона, и «У лукоморья дуб
     * зелёный» оказалось не в той, что выбрана по времени: разбор посмотрел
     * вторую, увидел русскую народную сказку и предложил снять заявку за
     * несоответствие. Участник тут ни при чём — работу он прислал.
     *
     * Поэтому прежде чем записывать отказ по номинации или репертуару,
     * пробуем остальные файлы папки: их немного, а цена ошибки — снятый с
     * конкурса человек. */
    $others = (array) ($GLOBALS['vf_pick_others'] ?? []);
    $mismatch = isset($formal['nomination']) && $formal['nomination'] === false;
    if ($mismatch && $others && (int) ($opt['__try'] ?? 0) < 3) {
        $next = array_shift($others);
        $GLOBALS['vf_pick_others'] = $others;
        $GLOBALS['vf_pick_force']  = (string) ($next['name'] ?? '');
        if ($GLOBALS['vf_pick_force'] !== '') {
            error_log('ai_grade(' . $appId . '): в папке несколько файлов, разбор не нашёл заявленного — '
                    . 'пробую «' . $GLOBALS['vf_pick_force'] . '»');
            return ag_grade_application($appId, ['__try' => (int) ($opt['__try'] ?? 0) + 1] + $opt);
        }
    }
    ...
    if ($formalFail) {
        try {
            ...
            if (!$doReject && $reason === '') {
                /* МОНТАЖ И НАЛОЖЕННЫЙ ВОКАЛ В ПРИЧИНУ НЕ ИДУТ.
                 *
                 * Оба — п. 8.5, а по нему центр работы не снимает. Наложенный
                 * студийный вокал вдобавок означает не отказ, а фонограмму:
                 * по п. 8.7 звание опускается до дипломанта, и участник
                 * получает диплом, а не снятие с конкурса. Заявка #1873 была
                 * помечена к отказу именно так — «нет наложенного студийного
                 * вокала поверх живого исполнения». */
                $skip = ['integrity' => 1, 'no_overdub' => 1];
                $names = [];
...
function ag_auto_reject(array $run, array $comp = []): array {
    ...
    /* МОНТАЖ И СКЛЕЙКА — НЕ ОСНОВАНИЕ ДЛЯ ОТКАЗА.
     *
     * Решение владельца (август 2026), дословно: «за монтаж склейку не
     * отклонять, хватит, это неправильно». Пункт 8.5 в положении есть, но на
     * практике центр по нему работы не снимает: почти всякая запись со
     * школьного концерта смонтирована — вступительный титр, обрезанные края,
     * переход между планами. Три работы последнего прогона были сняты именно
     * так, и все три сняты зря.
     *
     * Ключи integrity и no_overdub остаются в разборе: жюри полезно знать, что
     * запись собрана из кусков или что поверх живого голоса лежит студийный.
     * Но отказ из этого больше не следует. */
    $map = [
        'one_piece'  => 'odna',
        'nomination' => 'п. 4.2',
        'playable'   => 'video',
