 ...
 * жюри считала срок отправки результата, переделывала наградные материалы под
 ...
 *     Элитного клуба — через 3; наградные материалы платного конкурса — тем же
...
function grade_apply_result(int $appId, string $result, array $opt = []): array {
    ...
    // Наградные материалы приводятся в соответствие новому званию: бланк
    ...
    if (!$isLong && ($firstGrade || $changed) && $dueNow) {
        ...
        if (!empty($cur['user_id']) && function_exists('notify_user')) {
            $tail = $compPaid
                ? 'Наградные материалы придут на почту из заявки в течение ' . $wdays . ' рабочих дней.'
 ...
 * наградные материалы, вернуть учреждению партнёрскую скидку, вернуть оргвзнос
...
function grade_reject_application(int $appId, string $reason, string $source = 'jury'): array {
    ...
    // Наградные материалы отклонённой работы снимаются целиком: файлы, записи
