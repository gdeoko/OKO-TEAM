...
function inbox_strip_quote(string $body): string {
    ...
    $marks = ['~(?:^|\s)От:\s~u', '~(?:^|\s)From:\s~u',
              ...
              // ЦИТАТА МОБИЛЬНОЙ ПОЧТЫ. Яндекс и Mail на телефоне подписывают ответ
              // строкой «Отправлено из мобильной Яндекс Почты» и сразу за ней
              // цитируют наше письмо целиком — без «От:» и без «>». Без этого
              // маркера разбор читал наш собственный подвал со ссылкой
              // «Отписаться от рассылки» как просьбу участницы отписаться: живой
              // человек, оплативший два конкурса, попал в стоп-лист, и его дипломы
              // перестали отправляться вообще.
              '~--\s*Отправлено из мобильн\w*~u',
              '~\d{2}\.\d{2}\.\d{4},?\s*\d{1,2}:\d{2},?\s*"~u'];
    $cut = mb_strlen($body);
    foreach ($marks as $re) {
...
function inbox_classify(string $mailbox, string $subject, string $body, bool $isAuto): string {
    ...
    if (preg_match('~(удалит\w+|убер\w+|уберите|исключит\w+|снимит\w+)\s+'
                 ...
                 // «Отписаться от рассылки» — это текст НАШЕЙ ссылки в подвале
                 // каждого письма, а не просьба человека. Просьбу пишут в
                 // повелительном наклонении: «отпишите», «удалите адрес».
                 . '|удалите у себя|отпишите( нас| меня)?'
    ...
    // «ПИСЬМА НА ПОЧТУ НЕ РАССМАТРИВАЕМ» — ЭТО НЕ ПОДДЕРЖКА И НЕ ОТКАЗ.
    //
    // После 547-ФЗ ведомства всё чаще отвечают, что обращение в электронном виде
    // принимается только через их интернет-приёмную или ЕСИА. В таком письме
    // почти всегда есть слова «для направления обращения... воспользуйтесь», и
    // правило про «информация направлена» ловило их как согласие: Управление
    // культуры Казани (01.09) и Минкультуры Владимирской области (02.09) получили
    // от центра благодарность за поддержку, которой не было. Правило стоит выше
    // разбора дел ведомства — как и в mrep_classify_rules(), где тот же случай
    // разобран верно; два классификатора обязаны отвечать одинаково.
    if (preg_match('~(интернет|электронн\w*)[ -]?приемн|(интернет|электронн\w*)[ -]?приёмн'
                 . '|в форме электронного документа'
                 . '|есиа|единой системы идентификации'
                 . '|обращени\w*[^.!?]{0,80}не рассматрива'
                 . '|не рассматрива\w*[^.!?]{0,80}(электронн\w* почт|почте)'
                 . '|547-фз|547 - фз~ui', $t)) {
        return $mailbox === 'kc' ? 'ministry_eform' : 'question';
    }
