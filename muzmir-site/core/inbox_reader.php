...
function inbox_classify(string $mailbox, string $subject, string $body, bool $isAuto): string {
    ...
    if (preg_match('~недоставлен|undelivered|delivery status|mail delivery failed|returned to sender~ui', $t)) return 'bounce';

    // Согласие на партнёрство — только явные обороты.
    $yes = '~(подтвержда\w+ соглас|выража\w+ соглас|соглас(ны|ен|на|ie)|принима\w+ (предложени|услови|партнёрств|партнерств)'
         . '|готовы (стать|принять участие|сотруднича)|заинтересован\w* в (сотрудничеств|партнёрств|партнерств)'
         . '|просим (присвоить|оформить) статус|хотим стать (информационным )?партн)~ui';
    ...
    $no  = '~(отказыва\w+|не заинтересован\w*|не планиру\w+ участ|участвовать не будем|не намерен\w* участ'
         ...
         . '|не поддержива\w+|не находим оснований|не имеется оснований|не считаем возможным)~ui';
