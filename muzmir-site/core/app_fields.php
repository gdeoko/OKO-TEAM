...
/**
 * КЛЮЧИ ДЛЯ СВЕРКИ ПОВТОРНОЙ ЗАЯВКИ.
 *
 * Внутри конкурса одно выступление участвует один раз. Ловить повтор сравнением
 * строк напрямую бесполезно: люди пишут одно и то же по-разному, и в августе
 * так прошли две заявки на один танец —
 *
 *   «Хореографическая группа «Виктория»» · ««Лялечка»»
 *   «Хореографический коллектив «Виктория»» · «танец «Лялечка»»
 *
 * — ссылка одна, участник один, номер один, а строки разные. Поэтому сравниваем
 * не текст, а смысл: у коллектива берём собственное имя (то, что в кавычках),
 * отбрасывая тип — «ансамбль», «студия», «группа»; у номера отбрасываем
 * жанровую приставку «танец», «песня», «композиция» и знаки препинания.
 *
 * Ключ пустой — сверять нечего, повтор не объявляем.
 */
function app_dup_person_key(string $v): string {
    $s = mb_strtolower(trim(preg_replace('~\s+~u', ' ', $v) ?? $v));
    $s = str_replace('ё', 'е', $s);
    // Собственное имя коллектива стоит в кавычках — по нему и сверяем.
    if (preg_match('~[«"“]([^»"”]{2,})[»"”]~u', $s, $m)) return trim($m[1]);
    // Кавычек нет: снимаем тип и определения, оставляя имя.
    $s = preg_replace('~\b(образцов\w*|народн\w*|заслуженн\w*|детск\w*|юношеск\w*|эстрадн\w*|'
        . 'вокальн\w*|хоров\w*|хореографическ\w*|танцевальн\w*|фольклорн\w*|инструментальн\w*|'
        . 'ансамбл\w*|коллектив\w*|студи\w*|групп\w*|театр\w*|хор|дуэт|трио|квартет|квинтет|'
        . 'оркестр\w*|шоу|клуб\w*)\b~u', ' ', $s) ?? $s;
    $s = preg_replace('~[^\p{L}\p{N}\s-]+~u', ' ', $s) ?? $s;
    return trim(preg_replace('~\s+~u', ' ', $s) ?? $s);
}

/** Ключ конкурсного номера: без кавычек, знаков и жанровой приставки. */
function app_dup_work_key(string $v): string {
    $s = mb_strtolower(trim(preg_replace('~\s+~u', ' ', $v) ?? $v));
    $s = str_replace('ё', 'е', $s);
    $s = preg_replace('~[^\p{L}\p{N}\s-]+~u', ' ', $s) ?? $s;              // кавычки, точки, запятые
    // Жанровое слово в начале названия ничего не различает: «танец «Лялечка»».
    $s = preg_replace('~^\s*(танец|танго|вальс|песня|песнь|романс|ария|этюд|пьеса|сюита|попурри|'
        . 'композиция|произведение|номер|выступление|стихотворение|стих|чтение|проза|басня|'
        . 'вокал|соло|дуэт|рисунок|работа|картина|фото|фотография)\s+~u', ' ', $s) ?? $s;
    return trim(preg_replace('~\s+~u', ' ', $s) ?? $s);
}
