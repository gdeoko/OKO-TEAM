 ...
 *
 * РЕШЕНИЕ ВЛАДЕЛЬЦА: по России адрес записывается в виде справочника, а не так,
 * как его набрал человек. Одна и та же квартира перестаёт появляться в базе в
 * трёх написаниях, и посылка на неё собирается одна.
 *
 * Приводим только то, что сервис подтвердил ДО ДОМА и признал российским: ФИАС
 * знает Россию, а Минск и Алматы — нет, и там остаётся написанное человеком.
 * Квартиру дописываем сами, если сервис опустил её в ответе (иначе посылка
 * доедет до подъезда и останется у соседей), индекс ставим первым — так его ждёт
 * Почта России.
 *
 * Плюс у заказа есть отдельный ключ addr_key для сборки посылок: канонический
 * вид, приведённый к набору значимых слов. Сервис недоступен или ключа нет —
 * ключ считается по самой строке, как раньше, и хуже прежнего не станет.
 */
...
if (!function_exists('addr_validate')) {
     ...
     *
     * @return array{ok:bool, value:string, postal:string, reason:string, ru?:bool}
     */
    function addr_validate(string $addr): array {
        ...
        $flat    = trim((string) ($d['flat'] ?? ''));
        $flatT   = trim((string) ($d['flat_type'] ?? 'кв'));
        // Квартиру сервис включает в value не всегда — дописываем, иначе посылка
        // приедет в подъезд и останется у соседей.
        if ($flat !== '' && mb_stripos($value, $flat) === false) {
            $value .= ', ' . ($flatT !== '' ? $flatT . ' ' : '') . $flat;
        }
        // Индекс первым — так его ждёт Почта России на конверте и в описи.
        if ($postal !== '' && mb_strpos($value, $postal) === false) {
            $value = $postal . ', ' . $value;
        }
        ...
        return ['ok' => true, 'value' => $value, 'postal' => $postal, 'reason' => '',
                // Российский адрес приводим к виду справочника, зарубежный оставляем
                // как написал человек: ФИАС знает только Россию.
                'ru' => ($country === '' || mb_stripos($country, 'росс') !== false)];
    }
}
