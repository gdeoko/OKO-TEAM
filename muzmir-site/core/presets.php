<?php
/**
 * Общие справочники итогов конкурса — используются и «Оцениванием» (admin/grading.php),
 * и «Длинными конкурсами» (admin/longcomp.php). Вынесены отдельно, чтобы длинный раздел
 * не включал grading.php целиком (у той есть исполняемый вывод страницы).
 */
declare(strict_types=1);

if (!function_exists('RESULT_PRESETS')) {
    /** Заготовки итоговых званий (порядок = порядок кнопок). */
    function RESULT_PRESETS(): array {
        return ['ГРАН-ПРИ','ЛАУРЕАТ I СТЕПЕНИ','ЛАУРЕАТ II СТЕПЕНИ','ЛАУРЕАТ III СТЕПЕНИ',
                'ДИПЛОМАНТ I СТЕПЕНИ','ДИПЛОМАНТ II СТЕПЕНИ','ДИПЛОМАНТ III СТЕПЕНИ','УЧАСТНИК КОНКУРСА'];
    }
}
/**
 * ФОРМУЛИРОВКА ПРО ФОНОГРАММУ — ОДНА НА ВСЕ МЕСТА, СЛОВО В СЛОВО.
 *
 * Текст утверждён владельцем и пишется без изменений: у галочки в разделе
 * оценки, в сообщении после сохранения итога и первой строкой в комментарии
 * жюри, который участник читает в письме с результатом и в кабинете. Раньше в
 * каждом из этих мест была своя редакция — жюри отмечало одно, участник получал
 * другое. Ничего не дописывать и не сокращать: ни пояснений про минусовку, ни
 * «согласно номинации», ни точки в конце.
 */
const PHONOGRAM_NOTE = 'Оценка снижена до Дипломанта за использование голосовой фонограммы (Пункт положения конкурса 8.7)';

if (!function_exists('EXTRA_PRESETS')) {
    /** Заготовки дополнительных дипломов. */
    function EXTRA_PRESETS(): array {
        return ['ЗА АРТИСТИЗМ','ЗА ПАТРИОТИЗМ','ЗА ОРИГИНАЛЬНОЕ ИСПОЛНЕНИЕ','ЗА ВЫРАЗИТЕЛЬНОСТЬ','ЗА ВИРТУОЗНОЕ ИСПОЛНЕНИЕ'];
    }
}

/**
 * ОСНОВАНИЯ ДЛЯ ОТКЛОНЕНИЯ ЗАЯВКИ — ДОСЛОВНО ПО ПУНКТАМ ПОЛОЖЕНИЯ.
 *
 * Каждый отказ ссылается на конкретный пункт, и участник должен найти этот
 * пункт в своём положении слово в слово. Пока разделы «Требования к конкурсному
 * материалу» у платного и бесплатного конкурсов различались, список приходилось
 * держать двойным: один и тот же отказ ссылался то на 8.8, то на 8.7, а часть
 * оснований для бесплатного конкурса вовсе не существовала. Участник, оспоривший
 * такой отказ, был бы совершенно прав.
 *
 * Теперь раздел 8 одинаков во всех положениях, и список один.
 *
 * Ещё две формулировки пришлось выправить по тексту эталонов:
 *   - качество видео: в положении сказано «должно быть хорошего качества», и
 *     никакого порога в 480 пикселей там нет — цифра была выдумана;
 *   - фонограмма: по положению за неё оценка СНИЖАЕТСЯ до дипломанта, а не
 *     отклоняется заявка, поэтому основанием для отказа она быть не может и из
 *     списка убрана.
 *
 * @param array $comp строка конкурса; смотрим is_paid — по нему выбирается эталон
 */
function REJECT_REASONS(array $comp = []): array {
    /* ТРЕБОВАНИЯ К МАТЕРИАЛУ ОДИНАКОВЫ ВО ВСЕХ КОНКУРСАХ.
     *
     * Раздел 8 платного и бесплатного положений разъезжался: съёмка, монтаж и
     * качество стояли под разными номерами, а пунктов про видимость исполнителя
     * и про фонограмму в бесплатном не было вовсе. Отсюда и список отказов был
     * двойной, с оговорками «в этом положении такого пункта нет».
     *
     * Раздел приведён к общему виду (scripts/regulation_unify8.php), поэтому
     * нумерация теперь одна на все конкурсы: один и тот же отказ ссылается на
     * один и тот же пункт, что бы участник ни открыл. Тематика конкурса (8.0)
     * у конкурсов по-прежнему своя, но основанием для отказа она не бывает. */
    $p = ['odna' => '8.1', 'semka' => '8.2', 'vidno' => '8.3', 'montazh' => '8.5',
          'video' => '8.6', 'audio' => '8.7.1', 'resurs' => '8.8', 'zapret' => '8.9',
          'ssylka' => '8.10', 'god' => '8.11'];

    $t = static fn(string $num, string $text): array => ['п. ' . $num, $text . ' (п. ' . $num . ' положения).'];

    $list = [];
    foreach ([
        [$p['god'],     'Конкурсный материал старше 1 года с момента исполнения'],
        [$p['ssylka'],  'Ссылка на конкурсный материал недействительна: она должна быть актуальна вплоть до оглашения результатов'],
        [$p['resurs'],  'Ссылка не на разрешённый интернет-ресурс. Принимаются только: RuTube, Google Диск, Яндекс Диск, ОК видео, ВК видео, Дзен видео'],
        [$p['zapret'],  'Материал размещён на запрещённом интернет-ресурсе (Instagram, Facebook — принадлежат Meta, признанной в России экстремистской, TikTok, YouTube)'],
        [$p['montazh'], 'Видеоматериал содержит элементы монтажа, склейки, стоп-кадры или наложение видеоэффектов и аудиодорожек'],
        [$p['video'],   'Видео- или фотоматериал конкурсного номера ненадлежащего качества'],
        [$p['audio'],   'Ненадлежащее качество звука: аудиотрек, музыкальное сопровождение, аккомпанемент или вокал плохо прослушиваются'],
        [$p['vidno'],   'В конкурсном номере чётко не видны руки, ноги и лицо исполнителя (в соответствии с номинацией)'],
        [$p['semka'],   'Видеозапись не соответствует требованиям к съёмке: сцена, импровизированная сцена или домашние условия, статичная камера'],
        [$p['odna'],    'Несколько конкурсных материалов поданы одной заявкой: одна заявка — один конкурсный материал'],
    ] as [$num, $text]) {
        if ($num === null) continue;                 // пункта нет в этом положении
        [$k, $v] = $t($num, $text);
        $list[$k] = $v;
    }

    // Общие основания — нумерация у обоих эталонов одинаковая.
    $list['п. 4.2'] = 'Заявка на участие заполнена неверно (п. 4.2 положения).';
    $list['п. 4.1'] = 'Невыполнение требований, условий или правил, прописанных в положении конкурса (п. 4.1 положения).';
    $list['Пункт №1'] = 'Материал не допускается к конкурсу: нецензурная брань, сцены насилия или призыва к насилию, '
                      . 'материалы экстремистского толка, порнография, реклама табака, алкоголя и наркотиков, а также '
                      . 'иные материалы, нарушающие законодательство Российской Федерации (Пункт №1 положения).';
    $list['критерии'] = 'Отсутствие самостоятельности исполнения: срисовывание или плагиат (критерий «оригинальность замысла»).';

    return $list;
}
