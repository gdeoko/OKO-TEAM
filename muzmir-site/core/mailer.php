 ...
 *   awards — наградные материалы и заказы: сначала наградный ящик, далее любые
...
function mail_reply_box(string $pool): string {
    ...
    $box = match ($pool) {
        ...
        'awards'   => 'nagradi.on',  // наградные материалы и заказы
...
function mail_fallback_accounts(array $primary = [], string $pool = 'tx', string $to = ''): array {
     ...
     * письма: код для входа, пароль от кабинета, наградной материал. Человек
...
function mail_send_failover(string $to, string $subject, string $html, array $opt = []): bool {
    ...
    foreach ($accounts as $i => $acc) {
         ...
         * наградной материал. Человек его ждёт, и отказ сервиса про него ничего
