...
function mail_pool_names(string $pool): array {
    // Рассылочные ящики (news, news2) НЕ участвуют в личных письмах и наградах:
    // их адреса не публикуются в контактах, и человек не должен получать
    // диплом или счёт с ящика, на который потом некуда ответить.
    return match ($pool) {
