...
function mail_pool_names(string $pool): array {
    ...
    return match ($pool) {
        ...
        // ТЕ ЖЕ ДВА ЯЩИКА, НО НАПРЯМУЮ, МИМО СЕРВИСА РАССЫЛОК.
        //
        // Часть почтовых служб режет всё, что пришло через рассылочный сервис:
        // ведомственные шлюзы отвечают «blocked due to security reason», Яндекс
        // принимает, но кладёт в «Спам». Письмо с нашего же домена напрямую они
        // берут. Ящик при этом ТОТ ЖЕ, что и в обычном канале: своей базе —
        // news@, учреждениям — novosti@. Официальная почта центра kc@ в массовых
        // рассылках не участвует ни при каких условиях (правило владельца):
        // на ней заявки, результаты, сайт и переписка с ведомствами.
        'bulk_smtp' => ['news'],                           // своя база напрямую
        'cold_smtp' => ['news2'],                          // учреждения напрямую (novosti@)
