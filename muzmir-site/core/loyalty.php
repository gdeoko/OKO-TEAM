 ...
 *   • Суммарная скидка на аккаунт БЕЗ Элитного клуба — не больше 10%.
...
/** Суммарный потолок скидки на аккаунт без Элитного клуба, %. */
...
function app_discount_reasons(array $info, int $pct): array {
    ...
    if ($club > 0) $out[] = 'Скидка ' . $club . '% — участник Элитного клуба';
    ...
    // Партнёрская скидка идёт отдельным ключом и вместо остальных. Без этой ветки
    // карточка заявки писала «детали не сохранены» там, где причина как раз
    // известна: заявку привело учреждение-партнёр по своему коду.
    $prt = (int) ($info['partner_pct'] ?? 0);
    if ($prt > 0) {
        $code = trim((string) ($info['partner_promo'] ?? $info['promo_code'] ?? ''));
        $out[] = 'Скидка ' . $prt . '% — промокод учреждения-партнёра'
               . ($code !== '' ? ' ' . $code : '');
    }
