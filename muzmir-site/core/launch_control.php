 ...
 *   • рассылки: сколько ушло сегодня и всего по каждому типу (конкурсы/Элитный клуб/кабинет),
 ...
 *
 * Числа в подписи не зашиваем: воскресную волну расписание сдвигает (предупреждения
 * на субботу, закрытие и итоги на понедельник), и подпись «25-е, 09:00» рядом с
 * заданием, стоящим на 24-е, заставляет оператора верить не тому, что произойдёт.
 * Фактическую дату показывает сам пульт рядом с названием (admin/launch.php).
 */
function launch_wave_title(string $wave): string {
    return [
        ...
        'campaign_vip'     => 'Письмо по базе: Элитный клуб',
        'campaign_kabinet' => 'Письмо по базе: личный кабинет',
        'launch'           => 'Открытие · приём заявок',
        'd3'               => 'Осталось 3 дня до конца приёма',
        'last'             => 'Последний день приёма',
        'closed'           => 'Приём закрыт',
        'results'          => 'Результаты длинного конкурса',
    ][$wave] ?? $wave;
}
...
function launch_wave_note(string $wave): string {
    return [
        ...
        'campaign_vip'     => 'Приглашение в Элитный клуб по всей базе (кроме действующих членов клуба).',
...
function launch_mail_stats(): array {
    ...
    $out = [];
    foreach (['konkurs' => 'Новые конкурсы', 'vip' => 'Элитный клуб', 'kabinet' => 'Личный кабинет'] as $t => $lbl) {
        $quota = (int) ($split[$t] ?? 0);
