 ...
 * в базе она хранится уже в нём. Сверка же приносит должность как она написана
 * на сайте ведомства — в именительном.
 *
 * Склоняется ТОЛЬКО первое слово: остальное («культуры Республики Карелия»)
 * и так стоит в родительном и меняться не должно.
 */
function min_role_dative(string $role): string {
    ...
    if (preg_match('~(у|ю)$~u', $lower) && !preg_match('~(бюро|депо)$~u', $lower)) return $role;

    // «Врио» и «И.о.» не склоняются — за ними идёт уже родительный падеж.
    if (preg_match('~^(врио|и\.?о\.?|исполняющий)$~u', $lower)) return $role;

    $last = mb_substr($head, -1);
    if ($last === 'ь' || $last === 'й') {
        // Председатель → Председателю, Руководитель → Руководителю.
        $head = mb_substr($head, 0, -1) . 'ю';
    } elseif ($last === 'а') {
        // Глава → Главе.
        $head = mb_substr($head, 0, -1) . 'е';
    } elseif ($last === 'я') {
        $head = mb_substr($head, 0, -1) . 'е';
    } elseif (preg_match('~[бвгджзклмнпрстфхцчшщ]$~u', $lower)) {
        // Министр → Министру, Директор → Директору, Начальник → Начальнику.
        $head .= 'у';
    } else {
        // Незнакомая форма — оставляем как есть, лучше именительный, чем выдумка.
        return $role;
    }
    $parts[0] = $head;
