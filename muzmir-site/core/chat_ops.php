...
/** Общий рубильник бота: настройка chat_bot_enabled ('1' — отвечает, '0' — молчит). */
function chat_bot_enabled(): bool {
    return (string) setting('chat_bot_enabled', '1') === '1';
}

/**
 * Бот заглушён для диалога: выключен целиком, заблокирован, выключен вручную
 * или активен перехват оператора. Возвращает причину
 * ('off'|'blocked'|'disabled'|'operator') или '' если бот отвечает.
 *
 * ОБЩИЙ РУБИЛЬНИК — ПЕРВОЙ СТРОКОЙ.
 *
 * Выключить бота по одному диалогу можно было и раньше, а разом — нет: в дни,
 * когда центр оглашает итоги, ответ машины дороже молчания. Человек спросит про
 * своё звание, бот назовёт не то или не так, и разбираться будет уже оператор.
 * Проверка стоит здесь, потому что через chat_bot_muted проходят все три входа
 * (чат сайта — текстом и вложением, сообщения сообщества ВКонтакте) и ночной
 * дослов: одно место — и молчат все каналы сразу.
 *
 * Молчание не значит потерю: сообщение человека сохраняется в chat_messages до
 * этой проверки, диалог виден в админке, оператор отвечает руками. Включить
 * обратно — chat_bot_enabled=1.
 */
function chat_bot_muted(string $sessionKey): string {
    if (!chat_bot_enabled()) return 'off';
    $d = chat_dialog_get($sessionKey);
...
/**
 * ОТКЛАДЫВАЕМ ЛИ ВОПРОС ДО УТРА.
 *
 * Раньше ответ был один: не рабочее время — шаблон «мы отвечаем Пн–Пт…», а
 * разговор до девяти утра. Для живого оператора это правда: он спит. Но бот
 * не спит, и участник, спросивший в девять вечера «дипломы только платные?»,
 * получал отписку вместо ответа, который машина знает наизусть. Так и выглядит
 * «бот перестал отвечать»: он отвечал — только не по делу.
 *
 * Теперь по умолчанию бот отвечает круглосуточно, а про график говорит там,
 * где это важно: если вопрос требует человека (деньги, жалоба), он честно
 * предупреждает, что оператор подключится в рабочее время — это делает
 * chat_maybe_escalate, а не общий шаблон на каждое сообщение.
 *
 * Вернуть прежнее поведение — chat_answer_offhours=0.
 */
function chat_offhours_hold(): bool {
    if ((string) setting('chat_answer_offhours', '1') === '1') return false;
    return !chat_is_working_hours();
}
