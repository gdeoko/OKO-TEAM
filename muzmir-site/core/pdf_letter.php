...
function pdf_official_letter(array $o): string {
    ...
    //
    // У одного исходящего номера бывает ДВА документа: обращение и
    // сопроводительное письмо к нему. Различает их это же имя: пока оба звались
    // «obrashchenie-<номер>», второй писался поверх первого, и в письме уходил
    // один документ дважды. Имя видит получатель, поэтому оно и должно говорить,
    // что внутри.
    $kind = (string) ($o['kind'] ?? '');
    $head = $kind === 'thanks' ? 'blagodarnost'
          : (trim((string) ($o['title'] ?? '')) === 'Сопроводительное письмо' ? 'soprovoditelnoe' : 'obrashchenie');
    $path = $dir . '/' . $head . '-' . ($slug !== '' ? $slug : date('Ymd-His')) . '.pdf';

    // ГОТОВЫЙ БЛАНК ВТОРОЙ РАЗ НЕ РИСУЕМ.
    //
    // Номер обращения уникален и присвоен один раз, значит и лист по нему всегда
    // один и тот же. Перерисовка нужна была только потому, что её никто не
    // проверял: пересборка тела письма для очереди из шести тысяч приглашений
    // означала шесть тысяч лишних рендеров листа А4 со шрифтами и печатью.
    // Принудительно перерисовать: $o['force'] = true.
    $preview = (string) ($o['preview'] ?? '');
    if (empty($o['force']) && is_file($path) && filesize($path) > 1024
        && ($preview === '' || is_file($preview))) {
        return $path;
    }
