...
function regulation_pdf_cache_key(array $c): string {
    ...
    /* СЧИТАЕМ ТОЛЬКО СВОЙ ЭТАЛОН, А НЕ ВСЕ СРАЗУ.
     *
     * Здесь стоял максимум по ВСЕМ файлам etalon_*.docx. Пока эталона было два,
     * это работало. 25.08.2026 добавился третий — для конкурсов Клуба, — и его
     * свежая дата подняла отпечаток у КАЖДОГО конкурса: положения всех
     * действующих конкурсов встали на пересборку, хотя их собственный эталон
     * никто не трогал. Владелец увидел это как «положения зачем-то заменились».
     *
     * Берём дату того эталона, по которому собирается ИМЕННО этот конкурс:
     * правка своего эталона по-прежнему доезжает до открытых конкурсов, а
     * появление чужого больше ничего не сдвигает. */
    $etalonFile = ((int) ($c['club_only'] ?? 0) === 1)
        ? 'etalon_5.docx'
        : (((int) ($c['is_paid'] ?? 0) === 1) ? 'etalon_2.docx' : 'etalon_4.docx');
    $etalonTs = (int) @filemtime(BASE_PATH . '/docs/polozheniya/' . $etalonFile);
    /* ЦЕНА ТОЖЕ В КЛЮЧЕ.
     *
