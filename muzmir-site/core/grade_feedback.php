...
function gfb_extra_hint(): string {
    ...
    return "\n\nДОПОЛНИТЕЛЬНЫЙ ДИПЛОМ — КАК ЕГО ВЫДАЁТ ИМЕННО ЭТОТ ЦЕНТР (по " . $graded . " решениям жюри).\n"
         . "  Жюри отмечает дополнительным дипломом примерно " . $pct . " работ из ста"
         . ($everyN ? ' — каждую ' . $everyN . '-ю' : '') . ". " . $often . ":\n"
         . "  диплом здесь отмечает то, чего звание не показывает.\n"
         . "  Формулировки, которые центр действительно выдаёт (в скобках — сколько раз): " . implode(', ', $list) . ".\n"
         . "  Бери формулировку ИЗ ЭТОГО СПИСКА. Свою придумывай только если ни одна не описывает работу, —\n"
         . "  на бланке центра печатается именно то, что ты напишешь, и незнакомая формулировка выглядит чужой.\n"
