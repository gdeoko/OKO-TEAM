...
function pdf_regulation(array $competition): string {
    try {
        ...
        /* БЛАГОДАРНОСТИ У ЦЕНТРА ПЛАТНЫЕ.
         *
         * Здесь стояло «Педагоги и концертмейстеры отмечаются благодарственными
         * письмами» — обещание, которого нет в утверждённом положении. Участница
         * прочитала эту строку, пошла искать в форме заявки, где указать
         * концертмейстера, и не нашла: указывать негде, потому что благодарность
         * не выдаётся автоматически, а заказывается отдельно, и ФИО получателя
         * вписывается при заказе. Единственная бесплатная выдача — учреждениям-
         * партнёрам от пяти заявок, и это отдельная история, к положению
         * конкурса отношения не имеющая. */
        $doc->bullets([
            'Все участники получают именные наградные материалы в электронном виде.',
            'По итогам аттестации участникам присваиваются звания Лауреата, Дипломанта и Гран-при.',
            'По желанию оформляется оригинал диплома, кубок, статуэтка или медаль.',
            'По желанию оформляется именная благодарность педагогу, концертмейстеру или руководителю коллектива.',
        ]);
        $doc->spacer(20);
        ...
        if ($isPaid && $price > 0) {
            ...
            $doc->para('Участие в конкурсе бесплатное. Электронные наградные материалы предоставляются '
