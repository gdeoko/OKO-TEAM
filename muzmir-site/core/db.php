...
function db_migrate(PDO $pdo): void {
    ...
    foreach ([
        ...
        // Электронный документ или печатный оригинал. Реестр /verify обязан знать
        // и тех, и других: QR стоит на обоих бланках. Отличаются они судьбой —
        // электронный уходит письмом по расписанию, печатный едет почтой.
        ['diplomas', 'kind',              "TEXT DEFAULT 'digital'"],
        // Когда документ стал проверяемым в реестре. У электронного это момент
        // отправки письма (sent_at), у печатного — момент, когда бланк ушёл
        // почтой: отдельное поле, потому что sent_at у печатного занимать нельзя
        // (по нему крон отличает «ещё не отправлено письмо»).
        ['diplomas', 'issued_at',         "TEXT DEFAULT ''"],
