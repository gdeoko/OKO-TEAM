...
function nl_resolve_recipients(string $audience): array {
    ...
    // Элитный клуб — ПРИГЛАШЕНИЕ вступить, поэтому идёт по ВСЕЙ базе (со своей суточной
...
function nl_wrap_email(string $bodyHtml, string $unsubUrl, string $openPixel, string $preheader = '', array $opt = []): string {
    ...
    return mm_email_layout($bodyHtml, [
        ...
        'pixel'           => $openPixel,
        // Письмо про сам Элитный клуб не должно нести ещё и карточку клуба в подвале.
        'vip'             => $opt['vip'] ?? true,
    ]);
}
 ...
 * Элитный клуб и «личный кабинет» имеют отдельные дневные лимиты (см. nl_daily_split).
 ...
 *
 * Раньше объём делился на три (200 конкурсы + 100 Элитный клуб + 100 кабинет), и человек
 ...
 * чтобы соблюдать суточную квоту КАЖДОГО типа отдельно (конкурсы/Элитный клуб/кабинет).
 */
...
function nl_bulk_window_open(?\DateTime $at = null): array {
    ...
    // ВОСКРЕСЕНЬЕ — ВЫХОДНОЙ. Раньше оно было разрешено, и рассылка шла все семь
    // дней. Решение владельца: в воскресенье писем не отправляем — письмо о приёме
    // заявок, прочитанное в выходной, чаще уходит в архив, чем в работу, а
    // отправитель без единого тихого дня выглядит для почтовиков как автомат.
    if ((int) setting('nl_window_sunday', '0') !== 1 && (int) $t->format('N') === 7) {
        return ['open' => false, 'why' => 'воскресенье — рассылки не идут'];
    }
...
function nl_service_cap_today(): int {
    ...
    // Дату старта можно поставить и вперёд — так переносят запуск на завтра.
    // Без нижней границы день выходил отрицательным, лесенка бралась по индексу
    // минус один, и норма превращалась в null: рассылка падала бы на первом же
    // проходе крона.
...
function nl_box_gap_sec(string $box = ''): int {
    ...
    // серверам. Нужно другое — чтобы дневная норма легла РОВНО НА РАБОЧЕЕ ОКНО,
    // а не ушла залпом за полчаса.
    //
    // Пауза считается, а не задаётся числом: норма растёт по лесенке прогрева
    // (1000 писем сегодня, 2000 завтра, 3000 послезавтра), и фиксированные
    // «30 секунд» либо растянули бы тысячу на восемь часов и не дали дослать
    // три тысячи, либо выплюнули бы всё за час. Окно 9:00-18:00 — 32 400 секунд:
    // 1000 писем → 32 с, 2000 → 16 с, 3000 → 10 с, 10 000 → 3 с.
    if ($box !== '' && nl_box_is_service($box)) {
        $manual = (int) setting('nl_service_gap_sec', '');
        if ($manual > 0) return $manual;                    // владелец задал вручную
        ...
        //
        // Нижняя граница — секунда, а не две. Это не почтовый ящик, а HTTP-API
        // сервиса рассылок: он рассчитан на поток и сам разводит письма по своим
        // серверам. Две секунды упирали темп в 30 писем в минуту, и разослать
        // тридцать тысяч за два дня было нельзя чисто арифметически.
        return max(1, min(120, (int) floor($window * 0.9 / $cap)));
    }
        ...
        if (!$allowed) {
            ...
            foreach ($ready as $bi => $boxUser) {
                ...
                foreach ($bulk as $k => $row) {
                    ...
                    /* НА ПРИДЕРЖАННОМ ДОМЕНЕ НОРМУ ЗАНИМАЕТ СВОЯ БАЗА, А НЕ ХОЛОДНЫЙ ОХВАТ.
                     *
                     * Когда служба урезала нам норму до тридцати писем в сутки, важно
                     * не сколько мы отправим, а что именно. В очереди на Mail.ru Group
                     * стояло 21 827 писем, и 17 675 из них — приглашения учреждениям,
                     * которые нам не писали. Ровно за такие письма дверь и закрыли:
                     * с 25 августа на mail.ru не прошло ни одного, все с ответом
                     * «550 spam message rejected». Тратя дневной запас на них, мы
                     * каждый день подтверждали подозрение вместо того, чтобы его снять.
                     *
                     * Своя база ведёт себя иначе: человек оставил адрес сам, ждёт
                     * результат конкурса и письмо открывает. Открытия и переходы —
                     * единственный довод, который почтовая служба принимает. Норма
                     * вернётся к рабочей — холодные письма пойдут следом, они ждут
                     * в очереди и никуда не денутся. */
                    if ($ct === 'inst' && function_exists('mrep_cold_allowed')
                        && !mrep_cold_allowed((string) $row['to_email'])) continue;

                    /* ЧТО ОБЕЩАНО ПОЧТОВОЙ СЛУЖБЕ, ТО И ДЕЛАЕМ.
                     *
                     * В обращении к abuse-службе Mail.ru центр написал дословно:
                     * «Рассылку по учреждениям на ваши домены возобновлять не
                     * планируем до полной проверки контактов». 9 сентября служба
                     * ограничение сняла — и норма домена вернулась к рабочей, а
                     * вместе с ней сама собой открылась бы и холодная рассылка
                     * по школам: семнадцать тысяч писем по контактам, собранным
                     * из открытых источников, где каждый второй адрес устарел.
                     * Именно они в августе дали 41% отказов.
                     *
                     * Обещание должно держаться не памятью, а замком. Своя база
                     * (участники, которые сами оставили адрес) идёт как обычно —
                     * закрыт только холодный охват учреждений и только по
                     * доменам Mail.ru. Снять: nl_cold_mailru_off=0. */
                    if ($ct === 'inst' && (string) setting('nl_cold_mailru_off', '1') === '1'
                        && preg_match('~@(mail|bk|inbox|list|internet)\.ru$~i', (string) $row['to_email'])) continue;

                    /* АДРЕС ИЗ НАШЕГО ЖЕ СТОП-ЛИСТА ОТСЕИВАЕМ ЗАРАНЕЕ.
                     *
                     * Раньше такое письмо шло в общий путь отправки, там
                     * mail_send_failover() узнавал адрес в стоп-листе, ничего никуда
                     * не отправлял и возвращал отказ с текстом «ящика не существует».
                     * Отправки не было — а цикл ниже читал этот текст как отказ
                     * почтовика и наращивал счётчик подряд идущих отказов. Тридцать
                     * один такой пропуск подряд, и защита от порчи базы опускала
                     * стоп-кран на ВСЕ массовые: 24 августа рассылка встала в 09:34
                     * с 24 588 письмами в очереди и простояла весь день, хотя канал
                     * был исправен, лимиты свободны, а «отказы» пришли из нашей
                     * собственной таблицы.
                     *
                     * Считать своим стоп-листом чужой отказ нельзя. Письмо просто
                     * закрываем: адресат в стоп-листе, слать ему массовое нельзя, и
                     * держать строку в очереди на вечные повторы незачем. */
                    if (function_exists('mail_stop_kind') && mail_stop_kind((string) $row['to_email']) !== '') {
                        unset($bulk[$k]);
                        try {
                            q("UPDATE mail_queue SET status='cancelled', error=? WHERE id=?",
                              ['адрес в стоп-листе — массовое не отправляется', (int) $row['id']]);
                        } catch (\Throwable $e) {}
                        continue;
                    }
                    ...
                    if ($sendRow($row, $acc)) {
                        ...
                        $why = function_exists('mail_last_error') ? mail_last_error() : '';
                        /* Отказ, придуманный нами самими, не считается ни отказом
                         * адресата, ни признаком больного канала: письмо даже не
                         * покидало сервер. Пропуск по стоп-листу выше уже отсеян,
                         * но проверка остаётся здесь второй линией — именно эта
                         * подмена 24 августа остановила рассылку на целый день. */
                        if (mb_stripos($why, 'стоп-лист') !== false) {
                            continue;
                        }
                        if (nl_failure_kind($why) === 'hard') {
                            /* ВЕРДИКТ СЕРВИСА ПО КОНКРЕТНОМУ АДРЕСУ — ЭТО ПРО АДРЕС,
                             * А НЕ ПРО КАНАЛ, И В СЧЁТ «БОЛЬНОГО КАНАЛА» НЕ ИДЁТ.
                             *
                             * Сторож ниже задуман против другой беды: почтовик упёрся
                             * в лимит и отвечает «нет такого ящика» вообще на всё.
                             * Признак там — отказ БЕЗ имени адреса, голый 5xx.
                             *
                             * А сервис рассылок отвечает иначе: он называет адрес и
                             * причину (failed_emails: permanent_unavailable и т. п.) —
                             * это его собственный список подавления, письмо даже не
                             * ушло с нашего сервера. Такие адреса в очереди лежат
                             * кучей, подряд, и тридцать одна штука набирается на
                             * ровном месте. 16 и 17 сентября ровно так в 09:00 падал
                             * ВЕСЬ канал: за сутки не уходило ни одного массового
                             * письма при полной очереди на живые адреса.
                             *
                             * Такой отказ разбираем как факт об адресе (ниже) и идём
                             * дальше — счётчик подряд идущих не трогаем. */
                            $verdict = nl_service_verdict(mb_strtolower($why));
                            if ($verdict !== '') {
                                /* И НЕ ВЫЧЁРКИВАЕМ ЧЕЛОВЕКА ИЗ БАЗЫ ПО ЧУЖОМУ СПИСКУ.
                                 *
                                 * «Ящика не существует» (invalid) — проверяемый факт,
                                 * такой адрес вон. Остальное — решение сервиса или
                                 * самого человека: отписался, пожаловался, попал в
                                 * подавление после нашего же залпа. Ящик при этом
                                 * живой, и удалять участника нельзя: 15 сентября мы
                                 * уже разбирали 583 адреса, закрытых за нашу
                                 * собственную блокировку. Такому адресу закрываем
                                 * массовые письма и снимаем его с очереди, а история
                                 * участия, результаты и наградные остаются. */
                                if ($verdict === 'invalid') {
                                    nl_purge_person((string) $row['to_email'], 'отказ адресата: ' . mb_substr($why, 0, 120));
                                } else {
                                    $em = mb_strtolower(trim((string) $row['to_email']));
                                    try {
                                        q("INSERT OR IGNORE INTO mail_stop (email, reason, source) VALUES (?,?,?)",
                                          [$em, 'сервис рассылок: ' . $verdict, 'вердикт сервиса']);
                                        q("UPDATE mail_queue SET status='cancelled', error=?
                                            WHERE LOWER(to_email)=? AND status IN ('queued','paused') AND COALESCE(priority,0)>0",
                                          ['сервис рассылок: ' . $verdict, $em]);
                                    } catch (\Throwable $e) {}
                                }
