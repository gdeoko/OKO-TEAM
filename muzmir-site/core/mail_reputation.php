...
function mrep_intraday_brake(string $d, array $family, int $cap, int $probe): int {
    ...
    $pct = $hb * 100 / $tot;
    // Порог настраивается, потому что зависит от того, как мы едем. На ровном
    // ходу четверть отказов — честный признак беды. В день, когда владелец велел
    // выгрести остаток и норма поднята в разы против вчерашней, ждать четверти
    // нельзя: к этому моменту тысячи писем уже сожжены, а служба закрывает дверь
    // не на день, а на неделю. На таких днях порог опускается вручную.
    $limit = max(5, min(100, (int) setting('nl_domain_brake_pct', '25')));
    if ($pct < $limit) return $cap;
...
function mrep_domain_day_cap(string $domain): int {
    ...
    /* ПРОБА ОБЯЗАНА УМЕТЬ ЗАКОНЧИТЬСЯ.
     *
     * «Мало данных — стоим на месте» и пробная норма вместе давали замкнутый
     * круг: в пробе уходит 5–30 писем, полсотни наблюдений не набирается
     * никогда, значит норма не пересматривается никогда. Mail.ru стоял на пяти
     * письмах в сутки, и остался бы на них навсегда — даже после того, как
     * служба откроет дверь. Про это никто бы не узнал: в отчёте «0 доставлено,
     * 0 отказов», и выглядит как продолжающаяся блокировка.
     *
     * Поэтому у пробы свой, короткий счёт. Она и нужна для того, чтобы задать
     * службе один вопрос — принимает или нет, — а на такой вопрос хватает
     * нескольких писем. Приняла все и ни одного не отбила — дверь открыта,
     * возвращаемся сразу к полу (обычный рабочий темп), дальше растём как всегда.
     */
    $inProbe = $cap <= $probe * 2;
    $floorUp = max(200, (int) setting('nl_domain_floor_cap', '800'));

    if ($inProbe && $tot >= 3 && $hb === 0)      $new = $floorUp;   // дверь открылась
    elseif ($inProbe && $tot >= 3 && $pct < 10)  $new = $floorUp;   // почти чисто — тоже открылась
    elseif ($inProbe && $tot >= 3)               $new = $probe;     // всё ещё отбивает — ещё сутки пробы
    elseif ($tot < 50)    $new = $cap;                          // мало данных — стоим на месте
...
/**
 * МОЖНО ЛИ СЛАТЬ НА ЭТОТ ДОМЕН ХОЛОДНОЕ ПИСЬМО.
 *
 * У почтовой службы, которая нас придерживает, суточная норма мала — тридцать
 * писем вместо восьмисот. Вопрос в том, ЧЕМ занять эти тридцать.
 *
 * Сейчас их занимает холодный охват: в очереди на Mail.ru Group стоит 21 827
 * писем, из них 17 675 — приглашения учреждениям, которые нас не ждут и нам не
 * писали. Именно за такие письма служба и закрыла дверь: с 25 августа на
 * mail.ru не прошло ни одного, все с ответом «550 spam message rejected».
 * Каждый день мы тратим весь дневной запас на письма, которые никто не открывал
 * и не откроет, — и доказываем службе ровно то, в чём она нас подозревает.
 *
 * Поэтому на придержанном домене холодные письма ждут, а норму занимает своя
 * база: участник оставил адрес сам, ждёт результат конкурса и письмо откроет.
 * Открытия и переходы — единственный аргумент, который почтовая служба
 * принимает. Норма поднимется до рабочей — холодный охват пойдёт следом сам.
 */
function mrep_cold_allowed(string $email): bool {
    $dom = mrep_domain($email);
    if ($dom === '') return true;
    $cap   = mrep_domain_day_cap($dom);
    $floor = max(200, (int) setting('nl_domain_floor_cap', '800'));
    return $cap >= $floor;
}
