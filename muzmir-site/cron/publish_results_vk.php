...
// НАРУЖУ НИЧЕГО НЕ УХОДИТ В ВОСКРЕСЕНЬЕ И НОЧЬЮ (правило владельца).
// Этот крон публикует на стену сообщества и рассылает письма участникам, то есть
// он такой же наружный канал, как пульт. Раз в несколько лет 28-е выпадает на
// воскресенье (ближайшее — 28.02.2027), и без проверки итоги ушли бы в выходной.
require_once BASE_PATH . '/core/outreach_window.php';

// Страховка расписания: работаем 28 числа, а если 28-е воскресенье — 29-го.
// Пропустить оглашение нельзя: участники ждут итогов в объявленный день, поэтому
// вместо «выход, сегодня не 28-е» страховка сдвигается на понедельник вслед за
// волной пульта.
$today     = (int) date('j');
$sundayEnd = (int) date('w', strtotime(date('Y-m-28'))) === 0;
if (!($today === 28 || ($today === 29 && $sundayEnd))) {
    cron_log(JOB, 'сегодня не день оглашения - выход');
    exit(0);
}
if (!outreach_window_ok()) {
    cron_log(JOB, 'наружу сейчас нельзя (' . outreach_window_reason() . ') - выход');
    exit(0);
}
