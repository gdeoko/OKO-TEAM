...
if ($opened) {
    /* ПРОБА ПОДНИМАЕТ НОРМУ, НО НИКОГДА ЕЁ НЕ ОПУСКАЕТ.
     *
     * Здесь стояла безусловная запись рабочего пола. Пока домен лежал в
     * блокировке, это было ровно то, что нужно: проба дошла — вернули 800 и
     * поехали. А 12 сентября, когда норма была поднята под выгрузку остатка до
     * семи тысяч, утренняя проба честно дошла в 10:05 и... вернула 800. Уже
     * отправленные за утро 1 831 письмо оказались выше новой нормы, и рассылка
     * встала на ровном месте — на час, до первой проверки.
     *
     * Проба отвечает на один вопрос: принимает служба или нет. Приняла — норма
     * не должна быть НИЖЕ рабочего пола. Если она уже выше, значит её подняли
     * осознанно, и пробе там делать нечего. */
    $floor = max(200, (int) setting('nl_domain_floor_cap', '800'));
    if (!$dry) {
        mrep_ensure_caps();
        $raised = 0;
        foreach (MR_FAMILY as $d) {
            $cur = (int) (scalar("SELECT day_cap FROM mail_domain_caps WHERE domain=?", [$d]) ?: 0);
            if ($cur >= $floor) continue;                 // норма и так рабочая или выше — не трогаем
            q("INSERT OR REPLACE INTO mail_domain_caps (domain, day_cap, cap_date, note) VALUES (?,?,?,?)",
              [$d, $floor, date('Y-m-d'), 'проба дошла ' . date('d.m.Y H:i') . ' — норма поднята до рабочей']);
            $raised++;
        }
    ...
    $say($dry || ($raised ?? 0) > 0
        ? 'норма Mail.ru Group поднята до ' . $floor . ' писем в сутки'
        : 'норма Mail.ru Group и так не ниже рабочей — оставлена как есть');
