...
$sql = "SELECT a.id
         ...
         WHERE COALESCE(a.result,'') = ''
           ...
           /* Три неудачи подряд — вопрос к ссылке, а не к очереди: дальше это
              ...
              СЧИТАЕМ ТОЛЬКО СВЕЖИЕ. Счёт шёл за всё время, и это оборачивалось
              против центра: когда ломалась наша сторона — 2 сентября отвалилась
              загрузка записи в модель, — каждая работа за несколько дней
              набирала свои три отказа и выбывала НАВСЕГДА. Аттестацию починили,
              а очередь осталась пустой: тридцать одна заявка со статусом «новая»
              и «работ для аттестации нет» в каждом заходе. Окно в трое суток
              оставляет правило в силе для мёртвых ссылок и не превращает нашу
              поломку в приговор чужой заявке. */
           AND (SELECT COUNT(*) FROM grading_runs f2
                 WHERE f2.application_id = a.id AND f2.status = 'failed'
                   AND f2.created_at >= datetime('now','localtime','-3 days')) < 3
         ORDER BY a.id ASC
         LIMIT " . (int) $batch;
...
foreach ($rows as $r) {
     ...
     * центра: срок отправки результата по правилу «5 рабочих дней от подачи, Элитный клуб
