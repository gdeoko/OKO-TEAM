<?php
/**
 * Годовой отчёт Культурного центра «Музыкальный Мир» (PDF). Запуск: раз в год, 5 января.
 *
 ...
 *   - кладёт письмо владельцу на cfgv('owner_email') в mail_queue С ВЛОЖЕНИЕМ-PDF
...
try {
    ...
    $summary = "Годовой отчёт Культурного центра «Музыкальный Мир» за $year год сформирован.\n\n"
        . "Файл: " . basename($pdf) . " ($sizeKb КБ)\n"
        . "Ссылка: $url\n\n"
        . "PDF приложен к письму, отправленному на " . (string) cfgv('owner_email', (string) cfgv('org_email', '(email не задан)')) . ".";
