...
foreach ($ids as $id) {
    ...
    /* В ВИТРИНУ — ТОЛЬКО НАСТОЯЩАЯ ПОДДЕРЖКА И ТОЛЬКО С ДОКУМЕНТОМ.
     *
     * Здесь публиковалось всё, что не распознано как ОТКАЗ. Но «обращения по
     * электронной почте не рассматриваются», разъяснение о порядке обращения и
     * «размещение анонсов сторонних организаций не входит в наши полномочия» —
     * формально не отказы, а поддержки в них нет ни слова. Такие письма вставали
     * в галерею рядом с настоящими, и центр публично ссылался на поддержку,
     * которой ему не давали. Владелец увидел это 14.09.2026 и потребовал
     * публиковать только тех, кто поддержал.
     *
     * Два условия, оба обязательны:
     *   • разбор ответа сказал именно «support» — не «receipt», не «other»;
     *   • есть скан письма. Карточка без документа доказывает ровно ничего,
     *     а выглядит как поддержка.
     * Не прошло — ответ остаётся в реестре обращений и ждёт человека. */
    /* БЕЗ ДОКУМЕНТА КАРТОЧКИ НЕ БЫВАЕТ.
     *
     * Прямой отказ отсеян выше ($isRefusal) — до этого места он не доходит, и
     * ставить здесь вторую проверку на отказ незачем. А вот скан не проверялся
     * никак, и в витрину попадали карточки с одним названием региона: Казань с
     * ответом «обращения по электронной почте не рассматриваются» и Владимирская
     * область, которая работает только через интернет-приёмную. Пустая карточка
     * не доказывает ничего, а выглядит как поддержка.
     *
     * Мера строгости — по владельцу (14.09.2026): снимаем то, где отказ сказан
     * прямо, а «информация принята к сведению» стоять может — ведомство
     * обращение зарегистрировало и не отказало. Поэтому единственное условие
     * здесь: есть сам документ. */
    if ($imgRel === '') {
        mr_log('в витрину не идёт (нет скана письма): ' . $org);
        continue;
    }
