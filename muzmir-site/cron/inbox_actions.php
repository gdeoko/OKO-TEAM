...
try {
    ...
    foreach ($rows as $r) {
        ...
        /* ── ОТВЕТ УЧРЕЖДЕНИЯ, ПРИШЕДШИЙ НЕ В ТОТ ЯЩИК ──
         *
         * Классификатор решает по ящику: письмо в kc@ — значит ведомство, письмо
         * в novosti@ — значит учреждение. Пока партнёрская рассылка уходила с
         * novosti@, это работало. Но письма центра неделю шли с Gmail (Яндекс
         * держал kc@ закрытым), а часть учреждений отвечает на исторические
         * адреса из старых бланков — и их «согласны» приходило в kc@, mail.ru
         * или gmail, где помечалось как ministry_approve и тут же пропускалось:
         * ведомства ведёт другой крон, а он про партнёрство ничего не знает.
         *
         * Смотрим не на ящик, а на отправителя: письмо от адреса из справочника
         * учреждений — партнёрский ответ, где бы оно ни лежало.
         */
        $instOf = (int) ($r['inst_id'] ?? 0);
        if ($instOf > 0 && (int) ($r['ministry_id'] ?? 0) === 0 && str_starts_with($kind, 'ministry')) {
            $kind = $kind === 'ministry_decline' ? 'partner_decline'
                  : ($kind === 'ministry_approve' ? 'partner_accept' : 'question');
            if (!$dry) q("UPDATE inbox_messages SET kind=? WHERE id=?", [$kind, $id]);
            ia_log('ответ учреждения в ящике ' . $box . ' — читаем как ' . $kind . ': ' . $from);
        }
         ...
         */
        if ($kind === 'optout') {
            // ПИСЬМО ИЗ ПАПКИ «СПАМ» САМО НИЧЕГО НЕ ЗАКРЫВАЕТ.
            //
            // Спам читается специально: там оказываются ответы ведомств и учреждений,
            // которые почтовая служба посчитала подозрительными. Но там же лежат
            // письма с подделанным обратным адресом, и любое из них, попав в
            // классификатор как «удалите мой адрес», молча выключило бы рассылку
            // настоящему учреждению. Из спама такие просьбы уходят оператору.
            $folder = (string) ($r['folder'] ?? 'INBOX');
            if ($folder !== '' && $folder !== 'INBOX') {
                ia_log('просьба удалить адрес пришла из папки «Спам», нужен человек: ' . $from);
                $mark('human');
                $did['skip']++;
                continue;
            }
