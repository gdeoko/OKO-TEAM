...
//
// ОБЯЗАТЕЛЬСТВА ПЕРЕД УЧАСТНИКАМИ СТОП-КРАН НЕ ОТМЕНЯЕТ.
//
// Кран останавливает месячную кампанию — приглашения и напоминания. Но две
// волны не про кампанию, а про обещанное участникам, и час их выхода назван
// заранее:
//   closed  — конец приёма: переводит конкурсы в 'closed', открывает окно
//             «новые конкурсы с 1 числа», объявляет об этом постом;
//   results — оглашение итогов в назначенный день и письма с результатами.
// 25 августа владелец свернул кампанию в 17:45, а приём закрывался в 18:00: с
// общей проверкой приём остался бы открытым до следующего запуска пульта — на
// сутки дольше объявленного, — а итоги 28-го не вышли бы вовсе.
//
// Личка ВК из обеих волн при опущенном кране убирается: остановка массовых —
// это прежде всего про сообщения людям. Пост на стену уходит, приём
// закрывается, итоги публикуются. Почту оставляем только волне итогов: письмо
// со званием — личное письмо участнику, а не рассылка, и не отправить его
// нельзя.
if (function_exists('mass_sending_enabled') && !mass_sending_enabled()) {
    $due = [];
    try {
        $due = all("SELECT id, wave, channels FROM launch_jobs
                     WHERE status='scheduled' AND wave IN ('closed','results') AND run_at <= ?",
                   [date('Y-m-d H:i:s')]);
    ...
    $waves = [];
    foreach ($due as $j) {
        $keepMail = (string) $j['wave'] === 'results';
        $ch = array_values(array_filter(
            array_map('trim', explode(',', (string) $j['channels'])),
            static fn(string $c): bool => $c !== '' && $c !== 'vk_dm' && ($keepMail || $c !== 'email')
        ));
        update('launch_jobs', ['channels' => implode(',', $ch)], 'id=:id', ['id' => (int) $j['id']]);
        $waves[] = (string) $j['wave'];
    }
    cron_log('launch_scheduler', 'стоп-кран опущен: выполняем только обязательные волны ('
        . implode(', ', array_unique($waves)) . '), личка ВК из них убрана');
}
