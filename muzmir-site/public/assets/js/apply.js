...
(function () {
  ...
  function formatCity(raw) {
    ...
    var said = '';
    if (s.indexOf(',') !== -1) {
      // «Страна, г. Город»: последний кусок — населённый пункт, всё что перед ним —
      // возможно страна. Её и запоминаем, чтобы не потерять по дороге.
      var p = s.split(',');
      s = p[p.length - 1].trim();
      for (var i = 0; i < p.length - 1; i++) { var c = countryCanon(p[i]); if (c) { said = c; break; } }
    }
    var pref = 'г.';
    ...
    // Справочник город не знает. Если человек сам написал страну через запятую —
    // верим ему: раньше поле «Страна» было только для чтения и всегда «Россия»,
    // и участник из Столбцов физически не мог указать Беларусь.
    return (said || 'Россия') + ', ' + pref + ' ' + titleCity(s);
  }

  /* Страна, написанная как придётся: «Беларусь», «РБ», «Белоруссия». */
  var COUNTRY_ALIAS = {
    'россия':'Россия','российская федерация':'Россия','рф':'Россия','russia':'Россия',
    'беларусь':'Республика Беларусь','белоруссия':'Республика Беларусь','республика беларусь':'Республика Беларусь','рб':'Республика Беларусь',
    'казахстан':'Республика Казахстан','республика казахстан':'Республика Казахстан','рк':'Республика Казахстан',
    'киргизия':'Кыргызская Республика','кыргызстан':'Кыргызская Республика','кыргызская республика':'Кыргызская Республика',
    'узбекистан':'Республика Узбекистан','республика узбекистан':'Республика Узбекистан',
    'таджикистан':'Республика Таджикистан','туркменистан':'Туркменистан','туркмения':'Туркменистан',
    'армения':'Республика Армения','азербайджан':'Азербайджанская Республика',
    'молдова':'Республика Молдова','молдавия':'Республика Молдова','грузия':'Грузия','украина':'Украина',
    'абхазия':'Республика Абхазия','южная осетия':'Республика Южная Осетия',
    'латвия':'Латвийская Республика','литва':'Литовская Республика','эстония':'Эстонская Республика',
    'сербия':'Республика Сербия','болгария':'Республика Болгария','монголия':'Монголия',
    'китай':'Китайская Народная Республика','кнр':'Китайская Народная Республика',
    'израиль':'Государство Израиль','германия':'Федеративная Республика Германия','турция':'Турецкая Республика'
  };
  function countryCanon(raw) {
    var k = (raw || '').toLowerCase().replace(/ё/g, 'е').replace(/[.,;]+$/, '').trim();
