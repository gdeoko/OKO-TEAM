...
(function () {
  ...
  // Настоящий скрипичный ключ (G-clef): длинный стержень + петля через него +
  // внутренняя спираль-«глаз» + нижний завиток с терминальной точкой. Рисуется
  // векторно, одинаково на всех устройствах.
  function drawTreble(s) {
    var u = s * 0.42;
    ctx.lineWidth = Math.max(1.5, s * 0.15); ctx.lineCap = 'round'; ctx.lineJoin = 'round';

    // Единая линия ключа: верхний хвостик → стержень вниз → нижний завиток влево →
    // спираль вокруг центра → замыкание «глаза».
    ctx.beginPath();
    // верхний загиб вправо
    ctx.moveTo(0.75 * u, -3.35 * u);
    ctx.bezierCurveTo(1.55 * u, -2.75 * u, 1.15 * u, -1.75 * u, 0.35 * u, -1.15 * u);
    // левая петля, пересекающая стержень
    ctx.bezierCurveTo(-1.55 * u, 0.1 * u, -1.55 * u, 1.75 * u, 0.2 * u, 2.15 * u);
    // правая часть большой петли, разворот к центру
    ctx.bezierCurveTo(1.95 * u, 2.55 * u, 2.05 * u, 0.15 * u, 0.2 * u, -0.05 * u);
    // внутренняя спираль-«глаз»
    ctx.bezierCurveTo(-1.15 * u, -0.2 * u, -1.05 * u, 1.15 * u, 0.15 * u, 1.15 * u);
    ctx.bezierCurveTo(0.95 * u, 1.15 * u, 0.95 * u, 0.35 * u, 0.2 * u, 0.35 * u);
    ctx.stroke();

    // Стержень: длинная почти прямая вертикаль от верхнего хвостика вниз к завитку.
    ctx.beginPath();
    ctx.moveTo(0.75 * u, -3.35 * u);
    ctx.bezierCurveTo(0.5 * u, -1.2 * u, 0.35 * u, 1.6 * u, 0.12 * u, 3.15 * u);
    ctx.stroke();

    // Нижняя терминальная точка (ножка ключа).
    ctx.beginPath(); ctx.arc(0.0 * u, 3.55 * u, u * 0.34, 0, Math.PI * 2); ctx.fill();
  }
