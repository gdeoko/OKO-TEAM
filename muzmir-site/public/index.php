...
// /order-awards — форма заказа ПО КОНКРЕТНОЙ ЗАЯВКЕ (?app=…) или по конкурсу
// (?competition=…). Без параметров вести туда нечего — уводим на витрину наград.
// Раньше редирект стоял безусловно: переход из кабинета «Заказать награды» терял
// привязку к заявке, и участник попадал на общую страницу, где мог выбрать любой
// результат и заказать несоответствующий трофей.
if ($route === '/order-awards' && input('app') === '' && input('competition') === '') {
