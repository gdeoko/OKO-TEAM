...
// Благодарность выписывается НА ВЕДОМСТВО, а не на человека: поддержку оказал
// орган власти, руководитель может смениться, а документ останется в деле.
$org       = trim((string) ($_GET['org']   ?? 'Министерство культуры Тюменской области'));
$region    = trim((string) ($_GET['reg']   ?? ''));
$person    = trim((string) ($_GET['fio']   ?? ''));
...
$len = mb_strlen($org);
$scriptPt = $len <= 24 ? 34 : ($len <= 34 ? 28 : ($len <= 48 ? 23 : ($len <= 66 ? 19 : 16)));

$body = 'Оргкомитет Культурного центра «Музыкальный Мир» выражает искреннюю благодарность '
      . 'за информационную поддержку всероссийских творческих конкурсов и за содействие в '
      . 'распространении сведений о них среди учреждений культуры, образования и '
      . 'дополнительного образования детей'
      . ($region !== '' ? ' ' . htmlspecialchars($region, ENT_QUOTES) : '') . '. '
      . 'Благодаря этому участие в конкурсах стало доступным детям и педагогам из городов и '
      . 'посёлков, куда сведения о творческих состязаниях доходят редко.';
...
<div class="sheet">
  ...
  <div class="content">
    ...
    <div class="rule"></div>
    <div class="lead">объявляется</div>
    <div class="name"><?= htmlspecialchars($org, ENT_QUOTES) ?></div>
    <?php if ($region !== '' || $person !== ''): ?>
    <div class="region"><?= htmlspecialchars(trim($region . ($person !== '' ? ' · ' . $person : '')), ENT_QUOTES) ?></div>
    <?php endif; ?>
