...
## 2026-07-14 — v6.2 (премиум-полировка + фикс адаптации)
Даниэль: «не нравится, криво, адаптация плохая на ПК и телефоне, блоки не
помещаются, сделай шикарный». Диагностика Playwright на 6 размерах
(360/390/768/1366/1440/1920):
- НАЙДЕНА причина «не помещается»: reveal data-from=left/right выезжал на ±34px
  за край на full-width мобильных блоках → горизонтальный скролл. Фикс:
  html+section overflow-x:hidden/clip. Итог: 0 overflow на ВСЕХ 6 размерах.
- «Криво / блоки пропадают»: reveal мог оставлять секции пустыми (opacity:0).
  Сделал reveal пуленепробиваемым: моментальный показ near-viewport +
  страховка showAll через 3с. Пустых блоков больше нет.
- Добавлено для премиум-вида: прилипающая шапка #nav (лого-крылья + меню +
  CTA, solid-on-scroll), нумерация секций 01-06 (.kick .num), градиентная
  обводка карточек (.card/.step ::after mask), настоящий футер #foot
  (лого, ссылки, @diesel_cargo, dieselcompany.pro), scroll-padding-top под шапку.
- Герой h1 max 104→86px чтобы не разваливался на узких.
- Deploy: push main (a805ddb) + deploy_website production. LIVE=v6.2 (curl ok),
  шапка id="nav" на проде. Higgsfield падал ~10мин, деплой добит после реконнекта.
