...
#
# ЗАЧЕМ. Замер 10.08 (probe-prod.mjs): первая загрузка okoteam.top весит
# 7858 КБ. Сжатым приезжает РОВНО ОДИН файл — index.html. Всё остальное, все
# 37 файлов, идёт как есть: app.min.js 3270 КБ, app.min.css 962 КБ,
# three.module.min.js 655 КБ, oko-eye.glb 484 КБ и 27 слоёв из media/app.
#
# Под gzip те же файлы весили бы около 1,5 МБ — впятеро меньше. Это главная
# причина, по которой приложение «тормозит» на первом входе с телефона, и
# единственная оставшаяся крупная проблема запуска: лечится одной директивой
# в конфиге, никакой правки кода не требует.
#
# Пока сжатия нет, ядро отдаётся минифицированным (build-min.mjs) — это
# сняло 833 КБ из 5065. Минификация сжатие не заменяет: 16% против 80%.
#
