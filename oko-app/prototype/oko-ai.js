...
(function () {
  ...
  var CHIPS = [
    ...
    { i: 'megaphone', t: 'Сценарий ролика',      m: 'Собери сценарий ролика 30-45 секунд: хук, боль, решение, CTA' },
  ...
  function send(text) {
    ...
    function finish(reply, err) {
      ...
      if (reply) {
        ...
        var honest = (err === 'timeout')
          ? 'Сервер ОКО Ai не ответил за 45 секунд — ответа нет. Твоё сообщение осталось в переписке, повтори отправку чуть позже.'
          : 'Нет связи с сервером ОКО Ai — ответ не получен. Твоё сообщение осталось в переписке, повтори отправку, когда сеть вернётся.';
        push('ai', honest);
