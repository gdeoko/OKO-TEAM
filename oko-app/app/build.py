...
ORDER = [
    ...
    'chats-plus',      # мессенджер+: статусы, поиск, закреп, скорость голосовых, свайп-ответ
    'calls',           # звонки: групповая конференция (Zoom-like, админ) + личный (аудио/видео с getUserMedia, анимация колец)
    ...
    'partner-plus',    # партнёрка: уровни, лидерборд, промо, дип-линки, QR, история с фильтром
    ...
    'wow-eye',         # интерактивный 3D-стеклянный глаз на auth-экране (моргает + следит за пальцем)
    'wow-motion',      # финальный вау-пак: типографика, моушен, микро-интеракции, свечения (грузится самым последним)
]
