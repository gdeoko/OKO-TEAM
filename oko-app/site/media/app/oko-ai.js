...
(function () {
  ...
  function sendMedia(kind, prompt) {
    ...
    window.okoBridge.run(kind, prompt, {
      ...
      hideTyping();
      /* Если submit не прошёл (onQueued не вызвался) — освобождаем ввод и
         говорим честно, что задача НЕ поставлена; иначе задача уже в очереди. */
      var failedToQueue = /поставить задачу|bad json|ok/i.test(String(err && err.message || ''));
      var e = failedToQueue
        ? 'Не удалось поставить задачу в очередь — попробуй ещё раз чуть позже.'
        : 'Генератор OKO пока не отозвался — задача осталась в очереди. Загляни позже, результат придёт сюда.';
      push('ai', e); appendBubble('ai', e); setStatus('на связи');
    }).then(function () {
      busy = false; /* что бы ни случилось — ввод не должен залипнуть */
    });
  }
