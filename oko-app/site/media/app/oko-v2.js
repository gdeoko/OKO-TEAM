...
(function v2polite(){
  ...
  try{
    ...
    /* Момент последнего касания. Окно, открытое сразу после нажатия, — это
       ОТВЕТ на нажатие, и откладывать его нельзя, сколько бы кнопок в нём ни
       было. Поймано при сборке тарифов: «Как работает бесплатный тариф» —
       окно с одной кнопкой — не появлялось вовсе, потому что приложение
       считалось занятым. Человек жмёт и не получает ничего. */
    var последнееКасание = 0;
    ['pointerdown','click','keydown'].forEach(function(e){
      document.addEventListener(e, function(){ последнееКасание = Date.now(); }, true);
    });
    ...
    function вопрос(o){
      try{
        ...
        if(Array.isArray(o.actions) && o.actions.length > 1) return true;  /* есть выбор */
        /* открыто по нажатию — значит человек этого ждёт прямо сейчас */
        if(Date.now() - последнееКасание < 1200) return true;
        return false;
...
/* ============================================================================
   38. «Рулетка OKO» в шапке не помещается ни на одном телефоне

   Замер: заголовку в шапке достаётся 92 пикселя на 320 и 126 на 390, а
   «Рулетка OKO» просит 111 и 138. Резалось на обеих ширинах — как раз то
   «сверху обрезается», на которое жаловался Даниэль.

   Кеглем это не лечится: пришлось бы уйти до двенадцати пикселей. Место
   тоже брать неоткуда — слева «назад» и знак OKO, справа три действия.
   Поэтому в шапке остаётся короткое «Рулетка», а полное название читается
   на самом экране, где под него есть целая строка.
   ============================================================================ */
(function () {
  'use strict';
  function короче() {
    try {
      if (typeof regTitle !== 'function') return false;
      if (typeof TITLES === 'undefined' || TITLES.games === undefined) return false;
      if (TITLES.games === 'Рулетка OKO') regTitle('games', 'Рулетка');
      return true;
    } catch (e) { return false; }
  }
  if (короче()) return;
  var n = 0;
  var t = setInterval(function () { if (короче() || ++n > 80) clearInterval(t); }, 120);
})();

/* ============================================================================
   §39 · ПРОПАВШАЯ СЕТЬ — НЕ ОШИБКА ПРИЛОЖЕНИЯ

   Пробник доступности собрал 108 одинаковых записей:
   «TypeError: Failed to fetch» из библиотеки Supabase. Это не логическая
   ошибка приложения, а недоступная сеть: библиотека сама, в фоне, обновляет
   токен и держит канал живого чата, и эти промисы никто не ждёт — отказ
   всплывает как unhandledrejection.

   В песочнице так вышло из-за прокси, но у человека это будет постоянно:
   OKO живёт в Telegram на телефоне — метро, лифт, роуминг. Необработанное
   отклонение промиса засоряет консоль, а когда Даниэль подставит настоящий
   ключ Sentry — ещё и отчёты об ошибках, в которых утонут настоящие поломки.

   Гасим ТОЛЬКО сетевые отказы и ТОЛЬКО их: всё остальное обязано остаться
   видимым, иначе слой начнёт прятать настоящие дефекты. Счётчик и последняя
   причина доступны в okoNet — чтобы «тихо» не превратилось в «незаметно».
   ============================================================================ */
(function () {
  'use strict';
  var СЕТЬ = /Failed to fetch|NetworkError|Load failed|network error|ERR_INTERNET|ERR_NETWORK/i;
  var состояние = { сетевых: 0, последняя: null, прочих: 0 };
  try { window.okoNet = состояние; } catch (e) {}

  addEventListener('unhandledrejection', function (ev) {
    var r = ev && ev.reason;
    var текст = r ? String((r && r.message) || r) : '';
    var сетевая = (r instanceof TypeError || /TypeError/.test(текст)) && СЕТЬ.test(текст);
    if (!сетевая) { состояние.прочих++; return; }   /* настоящую ошибку не трогаем */
    состояние.сетевых++;
    состояние.последняя = текст.slice(0, 160);
    /* Не console.error: это не поломка, а состояние связи. */
    try { console.debug('[oko] сеть недоступна:', состояние.последняя); } catch (e) {}
    ev.preventDefault();
  });
})();
