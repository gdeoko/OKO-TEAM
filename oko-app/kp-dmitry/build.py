...
# --- адаптация под КП для Дмитрия (закрытый клуб по видеомонтажу) ---
tpl = tpl.replace("<title>OKO — Экосистема роста бизнеса под ключ</title>",
    "<title>OKO — КП для Дмитрия: закрытый клуб по видеомонтажу</title>")
tpl = tpl.replace("Три пакета под <span class=\"lime\">запуск и масштаб</span>",
    "Три варианта под <span class=\"lime\">запуск клуба</span>")
tpl = tpl.replace("Осталось 7 мест на этот месяц · цена действует 48 часов",
    "Персональное КП для Дмитрия · закрытый клуб по видеомонтажу · срок до 10 рабочих дней")
tpl = tpl.replace("свайп · клик по точкам · наклон телефона",
    "свайп · клик по точкам · наклон телефона · техрасходы отдельно: домен ~2 000 ₽/год, VPS 2 000–5 000 ₽/мес")
tpl = tpl.replace("<div class=\"gc-t\">Рассрочка 50/50</div><div class=\"gc-d\">Половина на старте — без скрытых доплат</div>",
    "<div class=\"gc-t\">Поэтапная оплата</div><div class=\"gc-d\">Прозрачные этапы по договору, без скрытых доплат</div>")
...
tpl = tpl.replace("50% на старте, 50% по готовности. Карта РФ и зарубежная — через Lava, также крипта (USDT), карта USD и ЮMoney. Всё по договору.",
    "Оплата поэтапно по договору. Карта РФ и зарубежная, крипта (USDT) и ЮMoney. По каждому платежу — чек.")
tpl = tpl.replace("Какие сроки запуска?<span class=\"p\">+</span></div><div class=\"faq-a\"><div>СТАРТ — от 5–7 дней, БИЗНЕС — 2–3 недели, ИМПЕРИЯ — 3–5 недель. Точные сроки фиксируем в договоре.",
    "Какие сроки запуска?<span class=\"p\">+</span></div><div class=\"faq-a\"><div>Любой вариант — до 10 рабочих дней с момента старта и передачи доступов. Сроки фиксируем в договоре.")
...
tiers = (
 tier("СТАРТ","Договор_OKO_СТАРТ.docx","Клуб внутри Telegram",False,"100 000&#8201;₽","50 000&#8201;₽","5 000&#8201;₽",[
   ("Telegram-бот + mini-app клуба","Закрытый клуб по подписке: бот выдаёт уроки, mini-app — личный кабинет ученика."),
   ("Личный кабинет ученика","Все уроки в одном месте, прогресс прохождения, предзагрузка. 1080p и 4 урока в 4К."),
   ("Оплаты по подписке · Lava API","Автопродление каждый месяц. Не оплатил — доступ закрывается автоматически.",True),
   ("Уроки: YouTube + свой сервер","Часть уроков на YouTube, часть на вашем VPS — быстро и надёжно."),
   ("Домен + VPS + запуск","Купим и подключим домен, настроим сервер, задеплоим и протестируем."),
 ], breakdown=[
   ("Разработка бота + Mini-app","20 000"),
   ("Дизайн UI","6 000"),
   ("Вёрстка Mini-app","6 000"),
   ("Подключение оплат Lava (подписка + кик)","6 000"),
   ("База: подписки, прогресс, предзагрузка","6 000"),
   ("VPS + домен + деплой + запуск","6 000"),
 ]),
 tier("БИЗНЕС","Договор_OKO_БИЗНЕС.docx","Клуб + браузер + админка",True,"150 000&#8201;₽","75 000&#8201;₽","7 000&#8201;₽",[
   ("Всё из варианта СТАРТ","Бот, mini-app, личный кабинет, Lava-подписка, уроки, домен и сервер."),
   ("Адаптация под браузер","Кабинет работает не только в Telegram: ПК, планшет, любой браузер.",True),
   ("Админ-панель","Аналитика подписок, редактирование уроков, цен и текстов — без программиста.",True),
   ("Расширенный дизайн UI","Более проработанный визуал и удобная навигация по урокам."),
 ], breakdown=[
   ("Разработка бота + Mini-app","22 000"),
   ("Адаптация под браузер","8 000"),
   ("Админ-панель (аналитика + редактор)","14 000"),
   ("Дизайн + вёрстка","12 000"),
   ("Подключение оплат Lava (подписка)","6 000"),
   ("База: подписки, прогресс, предзагрузка","6 000"),
   ("VPS + домен + деплой + запуск","7 000"),
 ]),
 tier("ПОД КЛЮЧ","Договор_OKO_ПОД_КЛЮЧ.docx","Максимум + ИИ-агент",False,"200 000&#8201;₽","100 000&#8201;₽","10 000&#8201;₽",[
   ("Всё из варианта БИЗНЕС","Браузерная версия, админ-панель, кабинет, подписки, домен и сервер."),
   ("ИИ-агент 24/7","Прогревает, отвечает на вопросы, доводит до оплаты — в боте и приложении.",True),
   ("Админ-панель PRO + рассылки","Клики, посещения, база клиентов, экспорт таблиц, сегменты «оплатил/не оплатил», рассылки.",True),
   ("Премиум mini-app","Больше функционала, красивее визуал, анимации."),
   ("Дублирующее меню в боте","Если mini-app не открылся — всё доступно прямо в боте."),
 ], breakdown=[
   ("Разработка расширенного бота + PRO Mini-app","22 000"),
   ("Адаптация под браузер","8 000"),
   ("Админ-панель PRO + сегменты + рассылки","16 000"),
   ("ИИ-агент 24/7 (Claude)","16 000"),
   ("Дизайн премиум + анимации","14 000"),
   ("Вёрстка Mini-app + веб","10 000"),
   ("Подключение оплат Lava (подписка)","6 000"),
   ("База: подписки, прогресс, аналитика","5 000"),
   ("VPS + домен + деплой + запуск","3 000"),
 ]),
)
...
outp = BASE/"kp_oko/kp_dmitry.html"
outp.write_text(out)
