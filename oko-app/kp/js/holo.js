...
export function initHolo(){
  ...
  var GESTURES=[
    ...
    function(g,t,w){var sw=osc(t,4.6,0);
      // рука (открытая сторона, LeftArm — для зрителя справа) поднята ВВЕРХ-ВБОК в плоскости экрана и машет ВВЕРХ-ВНИЗ.
      // Именно ось Z даёт видимое движение при фронтальной орто-камере сайта (ось X ушла бы в глубину и не читалась).
      addPose('LeftArm',0,0,-(1.50+0.35*sw)*w);              // подъём + махание вверх-вниз (без сгибания локтя)
      addPose('Head',0,-0.03*w,0);},
    // 1) СКЛАДЫВАЕТ ОБЕ РУКИ ПЕРЕД СОБОЙ на грудь/пузо: предплечья сходятся к центру В ПЛОСКОСТИ экрана (без ухода в глубину/назад)
    function(g,t,w){addPose('RightArm',0,0,0.12*w);addPose('LeftArm',0,0,-0.12*w);
      addPose('RightForeArm',0,0,1.28*w);addPose('LeftForeArm',0,0,-1.28*w);
