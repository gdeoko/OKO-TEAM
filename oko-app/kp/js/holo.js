...
export function initHolo(){
  ...
  const eyeLoad=document.getElementById('holoEyeLoad');   // фиксированный якорь глаза на экране загрузки (стабильно, без прыжков)
  ...
  var GESTURES=[
    ...
    function(g,t,w){var sw=osc(t,4.2,0);
      // ПРИВЕТ: ЛОКОТЬ остаётся ВНИЗУ (плечо не трогаем), поднимается ТОЛЬКО предплечье — ладонь выходит ПЕРЕД корпусом
      // и машет ВВЕРХ-ВНИЗ. Так рука не уходит вбок/назад (локоть прижат к боку).
      addPose('LeftForeArm',-(1.75+0.40*sw)*w,0,-0.20*w);
      addPose('Head',0,-0.03*w,0);},
    // 1) СКЛАДЫВАЕТ ОБЕ РУКИ на пузе СПЕРЕДИ: локти внизу, поднимаются только предплечья, ладони встречаются по центру перед корпусом
    function(g,t,w){addPose('RightForeArm',-1.40*w,0,0.70*w);addPose('LeftForeArm',-1.40*w,0,-0.70*w);
