...
export function initHolo(){
  ...
  var GESTURES=[
    ...
    function(g,t,w){var sw=osc(t,4.2,0);
      // ПРИВЕТ: рука с открытой стороны (LeftArm — для зрителя справа) выносится ВПЕРЁД к зрителю (локоть вперёд),
      // предплечье согнуто так, что ЛАДОНЬ поднята ПЕРЕД корпусом и машет ВВЕРХ-ВНИЗ. Рука спереди, не вбок и не назад.
      addPose('LeftArm',0.60*w,0,-0.20*w);
      addPose('LeftForeArm',-0.20*w,0,-(1.55+0.42*sw)*w);     // ладонь качается вверх-вниз перед корпусом (заметнее)
      addPose('Head',0,-0.03*w,0);},
    // 1) СКЛАДЫВАЕТ ОБЕ РУКИ ПЕРЕД СОБОЙ: плечи вынесены ВПЕРЁД к зрителю, предплечья сходятся, ладони встречаются на пузе спереди
    function(g,t,w){addPose('RightArm',0.60*w,0,0.15*w);addPose('LeftArm',0.60*w,0,-0.15*w);
      addPose('RightForeArm',0,0,1.40*w);addPose('LeftForeArm',0,0,-1.40*w);
      addPose('Head',0.04*w,0,0);},
