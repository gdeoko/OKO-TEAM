...
if (CHECK) {
  /* Ненулевой код возврата, чтобы --check годился как ворота перед коммитом:
     расхождение здесь означает, что правка слоя на прод НЕ поедет. */
  console.log(новых || обновлено
    ? '\n(--check: НЕ СИНХРОНИЗИРОВАНО — эти правки на прод не поедут, гони без --check)'
    : '\n(--check: всё синхронизировано)');
  process.exit(новых || обновлено ? 1 : 0);
}
