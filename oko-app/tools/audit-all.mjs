...
const детектор = (сверху0, конец, ширина) => `(() => {
  ...
  document.querySelectorAll('button, [role="button"], .prow, .pp2-row').forEach(el => {
    ...
    /* Модальное окно закрывает то, что под ним, — на то оно и модальное.
       На 1440 поверх шторок игр открывается пейволл, и кнопка «назад»
       самой шторки оказывается под его карточкой. Это не дефект: выход из
       пейволла есть у пейволла. Семь последних замечаний раунда 55 были
       ровно про это. */
    const окно = чужой.closest('.pw-modal, .pw-modal-card, .modal, [role="dialog"], .sheet.open, .sheet.on');
    if (окно && !окно.contains(el)) return;
