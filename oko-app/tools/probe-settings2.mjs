...
async function main() {
  ...
  for (const vp of VIEWPORTS) {
    for (const theme of THEMES) {
      ...
      await page.addInitScript(initScript(vp.mobile, vp.width, vp.height));
      const errors = [];
      ...
      for (const p of PANELS) {
        ...
        const shoot = (vp.id === 'phone' && theme === 'dark') || (vp.id === 'desk' && theme === 'light');
        if (shoot) {
  ...
  {
    const c = await browser.newContext({ viewport: { width: 390, height: 844 }, isMobile: true, hasTouch: true, colorScheme: 'dark', locale: 'ru-RU' });
    const page = await c.newPage();
    await page.addInitScript(initScript(true, 390, 844));
    const errors = [];
    page.on('pageerror', e => errors.push(String(e && e.message || e)));
    await page.goto(BASE, { waitUntil: 'domcontentloaded' });
    await sleep(1400);
    ...
    for (const pid of togglePanels) {
      await openPanel(page, pid, pid === 'notifChannel' ? { k: 'msg' } : null);
  ...
  /* ============ 3. СЕКРЕТЫ: ПАРОЛЬ И PIN НЕ ПОПАДАЮТ В DOM/localStorage ==== */
  {
    ...
    await sleep(200);
    await page.screenshot({ path: path.join(OUT, 'settings2-secrets-01-password-form.png') });
    report.shots.push('settings2-secrets-01-password-form.png');
    ...
    await page.screenshot({ path: path.join(OUT, 'settings2-secrets-02-password-honest.png') });
    report.shots.push('settings2-secrets-02-password-honest.png');
    ...
    await page.screenshot({ path: path.join(OUT, 'settings2-secrets-03-passcode-on.png') });
    report.shots.push('settings2-secrets-03-passcode-on.png');
    ...
    await page.screenshot({ path: path.join(OUT, 'settings2-secrets-04-2fa-key.png') });
    report.shots.push('settings2-secrets-04-2fa-key.png');
    await page.evaluate(() => { const b = document.querySelector('#okoPopup [data-pa="1"]'); if (b) b.click(); });
    await sleep(300);
