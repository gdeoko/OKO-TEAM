...
inp=["-i",W+"/vfull.mp4",
     "-i",f"{W}/ig/accents.mov",
     "-i",f"{W}/ig/titles.mov",
     "-i",f"{W}/ig/subs.mov"]
fc=("[0:v][1:v]overlay=0:0:eof_action=pass:format=auto[oa];"
    "[oa][2:v]overlay=0:0:eof_action=pass:format=auto[ot];"
    "[ot][3:v]overlay=0:0:eof_action=pass:format=auto[v]")
cmd=["ffmpeg","-y"]+inp+["-i",W+"/audio.m4a","-filter_complex",fc,
     "-map","[v]","-map","4:a","-c:v","libx264","-preset","medium","-crf","20",
