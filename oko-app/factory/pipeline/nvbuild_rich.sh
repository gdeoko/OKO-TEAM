#!/bin/bash
# RICH no-voice reel builder (Библия §6 + ЗАКОН РАЗНООБРАЗИЯ).
# Adds coded infographic accents (build_accents.py: speedo/donut/vs/route/odometer/…)
# UNDER the kinetic titles, so every reel carries 6-8 distinct motion-graphics mechanics.
# Usage: nvbuild_rich.sh <WORKDIR> <SPEC.json>
# SPEC extends the no-voice spec with:
#   "accents": [ {t,e,type,x?,data}, ... ]   # 6-12 distinct mechanics, timed to the reel
#   "ovl_style": 0..4                          # per-reel visual style of the accent boxes
...
cp $ROOT/pipeline/compose_rich.py "$W"/compose.py       # accents+titles compose
...
# ---- content from spec (silent VO timing per beat) ----
python3 - "$SPEC" "$W" <<'PY'
...
# ---- footage: 12 vertical Pexels clips by meaning (curl=proxy-aware) ----
python3 - "$SPEC" "$W" <<'PY'
...
def search(q,page):
    ...
    r=subprocess.run(["curl","-s","--max-time","30","-H",f"Authorization: {KEY}",u],capture_output=True,text=True)
...
# ---- kinetic TITLES from spec, timed to timing.json ----
python3 - "$SPEC" "$W" <<'PY'
import json,sys,re
spec=json.load(open(sys.argv[1])); W=sys.argv[2]
...
open(p,"w").write(txt)
# body length for accent auto-timing fallback
json.dump({"total":T["content_end"]},open(W+"/reel_total.json","w"))
print("titles",len(out),"span",out[0]["start"],out[-1]["end"],"body",T["content_end"])
PY
python3 "$W/build_titles.py" >/dev/null 2>&1 || python3 "$W/build_titles.py"

# ---- RICH ACCENTS: build coded infographic overlays from spec.accents ----
export OVL_A=$(python3 -c "import json;s=json.load(open('$SPEC'));print(json.dumps(s.get('accents',[]),ensure_ascii=False))")
export OVL_STYLE=$(python3 -c "import json;s=json.load(open('$SPEC'));print(s.get('ovl_style',0))")
if [ "$OVL_A" != "[]" ] && [ -n "$OVL_A" ]; then
  python3 "$W/build_accents.py" >/dev/null 2>&1 && echo "accents.html built (style $OVL_STYLE)"
fi
...
# ---- capture RICH accents -> accents.mov (only if built) ----
if [ -f "$W/ig/html/accents.html" ]; then
  node "$W/capture.js" "$W/ig/html/accents.html" "$N" "$W/fr_acc" 2>&1 | tail -1
  ffmpeg -y -loglevel error -framerate 30 -start_number 0 -i "$W/fr_acc/f%03d.png" -c:v qtrle -pix_fmt argb "$W/ig/accents.mov"
  rm -rf "$W/fr_acc"
  echo "accents.mov OK"
fi
...
# ---- compose (accents + titles) ----
python3 "$W/compose.py" | tail -2
echo "NVBUILD_RICH_DONE $W"

# ---- prune old builds (keep just-built for QC) ----
SCR=$(dirname "$W")
find "$SCR" -maxdepth 1 -type d -name 'build*' 2>/dev/null | while read d; do
  if [ -f "$d/reel.mp4" ] && [ -n "$(find "$d/reel.mp4" -mmin +25 2>/dev/null)" ]; then
    rm -rf "$d/foot" "$d/segs" "$d/fr_titles" "$d/fr_acc" "$d/base.mp4" "$d/vfull.mp4" "$d/cover_cand" "$d/ig/titles.mov" "$d/ig/accents.mov" 2>/dev/null
  fi
done
echo "PRUNED old builds under $SCR"
