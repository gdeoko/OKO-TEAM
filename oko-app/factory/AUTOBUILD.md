...
## NO-VOICE БИЛДЕР (durable, Библия §6) — основной путь виральных роликов
Голос-блокер обойдён: виральные ролики БЕЗ диктора (тренд-музыка + большие кинетик-титры).
Полностью воспроизводимый билдер лежит в репо:
```
bash oko-app/factory/pipeline/nvbuild_novoice.sh <WORKDIR> <SPEC.json>
```
- `SPEC.json` — формат в `pipeline/spec_example.json`: `{batch, music, seed, beats[6]{vo,title[2],y}, footage[12], cover{...}, meta{title,yt_desc,caption}}`.
- `vo` — текст бита (задаёт тайминг тихим сегментом); `title` — 2 строки БОЛЬШОГО кинетик-текста (акцент-строка амбер).
- `footage[12]` — поисковые фразы Pexels ПО СМЫСЛУ бита (портрет, дедуп внутри ролика; b07 = фон обложки).
- Собирает: тихий VO → plan → 12 стоков curl-ом (прокси!) → кинетик-титры (тайминг из timing.json) → обложка (env) → assemble → capture titles.mov → audio (music-only) → compose_novoice (titles-only).
- Выход: `<WORKDIR>/reel.mp4` + `cover.jpg` + `meta.json`. QC-сетка `ffmpeg fps=1/2 tile=4x3`, затем в очередь VPS (ЭТАП 4).
- ЗАКОН РАЗНООБРАЗИЯ: каждая пара A/B — РАЗНЫЕ темы, ноль пересечений; не повторять тему из прошлых 3–4 (реестр USED.md). Микс квадро/мото основа, гидро изредка, спецтехника реже всего.
- Пути: footage только через `curl` (Pexels мимо прокси = 403). `NODE_PATH=/opt/node22/lib/node_modules` для capture.js.
