...
# IG на паузе до подключения владельцем? — не проверяем и не алертим (флаг ig_hold.flag)
if [ -f analysis/ig_hold.flag ]; then IGA="IG на паузе (владелец подключит)"; else
  ...
  if ! echo "$IGA" | grep -q ALIVE; then
    ...
    if [ ! -f "$MARK" ]; then bad "IG-сессия мертва — нужен вход в @tappio.pro"; touch "$MARK"; fi
  fi
fi
