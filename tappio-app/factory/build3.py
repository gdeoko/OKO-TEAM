...
def main():
    ...
    for i,sh in enumerate(shots):
        vis=sh.get("visual","stock")
        if str(vis).startswith("DEMO:"):
            src=f"appdemo/{vis[5:]}.mp4"
        else:
    ...
    # обложка кадром 0 (полностью видна, затем уходит)
    cov_i=N+1; inputs+=['-loop','1','-framerate','30','-t',f'{total:.2f}','-i',f"{wd}/cover_ov.png"]
    fc.append(f"[{cov_i}:v]format=rgba,fps=30,fade=out:st={LEAD-0.35:.2f}:d=0.35:alpha=1[cov]")
    ...
    af.append(f"[{ii}:a]adelay=150|150,volume=0.6[imp]")
    # whoosh на каждый переход между кадрами
    whs=[]
    for k2 in range(1,N):
        tcut=k2*(D-XD)+LEAD*0  # переход в бэде; бэд стартует с 0, обложка сверху первые LEAD
        tcut=k2*(D-XD)
        whs.append(f"[{wi}:a]atrim=0:0.5,adelay={int(tcut*1000)}|{int(tcut*1000)},volume=0.28[wh{k2}]")
    af+=whs
    whmix="".join(f"[wh{k2}]" for k2 in range(1,N))
    af.append(f"{whmix}amix=inputs={N-1}:normalize=0[whall]")
    af.append(f"[mus][voice1]sidechaincompress=threshold=0.03:ratio=6:attack=5:release=250[mduck]")
    af.append(f"[voice2][mduck][imp][whall]amix=inputs=4:normalize=0,loudnorm=I=-14:TP=-1.5:LRA=11[aout]")
