...
- Разведка конкурентов+миллионников с РЕАЛЬНЫМИ цифрами и КРАСИВЫМ документом в бота:
  `python3 КОНТЕНТ-ЗАВОД/factory/pipeline/competitor_analysis.py` (YouTube Data API: топ роликов >1M, лайки/комменты/ER, топ-10 каналов по подписчикам). Дополни качественным разбором хука/воронки в COMPETITORS.md.
