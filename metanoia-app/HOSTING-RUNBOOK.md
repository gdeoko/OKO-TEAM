# МЕТАНОЙЯ · HOSTING RUNBOOK (развёртывание backend)
...
## 9. Что уже проверено на настоящей базе (перепроверено 08.09.2026)

Проверяли на MariaDB и PHP 8 в контейнере разработки: схема, запросы и права
работают, значит на хостинге останется только вписать доступы. Весь набор
поднимается одной командой: `bash metanoia-app/tests/сервер.sh`.

| Что | Как проверяли | Итог |
|---|---|---|
| Схема | импорт `db/schema.sql` целиком | 43 таблицы без ошибок |
| Регистрация и вход почтой | из самого приложения в браузере | аккаунт, ребёнок и токен заводятся |
| Забытый пароль | письмо со ссылкой, смена, вход новым | старый пароль перестаёт работать, ссылка одноразовая |
| Перенос прогресса | два устройства, обычный вход и Телеграм | урок и имя приезжают на чистое устройство |
| Двое детей | передача устройства, второе устройство | у каждого свой прогресс, оба видны после входа |
| Удаление аккаунта | кнопка в профиле | в таблицах пусто: родитель, дети, прогресс, серии |
| Права | чужой `child_id`, запрос без токена | «Это чужой профиль», 401 |
| Предел частоты | 130 запросов подряд одним токеном | 429 после 120, остальное проходит |
| Пустой `JWT_SECRET` | подменили строку в `.env` | сервер честно отказывается пускать |
| Подтверждение почты | регистрация, ссылка из письма, кнопка «прислать ещё раз» | письмо уходит, ссылка одноразовая, строка в профиле пропадает |

Чего проверить нельзя было и придётся на хостинге: письма настоящей почтой,
вход через Google (нужен домен в консоли Google) и работа за Apache с FastCGI
(заголовок авторизации прокинут в `.htaccess`, но подтвердить это можно только
на живом сервере).
