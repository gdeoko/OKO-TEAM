# МЕТАНОЙЯ — подключение бэкенда (инструкция OKO TEAM)
...
3. Создать БД MySQL 8.0 / MariaDB, импортировать:
   - `db/schema.sql` (структура, 43 таблицы)

   Всё это делает за вас `bash tools/установка.sh`: проверяет версию PHP и
   расширения, создаёт `config/.env` со свежим ключом подписи, заливает схему,
   ставит права и дёргает `/api/v1/health`, чтобы сразу увидеть, живой ли сервер.
...
Восстановление пароля работает так: родитель нажимает «Забыли пароль?», сервер
кладёт в `password_resets` одноразовую ссылку на два часа и шлёт письмо. Открыл
ссылку, задал новый пароль, вошёл; все прежние входы гасятся.

Чтобы письма уходили, в `config/.env` нужны две строки:

```
APP_URL=https://ваш-домен.ру
MAIL_FROM=noreply@ваш-домен.ру
```

Пока их нет, письма пишутся в `config/письма.log` (файл вне public_html): школа
ничего не теряет, ссылку можно достать оттуда руками. На хостинге с почтой
хватает встроенной `mail()` PHP; внешний сервис (SendGrid, Unisender) нужен,
только если хостинг режет отправку.
