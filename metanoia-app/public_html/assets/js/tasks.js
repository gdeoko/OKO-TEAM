/* ══════════════════════════════════════════════════════════════
   ...
   Шесть типов: пары, порядок, найди на картинке, пропущенное слово, ребус,
   кроссворд (вписать слова по подсказкам).
...
(function () {
  ...
  // Автоматически собираем только первые пять: кроссворд нужен со своими
  // подсказками, его пишет педагог, из текста урока он не выводится.
  ...
  function разметка(з) {
    ...
    if (з.тип === 'words') {
      // Кроссворд на бумаге — клетки и подсказки. На телефоне рисовать сетку
      // некуда, поэтому оставляем главное: подсказку, число букв и строку ввода.
      return `<div class="task task--words" data-task="words">${шапка}
        <div class="task__words">${з.слова.map((с, i) => `
          <div class="tword" data-word-row="${i}">
            <div class="tword__q"><span class="tword__n">${i + 1}</span>${с.вопрос}</div>
            <input class="tword__in" data-word-in="${i}" type="text" inputmode="text"
              autocomplete="off" autocapitalize="characters" spellcheck="false"
              maxlength="${с.ответ.length + 4}"
              aria-label="Ответ на подсказку ${i + 1}"
              placeholder="${'•'.repeat(с.ответ.length)}">
          </div>`).join('')}</div>
        <div class="task__res" hidden></div></div>`;
    }
    return '';
  ...
  function оживить(корень, з, готово) {
    ...
    if (з.тип === 'words') {
      // Слово засчитываем, как только оно набрано целиком: ребёнок не ищет
      // кнопку проверки и сразу видит, что попал.
      const сошлось = (а, б) => сравнимо(а) === сравнимо(б);
      let собрано = 0;
      корень.querySelectorAll('[data-word-in]').forEach((поле) => {
        const i = Number(поле.dataset.wordIn);
        const строка = поле.closest('.tword');
        поле.addEventListener('input', () => {
          if (строка.classList.contains('tword--ok')) return;
          const набрано = поле.value;
          if (сошлось(набрано, з.слова[i].ответ)) {
            поле.value = з.слова[i].ответ;
            поле.readOnly = true;
            строка.classList.add('tword--ok');
            собрано++;
            if (собрано === з.слова.length) победа('Все слова на месте!');
            return;
          }
          // Подсказываем только когда букв уже хватает, чтобы не мигать на каждой.
          строка.classList.toggle('tword--miss',
            сравнимо(набрано).length >= сравнимо(з.слова[i].ответ).length);
        });
      });
    }
  }

  /* Сравниваем ответы мягко: регистр, ё и лишние пробелы значения не имеют. */
  function сравнимо(слово) {
    return String(слово || '').toLowerCase().replace(/ё/g, 'е').replace(/[^а-я]/g, '');
  }
