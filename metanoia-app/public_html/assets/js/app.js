...
const CHAT_MSGS = {
  ...
  // Правила чатов родителей и учеников. Тексты школьные, не про веру,
  // поэтому написаны нами и ждут только слова Екатерины: захочет иначе,
  // меняется одна строка.
  2: { pinned: 'Здесь взрослые. Делимся опытом и поддерживаем друг друга, о детях говорим бережно.',
    msgs: [
  ...
  3: { pinned: 'Здесь ученики. Радуемся успехам друг друга, не дразнимся и не делимся личным: адресом, школой, телефоном.',
    msgs: [
      { who: 'Миша', text: 'Я собрал стих за 20 секунд!', time: '09:41' },
      { who: 'Аня', sticker: 'assets/img/stickers/sparkles.jpg', time: '09:43' },
    ] },
  // Поддержка. Пока сервера чатов нет, написанное здесь остаётся на
  // устройстве, и обещать ответ было бы нечестно: закреплённая строка
  // говорит об этом прямо и подсказывает живой путь.
  4: { pinned: 'Срочный вопрос задайте Екатерине в Телеграме школы.',
    msgs: [
      { who: 'Поддержка', text: 'Здравствуйте! Напишите, что случилось, и мы ответим, как только школа подключит сервер. Если вопрос срочный, найдите Екатерину в Телеграме школы.', time: 'вчера' },
    ] },
};
let cvChatId = null;
...
function openChatView(i) {
  ...
  // Пока сервера чатов нет, написанное остаётся на устройстве. Говорим это
  // один раз в самом чате: молча делать вид, что сообщение ушло, нечестно.
  // Появится сервер (заполнен mt-api) — строка пропадёт сама.
  const серверЕсть = !!(document.querySelector('meta[name="mt-api"]') || {}).content;
  const оговорка = (!серверЕсть && !data.readonly)
    ? 'Пока школа без сервера, ваши сообщения видны только на этом устройстве.' : '';
  const закреп = [data.pinned, оговорка].filter(Boolean).join(' ');
  $('#cvPinned').hidden = !закреп;
  if (закреп) $('#cvPinnedText').textContent = закреп;
...
function openAbout() {
  $('#aboutBody').innerHTML = `
    ...
    <div class="about-ver">Версия ${ВЕРСИЯ_ПРИЛОЖЕНИЯ} · сделано OKO TEAM для школы «Метанойя»<br>
      ...
      <a href="terms.html" data-doc="terms">Пользовательское соглашение</a> ·
      <a href="udalenie.html" data-doc="delete">Удаление аккаунта</a></div>
  `;
...
function askMatch(text) {
  ...
  if (ASK_SENSITIVE.some((k) => t.includes(k))) {
    // Тяжёлые темы разбирает взрослый, а не приложение. Телефон доверия
    // общероссийский, детский, бесплатный и круглосуточный.
    return { care: true, a: 'Мне важно то, что ты сейчас чувствуешь, и ты правильно сделал, что сказал об этом. С таким лучше всего поговорить с мамой, папой или с Екатериной — они любят тебя и помогут. Ты не один, и это не твоя вина. Пожалуйста, обратись к близкому взрослому прямо сейчас. А если рядом взрослого нет, позвони на детский телефон доверия 8 800 2000 122: там бесплатно, в любое время суток, и можно не называть своё имя.', ref: 'Псалом 33:19' };
  }
...
function initGrowth() {
  ...
  // Удаление аккаунта. Требование RuStore, Google Play и App Store: то, что
  // семья завела в приложении, она должна уметь стереть сама, из самого
  // приложения. Спрашиваем дважды и пишем прямо, что пропадёт.
  $('#mWipe')?.addEventListener('click', async () => {
    const первый = confirm(
      'Удалить аккаунт?\n\nПропадут профили детей, пройденные уроки, награды, '
      + 'переписка и сертификаты. Вернуть их будет нельзя.'
    );
    if (!первый) return;
    if (!confirm('Точно удалить? Это последнее предупреждение.')) return;
    try {
      if (window.MT_SYNC && MT_SYNC.включён()) await MT_SYNC.удалитьАккаунт();
    } catch (e) {
      toast('Сервер недоступен, стираем данные на этом устройстве');
    }
    try {
      Object.keys(localStorage).filter((k) => k.startsWith('mt_'))
...
const ДОКИ = {
  privacy: {
    ...
    parts: [
      ['Кто обрабатывает данные',
       'Оператор персональных данных — онлайн-школа духовно-нравственного воспитания «Метанойя», Екатерина Павленко. Связаться можно через раздел «Поддержка» в приложении, а пока школа работает без сервера — через Телеграм школы.'],
      ...
      ['Сколько храним',
       'Пока у вас есть аккаунт. Удалить аккаунт и все данные можно в любой момент самим: профиль, пункт «Удалить аккаунт и данные». Профили детей, прогресс, награды и переписка стираются сразу и на сервере, и на устройстве. Резервные копии сервера очищаются в течение 30 дней.'],
      ['Ваши права',
       'Вы можете запросить, какие данные о вас хранятся, исправить их и отозвать согласие: обращение уходит через «Поддержку», отвечаем в течение трёх рабочих дней. Удалить аккаунт можно без обращения, кнопкой в профиле.'],
      ['Изменения',
  ...
  // Google Play требует, чтобы порядок удаления аккаунта был описан
  // отдельно и открывался ещё и страницей сайта: public_html/udalenie.html.
  delete: {
    title: 'Удаление аккаунта',
    sub: 'Что стирается, когда семья уходит',
    parts: [
      ['Как удалить самим, за минуту',
       'Профиль, внизу пункт «Удалить аккаунт и данные». Подтвердите. Аккаунт стирается сразу, писать никому не нужно.'],
      ['Что удаляется',
       'Аккаунт родителя, профили всех детей, прогресс по урокам и играм, награды и сертификаты, переписка с педагогом вместе с голосовыми, кружочками и снимками, настройки семьи. Стирается и на сервере, и на устройстве.'],
      ['Что остаётся и насколько',
       'В работе не остаётся ничего. Резервные копии сервера перезаписываются в течение 30 дней. Документы, которые школа обязана хранить по закону, например подтверждение оплаты, хранятся в установленные законом сроки.'],
      ['Если войти не получается',
       'Напишите с той же почты, на которую заведён аккаунт, в Телеграм школы или на почту поддержки, со словом «удаление». Удаляем в течение трёх рабочих дней и отвечаем письмом.'],
    ],
  },
};
