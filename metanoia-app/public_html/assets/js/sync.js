...
(function () {
  ...
  /**
   * Снимок состояния ребёнка.
   * @param {number} предел  сколько знаков разрешаем одной записи. Обычно
   *   120 000, но если сервер отказался принять снимок целиком, собираем
   *   облегчённый: тяжёлое здесь это переписка со снимками и голосовыми,
   *   ей место на сервере чатов, а прогресс важнее.
   */
  function снимок(предел) {
    const порог = предел || ПРЕДЕЛ_ЗАПИСИ;
    const s = {};
    for (let i = 0; i < localStorage.length; i++) {
      const k = localStorage.key(i);
      ...
      if (v === null || v.length > порог) continue;
      s[k] = v;
    }
  ...
  async function отправить() {
    ...
    try {
      ...
      try {
        ...
        // 413: снимок не влез. Обычно это переписка с фотографиями. Шлём
        // облегчённый, чтобы прогресс ребёнка всё-таки уехал.
        if (тяжело.code !== 413) throw тяжело;
        if (window.console) console.warn('Метанойя: снимок великоват, шлём без тяжёлых записей');
        await запрос('/progress/' + ребёнок(), {
          method: 'PUT',
          body: JSON.stringify({ rev: rev, state: снимок(20000) }),
        });
      }
