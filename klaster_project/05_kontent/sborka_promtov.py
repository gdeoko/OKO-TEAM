...
def _без_кавычек(з):
    ...
    """
    # Косая черта в заголовке это авторский перенос строки внутри одной фразы,
    # а не граница заголовка и подписи: «ПОЛГОДА / БЕЗ ОТГРУЗОК» это один
    # заголовок. Вынос второй половины в подпись убивал хук, проверено на
    # приёмке 27.08. Склеиваем пробелом, а пунктуацию правим в самом плане.
    з = з.replace(" / ", " ").strip()
    for л, п in (("«", "»"), ('"', '"'), ("“", "”")):
