...
(function(){
  ...
  if(form) form.addEventListener('submit',function(e){
    ...
    fetch(API,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)})
      ...
      .then(function(res){ if(res && res.ok!==false){ done(); } else { fail(); } })
      .catch(function(){ fail(); });
    function done(){ form.style.display='none'; ok.classList.add('show'); }
    function fail(){
      btn.disabled=false; btn.textContent='Отправить заявку';
      var n=form.querySelector('.form-err');
      if(!n){ n=document.createElement('div'); n.className='form-note form-err'; n.style.color='#c0392b'; form.appendChild(n); }
  ...
  var KB=[
    ...
    {q:'Что с КРТ и технопарком?',a:'Объект вне зоны КРТ, снос не грозит. Парк в процессе получения статуса технопарка: после присвоения резиденты смогут претендовать на профильные льготы. Эффект кластера работает уже сейчас: 100+ предприятий на площадке.'},
  ...
  var opened=false;
  function openCb(){cbPanel.classList.add('open');if(!opened){opened=true;botSay('Здравствуйте! Я помогу с вопросами об аренде в бизнес-парке «Кластер». Выберите тему из списка ниже.');renderQuick();}}
