...
const список = await стр.evaluate(() => {
  ...
  сцена.traverse((о) => {
    ...
    /* Круглое: кольцо и диск видны по геометрии, а плоскость с
       радиальным спадом - по куску шейдера.

       ── НО КРУГ ОБЯЗАН БЫТЬ КРУГЛЫМ НА ЭКРАНЕ ──────────────────
       Одного шейдера мало, и это поймано первым же прогоном: под
       признак попала дымка горизонта Луны. У неё в шейдере тот же
       радиальный спад, но сама плоскость растянута 9.74 на 0.22, то
       есть сорок четыре к одному, и на кадре это полоса у земли, а не
       круг. Проверяем отношение сторон НА ЭКРАНЕ: от двух пятых до
       двух с половиной. Тело за этими краями круглым не читается, чем
       бы его ни рисовали. */
    let круг = null;
    const гео = о.geometry ? о.geometry.type : "";
    const ш = x1 - x0, в = y1 - y0;
    const кругловат = ш > 0 && в > 0 && (ш / в) > 0.4 && (ш / в) < 2.5;
    if (гео === "RingGeometry") круг = "кольцо";
    else if (гео === "CircleGeometry") круг = "диск";
    else if (кругловат && м.fragmentShader &&
             /length\s*\(\s*vUv\s*-\s*(vec2\s*\(\s*)?0\.5/.test(м.fragmentShader)) {
      круг = "пятно шейдером";
    }
    из.push({
      круг,
      центр: [+((x0 + x1) / 2).toFixed(2), +((y0 + y1) / 2).toFixed(2)],
...
/* ── КРУГЛОЕ НАЗЫВАЕМ ПОИМЁННО ────────────────────────────────
   Владелец сказал «какой-то круг ВЕЗДЕ появляется», то есть беда не в
   одном акте. Искать её глазами по шести актам и двум темам долго, а
   круг в сцене узнаётся по признакам: это либо кольцо и диск своей
   геометрией, либо плоскость с радиальным спадом в шейдере - у таких
   в коде стоит `length(vUv - 0.5)`, и по этому куску их видно прямо в
   исходнике материала.

   Плюс место: круг, который мешает, стоит в середине кадра. Корни
   актов мир ставит в точку прицела позы, поэтому забытое тело садится
   ровно туда, и заметнее его нет ничего. */
console.log("  ── круглое в кадре ──");
let круглых = 0;
for (const т of список) {
  if (!т.круг) continue;
  круглых++;
  const серединой = Math.hypot(т.центр[0], т.центр[1]) < 0.35;
  console.log(`  ${т.круг}: ${т.вид}/${т.гео} ${т.имя ? "«" + т.имя + "» " : ""}${т.путь}` +
              ` · центр ${JSON.stringify(т.центр)}${серединой ? " (В СЕРЕДИНЕ КАДРА)" : ""}` +
              ` · ${Math.round(т.доляКадра * 100)}% кадра · ${т.смешение}`);
}
if (!круглых) console.log("  ничего круглого");
