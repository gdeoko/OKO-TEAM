/* ВЕСЬ ФИЛЬМ ОДНИМ ПРОГОНОМ: снимок на каждой доле каждого акта.
   ...
   ── ЧЕГО ЭТОТ ИНСТРУМЕНТ НЕ ПОКАЗЫВАЕТ ───────────────────────────
   Камера тут ставится ПРЫЖКОМ (`RV_MOTION.кПунктy`), и это меняет не
   только положение. Корабль финала собирается ПО ХОДУ прокола, прыжок
   этот ход пропускает, и на финале 0.08 и 0.50 сборщик снимает пустое
   звёздное поле без корабля. Снимает так устойчиво, четырежды подряд,
   и переснимать бесполезно: с точки зрения сцены там и правда пусто.

   Я попалась на это дважды и один раз успела сказать владельцу, что
   нашла дыру в концовке. Дыры нет. Тем же местом при живой прокрутке:
   одиннадцать узлов корабля в сцене, четыре видимых, на снимке корабль
   на площадке, текст сверху, кнопка снизу (`тел-финал-0.08.png`).

   Вывод: пустой кадр финала у ЭТОГО инструмента ничего не доказывает.
   Проверять надо колесом - `tools/ракета-при-живом-ходе.mjs`.
...
/* ── ПАПКА ЧИСТИТСЯ ПЕРЕД ПРОГОНОМ ──────────────────────────────
   Кадры прежних прогонов оставались лежать рядом с новыми, и списки
   актов у них разные: у старого были «рубка» и «пуск», которых в ленте
   давно нет. Я на этом уже дважды попалась - смотрела кадр двухдневной
   давности и делала по нему вывод о сегодняшней правке. Один раз
   решила, что подписи не обновились; на самом деле прогон до них ещё не
   дошёл, а в папке лежал снимок от позавчера.

   Снимок, по которому судят, обязан быть ТОЛЬКО из этого прогона. */
try { rmSync(КУДА, { recursive: true, force: true }); } catch (e) {}
...
/* ── ПУСТОТУ ОТЛИЧАЕТ СТРУКТУРА, А НЕ ЯРКОСТЬ ────────────────────
   Здесь стоял один порог: верхний процент яркости ниже сорока. Он
   пропустил ровно тот кадр, ради которого написан. Финал 0.64 на
   телефоне снялся ПУСТЫМ - одно звёздное поле без рубки, - и снялся так
   ДВАЖДЫ подряд, а прямой зонд на той же доле показал нормальную рубку
   со стенами, остеклением и пультом. То есть сцена цела, врёт съёмка, и
   я успела сказать владельцу, что нашла дыру в концовке.

   Замер трёх кадров объяснил, почему порог не сработал:

     пустой (ложь)   среднее 37.9  разброс  5.2  p99  47  ярче 60:  0.4%
     настоящий       среднее 57.4  разброс 35.1  p99 161  ярче 60: 43.6%
     финал 0.92      среднее 50.2  разброс 34.8  p99 218  ярче 60: 19.8%

   У звёздного поля верхний процент равен сорока семи - выше порога, и
   кадр проходил. А отличает пустоту не яркость, а СТРУКТУРА: разброс
   5.2 против 35, и доля точек ярче шестидесяти 0.4 процента против
   двадцати и сорока.

   Берём оба признака вместе. Разброс ниже двенадцати это ровная заливка
   с редкими точками, какой бы яркой она ни была; доля ярче шестидесяти
   ниже трёх процентов это кадр без предмета. Настоящий тёмный кадр по
   замыслу так не выглядит: у него есть кромки, текст и блики. */
function пусто(файл) {
  try {
...
w, h = im.size
k = im.crop((int(w*0.05), int(h*0.18), int(w*0.95), int(h*0.86)))
s = ImageStat.Stat(k)
d = list(k.getdata()); n = len(d)
p99 = sorted(d)[n*99//100]
ярче = sum(1 for x in d if x > 60) * 100.0 / n
print(round(s.stddev[0], 1), p99, round(ярче, 2))
`, файл], { encoding: "utf8" });
    const [разброс, p99, ярче] = из.trim().split(/\s+/).map(Number);
    return разброс < 12 || ярче < 3 || p99 < 40;
  } catch (e) {
