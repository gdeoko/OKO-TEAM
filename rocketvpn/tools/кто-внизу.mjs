...
/* ── ВЫДЕРЖКА ТА ЖЕ, ЧТО У «ДВУХ ФИНАЛОВ» ─────────────────────────
   Первый заход этой пробы ждал тридцать кадров и получил полосу 9.3 -
   почти соседские 7.1, то есть беды не увидел вовсе. А `два-финала` на
   том же месте ленты даёт 104.6, и отличались они ровно выдержкой:
   шестьдесят кадров, две секунды покоя, ещё двадцать кадров.

   Значит виновник приходит по ВРЕМЕНИ, а не по прокрутке: что-то в
   финале разгорается на своих часах. Проба, снимающая раньше, чем беда
   появилась, честно докладывает «чисто» - и это худший вид мерки.
   Выдержку повторяем слово в слово, иначе числа несравнимы. */
async function осесть(стр) {
...
async function снять(файл) {
  /* Между состояниями тоже даём осесть: беда приходит по времени, и
     пять кадров её не застанут. */
  await стр.evaluate(() => new Promise((г) => {
    let i = 0; (function ш() { requestAnimationFrame(() => (++i >= 12 ? г() : ш())); })();
  }));
