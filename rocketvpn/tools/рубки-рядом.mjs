...
/* ── ГАБАРИТ ПУЛЬТА ─────────────────────────────────────────────
   Три числа про глаз сошлись, а настил в кадре VPN светил, и по трём
   числам этого не объяснить: зритель стоит одинаково. Значит вещь, на
   которую он смотрит, собрана по-разному.

   Мировой габарит и экранный разделяют два случая. Разошёлся мировой -
   дело в параметрах сборки (`RC_CABIN.build`): пульт физически другой.
   Мировой сошёлся, а экранный нет - дело в позе пульта: одна и та же
   вещь стоит под другим углом или на другом месте. Без такого деления
   правка была бы подбором.

   Порог по мирового габариту сантиметровый: пульт метровой ширины,
   и один процент по нему это уже процент кадра. Экранный меряем в
   долях кадра, порог два процента - меньше человек не различает. */
const ДОП_ПУЛЬТ_МИР = 0.02;
const ДОП_ПУЛЬТ_КАДР = 0.02;
if (впн["пультМир"] && кдн["пультМир"]) {
  const оси = ["ширина", "высота", "глубина"];
  for (let i = 0; i < 3; i++) {
    const а = впн["пультМир"][i], б = кдн["пультМир"][i];
    const р = +(а - б).toFixed(4);
    const плохо = Math.abs(р) > ДОП_ПУЛЬТ_МИР;
    console.log(`  ${("пульт мир, " + оси[i]).padEnd(22)} VPN ${String(а).padEnd(9)} CDN ${String(б).padEnd(9)}` +
                ` разница ${(р > 0 ? "+" : "") + р}` + (плохо ? "  ← за порогом " + ДОП_ПУЛЬТ_МИР : ""));
    if (плохо) беды.push(`пульт мир ${оси[i]}: ${а} против ${б}, разница ${р}`);
  }
  строка("пульт до глаза", "пультДоГлаза", ДОП_ДИСТАНЦИЯ, "");
} else {
  /* Печатаем ПРИЧИНУ, а не «не отдала». Мерка называет её словом
     (`пультНет`): «узла пульта нет», «console3 есть без group»,
     «коробка пуста». Без этого отказ отправлял искать наугад, и на
     угадывании тут уже ушло два круга. */
  console.log(`  ${"пульт мир".padEnd(22)} VPN ${впн["пультНет"] || "нет причины"}` +
              `  ·  CDN ${кдн["пультНет"] || "нет причины"}`);
  беды.push(`габарит пульта не измерен: VPN ${впн["пультНет"] || "?"}, ` +
            `CDN ${кдн["пультНет"] || "?"}`);
}
if (впн["пультКадр"] && кдн["пультКадр"]) {
  for (const к of ["ширина", "высота", "верх", "низ"]) {
    const а = впн["пультКадр"][к], б = кдн["пультКадр"][к];
    const р = +(а - б).toFixed(4);
    const плохо = Math.abs(р) > ДОП_ПУЛЬТ_КАДР;
    console.log(`  ${("пульт кадр, " + к).padEnd(22)} VPN ${String(а).padEnd(9)} CDN ${String(б).padEnd(9)}` +
                ` разница ${(р > 0 ? "+" : "") + р}` + (плохо ? "  ← за порогом " + ДОП_ПУЛЬТ_КАДР : ""));
    if (плохо) беды.push(`пульт кадр ${к}: ${а} против ${б}, разница ${р}`);
  }
} else {
  console.log("  пульт кадр           не измерен: " +
              `VPN ${JSON.stringify(впн["пультКадр"])}, CDN ${JSON.stringify(кдн["пультКадр"])}`);
}
