...
if (впн["пультМир"] && кдн["пультМир"]) {
  ...
  console.log("  пульт мир            мерка не отдала габарит" +
              ` (VPN ${JSON.stringify(впн["пультМир"])}, CDN ${JSON.stringify(кдн["пультМир"])})`);
  беды.push("габарит пульта не измерен ни на одной стороне");
}
