...
/* Ждём мир, а не вступление: вступление на программном отрисовщике
   идёт минутами, а поза акта ставится помимо него. */
await стр.waitForFunction(
  () => window.RV_MOTION && window.RV_MOTION["кПунктy"],
  null, { timeout: 240000 }).catch(() => console.log("плёнки не дождались"));
