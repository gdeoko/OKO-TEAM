  ...
  return стр.evaluate(() => {
    ...
    if (м && м.scene) м.scene.traverse(function (о) {
      ...
      for (const к in мат.uniforms) {
        ...
        if (typeof з === "number") { уни[к] = +з.toFixed(4); continue; }
        /* ── ВЕКТОРЫ ТОЖЕ, И ЭТО НЕ МЕЛОЧЬ ────────────────────────
           Первый заход печатал только числа и объявил, что материал в
           порядке: разошлись лишь uTime и uShov на две сотых. А дом на
           снимке чёрный. Направление солнца, цвет камня и положение
           камеры это ВЕКТОРЫ, и ровно они решают, сколько света упадёт
           на блок. Пропустить их значило опросить материал и не
           спросить главного. */
        if (з && typeof з.x === "number") {
          уни[к] = [з.x, з.y, з.z].map((ч) => +(+ч).toFixed(3)).join(",");
        } else if (з && typeof з.r === "number") {
          уни[к] = [з.r, з.g, з.b].map((ч) => +(+ч).toFixed(3)).join(",");
        }
      }
    });
