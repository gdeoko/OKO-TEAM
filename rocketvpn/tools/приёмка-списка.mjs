...
for (const к of КАДРЫ) {
  ...
  /* ── «на стартовой загрузке нельзя скроллить, пока камера не
       отпустится» ──────────────────────────────────────────────── */
  /* Замок ставит мир, а не разметка, и между отрисовкой страницы и
     первым кадром мира проходит момент. Первый заход читал признак
     сразу после domcontentloaded и в одном прогоне из четырёх видел
     ещё не поставленный замок. Ждём его появления до двух секунд:
     дольше это уже настоящая беда, короче - наша спешка. */
  const замокБыл = await стр.waitForFunction(
