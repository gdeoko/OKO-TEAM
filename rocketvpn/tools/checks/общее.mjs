...
/* ── ЛЕНТУ КРУТИМ ЕЙ САМОЙ, А НЕ ОКНУ ───────────────────────────
   Правило написано ценой пяти слепых проверок. Прокрутка у сайта своя
   (rv-скролл.js): лента живёт в контейнере, документ прибит к окну и
   всегда отдаёт высоту экрана. Замер 14.09.2026:

     document.documentElement.scrollHeight   900
     RV_СКРОЛЛ.лента()                     21510
     window.scrollTo(0, 10755)   ->  доли актов 0, y 0
     RV_СКРОЛЛ.к(10755)          ->  станция 1, периметр 1, прокол 0.104

   То есть `window.scrollTo` не двигает НИЧЕГО. Проверки, которые им
   гоняли ленту (обратно, полёт, посадка, скорость, ход-камеры),
   сравнивали один и тот же неподвижный кадр сам с собой и печатали
   хорошие числа. Такая проверка не находит ничего и при этом выглядит
   работающей, а это хуже её отсутствия: на неё ссылаются как на
   доказательство.

   Поэтому проводка ленты живёт ЗДЕСЬ, одной парой функций на все
   проверки: шестая, которую напишут завтра, возьмёт готовое и в ту же
   яму не попадёт. Запасной путь на документ оставлен - тем же набором
   проверяется предпоказ, где ленты может не быть вовсе. */
export async function лентаВсего(pg) {
  return pg.evaluate(() => {
    const с = window.RV_СКРОЛЛ;
    if (с && с["лента"] && с["высота"]) return Math.max(0, с["лента"]() - с["высота"]());
    return Math.max(0, document.documentElement.scrollHeight - window.innerHeight);
  });
}

export async function кПрокрутке(pg, y) {
  await pg.evaluate((цель) => {
    const с = window.RV_СКРОЛЛ;
    if (с && с["к"]) { с["к"](цель, false); return; }
    window.scrollTo(0, цель);
  }, Math.round(y));
}

export async function кДолеЛенты(pg, доля) {
  const всего = await лентаВсего(pg);
  await кПрокрутке(pg, всего * доля);
}
...
export async function кАкту(pg, имя, доля = 0.5) {
  ...
  /* ── ЖДЁМ КАДРАМИ, А НЕ СЕКУНДАМИ ──────────────────────────────
     Половина сцены живёт по времени, а не по прокрутке: превращение
     камня в металл, догон света, створки. На программном отрисовщике
     кадр идёт втрое дольше секунды, и полутора секунд ожидания хватает
     на четыре кадра вместо полусотни.

     Цена этого поймана проверкой света: на телефоне в финале 0.47 и
     0.55 она мерила кадр, в котором лампы корабля ещё не разгорелись,
     и объявляла салон тёмным - максимум 61 при пороге 90. Снимок того
     же места (tools/кадр.mjs ждёт девяносто кадров) показывал светлую
     рубку с горящим иллюминатором.

     Восемь кадров поверх секунд, а не тридцать: тридцать на восьмидесяти
     замерах программного отрисовщика съели четверть часа, и проверка
     умерла по своему же потолку. Восемь закрывают догон, которому
     осталось чуть-чуть, а длинные переходы чинятся в самой сцене, а не
     ожиданием (см. RV_КОМНАТА.победа, второй довод). */
...
/* ── СПИСОК АКТОВ БЕРЁТСЯ У СТРАНИЦЫ, А НЕ ИЗ ПАМЯТИ ─────────────
   Проверки света и контраста дважды краснели мимо цели, потому что
   держали имена актов у себя. В первый раз в их списках стояли
   «оболочка», «выход» и «стыковка» - таких актов не было в разметке
   уже давно, и обе проверки честно печатали проценты, ни разу не
   посмотрев на первый экран, на стену и на финал. Во второй раз там
   остались «рубка» и «пуск», снятые с ленты вместе со своей сценой.

   Ядро на незнакомое имя не падает, поэтому пропуск молчит. Теперь имена
   спрашиваются у самой страницы: сколько актов в разметке, столько и
   проверяется, и новый акт попадает под проверку в тот же день, когда
   появился. */
export async function актыСтраницы(pg) {
  return pg.evaluate(() =>
