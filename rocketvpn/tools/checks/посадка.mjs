...
/* Акты читаются ИЗ РАЗМЕТКИ. Здесь стоял список прежнего сценария -
   «видно», «оболочка», «выход», «стыковка», - и после того как
   владелец переписал сайт на пять экранов, проверка вставала на акты,
   которых нет: страница оставалась там, где была, а замер честно
   докладывал про чужой кадр. Сценарий будет меняться и дальше. */
let АКТЫ = [];
...
async function замер(pg) {
  return pg.evaluate(() => {
    ...
    /* ── УБРАНО С ЭКРАНА НАМЕРЕННО ───────────────────────────────
       Два приёма прячут узел от глаза, оставляя его в дереве, и оба
       законные. Проверка их не знала и находила по ним сотни бед.

       1. «Для скринридера»: размер в одну точку, overflow hidden и
          clip-path inset(50%). Так прячется разметка слов, когда они
          подняты в объём: её читают поисковик и голосовой помощник, а
          человеку слова показывает сцена. Сам такой предок отсеивался
          по размеру, а вот его ДЕТИ нет: getBoundingClientRect отдаёт
          разметочный прямоугольник, и `li` внутри коробки в одну точку
          честно докладывал свои две тысячи. Отсюда «вылезло за экран -
          li по ширине -1..2050 при 360». Отсюда же и «обрезан текст -
          h2 текст 60 в коробке 1»: у коробки в одну точку прокрутка
          больше её высоты по определению.

       2. Ловушка для роботов: поле формы, уведённое за левый край на
          девять тысяч точек. Робот его заполняет, человек не видит.
          Проверка докладывала «вылезло - div.rv-ловушка -9298..-9297»,
          то есть ругала защиту от спама за то, что она работает.

       Обе проверяются одной функцией: узел не на экране по замыслу. */
    const внеЭкрана = (э) => {
      let п = э;
      for (let i = 0; i < 12 && п && п !== document.body; i++, п = п.parentElement) {
        const с2 = getComputedStyle(п);
        const r2 = п.getBoundingClientRect();
        const режет = с2.overflow === "hidden" || с2.overflow === "clip" ||
                      с2.overflowX === "hidden" || с2.overflowY === "hidden";
        if (режет && (r2.width <= 2 || r2.height <= 2)) return true;
        if (с2.clipPath && с2.clipPath.indexOf("inset(50%") === 0) return true;
        /* Уведено далеко за кромку: ловушка, а не вёрстка. Тысяча
           точек это заведомо больше любого законного сдвига на входе
           акта и заведомо меньше девяти тысяч у ловушки. */
        if (r2.right < -1000 || r2.left > вид.ш + 1000) return true;
      }
      return false;
    };
    ...
    /* 4. Слова не должны уезжать под кромку окна.
       ...
       ── КОГДА СЛОВА В ОБЪЁМЕ, ЗДЕСЬ ИХ НЕТ ВОВСЕ ─────────────────
       При признаке `рв-слова-в-сцене` заголовок, подзаголовок и
       карточки рисует мир буквами (rv-слово3d.js), а разметка остаётся
       свёрнутой для поисковика. В слое от слов не остаётся ничего:
       только кнопки, и стоят они внизу законно.

       Правило после этого мерило кнопки и честно докладывало «слова
       прижаты к краю - сверху 698» на всех семи размерах разом. Это
       последние семь бед из двух тысяч восьмисот восьмидесяти четырёх,
       и они того же рода: мерилось не то, что нарисовано.

       Где стоят слова В ОБЪЁМЕ, держит замок зоны в самом слое слов
       (владелец: «размер текста - 1/4 часть сверху, не залазит на 3D
       ни в 1 сцене, ни во 2, ни в 3»), и он собран так, что блок
       ужимается, пока не влезет. Здесь остаётся запасной путь: если
       объём не собрался, слова лежат разметкой, и правило работает как
       работало. */
    let уехал = null;
    const вОбъёме = document.documentElement.classList.contains("рв-слова-в-сцене");
    const кадрВОкне = кр.top <= 2 && кр.bottom >= вид.в - 2;
    if (слой && кадрВОкне && !вОбъёме) {
      ...
      for (const д of слой.children) {
        ...
        /* Свёрнутая «для скринридера» разметка сюда не считается: её
           коробка ровно в одну точку проходила условие `height < 1`
           (единица не меньше единицы) и тянула объединённый
           прямоугольник к самому верху кадра. Отсюда «слова -6..13 при
           высоте окна 640» и «прижаты к краю - сверху 0»: мерился не
           блок слов, а невидимая подложка для поисковика. */
        if (внеЭкрана(д)) continue;
        есть = true;
...
for (const э of ЭКРАНЫ) {
  ...
  const подвал = await pg.evaluate(() => {
    ...
    for (const э of п.querySelectorAll("*")) {
      ...
      /* Ловушка для роботов уведена за левый край на девять тысяч
         точек: робот её заполняет, человек не видит. Ругать защиту от
         спама за то, что она работает, незачем. Тысяча точек это
         заведомо больше любого законного сдвига и заведомо меньше
         девяти тысяч у ловушки. */
      if (r.right < -1000 || r.left > innerWidth + 1000) continue;
