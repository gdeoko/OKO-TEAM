...
/* Плотность 3, а не 2: у телефона владельца она около трёх, и весь
   разбор контраста надо вести на том буфере, который он видит. */
const ТЕЛЕФОН = { имя: "телефон", vp: { width: 390, height: 844 }, dpr: 3, mob: true };
const ПК = { имя: "ПК", vp: { width: 1440, height: 900 }, dpr: 1, mob: false };

/* ── ЧЕТЫРЕ ИМЕНИ ИЗ ЭТОГО СПИСКА НЕ СУЩЕСТВУЮТ ────────────────────
   Стояли «видно», «оболочка», «выход» и «стыковка». В разметке
   таких актов нет: лента идёт станция, периметр, прокол, рубка,
   пуск, финал. Ядро на незнакомое имя не падает, и проверка честно
   меряла контраст НЕ ТАМ - в том числе ни разу на первом экране и ни
   разу в финале.

   То же самое нашлось и в проверке света. Имена сверены с
   `frame.html`; держать их надо оттуда, а не из памяти. */
/* Доли, на которых меряем каждый акт. Имена актов сюда НЕ пишутся:
   их отдаёт сама страница (общее.актыСтраницы), и пропустить акт
   молча больше нельзя. */
const ДОЛИ = [0.15, 0.50, 0.90];
...
for (const э of [ПК, ТЕЛЕФОН]) {
  ...
  const МЕСТА = [];
  for (const имя of акты) for (const д of ДОЛИ) МЕСТА.push([имя, д]);
