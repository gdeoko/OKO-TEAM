...
/* ── ГДЕ КАДР ОБЯЗАН СТОЯТЬ ─────────────────────────────────────
   Одно место на ленте стоит НАМЕРЕННО, и проверка про это знать
   обязана: иначе она вечно красная там, где сайт прав.

   Владелец дословно: «панель управления это финал каждого из сайтов
   дальше нее ничего не скроллится вниз вообще». Хвост финала это и есть
   та панель. Кадр там держится, человек читает вопросы и жмёт кнопки,
   и движение в этот момент мешало бы, а не помогало.

   Исключение узкое: один акт, один последний шаг. Всё остальное, включая
   голову того же финала, меряется как прежде. Список лежит здесь, а не
   разбросан по коду, чтобы каждое послабление было видно одним взглядом
   и со своей причиной. */
const СТОИТ_ПО_ЗАМЫСЛУ = [
  { "акт": "финал", "от": 0.86, "до": 1.00,
    "почему": "панель финала: дальше неё лента не идёт, кадр держится" }
];
function ждёмПокоя(акт, от, до) {
  return СТОИТ_ПО_ЗАМЫСЛУ.some((и) =>
    и["акт"] === акт && Math.abs(и["от"] - от) < 0.005 && Math.abs(и["до"] - до) < 0.005);
}
...
for (const акт of акты) {
  ...
  for (let i = 0; i < шаги.length; i++) {
    if (шаги[i] < ПОРОГ) {
      const от = i / (ШАГОВ - 1), до = (i + 1) / (ШАГОВ - 1);
      if (ждёмПокоя(акт, от, до)) {
        console.log(`   покой по замыслу: акт ${акт} на ${от.toFixed(2)}..${до.toFixed(2)} ` +
          `(${шаги[i].toFixed(1)}) - ` + СТОИТ_ПО_ЗАМЫСЛУ.find((и) => и["акт"] === акт)["почему"]);
        continue;
      }
      беды.push(`акт ${акт}: между долями ${от.toFixed(2)} и ${до.toFixed(2)} кадр почти не меняется (${шаги[i].toFixed(1)} при пороге ${ПОРОГ})`);
    }
  }
}
