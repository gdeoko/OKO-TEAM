...
/* ── СОСЕДНИЕ СТРОКИ ОДНОГО БЛОКА НЕ НАЛОЖЕНИЕ ───────────────────
   Заголовок в три строки это ТРИ отдельных меша поля расстояний, и их
   рамки соприкасаются по вертикали на несколько точек - это межстрочный
   просвет, а не беда. Первый заход считал такие пары наложениями, и на
   телефоне из сорока двух находок больше половины были ими: «Стена
   стоит.» против «Rocket её», «Rocket её» против «пробивает».

   Отличаем по трём признакам сразу, и все три должны сойтись:
   оба тела объёмные, они из одного акта и одной группы (то есть один
   блок текста), и перекрытие по высоте мелкое - меньше четверти
   меньшей рамки. Настоящее наложение строки на строку так не выглядит:
   там перекрываются десятки точек. */
function строкиОдногоБлока(а, б, в) {
  if (а.вид !== "объём" || б.вид !== "объём") return false;
  if (а.путь !== б.путь || а.группа !== б.группа) return false;
  const меньшая = Math.min(а.y1 - а.y0, б.y1 - б.y0);
  return меньшая > 0 && в < меньшая * 0.25;
}
...
function перекрытие(а, б) {
  ...
  if (строкиОдногоБлока(а, б, в)) return null;
  return { ш: Math.round(ш), в: Math.round(в) };
}

/* Надпись, вылезшая за кромку кадра, это отдельная беда и своя находка:
   рамка может ни с кем не пересекаться и при этом быть обрезанной. */
function заКромкой(т, ш, в) {
  const слева = т.x0 < 0 ? -т.x0 : 0;
  const справа = т.x1 > ш ? т.x1 - ш : 0;
  const сверху = т.y0 < 0 ? -т.y0 : 0;
  const снизу = т.y1 > в ? т.y1 - в : 0;
  const макс = Math.max(слева, справа, сверху, снизу);
  return макс > ЗАПАС ? { слева: Math.round(слева), справа: Math.round(справа),
                          сверху: Math.round(сверху), снизу: Math.round(снизу) } : null;
}
...
for (const акт of акты) {
  for (const д of ДОЛИ) {
    ...
    const срезаны = тела.map((т) => [т, заКромкой(т, экран.width, экран.height)])
                        .filter((п) => п[1]);
    if (!пары.length && !срезаны.length) {
      console.log(`  ${акт} ${д}: чисто (надписей в кадре ${тела.length})`); continue;
    }
    грязно += пары.length + срезаны.length;
    console.log(`  ${акт} ${д}: НАЛОЖЕНИЙ ${пары.length}, ЗА КРОМКОЙ ${срезаны.length}` +
                ` (надписей ${тела.length})`);
    for (const [т, к] of срезаны) {
      const куда = [];
      if (к.слева) куда.push(`слева на ${к.слева}`);
      if (к.справа) куда.push(`справа на ${к.справа}`);
      if (к.сверху) куда.push(`сверху на ${к.сверху}`);
      if (к.снизу) куда.push(`снизу на ${к.снизу}`);
      console.log(`      за кромкой ${куда.join(", ")}: ${т.вид} [${т.x0},${т.y0}..${т.x1},${т.y1}] «${т.имя}»`);
    }
