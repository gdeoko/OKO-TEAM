...
(function (g, d) {
  ...
  /* ── ФОРМЫ ОБРАТНОЙ СВЯЗИ НА VPN НЕТ ────────────────────────────
     Слово руководства через владельца: «форму обратной связи не нужно
     на VPN, а на CDN можно оставить». На сервере дверь уже закрыта -
     заявка с этой площадки получает отказ (`lead_sites` в конфиге
     соседнего сайта), и страница обязана совпадать с сервером: форма,
     которая собирает данные и получает отказ, хуже отсутствующей.

     Держим одним выключателем, а не вырезанным кодом. Входов у формы
     четыре - значок в шапке, строка в меню, плита в подвале и анкета в
     пульте финала, - и вырезать их по одному значит однажды пропустить
     пятый. Передумают - строка возвращается на место, и все четыре
     входа оживают разом. */
  var ФОРМА_ЕСТЬ = false;
  ...
  /* ── ЧУЖОГО ДОМЕНА В МЕНЮ БОЛЬШЕ НЕТ ──────────────────────────
     Стояли три адреса на rocketcdn.ru: сам сайт, игра и приём заявок.
     Первые два были кнопками, третий срабатывал сам - форма уносила
     имя человека и его способ связи на соседний домен у него на
     глазах.

     Заказчик просил закрыть дорожку между сайтами. Игра переехала на
     свой домен, кнопка ухода снята, заявки идут на свой приёмник, а
     тот пересылает их на общую панель уже с сервера. Свод по двум
     сайтам считается там же, где считался. */
  var ИГРА = "./?flight=1";
  var API  = "api.php";
      ...
      '<div class="rv-шапка-право">' +
        ...
        '<nav class="rv-шапка-органы" aria-label="Быстрые действия">' +
          (ФОРМА_ЕСТЬ
            ...
            : '') +
          '<a class="rv-орган" id="rvОрганБот" href="' + СВЯЗЬ.бот + '" target="_blank" rel="noopener" aria-label="Открыть бот RocketVPN">' + З.проба + '</a>' +
          '<a class="rv-орган" id="rvОрганИгра" href="' + ИГРА + '" aria-label="Играть, полёт в космосе">' + З.игра + '</a>' +
  ...
  function собрать() {
    ...
    var дела = [
      /* Срок везде сутки, и ведём в личный кабинет на сайте: цель
         руководства - довести человека до регистрации, а бот дать
         отдельной строкой, а не вместо неё. */
      { знак: З.проба,  имя: "Тест-драйв 24 часа", под: "тариф выбирается в кабинете после регистрации", адрес: СВЯЗЬ.кабинет, вне: true, главное: true },
      { знак: З.вход,   имя: "Открыть в Telegram", под: "бот RocketVPN",                 адрес: СВЯЗЬ.бот, вне: true },
      ...
      { знак: З.игра,   имя: "Играть",          под: "полёт в космосе",                 адрес: ИГРА }
  ...
  /* Копий формы на сайте три: окно из шапки, плита в подвале и пульт
     внутри ракеты. Поля у всех одни (поляФормы) и обработчик один
     (формаСлать) - три набора полей разошлись бы на первой же правке.
     Разное у них только заголовок, подпись и класс оформления. */
  function формаВГнездо(ид, класс, заг, под) {
    var гнездо = d.getElementById(ид);
    if (!гнездо || гнездо.querySelector("form")) return;
    var ф = d.createElement("form");
    ф.className = "rv-форма-плита " + класс;
    ф.setAttribute("novalidate", "");
    ф.innerHTML =
      (заг ? '<h2 class="rv-форма-заг">' + заг + "</h2>" : "") +
      (под ? '<p class="rv-форма-под">' + под + "</p>" : "") +
      поляФормы();
  ...
  function формаВПодвале() {
    формаВГнездо("rvСвязь", "rv-форма-вподвале", "Написать нам",
      "Отвечаем на почту или в Телеграм. Обычно в тот же день.");
    /* Пульт финала. Заголовок у него свой стоит в разметке рядом, и
       второй тут был бы дублем: на панели управления места на одну
       строку, а не на две. */
    формаВГнездо("rvПультАнкета", "rv-форма-впульте", "", "");
  }

  /* ── ССЫЛКИ «#форма» ОТКРЫВАЮТ ОКНО, А НЕ ВЕДУТ В НИКУДА ────────
     На первом экране стоит вторая кнопка «Оставить заявку» с
     href="#форма" (frame.html), и якоря с таким именем в разметке НЕТ
     ни одного. Нажатие не делало ничего: адресная строка получала
     решётку, страница оставалась на месте.

     Подвал ей тоже не подходит. Пока лента идёт объёмным фильмом,
     подвал снят из потока (rv.css: html.рв-слова-в-сцене .rv-подвал
     display none), и форма в нём недостижима. Достижимо только окно, и
     оно открыто всегда.

     Слушаем на документе, а не на кнопке: кнопок с этим адресом может
     стать больше, а разметку пишут в другом файле. */
  d.addEventListener("click", function (со) {
    var ц = со.target;
    if (!ц || !ц.closest) return;
    var сс = ц.closest('a[href="#форма"], [data-к-форме]');
    if (!сс) return;
    /* Формы нет - и ссылки на неё быть не должно. Гасим переход, чтобы
       человек не получил решётку в адресной строке вместо ответа, а
       уводим туда, где ему действительно ответят. */
    со.preventDefault();
    if (!ФОРМА_ЕСТЬ) { g.open(СВЯЗЬ.помощь, "_blank", "noopener"); return; }
    формаОткрыть();
  }, false);
  ...
  function старт() { собрать(); if (ФОРМА_ЕСТЬ) формаВПодвале(); }
