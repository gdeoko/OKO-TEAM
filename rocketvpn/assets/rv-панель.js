...
/* Списки команд здесь не держим намеренно. Их было два, они
   ...
   подпись на крышку всё равно берётся из RV_КЛАВИШИ.name по номеру
...
function keycaps(T, api, proj, fit, dep, inner, tgt, parent, ru, tiny, aniso) {
  ...
  var КЛ = (g.RV_КЛАВИШИ && g.RV_КЛАВИШИ.KEYS) ? g.RV_КЛАВИШИ.KEYS.length : 11;
  ...
  var drawn = g.RV_КЛАВИШИ ? g.RV_КЛАВИШИ.atlas(cellPx, ru) : null;
  if (!drawn) return [];
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    m.userData.halfW = hw;
    m.userData.halfH = hh;
    m.userData["имя"] = g.RV_КЛАВИШИ.name(i, ru);
...
function flatBuild(T, o, api, inner3, portrait, W, H, proj) {
  ...
  var имя = (g.RV_НАСТИЛ && g.RV_НАСТИЛ["какой"]) ? g.RV_НАСТИЛ["какой"](W, H)
  ...
  var п = (g.RV_НАСТИЛ && g.RV_НАСТИЛ["покрытие"])
    ? g.RV_НАСТИЛ["покрытие"](meta, W, H) : null;
...
function build(T, o) {
  ...
  var aniso = 8;
  try {
    var rr = g.RV_REAL && g.RV_REAL.renderer;
  ...
  try {
    if (g.RV_REAL && g.RV_REAL.env) { mat.envMap = g.RV_REAL.env; mat.envMapIntensity = 0.55; }
  } catch (eE) {}
  api.mat = mat;
