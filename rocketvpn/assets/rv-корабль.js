...
function buildDoor(C, env, hullMat) {
  ...
  /* ── ИМЯ НУЖНО, ЧТОБЫ ЭТОТ САЛОН МОЖНО БЫЛО ПОГАСИТЬ ─────────
     Эта комната за люком принадлежит МОДЕЛИ КОРАБЛЯ и нужна там, где
     корабль стоит сам по себе: створки разъезжаются, и в проёме видна
     комната, а не тёмный прямоугольник.

     В финале у нас СВОЯ рубка (rv-финал.js, узел «салон»), и она стоит
     в том же корпусе. Два интерьера в одной ракете человек видит как
     кашу: луч сквозь открытый люк на доле 0.50 попадал сначала в нашу
     стену на 11.8, а следом в ЭТИ тела на 13.57 и 13.62 - тёмные рамы
     `#26313a` и светлый настил `#9aa6af`. Владелец: «в открытых дверях
     видно кашу в салоне салон не тот что внутри».

     Без имени погасить было нечем: в дереве узлы шли безымянными
     `Group/Group/Group/Mesh`, и перебор по именам их не находил. */
  cabin.name = "салон корабля CDN";
...
g["RV_КОРАБЛЬ"] = {
  ...
  /* ── КОРПУС ГАСИТСЯ ОТДЕЛЬНО ОТ ПЛОЩАДКИ ─────────────────────
     До зала человек приходит по тоннелю, и на площадке его ждёт
     КАМЕННАЯ ракета, сложенная из тех же кирпичей (rv-ракета-кирпичи).
     Металлический корабль там лишний: два корпуса в одной точке.

     Гасить весь узел нельзя - вместе с кораблём уходит площадка, и
     каменная ракета повисает над полом без опоры. Снимок прокола 0.93
     показал ровно это. Гасим корпус, площадка остаётся стоять. */
  "корпусВидно": function (да) {
    if (собрано && М.ракета && М.ракета.root) М.ракета.root.visible = !!да;
  },

  /* ── СВОЙ САЛОН КОРАБЛЯ, ОТДЕЛЬНО ОТ КОРПУСА ─────────────────
     Нужен там, где корабль стоит сам по себе: створки разъезжаются, и
     в проёме видна комната. Не нужен там, где в тот же корпус вставлена
     ЧУЖАЯ рубка - в финале это наш узел «салон» (rv-финал.js), и два
     интерьера в одной ракете читаются кашей.

     Спрашивает это финал, а не модуль сам: корабль не знает, поставили
     ли в него чужую рубку, и решать за него нельзя. */
  "салонВидно": function (да) {
    if (!собрано || !М.ракета || !М.ракета.door || !М.ракета.door.cabin) return false;
    М.ракета.door.cabin.visible = !!да;
    return true;
  },
  ...
  /* ── ПЛОЩАДКА ОСТАЁТСЯ НА ЗЕМЛЕ, КОГДА КОРАБЛЬ САДИТСЯ ────────
     Посадка поднимает ВЕСЬ узел финала, а площадка лежит внутри него.
     Значит вместе с кораблём в воздух уходила и она: на снимке доли
     0.70 круглый решётчатый помост с бортиком и огнями парил в
     пустоте, а пол зала с кольцами оставался далеко внизу.

     Владелец увидел это как «камера гуляет туда-сюда» и сказал, как
     надо: «пусть подиум стоит на месте, когда с туннеля выходим,
     камера едет прямо, ракета просто сверху на готовый подиум перед
     нами прилетает».

     Компенсируем площадке ровно тот подъём, который посадка дала узлу.
     Подъём приходит в МИРОВЫХ единицах, а место площадки местное,
     поэтому делим на мировой масштаб её родителя: узел ужат втрое, и
     без деления помост уехал бы втрое дальше нужного.

     Своё начальное место запоминаем при первом вызове: правда о нём
     лежит в самом узле, а не в числе, записанном где-то ещё. */
  "площадкаНаМесте": function (подъёмМира) {
    if (!собрано || !М.корень) return false;
    var п = null;
    М.корень.traverse(function (о) { if (!п && о.name === "площадка") п = о; });
    if (!п) return false;
    if (п.__базаY === undefined || п.__базаY === null) п.__базаY = п.position.y;
    var род = п.parent || М.корень;
    род.updateMatrixWorld(true);
    var м = new T.Vector3();
    род.getWorldScale(м);
    var k = м.y || м.x || 1;
    var д = подъёмМира || 0;
    п.position.y = п.__базаY - д / k;
    return true;
  },
  "замер": function () {
