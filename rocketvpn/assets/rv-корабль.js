...
function собрать(мир, родитель) {
  ...
  /* Грунт вокруг. Он темнее площадки и уходит в дымку: по нему глаз
     считывает, что площадка возвышается, а не лежит наклейкой. */
  var грунт = new T.Mesh(
    new T.PlaneGeometry(PAD_SIZE * 6, PAD_SIZE * 6),
    new T.MeshBasicMaterial({ map: terrainTexture(C.weak), transparent: true,
                              depthWrite: false, toneMapped: false, opacity: 0.92 })
  );
  грунт.rotation.x = -Math.PI / 2;
  грунт.position.y = -PAD_LIFT;
  грунт.renderOrder = 0;
  площадка.add(грунт);
  М.грунт = грунт;
