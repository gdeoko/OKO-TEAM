...
(function (g, d) {
  ...
  function собрать() {
    ...
    /* ── ДВА УСЛОВИЯ, И ОБА ОБЯЗАТЕЛЬНЫ ────────────────────────
       Здесь стояло `обломки.visible = !днём`, и это ПЕРЕБИВАЛО
       признак подземки: в тёмной теме камни включались обратно,
       сколько бы актов их ни прятало. На снимке первого экрана в
       космосе над лунным домом висели восемь сотен гранёных глыб -
       ровно то, что владелец назвал «обломки нереалистичные, звёзды
       заменили эти 3D обломки, они угли какие-то глянцевые в
       воздухе».

       Условий два: камни видны, только когда камера НЕ под землёй и
       тема НЕ светлая. Оба живут в одной функции, и переключить одно
       мимо другого больше нельзя. */
    function пересчитать() {
      var днём = d.documentElement.getAttribute("data-тема") === "светлая";
      if (обломки) обломки.visible = !подЗемлёй && !днём;
      if (маяки) маяки.visible = !подЗемлёй;
    }
    _пересчитать = пересчитать;
    пересчитать();
  ...
  /* ── ОБЛОМКОВ В ЭТОМ ФИЛЬМЕ НЕТ НИГДЕ ─────────────────────────
     Признак начинался с «мы наверху, обломки видны». Он писался под
     прежний сценарий, где камера шла между станциями в открытом
     космосе, и обломки были содержанием перегона.

     Того сценария больше нет. Сегодня фильм это Луна, стена на её
     поверхности, тоннель и зал под ней: пять актов, и ни в одном из
     них камера в открытый космос не выходит. Обломкам взяться неоткуда
     ни на Луне, ни под ней.

     На снимке периметра доли 0.5 они и были видны - тёмный овал во
     весь кадр позади кладки, в пятидесяти двух единицах от глаза,
     угловым размером в сто пятнадцать градусов. Владелец сказал про
     такое прямо: «появились какие-то тёмные тени».

     Поэтому начальное состояние обратное: обломков нет, пока их не
     попросят. Просьба осталась рабочей - вернётся перегон в космосе,
     вернутся и они, снимать модуль ради этого не пришлось. */
  var подЗемлёй = true;
  var _пересчитать = null;
  function подземка(да) {
    подЗемлёй = !!да;
    if (_пересчитать) _пересчитать();
    else {
