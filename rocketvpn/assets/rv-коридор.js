...
(function (g, d) {
    ...
    м.onBeforeCompile = function (ш) {
      ...
      ш.vertexShader = ш.vertexShader
        ...
        .replace("#include <begin_vertex>",
          ...
          /* Кольцо, сквозь которое камера вот-вот пройдёт, гаснет. В
             упор оно занимает весь экран и обводит заголовок, на
             телефоне это кольцо через карточки станций и шаги пуска.
             Маяк это разметка пути впереди, а не рамка вокруг слов:
             ближе десяти единиц его нет, к тридцати он целый. Порог
             от замера на телефоне: кольцо в двенадцати единицах ещё
             обводило заголовок прокола в полэкрана. */
          + "float dCam = distance((modelMatrix * instanceMatrix * vec4(0.0, 0.0, 0.0, 1.0)).xyz, cameraPosition);\n"
          + "vBr *= smoothstep(10.0, 30.0, dCam);");
  ...
  function собрать() {
    ...
    /* Днём обломков нет. Светлая тема инвертирует холст мира, и
       освещённые камни превращаются в тёмные размытые кляксы на белом
       (снимок акта «видно» и «стыковка» днём). Маяки остаются: они
       после инверсии читаются чернильными кольцами и не мешают. */
    function поТеме() {
      var днём = d.documentElement.getAttribute("data-тема") === "светлая";
      if (обломки) обломки.visible = !днём;
    }
    поТеме();
  ...
  function расставить() {
    ...
    for (i = 0; i < перегоны.length && м < ёмк; i++) {
      ...
      for (j = 0; j < доля && м < ёмк; j++) {
        ...
        if (!далеко) continue;
        /* Верх масштаба ниже задуманного: глыба в единицу на двадцати
           единицах от объектива на снимке занимала сотню точек и
           спорила с заголовком. Обломки это фон для скорости, а не
           предмет кадра. */
        var ск = 0.12 * Math.pow(0.85 / 0.12, сл());
        if (r < 14) ск = Math.min(ск, 0.3);
  ...
  /* ── ПОД ЗЕМЛЁЙ ОБЛОМКОВ НЕТ ───────────────────────────────────
     Обломки и маяки это реквизит ПЕРЕГОНА: камни, летящие мимо, пока
     камера идёт между станциями в открытом космосе. В комнате под
     лунным грунтом им взяться неоткуда, а на снимке ангара они висели
     россыпью прямо поверх текста и фигуры.

     Запрет ставит тот, кто знает место: акты комнаты. Это не просьба
     погасить, а сообщение, что здесь их быть не может. */
  var подЗемлёй = false;
  function подземка(да) {
    подЗемлёй = !!да;
    if (обломки) обломки.visible = !подЗемлёй;
    if (маяки) маяки.visible = !подЗемлёй;
  }
