...
var PANELS = 8;
/* Азимут, на котором мы оказываемся, переступив порог.

   ОБОРОТА БОЛЬШЕ НЕТ (слово владельца 11.09.2026): «на CDN тоже 360
   внутри ракеты убери, и сделай так, чтобы после входа в ракету не
   было шва... вошли в ракету, подошли к панели и застыли, дальше
   вопросы, кнопки и всё». Тот же порядок уже стоит на соседнем сайте,
   и два сайта обязаны читаться одним миром.

   Значит входим сразу лицом к остеклению - на нулевом азимуте, там же
   стоит и приборная панель. Дальше камера только подходит к ней. */
var ENTER_AZ = 0;
...
/* Оборота больше нет (см. ENTER_AZ), но отметка осталась в двух
   местах и продолжает работать честно: по ней считается, где на
   ленте начинается блок контактов, и от неё же отмеряется тот
   короткий отрезок за порогом, после которого шапка сайта уходит
   за кромку кабины. Имя менять не стали намеренно: в разметке и в
   истории правок оно уже значит эту точку ленты. */
var P_TURN = 0.88;               /* блок контактов, дальше пульт */
...
function measure() {
  ...
  /* ── ОБОРОТА НЕТ: РАСПИСАНИЕ СТАЛО ПРЯМОЙ ─────────────────
     Здесь стояли семь остановок по числу экранов на стенах: камера
     шла по кругу и читала их один за другим. Владелец оборот снял на
     обоих сайтах, и семь остановок вместе с ним: угол держится нулём
     от входа до пульта, а стены остаются стенами помещения.

     Расписание не выбрасываем, а делаем прямой линией из двух узлов.
     Так все, кто спрашивает yawAt (сцена, карточки, щелчок фиксации),
     продолжают работать без единой правки, и возврат оборота, если он
     когда-нибудь понадобится, снова станет правкой одного места. */
  PLAN.n = 0;
  /* Углы разделов тоже сходятся в ноль. Кинематограф (rc-cinema)
     ставит карточки внутри рубки по этим углам и поворачивает комнату
     на тот же yaw: пока шёл оборот, справочник стоял на двухстах
     градусах и приходил в кадр вместе с камерой. Без оборота он бы
     туда так и не приехал - человек смотрел бы на пустую стену, а
     вопросы висели бы за спиной. */
  PLAN.rel = [];
  var карточек = relCards();
  for (var к = 0; к < карточек; к++) PLAN.rel.push(ENTER_AZ);
  PLAN.faq = ENTER_AZ;
  KNOT = [[P_IN, ENTER_AZ], [P_CON, ENTER_AZ]];
  PLAN.con = ENTER_AZ;

  /* Узлы обязаны идти строго по возрастанию: иначе на коротком
     блоке камера дёрнется назад посреди сектора. */
  for (var i = 1; i < KNOT.length; i++) {
...
function setProgress(p) {
  ...
  /* Снимаем флаг только на обратном ходу - когда люк закрывается.
     ...
     запасному порогу. */
  /* Число люка читается ТАК ЖЕ, как десятью строками выше: `|| 0`
     превращает отсутствие числа в ноль, то есть в закрытый люк, и
     признак «мы в люке» снимался бы на ровном месте, стоит модулю
     двери опоздать на кадр. Разбор этой самой ошибки стоит рядом, а
     здесь она осталась непереписанной. Нет числа - люк считаем
     открытым: ошибаться нужно в сторону «остаёмся снаружи». */
  var люкТут = (typeof g.RC_DOOR === "number") ? g.RC_DOOR : 1;
  ...
  /* Подъезд считается от ВХОДА, а не от конца оборота: оборота нет,
     и ждать его конца значило бы стоять на пороге треть эпизода. */
  var con = p > P_IN ? Math.min(1, (p - P_IN) / Math.max(1e-4, P_CON - P_IN)) : 0;
  st.con = con;
...
if (!phone) {
  addEventListener("mousemove", function (e) {
    ...
    st.pitchT = -ny * 0.21 + (st.con > 0.5 ? -0.1 : 0);      /* около 12 градусов */
    st.yawOff = nx * 0.1;
  }, { passive: true });
}
