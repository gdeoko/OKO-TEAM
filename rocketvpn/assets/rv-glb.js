...
function модель(T, разбор, опции) {
  ...
  return разбор.битмапы.then(function (картинки) {
    ...
    /* Картинки уезжают в видеопамять сейчас, а не в первом кадре с
       моделью: у станции их 37, и загрузка всех разом на первом же
       кадре давала рывок ленты (95-я доля кадра 617 мс при пороге 470
       в проверке скорости). С рисовальщиком в опциях загрузка идёт
       здесь, пока модель ещё не в кадре. */
    if (опции.рисовальщик && опции.рисовальщик.initTexture) {
      for (var к in кэшТек) if (кэшТек.hasOwnProperty(к)) {
        try { опции.рисовальщик.initTexture(кэшТек[к]); } catch (e) {}
      }
    }
...
/* ── ПОДГРУЗКА ПО БЛИЗОСТИ ────────────────────────────────────
   Станция весит 2.85 МБ, корабль 1.31, и оба нужны только в четвёртом
   и восьмом актах. Тянуть их на первом экране значит платить за них
   всем, кто дальше первого экрана не пошёл, и валить приёмку по весу
   страницы (6.00 МБ при пределе 6). Обещание отдаётся, когда секция
   акта подходит на полтора экрана к видимой области: модель успевает
   доехать раньше, чем камера доходит до акта, а на первом экране её
   нет в списке запросов вовсе. Без IntersectionObserver (очень старые
   браузеры) грузим сразу, как раньше. */
var близость = {};
function поБлизости(имяАкта) {
  if (близость[имяАкта]) return близость[имяАкта];
  близость[имяАкта] = new Promise(function (ок) {
    var секция = document.querySelector('section.rv-акт[data-акт="' + имяАкта + '"]');
    if (!секция || typeof IntersectionObserver !== "function") { ок(); return; }
    var н = new IntersectionObserver(function (записи) {
      for (var i = 0; i < записи.length; i++) {
        if (записи[i].isIntersecting) { н.disconnect(); ок(); return; }
      }
    }, { rootMargin: "160% 0px 160% 0px" });
    н.observe(секция);
  });
  return близость[имяАкта];
}

g.RV_GLB = { взять: взять, разобрать: разобрать, геометрия: геометрия, модель: модель, взятьМодель: взятьМодель, поБлизости: поБлизости };
