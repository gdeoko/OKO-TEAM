...
function watchFrame(dt, работа, резали) {
  if (!(работа >= 0)) работа = 0;
  if (noDeg) return;
  ...
  /* ── Две мерки тяжести, и главная теперь работа ────────────
     Работа кадра дольше 22 мс означает, что устройство не удержит и
     сорока пяти кадров, сколько бы раз в секунду мы его ни звали. Эта
     мерка честна всегда, потому что не зависит от нашего расписания.

     Промежуток между кадрами оставляем второй меркой, но засчитываем
     его ТОЛЬКО когда мы не режем частоту сами: иначе пропуск по нашей
     воле снова пошёл бы в зачёт как зависание устройства. */
  var bad = (работа > 22) ? 1 : 0;
  if (!резали && dt > 0.05) bad = 1;   /* ниже двадцати кадров в секунду */
  ...
  /* Рваность считаем только по нечиненому промежутку: при своей
     экономии частоты промежуток задан нами, и рвать ему нечего. */
  var рвано = (!резали && dt > порогРывка && dt > ПОЛ_РЫВКА) ? 1 : 0;
  ...
  /* Окно закрывается по времени ИЛИ по кадрам, а не по обоим сразу.

     Здесь стояло И, и это была вторая половина беды. Ниже семи кадров
     в секунду двенадцать кадров набираются дольше 1700 миллисекунд, и
     правил уже счётчик кадров, а не часы, ради которых условие и
     писалось: при 0.4 кадра в секунду одно окно занимало тридцать
     секунд, а первая ступень приходила через девяносто. То есть чем
     хуже устройству, тем медленнее приходила помощь.

     Четыре кадра как нижняя граница остаются: по одному кадру судить
     нельзя, это уже не наблюдение, а случайность. */
  var хватитВремени = win.length >= 4 && ts0 - окноС >= ОКНО_МС;
...
function loop(ts) {
  ...
  /* СУДИМ ВСЕГДА, но не по промежутку между кадрами, а по РАБОТЕ.

     Прежняя правка глушила сторожа, пока мы сами режем частоту. Она
     чинила настоящую ошибку (кадр, пропущенный по нашей воле, шёл в
     зачёт как рывок устройства), но лечила симптом и породила вторую,
     хуже первой: режем мы именно на простое, а на простое человек и
     стоит - в актах салона, пульта и выхода. То есть сторож молчал
     ровно там, где кадр и умирает. Замер приёмки: на медленной машине
     первая ступень приходила через 13.6 секунды и дальше не двигалась
     136 секунд, вторая и третья не достигались вовсе.

     Правильная мерка - не промежуток, а СВОЯ РАБОТА. Промежуток
     зависит от того, сколько раз в секунду мы решили считать кадр, а
     работа - только от устройства: машине, которой нужно восемьдесят
     миллисекунд на кадр, они нужны и при шестидесяти вызовах в
     секунду, и при двадцати. Поэтому измеряем время внутри самого
     кадра и судим по нему всегда - и когда режем, и когда нет.

     Промежуток при этом не выбрасываем: он остаётся второй меркой,
     но только когда мы НЕ режем, то есть когда он честно отражает
     устройство. */
  if (!чистоС) чистоС = ts;
  var работаС = ts;
  ...
  /* Отметка начала полезной работы. Всё, что выше, это разбор
     собственного расписания, и к тяжести устройства оно отношения не
     имеет. */
  работаС = (g.performance && g.performance.now) ? g.performance.now() : ts;
  ...
  /* Кадр отработал - судим его по СВОЕЙ работе.

     Триста миллисекунд после возврата к полной частоте всё ещё
     пропускаем: там доигрывают отложенные дела, и первый кадр после
     простоя честно тяжелее обычного. */
  var работа = ((g.performance && g.performance.now) ? g.performance.now() : ts) - работаС;
  if (ts - чистоС > 300) watchFrame(dt, работа, режем);
}
