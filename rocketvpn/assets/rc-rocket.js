...
function caps() {
  ...
  /* ── СЛАБАЯ ВИДЕОКАРТА РЕЖЕТ БУФЕР, А НЕ КАДР ───────────────
     Память и ядра про графику не знают. Дешёвый телефон вроде Infinix
     несёт восемь ядер и четыре гигабайта при видеокарте начального
     уровня: по строке выше он сильный, и буфер ему ставится в полторы
     точки. Заказчик на таком получил двадцать-тридцать кадров.

     Вердикт о карте уже посчитан один раз в rc-gl.js и опубликован -
     второй список моделей здесь заводить нельзя, разойдётся молча.

     Режем ровно то, что разрешает соседний абзац: РАЗРЕШЕНИЕ БУФЕРА и
     сглаживание. Ни одна деталь из кадра не уходит - ни искра, ни
     грань, ни блик, - а на плотном экране падение чёткости почти не
     читается. Гранёный силуэт и пропавшие искры видно сразу, поэтому
     `tiny` мы НЕ трогаем: он остаётся про совсем слабую машину. */
  var слабаяКарта = g.RC_СЛАБАЯ_КАРТА === true;
  return {
    mobile: mob,
    reduce: reduce,
    weak: tiny,
    tiny: tiny,
    картаСлабая: слабаяКарта,
    dpr: Math.min(g.devicePixelRatio || 1,
      tiny || слабаяКарта ? 1.1 : (mob ? 1.45 : 2)),
    aa: !tiny && !слабаяКарта,
    sparks: tiny ? 150 : 420,
    smoke: tiny ? 70 : 170,
    radial: tiny ? 28 : 64,
...
function buildAirlockInterior(env, R, HH, Y, seg) {
  ...
  /* Паспорт кабины спрашиваем, но НЕ надеемся на него.
     ...
     Первый заход полагался только на `RC_CAB_FLAT`, и это оказалось
  ...
  var ЗАПАСНЫЕ_СНИМКИ = {
    "широкая": { "файл": "assets/gen/cockpit-wide-hd.webp", w: 2688, h: 1536 },
    "средняя": { "файл": "assets/gen/cockpit-mid-hd.webp",  w: 2048, h: 1536 },
    "высокая": { "файл": "assets/gen/cockpit-tall-hd.webp", w: 1536, h: 2499 }
  };
  var снимок = null;
  try {
  ...
  if (снимок) {
    ...
    (function () {
      ...
      им.onload = function () {
        ...
        try {
          var видД = (g.RC_DECK && g.RC_DECK["какой"]) ? g.RC_DECK["какой"](снимок.w, снимок.h) : null;
          var планД = (видД && g.RC_CAB_DECK) ? (g.RC_CAB_DECK[видД] || g.RC_CAB_DECK["широкая"]) : null;
          if (планД && g.RC_DECK && g.RC_DECK["создать"]) {
...
/* ── Маршрут корабля по странице ─────────────────────────────
   Владелец описал маршрут дословно: «ракета изначально в первом
   блоке справа поверх планеты. как только я начинаю скроллить она
   сразу должна лететь в левую сторону вдоль сайта... справа налево,
   потом слева направо, потом подлетает к планете, облетает эту
   планету вокруг, потом от планеты вылетает и летит вправо, потом
   обратно влево, и потом приземляется».

   Прежняя петля этого не давала: она обходила кадр по кругу и
   пересекала экран поперёк ровно один раз. Замер живьём (телефон
   412x915) показал главное: на первой пятой части прокрутки корабль
   стоял справа и только мельчал - x шёл 325, 358, 324, 310 при
   масштабе 0.70, 0.70, 0.70, 0.44. Это и есть «летит на одном
   месте», и вина была не анимации, а формы маршрута.

   Второе, что мешало попасть узлом в нужный момент прокрутки:
   кривая three.js раскладывает узлы ПО ДЛИНЕ ДУГИ. Длинный перелёт
   съедает столько же доли, сколько короткий разворот, и назначить
   «крайняя левая точка на тринадцати процентах страницы» ей нельзя.
   Поэтому маршрут ведёт своя кривая: у каждого узла записана его
   доля прокрутки, и корабль приходит в узел ровно на ней.

   Доли выставлены по живому замеру сцены, а не на глаз. На обоих
   окнах (412x915 и 1440x900) виток вокруг планеты держит долю от
   0.30 до 0.42, а посадка забирает кадр себе около 0.52. Значит на
   «справа налево» и «слева направо» есть первые три десятых доли, а
   на «вправо и обратно влево» - десятая доля между витком и
   посадкой. Хвост петли (от 0.55) на экране не бывает никогда: там
   уже ведут посадка, подход к люку и салон, но он обязан быть
   гладким, иначе обратная прокрутка даёт рывок.

   Глубина по-прежнему держится в узком поясе (от -4.6 до -6.4):
   корабль всё время одного размера, и глаз следит за ним, а не
   ищет его заново на каждом экране. */
/* Узлы стоят так, чтобы скорость по горизонтали внутри одного
   пролёта почти не менялась: соседние секущие наклоны отличаются на
   считанные проценты. Это не эстетика, а требование ограничителя
   наклонов (см. ниже): если один отрезок вдвое медленнее соседнего,
   ограничитель поджимает наклон, и в середине быстрого отрезка
   появляется площадка - корабль на полпути через экран как будто
   притормаживает. Проверено сплайном: при неровных шагах на доле
   0.05 горизонтальная составляющая направления падала до 0.19. */
var ROUTE = [
  /* доля,  x,     y,     z */
  [0.000,  7.20, -2.60, -5.00],   /* герой: правый борт, поверх планеты */
  [0.030,  4.60, -0.90, -5.20],   /* тронулись - сразу пошла влево */
  [0.070,  1.00,  1.60, -5.50],   /* середина кадра, набирает высоту */
  [0.105, -3.20,  2.50, -5.60],
  [0.140, -6.90,  2.60, -5.60],   /* крайняя левая: пролёт справа налево */
  [0.175, -4.60,  0.60, -5.75],   /* развернулась, идёт вправо низом */
  [0.210, -1.00, -2.00, -5.90],
  [0.245,  3.20, -2.50, -5.50],
  [0.275,  6.90, -1.20, -5.15],   /* крайняя правая: слева направо */
  [0.310,  4.10,  1.10, -5.40],   /* поворот к планете */
  [0.345,  0.40,  2.90, -5.90],
  [0.380, -0.90,  3.30, -6.10],   /* подошла к планете: тут ведёт виток */
  [0.415,  1.30,  2.60, -5.85],   /* сход с витка */
  [0.446,  6.60,  0.90, -5.10],   /* от планеты вправо, крайняя точка */
  [0.478,  0.00, -0.30, -5.35],
  [0.508, -6.40, -1.60, -5.60],   /* и обратно влево, крайняя точка */
  [0.545, -2.40, -3.40, -5.80],   /* заход на посадку: дальше ведёт landK */
  [0.610,  1.60, -3.30, -5.40],   /* ── хвост петли: на экране не бывает */
  [0.690,  5.60, -1.80, -5.10],
  [0.770,  2.20,  1.60, -5.60],
  [0.850, -3.60,  2.80, -5.90],
  [0.920, -5.20, -0.60, -5.60],
  /* Возврат к герою идёт ИЗ-ЗА правого края кадра. Иначе касательная
     в нуле смотрит вправо, и корабль в первом экране стоит носом от
     страницы, а на первом же движении колеса разворачивается на сто
     восемьдесят градусов. Точка за кромкой этого не показывает: её
     доля прокрутки принадлежит салону, где сцена корабля вообще
     остановлена (класс rc-inside). */
  [0.965,  9.60, -1.80, -5.10]
];

/* Кривая по узлам с явной долей прокрутки.

   Считаем кубическим сплайном Эрмита: он проходит ровно через узлы,
   и производная на стыках сегментов совпадает - то есть скорость и
   направление не рвутся, стыков на экране не видно.

   Наклоны в узлах ограничены по Фричу-Карлсону. Без ограничения
   сплайн на резком развороте вылетает за пределы соседних узлов:
   после крайней правой точки он ушёл бы ещё правее, за кромку кадра,
   и корабль пропадал бы ровно на повороте. С ограничением каждый
   отрезок монотонен, а в точке разворота горизонтальная скорость
   честно обнуляется - это и есть вид разворота. */
function Маршрут(узлы) {
  var n = узлы.length;
  this.n = n;
  this.t = new Array(n);
  this.p = new Array(n);
  ...
  for (i = 0; i < n; i++) {
    this.t[i] = узлы[i][0];
    this.p[i] = [узлы[i][1], узлы[i][2], узлы[i][3]];
  }
  /* Шаги и секущие наклоны каждого сегмента; последний сегмент
     замыкает петлю, поэтому его длина считается через единицу */
  this.h = new Array(n);
  var d = new Array(n);
  for (i = 0; i < n; i++) {
    var j = (i + 1) % n;
    var шаг = this.t[j] - this.t[i];
    if (шаг <= 0) шаг += 1;
    this.h[i] = шаг;
    d[i] = [0, 0, 0];
    for (c = 0; c < 3; c++) d[i][c] = (this.p[j][c] - this.p[i][c]) / шаг;
  }
  /* Наклон в узле: среднее соседних секущих с ограничением */
  this.m = new Array(n);
  for (i = 0; i < n; i++) {
    var пред = (i + n - 1) % n;
    this.m[i] = [0, 0, 0];
    for (c = 0; c < 3; c++) {
      var a = d[пред][c], b = d[i][c];
      if (a * b <= 0) { this.m[i][c] = 0; continue; }
      var м = (a + b) / 2;
      var предел = 3 * Math.min(Math.abs(a), Math.abs(b));
      if (м > предел) м = предел;
      else if (м < -предел) м = -предел;
      this.m[i][c] = м;
    }
  }
  this._i = 0;
...
/* Какой сегмент держит эту долю. Ищем от последнего найденного:
   доля меняется на сотые между кадрами, и почти всегда это тот же
   сегмент, что и в прошлый раз. */
Маршрут.prototype.сегмент = function (t) {
  var n = this.n, i = this._i;
  for (var шаг = 0; шаг < n; шаг++) {
    var к = (i + шаг) % n;
    var t0 = this.t[к];
    var s = t - t0;
    if (s < 0) s += 1;
    if (s < this.h[к]) { this._i = к; return к; }
  }
  return 0;
};

/* Точка и касательная на доле t. Обе считаются по одним и тем же
   базисным функциям Эрмита, поэтому направление всегда согласовано
   с положением - нос не может смотреть не туда, куда корабль летит. */
Маршрут.prototype.getPointAt = function (t, out) {
  t = ((t % 1) + 1) % 1;
  var i = this.сегмент(t), j = (i + 1) % this.n, h = this.h[i];
  var s = (t - this.t[i]); if (s < 0) s += 1;
  s /= h;
  var s2 = s * s, s3 = s2 * s;
  var h00 = 2 * s3 - 3 * s2 + 1;
  var h10 = s3 - 2 * s2 + s;
  var h01 = -2 * s3 + 3 * s2;
  var h11 = s3 - s2;
  var A = this.p[i], B = this.p[j], MA = this.m[i], MB = this.m[j];
  out = out || new T.Vector3();
  out.set(
    h00 * A[0] + h10 * h * MA[0] + h01 * B[0] + h11 * h * MB[0],
...
Маршрут.prototype.getTangentAt = function (t, out) {
  t = ((t % 1) + 1) % 1;
  var i = this.сегмент(t), j = (i + 1) % this.n, h = this.h[i];
  var s = (t - this.t[i]); if (s < 0) s += 1;
  s /= h;
  ...
  out = out || new T.Vector3();
  out.set(
    g00 * A[0] + g10 * h * MA[0] + g01 * B[0] + g11 * h * MB[0],
    g00 * A[1] + g10 * h * MA[1] + g01 * B[1] + g11 * h * MB[1],
    g00 * A[2] + g10 * h * MA[2] + g01 * B[2] + g11 * h * MB[2]
  );
  /* В точке разворота производная по всем трём осям может сойтись в
     ноль: направление из неё не достать, и нормализация дала бы NaN.
     Берём тогда соседнюю точку - направление там уже определено. */
  if (out.lengthSq() < 1e-9) {
    var б = this.getPointAt(t + 0.004, new T.Vector3());
    out.copy(б).sub(this.getPointAt(t, new T.Vector3()));
...
Rocket.prototype.resize = function () {
  ...
  /* Тот же потолок, что в паспорте устройства: слабая карта идёт по
     нижней ступени вместе со слабой машиной. Две записи одного правила
     в двух местах уже расходились - здесь берём готовое число. */
  this.r.setPixelRatio(this.C.dpr);
...
/* Скорость витка. Владелец просил облетать планету заметно быстрее:
   на 0.62 радиана в секунду полный оборот занимал десять секунд, и
   за то время, что человек читает раздел, корабль успевал пройти
   меньше половины круга - облёта не читалось, читался медленный
   проход мимо. На 1.30 оборот укладывается в пять секунд: виток
   виден целиком и остаётся витком, а не мельканием. */
var ORB_BASE = 1.30;                                  /* сам по себе: радиан в секунду */
...
/* Заслонение планетой. Планета непрозрачна, значит на дальней
   половине витка корабля видно быть не должно вовсе - владелец
   поймал это дословно: «ракета когда планету облетает, она же
   должна за неё заходить, а её видно, когда она за планетой».

   Два условия и оба обязательны: корабль ушёл за плоскость шара И
   его экранная точка лежит на диске. Полосу перехода держим узкой
   (десятая доля радиуса): корабль обязан скрываться ЗА КРОМКОЙ, а
   не растворяться в воздухе на подлёте к ней. */
function hidden(o, c, planeZ, dz) {
  var d = (planeZ - o.z) / (dz * 0.12);
  if (d <= 0) return 0;
  if (d > 1) d = 1;
  var dx = (o.sx - c.cx) / c.r, dy = (o.sy - c.cy) / c.r;
  var ins = (1.005 - Math.sqrt(dx * dx + dy * dy)) / 0.09;
  if (ins <= 0) return 0;
  if (ins > 1) ins = 1;
  return d * ins;
}
...
Rocket.prototype.orbit = function (dt) {
  ...
  /* Разгон от прокрутки урезан с 2.6 до 1.4: базовая скорость витка
     выросла вдвое, и прежний множитель на резком листании раскручивал
     корабль до трёх четвертей оборота в секунду - виток превращался в
     смазанное кольцо. Потолок теперь около половины оборота. */
  var wantW = ORB_BASE * (1 + this._orbV * 1.4);
  ...
  this.occl = 1 - hidden(A, c, planeZ, dz) * this.orbK;
...
Rocket.prototype.landing = function (p, dt, pos, tan) {
  ...
  if (landR !== this._landPub) {
    ...
    /* Кадр начинает очищаться раньше: с той секунды, как корабль
       пошёл на снижение, а не когда уже стоит на грунте. Владелец
       увидел заголовок соседнего раздела прямо по корпусу - это была
       та самая задержка. */
    var cl = Math.max(0, Math.min(1, (this.landK - 0.18) / 0.42)).toFixed(2);
...
Rocket.prototype.doorOpen = function (dt) {
  var sc = g.RC_SCENE;
  var goal = 0;
  /* Акт «салон» приходит РАНЬШЕ, чем человек входит: акт выбирается по
     тому, чья середина ближе к середине экрана, а вход случается по
     распахнутому люку. Замер дал на мониторе 294 точки разницы. Пока
     эти двое расходятся, ветку «салон» нельзя понимать как «мы уже
     внутри»: она бросала цель двери сразу в единицу, и створки
     распахивались рывком, ещё когда человек стоял снаружи.

     Внутри мы или нет, знает признак на документе, а не название
     акта. Пока не вошли, дверь ведёт прокрутка, как и в проходе. */
  var внутри = false;
  try {
    var кл = document.documentElement.classList;
    внутри = кл.contains("rc-inside") || кл.contains("rc-in-hatch");
  } catch (eВ) {}
  if (this.appK > 0.45 && sc) {
    if (sc.act === "walk" || (sc.act === "cabin" && !внутри)) {
    ...
    /* Вошли - люк за спиной распахнут, что бы ни говорил акт */
    goal = 1;
  }
  ...
  /* Створки идут со скоростью прокрутки, а не со скоростью того
     слагаемого, которое сейчас связывает.

     Ход двери ведут два числа: положение раздела «надёжность» в
     кадре и затвор по подходу. На мониторе связывает первое, и
     половина двери проходит за 454 точки - ровно как задумано. На
     телефоне разделы короткие, положение раздела насыщается раньше,
     и остаётся один затвор, который на сжатой раскладке пролетает
     за сотню точек: замер дал половину двери за 51 точку, меньше
     половины щелчка колеса. Один свайп перепрыгивал всё открытие -
     это и есть «рывком, не реалистично».

     Ограничиваем прирост цели пройденной прокруткой: полный ход
     двери требует девять десятых экрана на ЛЮБОЙ раскладке. Прыжок
     по якорю сюда не попадает - там прокрутка сразу большая. */
  var yДв = (g.pageYOffset || document.documentElement.scrollTop || 0);
  var hДв = this.ch() || 800;
  if (this._doorY == null) this._doorY = yДв;
  var шагДв = Math.abs(yДв - this._doorY) / Math.max(1, hДв * 0.9);
  this._doorY = yДв;
  var цельПред = this._doorGoal == null ? 0 : this._doorGoal;
  if (goal > цельПред) goal = Math.min(goal, цельПред + шагДв);
  else if (goal < цельПред) goal = Math.max(goal, цельПред - шагДв);
  this._doorGoal = goal;
  ...
  /* Отдельный признак «карточки в проёме пора убирать».

     Раньше их гасило то же самое `rc-door-open`, а оно встаёт уже на
     четверти хода двери. Модуль же карточек рассчитан на другое: они
     проступают в щели с 0.14, читаются до 0.78 и растворяются к
     0.93, ровно перед входом. Из-за раннего гашения окно видимости
     сжималось до сотни точек прокрутки на мониторе, а на телефоне,
     где дверь проходила за один свайп, карточек не было видно
     вовсе - замер не поймал их ни на одном шаге. Владелец описывал
     этот кусок дословно: «там уже салон видно и карточки две,
     которые исчезнут». Гасим их там, где и задумано. */
  var gateOut = this.doorK > 0.88;
  if (gateOut !== this._gateOut) {
    this._gateOut = gateOut;
    document.documentElement.classList.toggle("rc-gate-out", gateOut);
  }
...
/* Порог передачи сцены рубке. Сценарий входа задан владельцем и
     ...
     не нужно. */
  /* Правду о признаке спрашиваем у САМОГО документа, а не у своей
     памяти.

     Здесь стояла защёлка: класс переключался только когда новое
     значение отличалось от запомненного в this._deep. А снимает этот
     класс не только корабль - салон снимает его напрямую при обратном
     ходе (rc-interior.js, две строки). После такого снятия память
     корабля продолжала говорить «глубоко внутри», условие больше
     никогда не выполнялось, и класс не возвращался НИКОГДА.

     Это и есть «вход в ракету абсолютно всегда зависает»: замер
     воспроизвёл на телефоне и на мониторе одинаково - вошли, шаг
     назад на сорок точек выкинул, и дальше хоть пятьсот шестьдесят
     точек вперёд при люке, распахнутом настежь, вход не случался ни
     разу. Человек стоял перед открытым люком до самого запасного
     порога, до которого ещё две тысячи точек прокрутки.

     Сравнение с классом в документе снимает вопрос о том, кто им
     владеет: условие сложилось - признак вернётся, кто бы его ни
     снял. */
  var deep = this.doorK > 0.97 && this.appK > 0.86;
  var естьКласс = document.documentElement.classList.contains("rc-in-hatch");
  if (deep !== естьКласс) {
    this._deep = deep;
...
Rocket.prototype.layout = function (p, dt) {
  ...
  if (this._fitX == null || this._fitW !== this.cw()) {
    ...
    /* Верхняя граница поднята с единицы: на мониторе размах петли
       упирался в неё и корабль ходил от 19 до 81 процента ширины -
       «вдоль сайта» из этого не читалось, оставалась середина. С
       1.16 крайние точки выходят на 11 и 89 процентов, то есть к
       самым полям, и пролёт виден как пролёт. Телефон границы не
       касается: там работает нижняя, 0.34. */
    this._fitX = Math.max(0.34, Math.min(1.16, (halfW - 1.3) / 7.2));
  ...
  /* Хватка держит корабль на месте героя, пока страницу не тронули,
     и отпускает его сразу, как только тронули.

     Раньше она тянулась до семи процентов прокрутки, да ещё вела
     корабль по собственной дуге (арке) вправо и вниз. Замер живьём
     показал, во что это обходилось: на телефоне корабль первую пятую
     часть прокрутки стоял справа (x = 325, 358, 324, 310 из 412) и
     только мельчал. Владелец назвал это дословно - «остаётся справа
     и просто уменьшается, летит на одном месте».

     Поэтому арки больше нет: она спорила с маршрутом, а маршрут по
     новой кривой сам стартует из этой же точки и сам уходит влево.
     Хватка осталась только затем, зачем нужна на самом деле: держать
     корабль в безопасной зоне первого кадра при нетронутой странице,
     на любом экране. Сход - за первые три процента прокрутки, с
     синусной формой (производная на обоих краях нулевая, поэтому
     переход к маршруту не виден). */
  if (scAct && (scAct.act === "pad" || p < 0.05) && app < 0.01 && (this.landK || 0) < 0.01) {
    var padRaw = 1 - Math.max(0, Math.min(1, (p - 0.001) / 0.016));
    var padK = padRaw * padRaw * (3 - 2 * padRaw);
    if (padK > 0.01) {
      var pw = this.cw();
      var ph = this.ch();
      /* Куда ставим корабль в первом кадре.

         Свободная зона на телефоне одна: правый край на уровне списка
         отметок. Строки там короткие, справа от них пусто, и корабль
         входит в кадр целиком, не заслоняя ничего. На широком экране
         уводим правее центра шара, иначе корпус тонет в его сетке.

         Числа подобраны под нулевой узел маршрута (x 7.2, y -2.6):
         его проекция ложится рядом, и в миг схода хватки корабль
         никуда не переставляется. */
      var pxA = pw * (this.C.mobile ? 0.78 : 0.84);
      var pyA = ph * (this.C.mobile ? 0.70 : 0.66);
  ...
  if (lock < 0.2 && !(scAct && scAct.act === "pad")) {
    ...
    if (pr.z < 1) {
      ...
      /* Сторона отвода липкая. Раньше знак брался от текущего
         положения, и в момент, когда корабль проходил середину
         кадра, отвод менял направление: корпус за долю секунды
         переезжал через весь экран. Владелец увидел ровно это -
         «не летит за скроллом, а просто исчезает и потом слева
         появляется». Теперь сторона переключается только когда
         корабль ушёл за середину заметно (на четверть колонки), и
         посреди кадра никаких прыжков нет. */
      var side = this._corrSide || (off >= 0 ? 1 : -1);
      if (off > colHalf * 0.25) side = 1;
      else if (off < -colHalf * 0.25) side = -1;
      this._corrSide = side;
      /* Отвод урезан с 26 до 14 процентов ширины кадра. По новому
         маршруту корабль пересекает экран поперёк четыре раза, и
         прежний отвод спорил с самим пролётом: у середины он толкал
         корпус вбок почти на четверть экрана, а на липком
         переключении стороны отдавал обратно. Пролёт из-за этого
         вилял. Читаемость держит второе средство, приглушение: оно
         работает по фактическому наложению на слова и от размаха
         отвода не зависит. */
      var maxPx = vw * 0.14;
  ...
  /* На телефоне корабль меньше: при 0.88 он не помещался в кадр
     целиком и был виден одним носом. 0.7 - размер, при котором
     ракета читается кораблём и не спорит с заголовком. */
  var s = this.C.mobile ? 0.70 : 1.12;
  ...
  /* Весь акт салона мы ИДЁМ в люк, а не стоим перед ним.

     Доля подхода в этом акте жёстко равна единице, люк открыт
     полностью, и корпус переставал двигаться вовсе. Замер живьём
     показал провал: от доли 0.72 до 0.81 страницы соседние кадры
     отличались на одну десятую единицы яркости при семи-сорока на
     живых участках. Десятая часть сайта, на которой человек крутит
     колесо, а кадр стоит намертво.

     Добавляем ход по доле самого акта: корпус продолжает расти, и
     проём наезжает на зрителя ровно до того мига, когда кадр
     принимает салон. Полста процентов - предел: дальше камера уходит
     за плоскость обшивки. */
  var вход = (scS && scS.act === "cabin")
    ? Math.max(0, Math.min(1, scS.k || 0)) : 0;
  var шаг = 1 + вход * вход * (3 - 2 * вход) * 0.52;

  var sNow = s * (1 - k * 0.70) * lndS * (1 + app * 3.9) * (1 + dk * dk * 1.25) * readS * stepIn * шаг;
  this._sNow = sNow;
...
Rocket.prototype.veil = function (dt) {
  ...
  if (this._veilT >= 0.1) {
    ...
    /* Затемнение под текстом было слишком сильным: в первом экране
       корабль растворялся до силуэта, и владелец видел, что ракеты
       нет вовсе. Уступаем тексту ровно настолько, чтобы буквы
       читались, а корабль оставался кораблём. */
    this._veilGoal = hit ? (this.C.mobile ? 0.74 : 0.78) : 1;
...
/* ── ПРОГРЕВ ШЕЙДЕРОВ ДО ПЕРВОГО КАДРА РАКЕТЫ ─────────────────
   Заказчик про эту сцену: «вход лагает в ракету». Дело не в машине.

   Three собирает и компилирует шейдер материала В ТОТ МОМЕНТ, когда
   материал впервые попал в кадр. У ракеты своя сцена и свой набор
   материалов - обшивка, створки, труба люка, салон, пульт, - и все они
   приходят в кадр разом, на подходе к люку. Каждый первый показ это
   синхронная пауза, и они складываются ровно там, где начинается
   движение внутрь.

   compile проходит сцену и компилирует всё сразу, до первого кадра.
   Стоит это сотни миллисекунд один раз, и платит их человек ещё до
   того, как что-то поехало.

   Ошибку глотаем нарочно: у старых сборок three свои доводы у compile,
   и упавший прогрев не имеет права не пустить человека в ракету. Без
   него вернётся прежний рывок, и только. */
Rocket.prototype.прогреть = function () {
  if (this._прогрет || !this.r || !this.scene || !this.cam) return;
  this._прогрет = true;
  try {
    var было = (typeof performance !== "undefined" && performance.now)
      ? performance.now() : 0;
    this.r.compile(this.scene, this.cam);
    this.прогревМс = было
      ? Math.round(performance.now() - было) : 0;
  } catch (e) { this.прогревМс = -1; }
};
...
Rocket.prototype.start = function () {
  ...
  this.running = true;
  this._last = 0;
  /* Прогрев идёт ПЕРЕД первым кадром, а не в первом кадре: иначе он
     сам и станет тем рывком, от которого заводился. */
  this.прогреть();
  this._raf = requestAnimationFrame(this.tick.bind(this));
};
