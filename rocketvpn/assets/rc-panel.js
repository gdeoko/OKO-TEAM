...
function inscribed(poly) {
  ...
  for (i = 0; i < ys.length; i++) {
    for (k = i + 1; k < ys.length; k++) {
      ...
      var l = -1e9, r = 1e9;
      /* Границы должны быть НАЙДЕНЫ, а не остаться на заглушке.

         Здесь была тихая беда. Рёбра делятся на левые и правые по
         знаку середины, и если для полосы левых рёбер не нашлось
         вовсе (так бывает у контура с длинной горизонтальной кромкой
         около центра), граница оставалась на минус миллиарде. Тогда
         площадь полосы выходила гигантской, побеждала в переборе, и
         наружу уходил прямоугольник с краем в минус миллиард.

         Дальше этим числом публикуется переменная остекления, по ней
         стоит весь верхний слой рубки - и он раздувался до тридцати
         трёх миллионов точек шириной при нулевой высоте, уезжая за
         экран вместе со всей начинкой. На новой плите это и
         случилось: досье, подсказки, чипы - всё было за кадром. */
      var естьЛево = false, естьПраво = false;
      for (e = 0, p = poly.length - 1; e < poly.length; p = e++) {
        ...
        var mid = (x1 + x2) / 2;
        if (mid < 0) { естьЛево = true; if (xmax > l) l = xmax; }
        else { естьПраво = true; if (xmin < r) r = xmin; }
      }
      if (!естьЛево || !естьПраво) continue;
...
/* ══════════════════════════════════════════════════════════
   Клавиши
   ══════════════════════════════════════════════════════════ */
/* Списки команд здесь не держим намеренно. Их было два, они
   разошлись с rc-keys.js и по порядку, и по составу (тут стоял
   ЗАЛП, которого в атласе нет вовсе), и ими никто не пользовался -
   подпись на крышку всё равно берётся из RC_KEYS.name по номеру
   ниши. Один список на весь пульт: assets/rc-keys.js. */
...
function layout(inner, tgt, portrait, n) {
  /* На мониторе клавиши идут одним рядом, на телефоне двумя.

     Одним рядом на телефоне пробовала: триста девяносто точек на
     дюжину клавиш это тридцать две точки на клавишу, подпись
     ...
     Число клавиш приходит снаружи и равно длине списка команд.
     Раньше здесь стояла жёсткая дюжина, а команд одиннадцать: на
     двенадцатую крышку в атласе лица нет, и она выходила пустым
     чёрным стеклом рядом с рабочими. Приёмка увидела ровно это -
     «пустые клавиши на рейках». За крышкой без команды и нажимать
     нечего: в physicalControlsFrame.nodes ей тоже не находилось
     пары. */
  n = Math.max(1, n | 0);
  ...
  for (var r = 0; r < rows; r++) {
    /* Последний ряд бывает короче: одиннадцать клавиш это шесть и
       пять. Короткий ряд ставим по центру, иначе он прижимается к
       левому краю и пульт выглядит собранным набок. */
    var вРяду = Math.min(perRow, n - r * perRow);
    if (вРяду <= 0) break;
    var сдвиг = (perRow - вРяду) * stepX / 2;
    for (var i = 0; i < вРяду; i++) {
      out.push({
        sx: -half + сдвиг + stepX * (i + 0.5),
        sy: top - rowH * (r + 0.5),
...
function keycaps(T, api, proj, fit, dep, inner, tgt, parent, ru, tiny, aniso) {
  /* Сколько клавиш - столько и лиц в атласе. Список один, в
     rc-keys.js, и он же задаёт порядок ниш. */
  var КЛ = (g.RC_KEYS && g.RC_KEYS.KEYS) ? g.RC_KEYS.KEYS.length : 11;
  var list = layout(inner, tgt, api.portrait, КЛ);
...
function build(T, o) {
  ...
  /* Плоская ветка стоит первой и всегда срабатывает, поэтому
     трёхмерная - с настоящей геометрией по карте глубины - никогда
     не выполнялась. Признак rc3d=1 в адресе даёт её пощупать, не
     трогая боевой путь. */
  var хочу3D = false;
  try { хочу3D = /[?&]rc3d=1/.test(location.search); } catch (e3) {}
  if (!хочу3D) {
  ...
  var mat = new T.MeshStandardMaterial({
    ...
    /* ── Снимок сам себе свет ─────────────────────────────
       Свет на снимке УЖЕ запечён: это фотография рубки, а не голая
       развёртка. Материал же считал освещение заново - и в
       полумраке салона пульт уходил в чёрное, хотя на картинке он
       светлый. Эталон для трёхмерной рамы один: та же фотография.

       Поэтому светимость ведёт САМ ЦВЕТ, а не отдельная карта
       свечения: albedo целиком становится собственным светом рамы,
       и с фронта модель повторяет снимок. Сцена добавляет к этому
       свой отклик - блик от планеты, тень от стойки, - и появляется
       то, чего у картинки не было никогда: силуэт и параллакс.

       Металличность ниже прежней: при 0.38 и шероховатости в
       единицу отражать нечего, и поверхность только темнела. */
    emissiveMap: mapAlbedo,
    emissive: new T.Color(0xffffff),
  ...
  try {
    /* RC_REAL.env это ФУНКЦИЯ, которая печёт окружение, а не готовая
       ...
       Проверка эта НЕ ЛИШНЯЯ и не «мёртвая ветка», хотя со стороны
       читается так: она и правда не проходит никогда. Отражения пульт
       получает не отсюда, а из `scene.environment` - его ставит сборка
       мира той же печёной картой (rc-flight.js, там же, где заводится
       плёнка), а три.js подхватывает окружение сцены сам, когда своей
       карты у материала нет. Поставить сюда envMap руками значит
       перебить общее окружение частным, и пульт перестанет отражать то
       же, что отражает вся рубка. */
