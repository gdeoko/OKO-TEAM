...
(function (g, d) {
  ...
  var R = 4.0;
  /* ── ВЫСОТА СВОДА, А НЕ ПОЛУСФЕРА ────────────────────────────
     Владелец: «размер этого домика должен быть как у igloo - размеры,
     положения, формы, текстуры и цвет».

     Замер их снимка с телефона: свод занимает 510 точек по ширине и
     475 по высоте, то есть высота это девять десятых ДИАМЕТРА. Наш
     купол был ровной полусферой: высота ровно половина диаметра, и
     сколько его ни двигай, силуэт оставался блином, а не домом. У них
     свод почти шаровой, чуть срезанный снизу, - так и кладут настоящее
     иглу, иначе оно не держит форму.

     1.80 радиуса это 7.2 при диаметре 8: те самые девять десятых. */
  var ВЫС = R * 1.80;
  ...
  function лунныйКадр() {
    ...
    var верхД = узкий ? 0.18 : 0.34;
    var низД = узкий ? -0.36 : -0.45;
  ...
  function кладка() {
    ...
    for (var ряд = 0; ряд < рядов; ряд++) {
      var шагФи = (Math.PI / 2) / (рядов + 0.35);
      var фи = (ряд + 0.5) * шагФи;
      /* Профиль свода это ЧЕТВЕРТЬ ЭЛЛИПСА: по горизонтали радиус R, по
         вертикали ВЫС. На окружности длина дуги ряда была одна и та же и
         высота блока бралась числом; на эллипсе она меняется от ряда к
         ряду, и высоту камня надо считать по дуге, иначе внизу блоки
         разойдутся щелями, а на макушке налезут друг на друга. */
      var rРяда = Math.cos(фи) * R, yРяда = Math.sin(фи) * ВЫС;
      var выс = Math.sqrt(R * R * Math.sin(фи) * Math.sin(фи) +
                          ВЫС * ВЫС * Math.cos(фи) * Math.cos(фи)) * шагФи;
      var шир0 = R * 0.52;
      ...
      for (i = 0; i < n; i++) {
        ...
        /* Нормаль эллипсоида, а не сферы: градиент x²/R² + y²/ВЫС² + z²/R².
           Взять центр и нормировать, как раньше, значило бы наклонить
           каждый камень к оси сильнее, чем лежит сама поверхность, и свет
           лёг бы мимо граней. */
        var норм = new T.Vector3(центр.x / (R * R), центр.y / (ВЫС * ВЫС), центр.z / (R * R)).normalize();
    ...
    for (i = 0; i < 4; i++) {
      ...
      var rк = R * 0.17;
      var цк = new T.Vector3(Math.cos(тк) * rк, ВЫС * 0.985, Math.sin(тк) * rк);
