...
(function (g, d) {
  ...
  var R = 4.0;
  /* ── СВОД ЭТО СЕКТОР ШАРА, А НЕ ПОЛУСФЕРА И НЕ ЭЛЛИПСОИД ────
     Владелец: «размер этого домика должен быть как у igloo - размеры,
     положения, формы, текстуры и цвет», и сразу следом: «форма дома
     сплющенная стала, выглядит как деформация».

     Оба замечания про одно и то же, и вот в чём было дело.

     ПОЛУСФЕРА (как было сначала) даёт высоту ровно в половину диаметра.
     Замер их снимка: свод 510 точек в ширину и 475 в высоту, то есть
     высота это три четверти ширины. Наш купол рядом читался блином.

     ЭЛЛИПСОИД (первая попытка это поправить) высоту дал, но ценой
     деформации: у растянутого по вертикали шара каждый камень поехал
     вместе с поверхностью, кладка сверху вытянулась в узкие полосы, и
     владелец назвал это ровно тем, чем оно было.

     СЕКТОР ШАРА снимает противоречие целиком. Шар радиусом R срезан
     плоскостью грунта НИЖЕ своего центра, на глубине С. Поверхность
     остаётся настоящей сферой - ни один камень не растянут, нормали
     честные, кладка идёт ровными поясами, - а пропорции становятся
     теми, что нужно:

         высота = R + С,  ширина = 2R,  отношение = (R + С) / 2R

     При С = 0.5R это 0.75 - ровно их три четверти. Самое широкое место
     свода при этом ВЫШЕ грунта, а книзу кладка подбирается, и именно
     так выглядит настоящее иглу и их модель.

     Основание сужается до sqrt(1 - 0.25) = 0.866 радиуса. */
  var СРЕЗ = R * 0.50;              /* насколько центр шара ниже грунта */
  var ВЫС = R + СРЕЗ;               /* высота свода над грунтом */
  var ФИ0 = -Math.asin(СРЕЗ / R);   /* широта нижнего пояса кладки */
  ...
  function лунныйКадр() {
    ...
    /* ── ДОМ ВПИСЫВАЕТСЯ ПО ВЕРХУ И НИЗУ, А НЕ ПО ШИРИНЕ ────────
       Владелец: «домик слишком близко и на ПК, и на мобильном, нужно
       немного подальше, размер как у igloo».

       Замер их снимка (390 на 844 без полос браузера): свод стоит с
       34-го процента высоты по 62-й, то есть от +0.31 до -0.25 в долях
       кадра, а по ширине берёт семьдесят один процент. Раньше кадр
       считался от ШИРИНЫ, и высота выходила какая придётся - при
       полусфере это была пятая часть экрана, дом читался блином, и
       чтобы он хоть что-то занимал, приходилось подъезжать вплотную.

       Теперь наоборот: объявлены верх и низ силуэта в долях кадра, из
       них считается расстояние. Ширина встаёт сама, потому что свод
       стал высоким (см. ВЫС) и его пропорции сошлись с их иглу. */
    /* Доли считаны по СИЛУЭТУ (свод плюс крыльцо входа), а замер
       снимает их по центрам камней, поэтому измеренный размах выходит
       на пятую часть больше объявленного: крыльцо вынесено к камере и
       в кадре крупнее свода. Числа ниже уже с этой поправкой -
       на лежачем кадре замер даёт 0.34..0.81 высоты, на стоячем
       0.41..0.70, то есть ровно igloo. */
    /* Числа с поправкой на замер: он снимает рамку по центрам камней, а
       крыльцо входа вынесено к камере и в кадре крупнее свода, поэтому
       измеренный размах выходит на десятую-пятую больше объявленного.
       Итог живьём: телефон 71% ширины и 25% высоты, монитор 33% и 40% -
       ровно та доля кадра, что у igloo, и дом стоит ДАЛЬШЕ, а не
       упирается в объектив. */
    var верхД = узкий ? 0.155 : 0.29;
    var низД = узкий ? -0.31 : -0.39;
    var ПОЛУДОМ = R * 1.145, ЦЕНТРДОМ = R * 0.145;
  ...
  function кладка() {
    ...
    for (var ряд = 0; ряд < рядов; ряд++) {
      /* Пояса идут по широте от среза до макушки. Шаг по широте один и
         тот же, значит и высота камня одна и та же: длина дуги на шаре
         не зависит от того, где этот шаг взят. Ровно поэтому сектор шара
         и не деформирует кладку. */
      var шагФи = (Math.PI / 2 - ФИ0) / (рядов + 0.35);
      var фи = ФИ0 + (ряд + 0.5) * шагФи;
      var rРяда = Math.cos(фи) * R;
      var yРяда = Math.sin(фи) * R + СРЕЗ;
      var выс = R * шагФи;
      var шир0 = R * 0.52;
      ...
      for (i = 0; i < n; i++) {
        ...
        /* Нормаль настоящей сферы, взятая от ОПУЩЕННОГО центра: он лежит
           на СРЕЗ ниже грунта, а не в нуле. Мерить от нуля значило бы
           наклонять каждый камень сильнее, чем лежит поверхность под
           ним, и свет пошёл бы мимо граней. */
        var норм = new T.Vector3(центр.x, центр.y - СРЕЗ, центр.z).normalize();
    ...
    for (i = 0; i < 4; i++) {
      ...
      var rк = R * 0.17;
      var цк = new T.Vector3(Math.cos(тк) * rк, СРЕЗ + R * 0.985, Math.sin(тк) * rк);
