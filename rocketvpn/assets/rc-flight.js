...
function buildWorld() {
  ...
  if (g.RC_REAL && g.RC_REAL.upgradeTree) {
    try {
      g.RC_REAL.upgradeTree(T, ядроК, function () {
        return { kind: "hull", roughness: 0.94, metalness: 0.02, normalScale: 1.5, repeat: 2, envMapIntensity: 0.35 };
      });
    } catch (eЯ) {}
  }
  comet.add(ядроК);
  /* Струи газа из трещин: узкие конусы наружу, светлые и полупрозрачные */
  /* Струя узкая и почти прозрачная. Первый заход дал конус радиусом
     полторы единицы при ядре в две с половиной, да ещё непрозрачностью
     в две десятых со сложением по обе стороны: на портрете это вышло
     двумя белыми клиньями из бумаги. Настоящий выброс это тонкий луч
     пыли, светящийся на просвет, и заметен он как раз своей
     тонкостью. */
  var струяМат = new T.MeshBasicMaterial({
    color: 0xbfe6ff, transparent: true, opacity: 0.085,
  ...
  for (var ст = 0; ст < СТРУИ.length; ст++) {
    var струя = new T.Mesh(new T.ConeGeometry(0.55, 7.5, 12, 1, true), струяМат);
    var нп = new T.Vector3(СТРУИ[ст][0], СТРУИ[ст][1], СТРУИ[ст][2]).normalize();
    струя.position.copy(нп).multiplyScalar(4.6);
    /* Конус в THREE смотрит вершиной вверх, разворачиваем его по
       направлению выброса. */
    струя.quaternion.setFromUnitVectors(new T.Vector3(0, 1, 0), нп);
    comet.add(струя);
  }
  /* ── Хвосты кометы ────────────────────────────────────────
  ...
  var панельМат = new T.MeshPhongMaterial({
    ...
    side: T.DoubleSide, emissive: 0x070f1c
  });
  ...
  if (g.RC_REAL && g.RC_REAL.upgradeTree) {
    try {
      g.RC_REAL.upgradeTree(T, sat, function (о, м) {
        if (м === панельМат) return { kind: "glass", roughness: 0.22, metalness: 0.30, envMapIntensity: 1.25 };
        if (о === satBody) return { kind: "hull", roughness: 0.34, metalness: 0.92, normalScale: 0.9, envMapIntensity: 1.4 };
