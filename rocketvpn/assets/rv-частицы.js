...
(function (g, d) {
  ...
  var Ф_РОЙ = [
    ...
    /* ── ОБЁРНУТЫЙ ДИФФУЗ И ОТСКОК ───────────────────────────────
       В заголовке этого файла их числа записаны верно - «обёрнутый
       диффуз с wrap 0.25 и отскоком 0.1», - а в самом расчёте стояло
       голое max(0, dot(L,n)). Разница видна ровно тогда, когда рой
       становится плотным: без обёртки теневая половина каждого шарика
       садится в uColorDark (34/43/66 из 255), и стоит частицам начать
       перекрывать друг друга, вся фигура уходит в черноту. Замер это и
       показал: при зерне, равном их 4.6, наше тело стояло на 79 из 255
       против их 182 - силуэт есть, статуи нет.

       Обёртка тянет свет за терминатор (свет в тумане приходит со всех
       сторон, а не из точки), отскок держит низ. Оба числа их. */
    /* ── ОБЁРТКА ТОЛЬКО НА ПОВЕРХНОСТИ, НЕ НА ШАРИКЕ ─────────────
       Обёрнутый диффуз у них стоит на нормали ПОВЕРХНОСТИ (её считает
       вершинный шейдер, vShadow). Здесь, на нормали самого шарика,
       нужен голый косинус: именно он делает из точки шарик, а не
       кружок. Обёртка стояла в обоих местах разом, и произведение двух
       сглаженных величин съедало форму - фигура выходила ровно светлой
       без единой тени.

       Само произведение двух косинусов меньше каждого из них, и с их
       числами тело садилось в черноту. Поднимаем произведение
       множителем, а не обёрткой: тень остаётся тенью, уровень встаёт
       на замер их кадра. */
    "  float lightShadow = max(0.0, dot(L, normalize(n)));",
    "  float ramp = clamp(lightShadow * vShadow * 1.45 + 0.05, 0.0, 1.0);",
  ...
  function собрать(мир, гнездо, опц) {
    ...
    /* ── ТОЧЕК БОЛЬШЕ, САМА ТОЧКА МЕЛЬЧЕ ─────────────────────────
       Сверка зерна с их кадром (плёнка-ПК-017, фигура крупным планом).
       Мера простая и честная: средний модуль разности соседних точек
       по строке. У них 4.6 из 255, у нас было 12.4 - зерно втрое
       крупнее, и оттого наша ракета читалась насыпью гравия, а их
       фигура - статуей из песка. Заодно у нас проваливались просветы:
       пятый процентиль 78 против их 135, то есть между частицами
       чернел фон.

       Оба числа лечит одно: точку уменьшить, число увеличить. Заливка
       кадра идёт как N на квадрат размера, поэтому при размере в 0.58
       прежнего число берётся втрое с небольшим - плотность выходит чуть
       выше прежней, а зерно вдвое мельче.

       Точка стоит одну вершину и ни одного треугольника, дорога только
       её заливка. Заливка при этом почти не изменилась, значит и цена
       кадра осталась прежней. */
    var сколько = ст === 0 ? 70000 : (ст === 1 ? 260000 : 900000);
    ...
    М.мат = new T.ShaderMaterial({
      uniforms: {
        ...
        uH: { value: 900 },
        /* Размер точки. У них 10 при их масштабе фигуры; наш рой крупнее
           и камера дальше, поэтому число живёт вместе с uScale. 5.8
           это те же 10, ужатые под замер зерна (см. «ТОЧЕК БОЛЬШЕ» ниже):
           зерно кадра падает с 12.4 до их порядка, потолок обрезки
           опущен с 14 до 9 по той же причине. */
        uSize: { value: опц["размер"] != null ? опц["размер"] : 8.0 },
        uScale: М.uScale,
        ...
        /* ── ПАРА ЦВЕТОВ ПОДНЯТА ПОД НАШ ВОЗДУХ ───────────────────
           Их #bdc6d4 и #222b42 сняты с их кадра, где воздух зала стоит
           на 150 из 255. После сверки подземелья наш воздух стоит на
           155, но и рой у нас плотнее (точка мельче, число больше), а
           при плотном рое каждая частица умножает фон под собой на свой
           цвет: с их тёмным концом (34/43/66) фигура уходила в черноту -
           замер дал тело 79 при их 182.

           Пара поднята так, чтобы совпал ЗАМЕР ИХ КАДРА, а не числа их
           кода: тело к фону 1.21 при их 1.21, зерно 4.7 при их 4.6.
           Тени у нас пока мельче (пятый процентиль 168 против их 135),
           света ярче (220 против 204) - вилка та же, стоит выше. */
        uColorLight: { value: new T.Color(0xDCE6F2) },
        uColorDark: { value: new T.Color(0x5A6478) },
      ...
      /* ── ЗАПИСЬ ГЛУБИНЫ ВКЛЮЧАЕТСЯ И ВЫКЛЮЧАЕТСЯ ─────────────────
         У соседей рой пишет глубину, и это верно, пока он в кадре ОДИН:
         тёмная частица обязана гасить фон под собой, иначе рой светится
         насквозь вместо того, чтобы стоять телом.

         Но в зале рой превращается в корабль, и вот тут запись глубины
         становится бедой: невидимые к концу превращения точки честно
         пишут свой силуэт, и корпус за ними вычищается из кадра
         целиком. Поймано прямой пробой - красный куб на месте корабля
         выходил чёрной дырой в рое. Из-за этого раздел с ракетой и был
         «кашей»: корабль не показывался никогда, сколько бы превращение
         ни считало единицу.

         Поэтому глубину переключает зал: рой один - пишет, появился
         корпус - перестаёт (см. rv-комната.js, «кадр»). */
  ...
  function кадр(dt, часы) {
    ...
    /* ── ГУЛ ФИГУРЫ ИДЁТ ЗА РУКОЙ ──────────────────────────────
       У igloo фигура из частиц звучит ВСЁ ВРЕМЯ, пока она в кадре, и
       разгорается под пальцем: `_particlesVolume = 0.04*e + v*0.21`,
       где v это сглаженная скорость руки по фигуре (разбор бандла,
       строка 18772). Разовых щелчков по касанию у них нет.

       Наш отклик живёт полсекунды и описывается синусом фазы - это и
       есть та самая скорость руки, только считанная иначе. Сглаживаем
       её тем же лерпом 0.05, что у них, и отдаём звуку. Прозрачность
       роя служит признаком «фигура в кадре»: пока её нет, гула нет. */
    var сила = М.uPush ? М.uPush.value : 0;
    _гул += (сила - _гул) * 0.05;
    if (g.RV_SOUND && g.RV_SOUND["рой"]) {
      g.RV_SOUND["рой"](М.uAlpha && М.uAlpha.value > 0.02 ? 1 : 0, _гул);
    }
  }
  var _гул = 0;
  ...
  /* Пишет ли рой глубину. Зовёт зал по ходу превращения. */
  function глубина(да) {
    if (М.мат && М.мат.depthWrite !== !!да) {
      М.мат.depthWrite = !!да;
      М.мат.needsUpdate = true;
    }
  }
