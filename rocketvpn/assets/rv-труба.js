...
(function (g, d) {
  ...
  var Ф_ПОЛЕ = [
    ...
    /* ВЕС СПУТНИКОВ УБАВЛЕН ВДВОЕ. Замер кадра тоннеля по слоям
       рендера: с трубой средняя яркость 101 при двенадцати процентах
       чёрного, без неё 56 при двадцати восьми. Сорок пять единиц из ста
       давали именно эти три слоя, сложенные поверх стенки: у igloo они
       ЛОВЯТ свет кольца, а не заливают им кадр. Тоннель обязан быть
       тёмным с яркими прожилками, а не ровно светлым. */
    "  float a = (telo + kromka * 0.85) * smoothstep(1.0, 0.86, r) * 0.45;",
  ...
  var Ф_ПЛАЗМА = [
    ...
    "  float a = pow(v * 1.7, 4.0) * polosa * 1.0;",
  ...
  var Ф_ДЫМ = [
    ...
    "  float a = pow(v * 2.75, 3.0) * fade * 0.5;",
