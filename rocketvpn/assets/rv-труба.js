...
(function (g, d) {
  ...
  var Ф_СТЕНКА = [
    ...
    /* КОНТРАСТ БОРОЗД ПОДНЯТ. Пять процентов туда-сюда читались на
       кадре у самого устья и полностью пропадали в глубине: замер
       снимка на середине трубы показывал ровное серое поле без единой
       детали, то есть тоннель, неотличимый от тумана. У igloo стенка
       шахты держит рисунок на всю длину - камень виден, пока виден
       вообще. Двадцать процентов на бороздах и семь на крупной волне
       возвращают ей поверхность. */
    "  color *= mix(0.80, 1.10, borozda) * mix(0.93, 1.05, volna);",
  ...
  function кадр(доля, dt, часы, поворотВерха) {
    ...
    /* Стенка, дым и снег живут, пока камера внутри трубы. Её длина
       девять их единиц; полторы за концом хватает, чтобы выход не
       обрубался кромкой. */
    var внутри = глуб < 9 + 1.5;
