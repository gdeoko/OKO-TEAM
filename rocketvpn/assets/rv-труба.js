...
(function (g, d) {
  ...
  function фОбод(дешево) {
    return [
      ...
      "  float a = polosa * 0.62 + plato * 1.55 + zhilka * 0.45;",
      /* Разгон по ходу. На кадре 012 каймы почти не видно, на 014 она
         слепит. Две трети силы на входе и полторы на выходе дают тот же
         ход, и при этом кайма НЕ гаснет в начале до нуля. */
      "  a *= vBliz * uAlpha * (0.66 + 0.9 * clamp(uSvet, 0.0, 1.0));",
      "  if (a < 0.004) discard;",
      ...
      "  gl_FragColor = vec4(vec3(clamp(a, 0.0, 3.0)), 1.0);",
      "}"
    ].join("\n");
  }
