...
## Мастер-хранилище (ВСЕ ключи/пароли/доступы одним файлом)

Полный детальный свод всего (инфра, AI, соцсети, оплата, паспорт/банк, аккаунты клиентов,
сервисы сборки) — единый приватный файл на VPS. Читать из ЛЮБОГО чата:

```bash
vexec 'cat /opt/oko-poster/cfg/OKO_MASTER_VAULT.md'
```

Файл приватный (chmod 600, только VPS) — в git НЕ коммитится, паспорт/банк/карты в публичные
репозитории не попадают. Обновлять при добавлении ключа (см. правило синхронизации ниже).
