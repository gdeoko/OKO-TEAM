...
**ДИАГНОСТИКА БЛОКИРОВКИ — ПРАВИЛЬНЫМ МЕТОДОМ (11.09.2026).** «Flood control»
на закрытой странице приходит не от всех методов: `users.get` отвечает именно
девяткой, а `account.getAppPermissions` и `messages.getConversations` говорят
прямо — `error 5, User authorization failed: user is blocked` и кладут в ответ
`ban_info.restore_url`. От домена это не зависит: api.vk.ru и api.vk.com
отвечают одинаково, проверено обоими. Проверка одной строкой:

    curl -s 'https://api.vk.ru/method/account.getAppPermissions?access_token=<ключ>&v=5.199'

Пока страница закрыта, сайт личным ключом НЕ ходит вовсе (`vk_user_key_usable()`
помнит хвост закрытого ключа); раз в шесть часов пробует снова, и первый удачный
ответ снимает отметку сам. Ссылка разблокировки лежит в настройке
`vk_user_restore_url` — открывать её владельцу со своего телефона, с российского
адреса и без VPN.
