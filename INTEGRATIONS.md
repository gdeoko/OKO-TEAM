...
## AMBERRY — админ-панель (23.09.2026)

**Что даёт.** Тот же адрес, что и страница каталога, но теперь это
полноценная панель: сводка с плитками и графиками по дням, люди (поиск,
блокировка, удаление), оплаты, генерации, переписка с поддержкой с ответом
прямо из браузера, рассылка по всей базе или по части, услуги (ступени
оплаты можно спрятать) и франшиза. Прежняя страница кнопок цела и открыта
разделом «Каталог» — она же по адресу `/каталог`.

**Адрес и пароль** — те же, что у каталога (раздел ниже).

**Как проверить.**
```
curl -u amberry:ПАРОЛЬ https://62-112-10-168.nip.io/api/сводка
```

**Грабли.**
- Рассылку шлёт ПАНЕЛЬ, а не бот: несколько тысяч адресов это полчаса пауз,
  и делать их в потоке бота значит занять того, кто в эту минуту платит.
  Темп 4 письма в секунду — пятая часть потолка бота. Две рассылки разом не
  заводятся.
- Кнопки «выплатить партнёру» в панели НЕТ и не будет. Панель считает,
  сколько причитается; переводит владелец руками.
- Токен чужого бота (франшиза) в общем списке только хвостом. Целиком —
  отдельным запросом `/api/франшиза/токен/<id>`.
- `hmac.compare_digest` на строках падает на не-ASCII: пароль с кириллицей
  ронял страницу пятисоткой вместо «нужен пароль». Сравниваются байты.

---

## APIMODELS — картинки и видео дешевле официала (23.09.2026)

**Что даёт.** Агрегатор: gpt-image-2.5, gemini-3-pro-image, grok, qwen3 и
ещё полторы сотни моделей одним ключом, 60–95 % дешевле официальных API.
Рекламная обложка франшизы AMBERRY сделана здесь, `gpt-image-2.5-sunburst`,
2K, 4:5 — стоила 0,04 кредита.

**Ключ** `APIMODELS_KEY` — в `OKO_MASTER_VAULT.md` (аккаунт
okoteam.top@gmail.com). База `https://api.apimodels.app/v1`.

**Как проверить.**
```
curl -H "Authorization: Bearer $APIMODELS_KEY" https://api.apimodels.app/v1/models
```

**Как генерировать.** `POST /v1/images/generations` с
`{model, prompt, aspect_ratio, resolution}` возвращает `taskId`; результат
забирается опросом `GET /v1/images/generations?task_id=ID`.

**Грабли.**
- Состояние задачи проходит `pending → processing → completed`, а не
  `success`: ждать надо именно `completed`, иначе опрос крутится вечно.
- Это ПЛАТНЫЕ кредиты. Правило №8 из CLAUDE.md действует: спрашивать
  Даниэля перед каждой генерацией, кроме той, что он назвал сам.

---

## AMBERRY — страница каталога (21.09.2026)

**Что даёт.** Правка бота из браузера с телефона: у каждого из 48 вариантов
переименовывается кнопка и вписывается своя строка. Сохранение пишет
`/srv/amberry/каталог.json`, бот замечает изменение файла сам — в пределах
секунды, без перезапуска и без root.

**Адрес.** `https://62-112-10-168.nip.io`, логин `amberry`, пароль — в
`OKO_MASTER_VAULT.md` и `ROCKET_ДОСТУПЫ.md` (ключ `AMBERRY_ADMIN_PASS` в
`/etc/amberry.env` на 62.112.10.168).

**Как проверить.**
```
curl -u amberry:ПАРОЛЬ https://62-112-10-168.nip.io/api/дерево | head -c 200
```

**Грабли.**
- Имя `62-112-10-168.nip.io` — не домен в аренде, а DNS, который отдаёт
  A-запись прямо из имени. Сертификат настоящий (Let's Encrypt через Caddy),
  адрес постоянный. Быстрый туннель cloudflared сюда не годился: он меняет
  имя при каждом перезапуске, а страница живёт в закладках.
- `каталог.json` ОБЯЗАН лежать вне `/opt`: код там под
  `ProtectSystem=strict` и доступен только на чтение. Путь задаёт
  `AMBERRY_DATA_DIR`.
- Полей у строки два, и оба нужны: русское видит клиент на экране до
  оплаты, английское уходит модели. Русский в промпте даёт мусор молча —
  ошибку видно на готовой картинке, когда коины уже списаны.
