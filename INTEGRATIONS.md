...
Токен в .env/secrets намертво. Остаётся опционально: сторис/репост-видео в ВК (по запросу). Получен через OAuth Implicit Flow (client_id 2685278, response_type=token) —
