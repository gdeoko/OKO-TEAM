...
запятую) и в `secrets.env`. **Порядок в списке — это порядок перебора, и менять
его нельзя**: первым идёт ключ с бесплатной квотой (проект «Muzmir»), вторым —
оплаченный («MUZMIR PAY»). Пока жива бесплатная квота, центр не платит; когда
первый ключ отвечает `429`, разговор без паузы подхватывает второй.
