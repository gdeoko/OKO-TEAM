...
# Норма дня считается от даты старта, а не стоит числом в файле. Раньше её
# поднимали руками, и это ровно тот шаг, который забывают: лента неделю идёт
# по одному ролику, потому что число никто не поправил.
python3 /opt/oko-poster/diesel_norma_dnya.py >> "$LOG" 2>&1
# Hooppy снят с маршрута 07.09.2026. TikTok публикуется прямой сессией
# внутри post_direct.py (tt_upload.mjs), до-публикация не нужна.
python3 - >> "$LOG" 2>&1 <<'PY'
