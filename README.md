...
Production addresses: [VPN](https://rocketvpn.top/), [CDN](https://rocketcdn.ru/), [flight](https://rocketcdn.ru/?flight=1), [admin](https://rocketcdn.ru/admin.html). Current publication status and acceptance limits are recorded in `RELEASE_R7.md`; `RELEASE_R6.md`, `RELEASE_R5.md`, `RELEASE_R4.md` and `RELEASE.md` preserve the earlier reports.

The client now runs **r7**, correcting the remaining ship PMREM path and the WebGL 1 tunnel shader. All six VPN acts were inspected at 72 native checkpoints across landscape/portrait and forward/reverse traversal, without GL, shader or act errors. These captures exclude browser layout, HTML text, audio, continuous rendering and physical-device FPS. Activation passed 27 server checks and retained all five pre-existing requests. The local r7 commit has not reached GitHub because automatic approval review requires explicit authorization for that payload and destination; client publication succeeded separately.
...
`deploy/prepare.py` records the one-off recovery using the original transfer archives; it is not a generic installer. For the active r7 release, run `sudo python3 /var/www/rocket-releases/20260908-r7/deploy/followup.py health`. Its `rollback` action restores r6 without reverting runtime records. `deploy/stage.py` maintains the isolated review routes.
